...
class FuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, tied_operands=None, sym_visibility=None, arg_attrs=None, res_attrs=None, inlining_policy=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_583')) else
      _ods_ir.AttrBuilder.get('anonymous_583')(function_type, context=_ods_context))
    if tied_operands is not None: attributes["tied_operands"] = (tied_operands if (
        issubclass(type(tied_operands), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('Util_TiedOpStorageAttr')) else
          _ods_ir.AttrBuilder.get('Util_TiedOpStorageAttr')(tied_operands, context=_ods_context))
...
def global_load_indirect(result, global_, *, is_immutable=None, loc=None, ip=None) -> _SubClassValueT:
  return _get_op_result_or_op_results(GlobalLoadIndirectOp(result=result, global_=global_, is_immutable=is_immutable, loc=loc, ip=ip))
...
def global_load(result, global_, *, is_immutable=None, loc=None, ip=None) -> _SubClassValueT:
  return _get_op_result_or_op_results(GlobalLoadOp(result=result, global_=global_, is_immutable=is_immutable, loc=loc, ip=ip))
