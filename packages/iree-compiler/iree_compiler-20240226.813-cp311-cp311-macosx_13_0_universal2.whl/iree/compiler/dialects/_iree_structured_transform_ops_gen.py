...
def iree_emit_remark(handle, message, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(EmitRemarkOp(handle=handle, message=message, loc=loc, ip=ip))
...
def iree_match_callback(outputs, callback_name, failure_propagation_mode, inputs, *, loc=None, ip=None) -> _SubClassValueT:
...
def iree_register_match_callbacks(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(RegisterMatchCallbacksOp(loc=loc, ip=ip))
...
def iree_take_first(first, rest, inputs, *, loc=None, ip=None) -> _Sequence[_SubClassValueT]:
  return _get_op_result_or_op_results(TakeFirstOp(first=first, rest=rest, inputs=inputs, loc=loc, ip=ip))
