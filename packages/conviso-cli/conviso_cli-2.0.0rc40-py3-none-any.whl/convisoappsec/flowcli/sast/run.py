...
def hash_issues_from_cp(context, conviso_api, issues):
    ...
    original_issue_ids = [item['originalIssueIdFromTool'] for item in issues_from_cp]

    data = [item for item in issues if item['hash_issue'] not in original_issue_ids]

    msg = "\U0001F4AC %s new Issue/Issues founded. Nothing to do." % len(data)
    log_func(msg)
