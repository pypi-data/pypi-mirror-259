class Plot:
    ...
    def plotBestFit(self):
        ...
        max_strain = (max_stress - self.fit_results['Intercept [kPa]']) / self.fit_results['Young Modulus [kPa]']
        max_strain = max_strain[0]
        strain_range = self.data[self.data['strain'] < max_strain]['strain']
        line = self.fit_results['Young Modulus [kPa]'][0] * strain_range + self.fit_results['Intercept [kPa]'][0]
        other = self.fit_results['Young Modulus [kPa]'][0] * (strain_range) + self.fit_results['Intercept [kPa]'][0]
