...
from django_keycloak_lma.services.exceptions import TokensExpired
from django_keycloak_lma.remote_user import KeycloakRemoteUser
...
def get_or_create_from_id_token(client, id_token):
    ...
    :param django_keycloak_lma.models.Client client:
    :param str id_token:
    :rtype: django_keycloak_lma.models.OpenIdConnectProfile
    """
    issuer = django_keycloak_lma.services.realm.get_issuer(client.realm)
...
def get_from_id_token(client, id_token):
    """
    Get OpenID Connect profile from given id_token.
    ...
    return get_oidc_profile_by_token(
        client=client, id_token_object=id_token_object)
...
def update_or_create_oidc_profile(client, id_token_object, user=None):
    """
    :param client:
    :param id_token_object:
    :param user:
    ...
    if user is None:
        raise Exception('User does not specified')
...
def get_oidc_profile_by_token(client, id_token_object):
    """
    :param client:
    :param id_token_object:
    :return:
    ...
    oidc_profile = OpenIdConnectProfileModel.objects.get(
        sub=id_token_object['sub'],
        realm=client.realm,
    )
...
def update_or_create_from_password_credentials(username, password, client, user=None):
    """
    ...
    return _update_or_create(client=client, token_response=token_response,
                             initiate_time=initiate_time, user=user)


def _update_or_create(client, token_response, initiate_time, user=None):
    """
    ...
    if user is None:
        ...
        oidc_profile = update_or_create_oidc_profile(
            client=client,
            id_token_object=token_object,
            user=user)
...
def update_tokens(token_model, token_response, initiate_time):
    ...
    :param django_keycloak_lma.models.TokenModelAbstract token_model:
...
def get_active_access_token(oidc_profile):
    ...
    :param django_keycloak_lma.models.KeycloakOpenIDProfile oidc_profile:
    :rtype: string
    :raise: django_keycloak_lma.services.exceptions.TokensExpired
    """
    initiate_time = timezone.now()
...
def refresh_token(client, refresh_token_obj):
    try:
        token_response = client.openid_api_client.refresh_token(refresh_token=refresh_token_obj)
    except Exception as ex:
        raise Exception(messages.REFRESH_TOKEN_ERROR)

    initiate_time = timezone.now()
    oidc_profile = get_from_id_token(client, token_response['access_token'])
    oidc_profile = update_tokens(token_model=oidc_profile,
                                 token_response=token_response,
                                 initiate_time=initiate_time)
    return oidc_profile
...
def logout(oidc_profile):
    oidc_profile.realm.client.openid_api_client.logout(
