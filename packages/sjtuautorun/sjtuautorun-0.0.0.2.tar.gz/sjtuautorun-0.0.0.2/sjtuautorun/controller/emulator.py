...
class Emulator:
    ...
    def is_running(self, app="sjtu"):
        """检查一个应用是否在运行

        Args:
            app (str, optional): 应用名, 默认为 "交我办".
    ...
    def get_screen(self, resolution=(720, 1280), need_screen_shot=True):
        if need_screen_shot:
            self.update_screen()
    ...
    def get_pixel(self, x, y, screen_shot=False) -> list:
        ...
        if screen_shot:
            self.update_screen()
        if len(self.screen) != 960:
            self.screen = cv2.resize(self.screen, (540, 960))
    ...
    def get_image_position(self, image, need_screen_shot=1, confidence=0.85, this_methods=None):
        ...
        Returns:
            如果找到:返回一个二元组表示相对坐标 (相对 540x960 屏幕)
