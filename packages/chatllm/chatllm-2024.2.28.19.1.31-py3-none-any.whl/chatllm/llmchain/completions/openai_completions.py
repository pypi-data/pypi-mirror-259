...
class Completions(object):
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, **kwargs):
        ...
        self.backup_llm_map = {
            ...
            "backup-gpt-3.5": "gpt-3.5-turbo-0125",
    ...
    def create(self, request: ChatCompletionRequest, **kwargs):  # 非openai兜底 backup-llm、openai兜底 backup-gpt、特殊标识直接走官转
        ...
        try:
            ...
            # 分支：夜里非流3.5
            if not request.stream and request.model.startswith(('gpt-3.5',)):
                ...
                # 代码类走deepseek
                if request.messages[-1].get('content', '').startswith('CREATE TABLE'):  # todo: 待补全
            ...
            if response:
                ...
                data['model'] = "backup-gpt-4" if data['model'].startswith('gpt-4') else "deepseek-chat"
                return self.backup_client.chat.completions.create(**data)
    ...
    def post_process(self, response, data):
        """兜底判断
    |     if not (chunk.choices and chunk.choices[0].delta.content):  # 都跑错的时候如何兜底
    |             │                 └ ('id', 'chatcmpl-id')
    |             └ ('id', 'chatcmpl-id')
    |
    ...
    def _handle_error(self, data: Dict[str, Any]) -> Any:
        send_message("兜底出错")
