...
def download_images(image_urls, dst_dir, file_prefix, concurrency=50, timeout=20, proxy_type=None, proxy=None):
    """
    Download image according to given urls and automatically rename them in order.
    :param timeout:
    :param proxy:
    ...
    :return: none
    """
    assert image_urls.__len__() == file_prefix.__len__(), "image_urls and file_prefix must have the same length"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for index,image_url in enumerate(image_urls):
            ...
            file_name = file_prefix[index]
            future_list.append(executor.submit(
                download_image, image_url, dst_dir, file_name, timeout, proxy_type, proxy))
            count += 1
        completed, _ = concurrent.futures.wait(future_list, timeout=180)
