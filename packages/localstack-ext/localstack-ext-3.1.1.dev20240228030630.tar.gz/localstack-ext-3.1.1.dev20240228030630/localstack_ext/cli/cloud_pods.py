...
@click.argument(_E,required=_B)
@click.argument('url',required=_B)
...
@click.option(_H,_I,_J,type=click.Choice([_F,_C]),default=_F,help='The formatting style for the remotes command output.')
def cmd_remotes(format_):
	...
	if format_==_C:console.print_json(json.dumps(A));return
...
@click.option(_H,_I,_J,type=click.Choice([_C]),help='The formatting style for the save command output.')
@publish_invocation
def cmd_pod_save(name=_A,remote=_A,services=_A,message=_A,visibility=_A,format_=_A):
	F=visibility;E=remote;B=services;A=name;G=CloudPodsClient(_B);_assert_host_reachable()
	...
	if F:
		try:G.set_remote_attributes(pod_name=A,attributes=CloudPodRemoteAttributes(is_public=_B),remote=H)
		except Exception as C:raise CLIError(str(C))
		console.print(f"Cloud Pod {A} made {F}")
	I=[A.strip()for A in B.split(',')]if B else _A
	try:D=G.save(pod_name=A,attributes=CloudPodRemoteAttributes(is_public=_D,description=message,services=I),remote=H)
	except Exception as C:raise CLIError(f"Failed to create Cloud Pod {A} ❌ - {C}")from C
	if format_==_C:console.print_json(json.dumps(D))
...
@click.option('--yes','-y',help='Automatic yes to prompts. Assume a positive answer to all prompts and run non-interactively',is_flag=_B,default=_D)
@publish_invocation
def cmd_pod_load(name=_A,remote=_A,merge=_A,yes=_D):
	B=remote;A=name;D=CloudPodsClient(_B);_assert_host_reachable()
...
@click.argument(_G,required=_D)
@click.option('--public','-p',help='List all the available public Cloud Pods',is_flag=_B,default=_D)
@click.option(_H,_I,_J,type=click.Choice([_F,_C]),default=_F,help='The formatting style for the list pods command output.')
@publish_invocation
def cmd_pod_list_pods(remote=_A,public=_D,format_=_A):
...
@click.argument(_E)
@click.option(_H,_I,_J,type=click.Choice([_F,_C]),default=_F,help='The formatting style for the version command output.')
@publish_invocation
