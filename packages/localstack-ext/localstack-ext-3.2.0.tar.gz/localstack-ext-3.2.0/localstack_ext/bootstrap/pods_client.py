...
from localstack_ext.bootstrap.pods.remotes.configs import DEFAULT_REMOTE_SCHEME,RemoteConfig,RemoteConfigParams
...
class CloudPodsClient(CloudPodsService):
	...
	def _process_response(J,response,message):
		...
		B.stop()
	def save(C,pod_name,attributes=_A,remote=_A,local=_B,version=_A):
		G=version;D=pod_name;E=f"{get_pods_endpoint()}/{D}?"
		if local:E+='&local=true'
		if G:E+=f"&version={G}"
		H=_get_remote_params_payload(remote);H.update({'attributes':attributes});B=requests.post(url=E,json=H,headers=_get_headers(),stream=C.interactive)
		if not B.ok:_raise_exception_with_formatted_message(f"Unable to save pod {D}",B)
		F={}
		if C.interactive:F=C._process_response(B,message=f"Saving Cloud Pod {D}")
		else:
			for A in B.iter_lines():
				A=json.loads(A)
				if A[_D]=='pod_info':F=PodInfo(**A['extra'])
	...
	def load(A,pod_name,remote=_A,version=_A,merge_strategy=_A,ignore_version_mismatches=_B):
		F=ignore_version_mismatches;D=version;C=remote;B=pod_name
		if in_ci():F=_C
		G=_get_remote_configuration(C,render_params=_B)if C else _A;K=G and G.scheme!=DEFAULT_REMOTE_SCHEME
		if not K and not F:
			H=A._get_localstack_pod_version(pod_name=B,version=D);I=get_ls_version_from_health()
			if not is_compatible_version(H,I)and not click.confirm(f"This Cloud Pod was created with LocalStack {H} but you are running LocalStack {I}. Cloud Pods might be incompatible across different LocalStack versions.\nLoading a Cloud Pod with mismatching version might lead to a corrupted state of the emulator. Do you want to continue?"):raise click.Abort('LocalStack version mismatch')
		J=f"{get_pods_endpoint()}/{B}"
		if D:J+=f"?version={D}"
		L=_get_remote_params_payload(C);E=requests.put(url=J,json=L,headers=_get_headers(),stream=A.interactive)
		if not E.ok:_raise_exception_with_formatted_message(f"Unable to load pod {B}",E)
		if A.interactive:A._process_response(E,message=f"Loading Cloud Pod {B}")
