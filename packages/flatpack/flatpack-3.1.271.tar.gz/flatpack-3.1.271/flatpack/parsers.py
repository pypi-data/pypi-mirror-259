...
if ! command -v {cmd} >/dev/null; then
  ...
  exit 1
    ...
    # Determine Python command outside of Colab check to ensure it's always set
    python_cmd_setup = """
...
if command -v python3 &>/dev/null; then
    ...
    """.strip()
    script.append(python_cmd_setup)
...
if [[ $IS_COLAB -eq 1 ]]; then
    ...
    if package_list_unix:
        apt_install = f"""
OS=$(uname)
if [[ ! -d "/content" && "$OS" = "Linux" && -f /etc/debian_version ]]; then
