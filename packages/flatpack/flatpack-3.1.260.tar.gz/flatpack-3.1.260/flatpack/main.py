...
def fpk_install(directory_name: str, session, verbose: bool = False):
    ...
    # Construct the unique path for the flatpack's files
    flatpack_dir_path = os.path.join(os.path.expanduser('~'), '.flatpack', directory_name)
    os.makedirs(flatpack_dir_path, exist_ok=True)  # Ensure the directory exists

    # Paths for the temporary TOML and the bash script within the flatpack directory
    temp_toml_path = os.path.join(flatpack_dir_path, 'temp_flatpack.toml')
    bash_script_path = os.path.join(flatpack_dir_path, 'flatpack.sh')

    with open(temp_toml_path, 'w') as f:
        f.write(toml_content)

    bash_script_content = parse_toml_to_venv_script(temp_toml_path, '3.10.12', directory_name)
    with open(bash_script_path, 'w') as f:
        f.write(bash_script_content)

    # Remove the temporary TOML file after creating the bash script
    os.remove(temp_toml_path)
    print(f"Installing {directory_name}...")

    command = ["bash", shlex.quote(bash_script_path)]
    try:
        if verbose:
            process = subprocess.Popen(command, cwd=flatpack_dir_path)
            process.wait()
        else:
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=flatpack_dir_path)
        ...
        if process.returncode == 0:
            ...
            print(f"🎉 All done! Installed in {flatpack_dir_path}")
...
def fpk_vector(directory_name: str):
    """Create a Vector instance for a specific flatpack."""
    # Construct the unique path to the .index file for this flatpack
    flatpack_dir_path = os.path.join(os.path.expanduser('~'), '.flatpack', directory_name)
    index_file_path = os.path.join(flatpack_dir_path, f'{directory_name}.index')

    # Instantiate CataDB with the unique index file path
    vector_db = Vector(vector_dimension=128, index_file=index_file_path)

    # Now, vector_db is ready to be used for this specific flatpack
    # For example, adding vectors, building the index, or searching
    return vector_db
