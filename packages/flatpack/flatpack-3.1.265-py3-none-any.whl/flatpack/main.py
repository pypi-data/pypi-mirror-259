...
def fpk_vector(directory_name: str, vector_dimension=128, metric='angular', n_trees=10, verbose=False):
    """
    Simplified function to create and manage a vector index for a specific directory.
    Validates the directory name and ensures the index file is created or loaded correctly.
    """
    ...
    index_file_name = f'{directory_name}.index'

    # Initialize the Annoy index
    index = AnnoyIndex(vector_dimension, metric)
    ...
    if os.path.exists(index_file_name):
        ...
        # Note: The new index file is actually created upon calling index.save()

    # Define function to add items to the index
    def add_item(item_id, vector):
        index.add_item(item_id, vector)

    # Define function to build and save the index
    def build_index():
        index.build(n_trees)
        index.save(index_file_name)
        if verbose:
            print(f"Index file '{index_file_name}' created and saved.")

    # Define function to retrieve nearest neighbors by vector
    def get_nns_by_vector(vector, n_neighbors=5, include_distances=True):
        return index.get_nns_by_vector(vector, n_neighbors, include_distances)

    # Immediately build and save the index to ensure file creation
    build_index()

    # Return functions for further operations
    return add_item, build_index, get_nns_by_vector
    ...
    try:
        with SessionManager() as session:
            ...
            if command == "find":
                ...
                add_item, build_index, get_nns_by_vector = fpk_vector(directory_name, verbose=True)
