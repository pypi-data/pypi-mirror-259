...
def fpk_vector(directory_name: str, vector_dimension=128, metric='angular', n_trees=10, verbose: bool = False):
    """Create and manage a vector index for a specific flatpack, including file handling."""
    ...
    if os.path.exists(index_file_name):
        ...
        index.load(index_file_name)  # Load the index
    else:
        if verbose:
    ...
    def build_index():
        ...
        try:
            ...
            if verbose:
                print(f".index file built and saved to {index_file_name}")
        except Exception as e:
            print(f"❌ Failed to save the index: {e}")

    def get_nns_by_vector(vector, n_neighbors=5, include_distances=True):
        """Retrieve nearest neighbors by vector."""
        return index.get_nns_by_vector(vector, n_neighbors, include_distances=include_distances)

    # Example of using the functions (replace with actual usage)
    # add_item(item_id=1, vector=[0.5]*vector_dimension)
    # build_index()
