...
from different_wrapping.yamlyaml import build_k8s_manifests, bulid_kustomize_manifest
...
def run_converter(args):
    ...
    with buildlist.open("w") as f:
        json.dump([image.serialize() for image in images], f)
    ...
    bulid_kustomize_manifest(
        challenges, static_folder, lambda kv: "dynamic" not in kv[0]
    )
...
def main():
    ...
    parser_convert.add_argument("--ingress_type", choices=["ingress", "gateway"], default="ingress", help="Controls what k8s primitive is used to configure http ingress")
    parser_convert.add_argument("--gateway_parent_name", help="If ingress-type gateway is used, controls the name of the parent ref")
    parser_convert.add_argument("--gateway_parent_namespace", help="If ingress-type gateway is used, controls the namespace of the parent ref")
    ...
    parser_convert.add_argument(
        "--dns_host", type=str, default="chal.cyberlandslaget.no", help="The DNS host to be used for challenge ingresses"
    )
    parser_convert.add_argument(
        "--docker_image_repo", type=str, default="docker.cyberlandslaget.no", help="The docker registry the images will reside in"
    )
