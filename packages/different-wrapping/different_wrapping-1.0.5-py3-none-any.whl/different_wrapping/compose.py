...
def normalize(text):
    return re.sub(r"-*$", "", re.sub(r"[^a-zA-Z\-]", "-", text))
...
def service_name_requirement(container, svc_name):
    if re.match(r"^[a-zA-Z\-]+$", svc_name) is None:
        return "Docker-compose service name must be in the form [a-zA-Z\-]+ due to kompose renaming them otherwise."
    return None
...
class Container:
    ...
    def patch_build(self, args, challenge_dir: Path, version_tag: str):
        ...
        if len(image_name) > 0:
            image_url = "%s/%s/%s" % (args.docker_image_repo, normalize(challenge_dir.name), image_name)
        else:
            image_url = "%s/%s" % (args.docker_image_repo, normalize(challenge_dir.name))
    ...
    def get_dns_name(self, dns_host):
        host = f"{self.name}.{self.challenge.name()}.{dns_host}"
    ...
    def serialize(self, dns_host):
        access_type = "none"
        access = self.get_dns_name(dns_host)
