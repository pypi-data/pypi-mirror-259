...
def display_html(html_content, notebook=False):
    ...
    server_thread.start()
    if notebook:
        return IFrame(src=f"http://{conf.LOCALHOST}:{server.server_port}", width=800, height=600)
    else:
...
def js_functions():
    import pkgutil
    js_code = pkgutil.get_data(__name__, 'static/lcpy.js').decode()
    return js_code
...
<html lang="en">
    <head>
        ...
        <script src="https://lightningchart.com/lightningchart-js-demos/lcpy/lcjs.iife.js"></script>
        <script src="https://cdn.jsdelivr.net/npm/@arction/lcjs-themes@3.1.0/dist/iife/lcjs-themes.iife.js"></script>
...
class Instance:
    ...
    def open_in_notebook(self, width: int = 800, height: int = 600):
        if self.live:
            return IFrame(src=f'http://{conf.LOCALHOST}:{conf.server_port}/?id={self.id}', width=width, height=height)
        else:
            html = create_html(self.items)
            return display_html(html, notebook=True)

    def send(self, id: str, command: str, parameters: dict = {}):
        if not self.live:
            self.items.append({
                'id': str(id),
                'command': command,
