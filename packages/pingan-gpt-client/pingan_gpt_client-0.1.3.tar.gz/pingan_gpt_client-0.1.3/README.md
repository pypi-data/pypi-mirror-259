...
```python
    def pingan_gpt_inference(self, prompt: str,
                             ...
                             timeout=120) -> str:
```
- temperature: 温度越低，输出确定性越高
- top_p: 值越低，输出确定性越高（top_p的概念为：模型输出的候选结果的概率依次累加，当达到top_p阈值时，不再使用更低概率的候选结果）
- session_id: 对话ID。如果设置为同一个值，则代表是同一通对话，代表模型能记忆历史上文。
- is_use_own_history和own_history：用于手动输入自己需要的上文信息
- max_new_tokens：模型最大输出长度
- timeout：超时时间
