...
class ProxyEnergyPlusModel(ProxyBuildingEnergyModel):
    ...
    def create_parameters(self, category):
        ...
        if category == CategoryParameterBuildingEnergyModel.OCCUPANCY:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWindowThermalResistance().get_classname()]:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWindowSolarHeatGainCoefficient().get_classname()]:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWallThermalResistance().get_classname()]:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWallDensity().get_classname()]:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWallSpecificHeatCapacity().get_classname()]:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWallThermalEmissivity().get_classname()]:
            ...
            for obj in self.idf.idfobjects[ParameterEnergyPlusModelWallThermalAbsorptivity().get_classname()]:
...
class UrbanBuildingEnergyModelOutputVariables(Enum):
    """
    Enumeration of output variables for an urban energy model.
    """
    TOTAL_ENERGY_LOAD = 1
...
class UrbanBuildingEnergyModel():
    ...
    Attributes:
        ...
        output_vars: list of output variables to be used for calibration.
    """
    __metaclass__ = ABCMeta

    def __init__(self, input_dir, output_vars, output_dir = '.'):
        """
...
class StandaloneEnergyPlusModels(UrbanBuildingEnergyModel):
    ...
    def __init__(self, input_dir, weather_file, output_vars, output_dir = '.'):
        """
        :param input_dir: directory in which building energy models are stored.
        ...
        :param output_dir: directory in which modified building energy models are stored.
        """
        UrbanBuildingEnergyModel.__init__(self, input_dir, output_vars, output_dir)
    ...
    def simulate(self, from_date, to_date, dt):
        ...
        :return: outputs of simulation.
        """
        bem_pool = EnergyPlusSimulationPool(EPWDataLoader(self.weather_file, year=from_date.year))
        bem_pool.pool = []
        ...
        for bem in bem_pool.pool:
            bem_output_data = {}
            for outvar in self.output_vars:
                if outvar == UrbanBuildingEnergyModelOutputVariables.TOTAL_ENERGY_LOAD:
                    bem_output_data['total_energy_load'] = bem.building.get_sensible_cooling_load()[from_date:to_date].resample(dt).interpolate().values + \
                                                           bem.building.get_latent_cooling_load()[from_date:to_date].resample(dt).interpolate().values
            output_data[bem.building.name] = bem_output_data
        return output_data
...
class CoupledEnergyPlusModels(StandaloneEnergyPlusModels):
    ...
    def __init__(self, input_dir, weather_file, cfg_file, neighborhood_file, output_vars, output_dir = '.', max_iter = 100, ddm = 'dummy', training_split_ratio = 0.0):
        """
        :param input_dir: directory in which building energy models are stored.
        :param weather_file: file containing weather data to perform EnergyPlus simulations.
        :param output_vars: list of output variables to be used for calibration.
        ...
        """
        StandaloneEnergyPlusModels.__init__(self, input_dir, weather_file, output_vars, output_dir)
    ...
    def simulate(self, from_date, to_date, dt):
        ...
        if not self.was_trained:
            cosim = ExclusiveCosimulationEnergyPlusDataDriven(self.cfg_file, self.neighborhood_file, self.weather_file, max_iter=self.max_iter, ddm=self.ddm, training_split_ratio=self.training_split_ratio)
            self.was_trained = True
        else:
            cosim = ExclusiveCosimulationEnergyPlusDataDriven(self.cfg_file, self.neighborhood_file, self.weather_file, max_iter=self.max_iter, ddm=self.ddm)
        cosim.cosimulate()
        output_data = {}
        for bem in cosim.bems:
            ...
            for outvar in self.output_vars:
                if outvar == UrbanBuildingEnergyModelOutputVariables.TOTAL_ENERGY_LOAD:
                    bem_output_data['total_energy_load'] = bem.building.get_sensible_cooling_load()[
                                                           from_date:to_date].resample(dt).interpolate().values + \
                                                           bem.building.get_latent_cooling_load()[
                                                           from_date:to_date].resample(dt).interpolate().values
            output_data[bem.building.name] = bem_output_data
        return output_data
...
class Calibration():
    ...
    Attributes:
        ...
        discrepancy: method to calculate the discrepancy between measurements and estimates.
        must_be_trained: True if the UBEM needs some prior training before calibration.
    """
    __metaclass__ = ABCMeta

    def __init__(self, ubem, metered_data, categories, upper_bounds, lower_bounds, from_date, to_date, dt, discrepancy, must_be_trained=False):
        """
        ...
        :param discrepancy: method to calculate the discrepancy between measurements and estimates.
    ...
    def calibrate(self):
        ...
        for building_name, proxy in self.ubem.bem_proxies.items():
            ...
            if platform.system() == 'Linux':
                proxy.output_dir = 'calibration/' + building_name + '.idf'
...
class BayesianCalibration(Calibration):
    ...
    def __init__(self, ubem, metered_data, categories, upper_bounds, lower_bounds, from_date, to_date, dt, discrepancy, parameters_selector,
                 num_training_samples, surrogate_model_loader, num_posterior_distribution_samples, must_be_trained=False):
        """
        :param ubem: urban building energy model to calibrate
        :param metered_data: metered data used to calibrate the urban building energy model
        :param categories: categories of parameters that must be calibrated.
        ...
        """
        Calibration.__init__(self, ubem, metered_data, categories, upper_bounds, lower_bounds, from_date, to_date, dt, discrepancy, must_be_trained)
        ...
        self.parameters_selector.discrepancy = discrepancy
    ...
    def get_calibrated_parameters(self):
        ...
        for building_name in self.ubem.get_building_names():
            ...
            training_samples = pd.read_csv(SAMPLES_DIR + building_name + '.csv').sample(n=self.num_training_samples)
            self.surrogate_model_loader.building_name = building_name
            self.surrogate_model_loader.inputs = training_samples[selected_parameters[building_name]['names']].values
            self.surrogate_model_loader.outputs = training_samples[self.discrepancy.get_name()].values
            ...
            output_samples = surrogate_model.predict(input_samples)
            def model(output_samples, input_names, lower_bounds, upper_bounds, surrogate_model):
                priors = []
                ...
                pyro.sample("discrepancy", dist.Normal(expected_output[0, 0], 0.1), obs=output_samples)
            nuts_kernel = NUTS(model, adapt_step_size=True)
            mcmc = MCMC(nuts_kernel, num_samples=self.num_posterior_distribution_samples, warmup_steps=1)
            mcmc.run(torch.tensor(output_samples), input_names, lower_bounds, upper_bounds, surrogate_model)
...
class DiscrepancyBetweenEstimatesAndMeasurements():
    """
    Class used to calculate the discrepancy between the estimates provided by an urban building energy model
    and measurements with respect to an output variable.
    """
    __metaclass__ = ABCMeta

    def __init__(self, estimates, measurements, output_var = 'total_energy_load'):
        """
        :param estimates: estimates provided by the urban building energy.
        :param measurements: measurements collected during an experiment and obtained from a data source.
        :param output_var: name of the output variable it is expected to calculate the discrepancy.
        """
    ...
    def get_discrepancy(self):
        """
        :return: discrepancy between estimates and measurements.
        """
        return self.calculate_discrepancy(self.estimates[self.output_var], self.measurements[self.output_var])

    @abstractmethod
    def get_name(self):
        """
        :return: name of the discrepancy method.
        """
        pass

    @abstractmethod
    def calculate_discrepancy(self, estimates, measurements):
        """
        Calculate the discrepancy between estimates and measurements.
        :param estimates: estimates provided by the urban building energy model with respect to an output variable.
        :param measurements: measurements of the given output variable.
        :return: discrepancy between estimates and measurements with respect to the output variable.
        """
        pass

class NormalizeMeanBiasError(DiscrepancyBetweenEstimatesAndMeasurements):
    """
    Class used to calculate the normalized mean bias error between estimates and measurements.
    """

    def __init__(self, estimates = None, measurements = None, output_var = 'total_energy_load'):
        """
        :param estimates: estimates provided by the urban building energy.
        :param measurements: measurements collected during an experiment and obtained from a data source.
        :param output_var: name of the output variable it is expected to calculate the discrepancy.
        """
        DiscrepancyBetweenEstimatesAndMeasurements.__init__(self, estimates, measurements, output_var)
    ...
    def calculate_discrepancy(self, estimates, measurements):
        ...
        :return: discrepancy between estimates and measurements with respect to the output variable.
        """
        return 100 * (np.mean(estimates) - np.mean(measurements)) / np.mean(measurements)

class CoefficientOfVariationOfRootMeanSquareError(DiscrepancyBetweenEstimatesAndMeasurements):
    """
    Class used to calculate the coefficient of variation of the root mean square error between estimates and measurements.
    """
    ...
    def calculate_discrepancy(self, estimates, measurements):
        ...
        :return: discrepancy between estimates and measurements with respect to the output variable.
        """
        rmse = mean_squared_error(estimates, measurements, squared=False)
        return 100 * (rmse / np.mean(measurements))
...
class ParametersSelector():
    ...
    def __init__(self, ubem = None, metered_data = None, categories = None, upper_bounds = None, lower_bounds = None,
                 from_date = None, to_date = None, dt = None, discrepancy = None, iter_saved = 5):
        """
        ...
        :param discrepancy: method used to evaluate the discrepancy between measurements and estimates.
    ...
    def select_parameters(self):
        ...
        for building_name in self.ubem.get_building_names():
            ...
            calibration_data[building_name]['input_samples'] = self.generate_input_samples(all_parameters, parameters_upper_bounds, parameters_lower_bounds)
        ...
        if not (len(os.listdir(path_to_save)) == 0):
            for building_name in self.ubem.get_building_names():
                ...
                for n in range(len(df)):
                    calibration_data[building_name]['output_samples'][n] = df[self.discrepancy.get_name()][n]
                    for m, pname in enumerate(all_parameters):
        ...
        for n in range(n_start, number_samples):
            ...
            for building_name in self.ubem.get_building_names():
                self.discrepancy.measurements = self.metered_data[building_name][self.from_date:self.to_date].resample(self.dt).interpolate()
                self.discrepancy.estimates = outputs[building_name]
                calibration_data[building_name]['output_samples'][n] = self.discrepancy.get_discrepancy()
                if (n == number_samples - 1) or ((n % self.iter_saved) == 0):
                    print('Samples being saved ...')
                    column_names = calibration_data[building_name]['parameters'] + [self.discrepancy.get_name()]
...
class ParametersSelectorMorris(ParametersSelector):
    ...
    Attributes:
        ...
        to_date: date to which estimates provided by the UBEM and measurements must be compared.
        dt: timestep with which estimates provided by the UBEM and measurements must be compared.
        discrepancy: method used to evaluate the discrepancy between measurements and estimates.
        iter_saved: number of iterations to save samples for parameter selections.
    ...
    def __init__(self, ubem = None, metered_data = None, categories = None, upper_bounds = None, lower_bounds = None,
                 from_date = None, to_date = None, dt = None, discrepancy = None, iter_saved = 5, num_trajectories=10,
        ...
        """
        ParametersSelector.__init__(self, ubem, metered_data, categories, upper_bounds, lower_bounds, from_date, to_date, dt, discrepancy, iter_saved)
    ...
    def generate_input_samples(self, parameters, upper_bounds, lower_bounds):
        ...
        return morris_sampler.sample(self.problem, N=self.num_trajectories, num_levels=self.num_levels, optimal_trajectories=self.optimal_trajectories)
    ...
    def perform_sensitivity_analysis(self, building_name, inputs, outputs):
        ...
        results = morris.analyze(self.problem, inputs, outputs, conf_level=0.95, num_levels=4)
        sensitivity_analysis_data = {}
...
class SurrogateBuildingEnergyModel():
    ...
    def save(self, folder):
        ...
        :param folder: folder in which to save the surrogate model.
        """
        pass
...
class GaussianProcessModel(SurrogateBuildingEnergyModel):
    ...
    def __call__(self, inputs):
        return self.gaussian_process_model.predict_f(inputs)
    ...
    def save(self, folder):
        ...
        :param folder: folder in which to save the surrogate model.
        """
        with open(folder + self.building_name + '.pkl', 'wb') as file:
            pickle.dump(self.gaussian_process_model, file)
    ...
    def predict(self, inputs):
        ...
        :return: predictions
        """
        m, v = self.gaussian_process_model.predict_f(inputs)
        return m.numpy()
...
class SurrogateBuildingEnergyModelLoader():
    ...
    def __init__(self, building_name = None,  inputs = None, outputs = None):
        ...
        :param inputs: inputs used to train the surrogate model in case it has not been previously saved.
        :param outputs: outputs used to train the surrogate model in case it has not been previously saved.
        """
        self.building_name = building_name
        self.inputs = inputs
    ...
    def load(self):
        ...
        if self.has_not_been_saved(SURROGATE_MODELS_DIR):
            surrogate_model = self.get_instance()
            surrogate_model.train(self.inputs, self.outputs)
            surrogate_model.save(SURROGATE_MODELS_DIR)
        else:
            surrogate_model = self.read(SURROGATE_MODELS_DIR)
