...
class EPWDataLoader(WeatherDataLoader):
    ...
    def __init__(self, weather_file):
        ...
        """
        self.weather_file = weather_file
        self.epw_data = op.WeatherData.load(weather_file)
    def get_instance(self):
        """
        :return: instance of weather data
        """
    ...
    def get_timestamps(self):
        ...
        timestamps = []
        for n in range(len(weather_dataframe)):
            minute = int(weather_dataframe.minute[n]) if not np.isnan(weather_dataframe.minute[n]) else 0
            if weather_dataframe.hour[n] != 24:
                if minute != 60:
                    d = datetime(self.get_year(),
                                weather_dataframe.month[n],
                                weather_dataframe.day[n],
                                weather_dataframe.hour[n],
                                minute)
                else:
                    d = datetime(self.get_year(),
                                 weather_dataframe.month[n],
...
class IDFBuildingLoader(BuildingLoader):
    ...
    Attributes:
        building_file: file in which details of the building are stored.
        idf_file: IDF file containing details of the building.
        idf_objects: IDF objects.
    """

    def __init__(self, idf_file):
        """
        :param idf_file: IDF file containing details of the building.
        """
        BuildingLoader.__init__(self)
        self.building_file = idf_file
        self.idf_objects = op.Epm.load(idf_file)
        ...
        for zone_info in self.idf_objects.Zone:
            ...
            for surface in self.idf_objects.BuildingSurface_Detailed.select(
                ...
                if surface.surface_type == 'wall':
                    zone.exterior_walls.append(ExteriorWall(surface.name, points))
...
class EnergyPlusSimulationPool(BuildingEnergySimulationPool):
    ...
    def create_simulation_environment(self):
        ...
        if self.weather_data is not None:
            self.weather_data_loader = EPWDataLoader(ENERGYPLUS_DIR + '\\WeatherData\\LOCAL_CLIMATE.epw')
    def get_weather_data_filename(self):
        """
        :return: filename under which weather data must be saved.
        """
