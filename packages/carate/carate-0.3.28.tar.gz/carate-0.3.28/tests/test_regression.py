...
def test_regression_override():
    ...
    verify_len_json(result_dir, dataset_name=run_title)
...
def test_molecule_net_regression(): 

    #check_dir_paths()
    config_filepath = "tests/config/regression_moleculenet.py"
    run_title = "ESOL"
    data_set_name = "ESOL"
    runner = RunInitializer.from_file(config_filepath=config_filepath)
    result_dir = f"tests/results/{run_title}"
    assert str(runner.data_set) == "StandardMoleculeNet"
    runner.run()  # takes instance attributes as parameters for the run() function
