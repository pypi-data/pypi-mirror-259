...
class Attributes(IntEnum):
    ...
    ATTR_C_GA_SLOT_ADDRESS          = 0x1103,   # Retrieve Global address slot address of a PXIe card (DWORD)
...
class Pi_Base:
    ...
    def Discover(self, port=1024, timeout=5000, address=None):
        ...
        LXIcount.value = self._handleCMLX.PICMLX_GetAvailableLXICount()
...
class Pi_Card_Base(Pi_Base):
    ...
    def Diagnostic(self):
        ...
        err = self._handlePLX.PIPLX_Diagnostic(self.session, self.card, ctypes.byref(diag_string))
        self._handlePIPLXError(err)
    ...
    def SetAttenuation(self, subunit, attenuation):
        err = self._handlePLX.PIPLX_AttenSetAttenuation(self.session, self.card, int(subunit), float(attenuation))
        self._handlePIPLXError(err)
        return
    ...
    def ResGetResistance(self, subunit):
        ...
        err = self._handlePLX.PIPLX_ResGetResistance(self.session, self.card, int(subunit), ctypes.byref(resistance))
        self._handlePIPLXError(err)
    ...
    def SetAttributeDWORD(self, subunit, outputSubunit, attribute, value):
        ...
        self.self._handlePIPLXError(err)
