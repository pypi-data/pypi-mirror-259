...
class Quill:
    ...
    def query(self, org_id, data):
        ...
        try:
            ...
            final_query_results = self.run_queries(
                quill_results.get("queries"), metadata, metadata.get("runQueryConfig")
            )
    ...
    def run_queries(self, queries, pkDatabaseType, databaseType, metadata=None, runQueryConfig=None):
        ...
        if not queries:
            return {"queryResults": []}
        if databaseType and databaseType != pkDatabaseType:
        ...
        if runQueryConfig and runQueryConfig.get("arrayToMap"):
            ...
            query_results = [self.target_connection.query(query + ' limit 1') for query in queries]
            results["columns"] = [{'name': result['name'], 'dataTypeID': convert_type_to_postgres(result['dataTypeID'])} for result in query_results[0]['fields']]
