...
class ConfigurationFile(object):
    ...
    def __init__(self, fn = None, base_dir=None):
        # Needed for calculations with is_filename

        self.base_dir = os.path.abspath(os.path.split(fn)[0]) if (fn and not base_dir) else base_dir
        self._conf = ConfigParser(strict=False)
        self._override_values = collections.defaultdict(lambda : collections.defaultdict(lambda : None))

        if fn:
          if not os.path.isfile(fn):
              err_msg = "No such file: '{}'".format(fn)
              raise IOError(err_msg)
          self._conf.read(fn)
        return

    def override(self, section, key, value):
      self._override_values[section][key] = str(value)
      return
    ...
    def get(self, sect, key, default=None, mandatory=True,
        ...
        if mandatory and not self.has(sect, key) and not self._override_values[sect][key]:
            err_msg = "Missing configuration exception '{0}::{1}'".format(sect, key)
            raise KeyError(err_msg)

        if not self.has(sect, key) and not self._override_values[sect][key]:
            value = default
            if value is None: return
            if not isinstance(value, str): return value
        else:

            if not (value := self._override_values[sect][key]):
...
class Configuration(object):
    ...
    def __init__(self, *config_fn_list):
        base_dir = os.path.split(os.path.abspath(config_fn_list[0]))[0]
        self._config_list = [ConfigurationFile(base_dir=base_dir)] + [ConfigurationFile(fn) for fn in config_fn_list]
        return

    def override(self, section, key, *args, **kwds):
        return self._config_list[0].override(section, key, *args, **kwds)

    def set_override_basedir(self, base_dir):
        self._config_list[0].set_base_dir(base_dir)
        return
