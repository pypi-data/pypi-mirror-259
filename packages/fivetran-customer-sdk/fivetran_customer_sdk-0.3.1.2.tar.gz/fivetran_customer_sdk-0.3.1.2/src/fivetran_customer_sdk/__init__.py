...
class Connector(connector_sdk_pb2_grpc.ConnectorServicer):
    ...
    def deploy(self, project_path: str, deploy_key: str, group: str, connector: str, configuration: dict = None):
        ...
        if configuration:
            if "secrets" in configuration:
                for k, v in configuration['secrets']:
                    secrets_list.append({"key": k, "value": v})
            if "payloads" in configuration:
                for k, v in configuration['payloads']:
                    payloads_list.append({"key": k, "value": v})
        group_id, group_name = self.__get_group_id(group, deploy_key)
        print(f"Deploying '{project_path}' to '{group_name}/{connector}' using key '{deploy_key}'")
        ...
        if not self.__connector_exists(connector, group,group_id, deploy_key):
            configuration = {
                "schema": connector,
            ...
            response = self.__create_connector(deploy_key, group_id, configuration)
            if response.ok:
    ...
    @staticmethod
    def __connector_exists(name: str, group: str, group_id: str, deploy_key: str):
        resp = requests.get(f"https://api.fivetran.com/v1/groups/{group_id}/connectors",
        ...
        if not resp.json()['data']['items']:
            return False

        print(f"Connector '{name}' already exists in group {group}")
        return True

    @staticmethod
    def __create_connector(deploy_key: str, group_id: str, configuration: dict):
        response = requests.post(f"https://api.fivetran.com/v1/connectors",
                                 headers={
    ...
    def __upload(local_path: str, deploy_key: str, group_id: str, connector: str):
        response = requests.post(f"https://api.fivetran.com/v2/deploy/{group_id}/{connector}",
                                 files={'file': open(local_path, 'rb')},
                                 headers={"Authorization": f"Basic {deploy_key}"})
        if response.ok:
            print("Deploy was successful")
        else:
            print("Error deploying:", response.reason)
    ...
    @staticmethod
    def __get_group_id(group: str, deploy_key: str) -> tuple[str, str]:
        resp = requests.get("https://api.fivetran.com/v1/groups",
                            headers={"Authorization": f"Basic {deploy_key}"})
