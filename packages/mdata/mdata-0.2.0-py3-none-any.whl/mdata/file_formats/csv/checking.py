...
from mdata.core.shared_defs import ObservationTypes, ObservationConcepts
...
def check_if_readable_header_definition_file(file: DataSource, do_full_check=True):
    ...
    if do_full_check:
        for k, row in enumerate(lines_from, start=1):
            if row[0] == ObservationConcepts.Type and row[1] == ObservationConcepts.Label:
                # skip header if included
                continue

            row_identifier = row[0]
            non_empty_idx = [0]
            if row_identifier in ObservationTypes or row_identifier in {segments_ext.CSV_KEY,
                ...
                spec_type = row[2]
                if spec_type in ObservationTypes or spec_type in {segments_ext.CSV_KEY, segments_ext.CSV_KEY_DATA}:
