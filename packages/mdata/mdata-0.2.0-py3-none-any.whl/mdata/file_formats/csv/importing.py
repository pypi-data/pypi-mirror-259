...
from mdata.core.extensions import metadata_ext, segments_ext, directionality_ext, registry
...
def read_raw_observations(source: DataSource) -> pd.DataFrame:
    ...
    with use_for_pandas_io(source) as f:
        df = pd.read_csv(f, parse_dates=False, low_memory=False)
...
def read_raw_header_csv(source: DataSource) -> raw_v2.RawHeaderSpecV2:
    ...
    def get_datapointspec_dict(row: list[str]):
        ...
        elif type_spec_key == segments_ext.CSV_KEY_DATA:
            if 'segmentdata_specs' not in result:
                result['segmentdata_specs'] = {}
            subdict = result['segmentdata_specs']
        return subdict

    def get_segmentspec_list(row: list[str]):
        type_spec_key = row.pop(0)
        if type_spec_key == segments_ext.CSV_KEY:
    ...
    for row in read_csv_lines_from(source):
        ...
        if statement_identifier == registry.CSV_KEY:
        ...
        elif statement_identifier in {ObservationTypes.E, ObservationTypes.M, segments_ext.CSV_KEY_DATA}:
            specs = get_datapointspec_dict(row)
            label = row.pop(0)
            specs[label] = [f for f in row if f != '']
        elif statement_identifier == segments_ext.CSV_KEY:
            ...
            row.pop(0)
            key = row.pop(0)
            if row[0] in {ObservationTypes.E, ObservationTypes.M, segments_ext.CSV_KEY_DATA}:
                ...
                for j, f_spec in enumerate(features):
                    if type(f_spec) is str and target_feature == f_spec:
                        ...
                        break
            elif row[0] == segments_ext.CSV_KEY:
                relevant_list = get_segmentspec_list(row)
                label = row.pop(0)
