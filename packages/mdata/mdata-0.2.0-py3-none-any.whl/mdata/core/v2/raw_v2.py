...
from mdata.core.v2.shared_defs import SegmentPropertyValue, SegmentConcepts, SegmentDefinitionType
...
def squash_segment_data_series_containers(series: Collection[SDSContainer]) -> pd.DataFrame:
    ...
    for s_container in series:
        ...
        df.columns = SegmentConcepts.segmentdata_columns + cs
        dfs.append(df)
...
pd.DataFrame, list[SContainer]):
    ...
    categories = derive_categoricals(df,
                                     [SegmentConcepts.Object, SegmentConcepts.Segment, SegmentConcepts.Label])

    overall = pd.DataFrame(df, columns=SegmentConcepts.segmentdata_columns, copy=True)
    ...
    series_containers = []
    for group, idx in overall.groupby([SegmentConcepts.Label],
                                      observed=True).groups.items():
        label = group
        spec: SegmentDataSpec = header.lookup_spec(SegmentDefinitionType.SegmentData, label)
        sds_spec: SegmentDataSeriesSpec = factory.make_segment_data_spec(label, spec)
        ...
        df_g = pd.concat(
            [overall.loc[idx, SegmentConcepts.segmentdata_columns], df.loc[idx, placeholder_feature_labels]],
    ...
    placeholder_cols = list(set(overall.columns).difference(SegmentConcepts.segmentdata_columns))
...
def gen_segments_container(seg_df, header, factory) -> SSContainer:
    ...
    segment_names = seg_df[SegmentConcepts.Segment].unique()
    seg_specs = []
...
def create_machine_data_from_raw(raw_md: RawMD, sort_by_time=False, sort_by_index=False) -> MD:
    ...
    from mdata.core.factory import get_factory, ExtendedFactory
    factory: ExtendedFactory = get_factory(header.meta.extensions)
    ...
    from mdata.core import ObservationType
    return factory.make(meta=header.meta,
                        events=(tsc for tsc in series_containers if tsc.observation_type == ObservationType.E),
                        measurements=(tsc for tsc in series_containers if tsc.observation_type == ObservationType.M),
