...
class HeaderV2(Header):
    ...
    def lookup_spec(self, spec_type, label) -> DatapointSpec | SegmentSpec:
        ...
        elif spec_type == SegmentDefinitionType.Segments:
            return self.segment_specs.get(label)
        elif spec_type == SegmentDefinitionType.SegmentData:
            return self.segment_data_specs.get(label)
        else:
            raise KeyError(f'unknown spec type {spec_type}')
