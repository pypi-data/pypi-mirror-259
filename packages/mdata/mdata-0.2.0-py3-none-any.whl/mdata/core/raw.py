...
from .shared_defs import ObservationConcepts, ObservationType
...
def convert_to_raw_data_legacy(md: MD) -> pd.DataFrame:
    ...
    for ts_container in md.series_containers:
        ...
        for tup in df.itertuples(index=True):
            rows.append(
                [getattr(tup, ObservationConcepts.Time), getattr(tup, ObservationConcepts.Object), tt.type,
...
def split_squashed_df_into_ts_series_containers(df: pd.DataFrame, header, factory, sort_by_time=False) -> (pd.DataFrame, list[SContainer]):
    categories = derive_categoricals(df,
                                     [ObservationConcepts.Object, ObservationConcepts.Label, ObservationConcepts.Type])
    ...
    series_containers = []
    for group, idx in overall.groupby([ObservationConcepts.Type, ObservationConcepts.Label],
