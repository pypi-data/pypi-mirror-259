...
from mdata.core.shared_defs import Extension, SpecLabel, ObservationConcepts, ExtendedObservationConcepts, \
    MeasurementSpecLabel, EventSpecLabel, SeriesFeatureLabel, ObservationTypes, ObservationTypeValue
...
from mdata.core.v2.shared_defs import SegmentSpecLabel, SegmentConcepts, SegmentProperty, SegmentDataSpecLabel, \
    ExtendedSegmentConcepts, SpecIdentifier, SegmentDefinitionTypes
...
class SegmentDataSeriesSpec(SeriesSpec[SegmentDataSpecLabel, SegmentDataSpec], SegmentDataSpecProtocol):

    @classmethod
    def of(cls, label: SpecLabel, base_spec: SegmentDataSpec) -> Self:
        return super().of(label, base_spec)

    @property
    def identifier(self) -> SegmentDataSpecLabel:
...
class SegmentSeriesSpec(SegmentSeriesSpecProtocol):
    _label: SegmentSpecLabel
    _base: SegmentSpec

    @classmethod
    def of(cls, label: SegmentSpecLabel, base: SegmentSpec) -> Self:
        return cls(label, base)

    @property
    def label(self) -> SegmentSpecLabel:
        return self._label
...
class SegmentDataContainer(AbstractSeriesContainer[SDSSpec, SDSView],
    ...
    def merge(self, other: Self,
        ...
        if axis == 'horizontal':
            ...
            if new_fs:
                assert self.df.loc[:,
                       [SegmentConcepts.Segment, SegmentConcepts.Index, SegmentConcepts.Object]].equals(
                    other.df.loc[:, [SegmentConcepts.Segment, SegmentConcepts.Index, SegmentConcepts.Object]])
            ...
            df.sort_values([SegmentConcepts.Segment, SegmentConcepts.Index], ignore_index=True, inplace=True)
            return self.__class__(self.series_spec, df)


class SegmentsContainer(SegmentSeriesContainerProtocol[SegmentSeriesSpec]):

    def __init__(self, segment_specs: Mapping[SegmentSpecLabel, SegmentSeriesSpec], df: pd.DataFrame) -> None:
        self.df = df.set_index(
            keys=[SegmentConcepts.Object, SegmentConcepts.Segment, SegmentConcepts.Index]).sort_index()
    ...
    def empty(cls) -> Self:
        return SegmentsContainer.of({}, pd.DataFrame(
            columns=[SegmentConcepts.Object, SegmentConcepts.Segment, SegmentConcepts.Index]).astype(
            {SegmentConcepts.Object: 'str', SegmentConcepts.Segment: 'str', SegmentConcepts.Index: 'int'}))

    @classmethod
    def of(cls, segment_specs: Iterable[SegmentSeriesSpec] | Mapping[SegmentSpecLabel, SegmentSeriesSpec],
           df: pd.DataFrame) -> Self:
        if isinstance(segment_specs, list):  # this is unclean
            segment_specs = typing.cast(Iterable[SegmentSeriesSpec], segment_specs)
            segment_specs = {s.label: s for s in segment_specs}
    ...
    @property
    def segment_specs(self) -> Mapping[SegmentSpecLabel, SegmentSeriesSpec]:
    ...
    def get_intervals(self, segment_label: SegmentSpecLabel, objs: StrIndexer) -> pd.DataFrame:
        ...
        outer_indices = [SegmentConcepts.Segment]
        if isinstance(objs, str):
            outer_indices = [SegmentConcepts.Object] + outer_indices
        return sliced.droplevel(level=outer_indices, axis=0)

    def view(self, segment_label: SegmentSpecLabel, objs: StrIndexer = slice(None)) -> pd.DataFrame:
        return self.get_intervals(segment_label, objs=objs)

    def gen_interval_index(self, segment_label: SegmentSpecLabel, objs: StrIndexer) -> pd.MultiIndex:
    ...
    def segment_df(self, observations: pd.DataFrame, by: SegmentSpecLabel, obj: str,
    ...
    def segment(self, tsc: TSContainer, by: SegmentSpecLabel, objs: StrIndexer = None) -> TSContainer:
        raise NotImplemented
        intervals = self.get_intervals(by, objs)
        intervals.apply()
        sliced = self.df.loc[pd.IndexSlice[:, by, :], [SegmentConcepts.Start, SegmentConcepts.End]]
        sliced = sliced.droplevel(level=SegmentConcepts.Segment, axis=0)
...
class AbstractMachineDataV2(MachineDataV2Protocol[
    ...
    def __init__(self, meta: Meta, events: Iterable[ETSC], measurements: Iterable[MTSC],
        ...
        self._segment_data: Mapping[SegmentDataSpecLabel, SDSContainer] = immutabledict(
            {sdc.series_spec.label: sdc for sdc in (segment_data if segment_data is not None else ())})
        ...
        self._segment_specs: Optional[Mapping[SegmentSpecLabel, SSSpec]] = None
        self._segment_data_specs: Optional[Mapping[SegmentDataSpecLabel, SDSSpec]] = None
    ...
    def _repopulate_maps(self):
        self._segment_data_specs = immutabledict(
            {sdc.series_spec.label: sdc.series_spec for sdc in self.segment_data.values()})
        self._segment_specs: Mapping[SegmentDataSpecLabel, SSSpec] = immutabledict(
    ...
    @property
    def segment_data(self) -> Mapping[SegmentDataSpecLabel, SDSContainer]:
        return self._segment_data

    @property
    def segment_specs(self) -> Mapping[SegmentSpecLabel, SSSpec]:
        if self._segment_specs is None:
            self._repopulate_maps()
        return self._segment_specs

    @property
    def segment_data_specs(self) -> Mapping[SegmentSpecLabel, SDSSpec]:
    ...
    def project(self, *, measurement_feature_selection: Optional[Mapping[
        ...
        Mapping[EventSpecLabel, bool | Collection[SeriesFeatureLabel]]] = None, project_underlying_dfs=False,
                ...
                SegmentSpecLabel, bool | Collection[SeriesFeatureLabel]]] = None, **kwargs) -> Self:
        raise NotImplemented
    ...
    def __str__(self):
        bmd_string = super().__str__()[:-1]
        return (
                'Extended' + bmd_string + f'Segment: {[f"{ss} ({len(self.segments.view(ss))})" for ss in self.segment_specs]}' + '\n'
                + f'Segment Data Specs: {[f"{sds} ({len(self.segment_data[sds].datapoint_count)})" for sds in self.segment_data_specs]}' + '\n' + '}')
    ...
    def get_spec(self, identifier: SpecIdentifier) -> ETSSpec | MTSSpec | SSSpec | SDSSpec:
        t, label = identifier
        if t in ObservationTypes:
            t: ObservationTypeValue = t
            return self.bmd.get_spec((t, label))
        else:
            match t:
                case SegmentDefinitionTypes.Segments:
                    return self.segment_specs[label]
                case SegmentDefinitionTypes.SegmentData:
                    return self.segment_data_specs[label]
                case _:
                    raise KeyError

    def fit_to_data(self, ignore_index=False):
        self.bmd.fit_to_data(ignore_index=ignore_index)
        self.segments.fit_to_data()
        for sdsc in self.segment_data.values():
            sdsc.fit_to_data()
    ...
    def create_observation_index_view(self,
                                      types: Optional[ObservationTypeValue | Iterable[ObservationTypeValue]] = None,
                                      objs: Optional[str | Iterable[str]] = None,
                                      labels: Optional[SpecLabel | Iterable[SpecLabel]] = None) -> pd.DataFrame:
        return self.bmd.create_observation_index_view(types, objs, labels)
