...
from mdata.core.shared_defs import ObservationType, ObservationSpecIdentifier, ObservationTypes, \
    ObservationTypeValue, ObservationConcepts, only_feature_columns, Extension

md_columns_def = {ObservationConcepts.Object: str, ObservationConcepts.Time: str,
                  ObservationConcepts.Label: str, ObservationConcepts.Time + '_col': str,
                  ObservationConcepts.Object + '_col': str,
                  ObservationConcepts.Type + '_col': str, ObservationConcepts.Label + '_col': str}
...
def prepare_observations_df(df: pd.DataFrame, copy=False, sort_by_time=False, **kwargs: MDColumnsDef) -> tuple[
    ...
    time_def = match_def_and_df(kwargs, df, ObservationConcepts.Time, 0,
                                fallback=lambda: df_utils.create_artificial_daterange(df), is_dt_type=True)
    object_def = match_def_and_df(kwargs, df, ObservationConcepts.Object, 1)
    type_def = match_def_and_df(kwargs, df, ObservationConcepts.Type, 2)
    label_def = match_def_and_df(kwargs, df, ObservationConcepts.Label, 3)
...
class TimeseriesSpecFactory(SpecFac[ObservationSpec, TSSpec]):
    ...
    def __init__(self, ts_spec_cls: type[TSSpec] = None, observation_type: ObservationType = ObservationType.E) -> None:
        ...
        self.constr = self.constructors[observation_type][ts_spec_cls]
