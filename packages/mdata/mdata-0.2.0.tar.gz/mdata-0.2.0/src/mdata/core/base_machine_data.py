...
from mdata.core.shared_defs import Extension, SeriesSpecMergeException
...
from .shared_defs import SeriesFeatureLabel, SeriesFeatureLabels, ObservationTypeValue, \
    ObservationType, SpecLabel, ObservationSpecIdentifier, EventSpecLabel, MeasurementSpecLabel, \
    ObservationTypes, ObservationConcepts, only_feature_columns
...
class SeriesSpec(SeriesSpecProtocol[Identifier, BaseSpec]):
    ...
    @classmethod
    def of(cls, label: SpecLabel, base_spec: BaseSpec) -> Self:
        return cls(label, base_spec, tuple((f.name for f in base_spec)))

    @property
    def label(self) -> SpecLabel:
        return self._label
    ...
    def project(self, feature_selection: bool | str | Collection[str]) -> Self:
        ...
        return self.__class__.of(self.label, self._base.__class__.of(*(self.base[f] for f in feature_selection)))
    ...
    def merge(self, other: Self) -> Self:
        ...
        return self.__class__.of(self.label, self._base.__class__.of(*specs))
...
class TimeseriesView(SeriesView[TSSpec], TimeseriesViewProtocol[TSSpec], Generic[TSSpec]):
    ...
    def observation_type(self) -> ObservationType:
        return self.series_spec.observation_type
...
class AbstractSeriesContainer(SeriesContainerProtocol[SSpec, SView], Generic[SSpec, SView], ABC):
    ...
    def _update_series_spec(self, series_spec: SSpec = None) -> None:
        self.series_spec = self._derive_series_spec() if series_spec is None else series_spec
        assert self._check_series_features()

    def _derive_series_spec(self) -> SSpec:
        ...
        return self._s_spec_cls.of(self.series_spec.label, self.series_spec.base.__class__.of(*specs))
...
class AbstractMachineData(MachineDataProtocol[ETSSpec, ETSView, ETSC, MTSSpec, MTSView, MTSC],
    ...
    def __init__(self, meta: Meta, events: Iterable[ETSC],
        ...
        self._event_series: Mapping[EventSpecLabel, ETSC] = immutabledict(
            {etc.series_spec.label: etc for etc in events})
        self._measurement_series: Mapping[MeasurementSpecLabel, MTSC] = immutabledict(
            {mtc.series_spec.label: mtc for mtc in
             measurements})
    ...
    def _repopulate_maps(self):
        self._series_containers = frozenset(
            itertools.chain(self.event_series.values(), self.measurement_series.values()))
        self._event_specs = immutabledict({es.series_spec.label: es.series_spec for es in
                                           self.event_series.values()})
        self._measurement_specs = immutabledict({ms.series_spec.label: ms.series_spec for ms in
    ...
    def create_observation_index_view(self,
        ...
        if isinstance(labels, SpecLabel):
            mask &= (self.observation_index[ObservationConcepts.Label] == labels)
        elif labels is not None:
            mask &= (self.observation_index[ObservationConcepts.Label].isin(mangle_arg_to_set(labels)))
        if isinstance(types, ObservationTypeValue):
...
def build_shared_index(series: Iterable[TSContainer], index_cols=None,
    ...
    cats = derive_categoricals(frame, [ObservationConcepts.Object, ObservationConcepts.Type, ObservationConcepts.Label])
