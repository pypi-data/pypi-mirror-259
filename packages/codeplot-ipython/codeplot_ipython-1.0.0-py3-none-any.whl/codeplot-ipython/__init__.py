...
class Magics(Magics):
    ...
    @line_magic
    def cP_cell_id(self, id):
        "Define the cell id/plot id in a magic."
        _cP_extension = self.shell.user_ns.get('_cP_extension')
        _cP_extension._magic_current_cell_id = id
...
class IPythonExtension:
    ...
    def post_run_cell(self, result):
        ...
        if result.result is not None and self.cP is not None:
            ...
            # Construct the kwargs dictionary conditionally
            kwargs = {}

            # Only add 'id' if it's not None
            if result.info.cell_id is not None:
                kwargs["id"] = result.info.cell_id
                kwargs["ipython_cell_id"] = result.info.cell_id
            if self._magic_current_cell_id is not None:
                kwargs["id"] = self._magic_current_cell_id
                kwargs["ipython_cell_id"] = self._magic_current_cell_id
            if result.info.raw_cell is not None:
                kwargs["ipython_raw_cell"] = result.info.raw_cell

            asyncio.create_task(self.cP.plot(result.result, **kwargs))
        if self._supress_output and self.connection_established is True and result.result is not None:
