...
class StrangeworksBinaryQuadraticModelJiJProblemConverter(StrangeworksConverter):
    ...
    def convert(self) -> tuple[jm.Problem, dict[str, np.ndarray], Any]:
        ...
        if self.model.offset != 0:
            Qmat += np.eye(self.model.num_variables) * self.model.offset
...
class StrangeworksBinaryQuadraticModelHitachiDictConverter(StrangeworksConverter):
    ...
    def convert(self) -> tuple[HitachiModelList, dict[Any, Any]]:
        ...
        def get_target_graph(machine_version):
            # Set the size of the target graph
            if machine_version == "ASIC":
                ll = 384
            elif machine_version == "GPU":
                ll = 512
            else:
                raise ValueError("machine_type must be 'ASIC' or 'GPU'.")
        ...
        def get_hitachi(linear: dict, quadratic: dict, offset: float = 0.0):
            out_list = []
        ...
        target_graph = get_target_graph("GPU")
        embedding = find_embedding(problem_graph, target_graph)

        target_linear, target_quadratic = embed_ising(linear, quadratic, embedding, target_graph)
        target_list = get_hitachi(target_linear, target_quadratic, bqm.offset)

        return HitachiModelList(target_list), embedding


class StrangeworksBinaryQuadraticModelToshibaDictConverter(StrangeworksConverter):
    def __init__(self, model: BinaryQuadraticModel):
        self.model = model

    def convert(self) -> tuple[ToshibaModelList, dict[Any, Any]]:
        bqm = self.model
        mapping = {}
        iter = 0
        ...
        for k, v in bqm.linear.items():
            qubo_str += f"\n{mapping[k] + 1} {mapping[k] + 1} {int(v)}"
        for k, v in bqm.quadratic.items():
            if mapping[k[0]] > mapping[k[1]]:
                qubo_str += f"\n{mapping[k[0]] + 1} {mapping[k[1]] + 1} {int(v)}"
            else:
                qubo_str += f"\n{mapping[k[1]] + 1} {mapping[k[0]] + 1} {int(v)}"

        return ToshibaModelList(qubo_str), mapping
...
class StrangeworksConverterFactory:
    ...
    def from_model(model_from: Any, model_to: Any) -> StrangeworksConverter:
        if isinstance(model_from, BinaryQuadraticModel) and model_to == jm.Problem:
            ...
            return StrangeworksBinaryQuadraticModelHitachiDictConverter(model=model_from)
        elif isinstance(model_from, BinaryQuadraticModel) and model_to == ToshibaModelList:
            return StrangeworksBinaryQuadraticModelToshibaDictConverter(model=model_from)
        else:
