...
@njit(fastmath=True)
def unique(x: np.ndarray) -> np.ndarray:
    """Return the unique values of an array.

    This function is necessary to JIT the :func:`np.unique` function.
...
@njit(parallel=True, fastmath=True)
def ppf(p: np.ndarray) -> np.ndarray:
...
def polyserial(x: np.ndarray, y: np.ndarray) -> float:
    ...
    p = np.cumsum(xt)[:-1] / _sum(xt)
    tau = ppf(p)
    return pearson(x, y) * np.std(x) / _sum(pdf(tau))
...
def _pxy(p: np.ndarray) -> np.ndarray:
    """Apply the ppf function to the probabilities padded with -23 and 23.

    This wrapper function is necessary to JIT the :func:`ppf` function.
...
@njit(parallel=True, fastmath=True)
def _uniques(data: np.ndarray) -> np.ndarray:
    """Return the number of unique values in each column of the input array.
...
@njit(parallel=True, fastmath=True)
def _uniques_m(data: np.ndarray) -> np.ndarray:
    """Return the number of unique values in each column of the input array.
    ...
    """
    out = np.empty(data.shape[2], dtype=np.int64)
...
def parallel_analysis(
    ...
    times; (5) calculate the `q`th ppf of the distribution of each random-data
    ...
    eigenvalues that are greater than their corresponding ppf until we encounter
    the first eigenvalue that is not greater than its ppf".
    ...
    Args:
        ...
        quantile: Must be a float in the range ``0<q<1``. Defaults to ``0.95``.
    ...
    crit = np.quantile(eigs, quantile, axis=0)
...
def factor_analysis(
    ...
    if factors is None:
        factors = parallel_analysis(data, analysis_type="fa")
