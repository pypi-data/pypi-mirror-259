...
@refreshToken
def get_standard_requirement_compliance_statistics(prismaSession: PrismaSession, standard_id, requirement_id, account_name = None) -> json:
    """
    Obtiene un json con las estadisticas de un requerimiento de standard de Prisma.

    :param prismaSession: datos de la sesión con el tenant (url, token)
    :param standard_id: ID del standard
    :param requirement_id: ID del standard
    :return: json con estadisticas del standard de Prisma
    """
    ...
    if (account_name != None):
        querystring["cloud.account"] = account_name



    response = requests.request(
        "GET", prismaSession.apiUrl + '/compliance/posture/' + standard_id + '/' + requirement_id, headers=headers,
        params=querystring)
...
def config_search(prismaSession: PrismaSession, query, timerange) -> json:
    """
