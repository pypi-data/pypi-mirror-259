...
def text2image(
        ...
        base_model: str,
        vae: str,
        lora_specs: list[tuple[str, str | float | None]],
        embedding_specs: list[tuple[str, str | float | None]],
        # ------
        prompt: str,
        neg_prompt: str,
        scheduler: str,
    ...
    lora_specs = [(k, float(v)) for k, v in lora_specs] if lora_specs else []
    embedding_adapters = {i[0] for i in embedding_specs} if embedding_specs else set()
    lora_adapters = {i[0] for i in lora_specs}
    ...
    m.init(base_model=base_model,
           ...
           embedding_adapters=list(embedding_adapters))
    ...
    res = m.text2image(prompt=prompt,
        ...
        with gr.Group():
            ...
            with gr.Accordion("additional networks", open=False):
                with gr.Row():
                    ...
                    self.enable_ip_adapter = gr.Checkbox(value=False, label="enable ip_adapter")
                    self.enable_controlnet = gr.Checkbox(value=False, label="enable controlnet")
...
class InferenceUI(UI):
    ...
    def _build(self):
        with gr.Group():
            with gr.Row():
                self.prompt = gr.Textbox(label="prompt", placeholder="inputs prompt", lines=4, max_lines=4)
                self.neg_prompt = gr.Text(label="neg prompt", placeholder="inputs negative prompt", lines=4,
                                          max_lines=4)
            with gr.Row():
        ...
        self.gallery = gr.Gallery(label="output", columns=4, object_fit="contain", height=600)
    ...
    def run(self, network_type, base_model, vae, load_lora, load_embedding):
        ...
        self.inference.click(text2image, inputs, [self.gallery])


@dataclass
class _Text2ImgUI(UI):
    network_type: NetworkType = None
    hubs: dict[tuple[NetworkType, ModelType], CachedModelHub] = None
...
class Text2ImgUI(UI):
    ...
    def _build(self):
        ...
        self.network_type = gr.Dropdown(choices=network_types,
                                        ...
                                        interactive=True,
                                        label="network type")
        blocks = [_Text2ImgUI(hubs=self.hubs, network_type=NetworkType.SD_1_5),
                  _Text2ImgUI(hubs=self.hubs, network_type=NetworkType.SDXL_1_0)]
