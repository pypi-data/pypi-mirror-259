...
def get_tavily_api_key():
    return os.environ["TAVILY_API_KEY"] if "TAVILY_API_KEY" in os.environ else lambda_fetch_tavily_api_key()


def lambda_fetch_tavily_api_key():
    return lambda_fetch_secret("TAVILY_API_KEY")


def setup_tavily_search_action():
    add_tool(TAVILY_SEARCH_ACTION_NAME, tavily_search_definition(), tavily_search_action, web=True)


def tavily_search_tool():
    return KomodoTool(name=TAVILY_SEARCH_ACTION_NAME,
                      purpose="Accesses real-time data and insights from a wide range of internet sources, tailored to specific queries",
                      definition=tavily_search_definition(),
                      action=tavily_search_action)
