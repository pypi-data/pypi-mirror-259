...
from DLMS_SPODES.enums import (
    ...
    VariableAccessSpecification, AcseServiceUser, RelationGroup
)
...
class Client:
    ...
    async def init_type(self):
        ...
        try:
            ...
            new_collection = Collection(ldn=ldn)
            # read association
            ...
            for c_id, ver, ln, _ in object_list:
                ...
                if new_obj.CLASS_ID == overview.ClassID.ASSOCIATION_LN:  # search association for keep object_list into
                    await self.read_attr(new_obj.get_attr_descriptor(3))
                    if new_obj.associated_partners_id.client_SAP == self.SAP:
                        new_obj.set_attr(2, object_list)
            new_collection.get_class_version()
            self.objects = new_collection
            a = self.objects.LDN
            # read all DeviceIDObjects
            for d_id in collection.group_filter(new_collection, (RelationGroup.DEVICE_ID_OBJECTS, RelationGroup.OTHER_ABSTRACT_GENERAL_PURPOSE_OBIS_CODES)):
                await self.read_attribute(d_id, 2)
        ...
        try:
            ...
            for ex in exchanges:
                match ex:
                    ...
                    case tasks.ReadAttribute():
                        # TODO: check is_readable
                        await self.read_attribute(ex.ln, ex.index)
                    case tasks.WriteAttribute():
                        if self.objects.is_writable(
                                ln=(obj := self.objects.get_object(ex.ln)).logical_name,
                                index=2,
                                association_id=self.objects.get_association_id(self.SAP)):
                            if isinstance(ex.value, str):
                                value2 = await self.encode(
                                    obj=obj,
                                    index=ex.index,
                                    value=ex.value)
                                enc = value2.encoding
                                ...
                                enc = ex.value
                            await self.write_attr(
                                obj=obj,
                                attr_index=ex.index,
                                value=enc)
                        else:
                            raise exc.ITEApplication(F"Access Error. From current association: {obj}: {ex.index} is not writable")
                    case _:
            ...
            self.set_error(Transmit.NO_TRANSPORT, F"При соединении{e}")
        except Exception as e:
