...
def add_submit_cli_options(*model_classes):
    ...
    def generate_cli_option(field_name, model_field):
        ...
        multiple = model_field.outer_type_ == typing.List[field_type]
        option = click.option(f'--{field_name}',
                type=field_type,
                multiple=multiple,
    ...
    def decorator(func):
        ...
        for field_name, model_field in cli_param_fields.items():
            ...
            warpped_func = option(wrapped_func)
...
def add_formatter_cli_options(model_class):
    def decorator(func):
        ...
        for formatter_field in FORMATTER_FIELDS:
            option = click.option(f"--{formatter_field.replace('_','-')}",
                type=str,
