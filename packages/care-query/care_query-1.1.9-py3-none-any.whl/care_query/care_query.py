...
class CareQuery:
    ...
    def orgPayorAggs(self,
        ...
        Parameters:
            ...
            npi (list(string)):
                ...
                option(s): Any valid, registered individual or organizational National Provider Identifier in the US
                default(s): Defaults to all National Provider Identifiers (NPIs) if left unspecified
            name (str or list(str) or list(list(str),list(str))):
                Name of the organization or individual provider(s) of interest
                option(s): Any valid, registered organization or individual provider name in the US
                default(s): Defaults to all organization or individual provider names if left unspecified
        ...
        if min_year == None:
            raise ValueError(
                f"min_year is not specified: Please specify the date range minimum for the query to be executed.")
        if max_year == None:
            raise ValueError(
                f"max_year is not specified: Please specify the date range maximum for the query to be executed.")
        if min_month == None:
            min_month = 1
        if max_month == None:
            max_month = 12
        ...
        # ensure valid mixed data type
        mixed = {"payor_channel": payor_channel, "payor": payor, "name": name, "npi": npi}
        ...
        # ensure valid integer data type
        ints = {"limit": limit, "min_year": min_year, "max_year": max_year, "min_month":min_month, "max_month": max_month}
        for x in list(ints.items()):
            if (x[1] != None):
                cqu.checkTypeInt(x[1], x[0])
        ...
        # diagnosis hierarchy logic
        # diags, diag_greedy, warnings_list1 = cqu.diagnosisPriority(diag_code, short_diag_code, diag_subcategory,
        #                                                            diag_category, diag_greedy, self.field_values.data)

        # procedure hierarchy logic
        # procs, proc_greedy, warnings_list2 = cqu.procedurePriority(proc_code, proc_subcategory, proc_category,
        #                                                            proc_greedy, self.field_values.data,
        #                                                            query_type="supplemental")

        # warning around drg and diag/proc overlap
        warnings_list3 = []  # cqu.checkDRGOverlap(drg_code, diags, procs)

        # procedure hierarchy logic
        # taxonomies, taxonomy_greedy, warnings_list4 = cqu.taxonomyPriority(taxonomy_code, subspecialty, specialty,
        #                                                                    specialty_category, taxonomy_greedy,
        #                                                                    self.field_values.data)
        # cqu.checkLevels(taxonomies, "taxonomies", levels=2)

        # geography hierarchy priority
        # states, zips, warnings_list5 = cqu.geoPriority(state, division, region, metro, short_zip,
        #                                                self.field_values.data)
    ...
    def orgPayorDiagAggs(self,
        ...
        Description:
            ...
            Query the aggregated payor mix for an organization or individual provider(s) by payor, payor channel, and diagnosis code.
        ...
        Parameters:
            ...
            payor_channel (str or list(str) or list(list(str),list(str))):
                ...
                default(s): Defaults to all payor channel names if left unspecified
            diag_code (str or list(str) or list(list(str),list(str))):
                International Classification of Diseases (ICD) diagnosis code(s) of interest
                option(s): Any valid, registered ICD diagnosis code in the US
                default(s): Defaults to all ICD diagnosis codes if left unspecified
            diag_position (list(int)):
                Position of the diagnosis code in the encounter
                option(s): 1 to 5, or [1,3,5] to select multiple positions
                default: None
            show (bool):
                Binary indicator as to whether you'd like your query to be printed for review upon it's creation. Regardless, this summary may be accessed via the .summarize(show=True) method on the query object itself.
                option(s): True indicates print, False indicates do not print
        ...
        # ensure valid integer data type
        ints = {"limit": limit, "min_year": min_year, "max_year": max_year, "min_month": min_month,
                "max_month": max_month, "diag_position": diag_position}
        for x in list(ints.items()):
            if (x[1] != None):
                cqu.checkTypeInt(x[1], x[0])
        ...
        # make it pretty for Ray's SQL
        diag_code = cqu.stringNorm(cqu.listWrapString(diag_code), stringtype="upper")
        # procs = cqu.stringNorm(cqu.listWrapString(procs), stringtype="upper")
        # states = cqu.stringNorm(cqu.listWrapString(states), stringtype="upper")
        # taxonomies = cqu.stringNorm(cqu.listWrapString(taxonomies), stringtype="upper")
        payor_channel = cqu.stringNorm(cqu.listWrapString(payor_channel), stringtype="lower")
    ...
    def orgPayorProcAggs(self,
        ...
        Description:
            ...
            Query the aggregated payor mix for an organization or individual provider(s) by payor, payor channel, and procedure code.
        ...
        Parameters:
            ...
            payor_channel (str or list(str) or list(list(str),list(str))):
                ...
                default(s): Defaults to all payor channel names if left unspecified
            proc_code (str or list(str) or list(list(str),list(str))):
                Common Procedural Terminology (CPT) procedure code(s) of interest
                option(s): Any valid, registered ICD diagnosis code in the US
                default(s): Defaults to all ICD diagnosis codes if left unspecified
            show (bool):
                Binary indicator as to whether you'd like your query to be printed for review upon it's creation. Regardless, this summary may be accessed via the .summarize(show=True) method on the query object itself.
        ...
        # make it pretty for Ray's SQL
        proc_code = cqu.stringNorm(cqu.listWrapString(proc_code), stringtype="upper")
        payor_channel = cqu.stringNorm(cqu.listWrapString(payor_channel), stringtype="lower")
        payor = cqu.stringNorm(cqu.listWrapString(payor), stringtype="lower")
        npi = cqu.listWrapString(npi)
    ...
    def orgProcAggs(self,
        ...
        Description:
            ...
            Query the aggregated proc mix for an organization or individual provider(s) by procedure code.
    ...
    def payorDiagAggs(self,
        ...
        Description:
            ...
            Query the aggregated case mix for an payor, payor channel, and procedure code.
        ...
        # ensure valid mixed data type
        mixed = {"payor_channel": payor_channel, "payor": payor,
                 "diag_code": diag_code}
    ...
    def providerDiagAggs(self,
        ...
        Description:
            ...
            Query the aggregated diag mix for an organization or individual provider(s) by procedure code.
    ...
    def providerPayorAggs(self,
        ...
        Description:
            ...
            Query the aggregated case mix for an organization or individual provider(s) by payor and payor channel.
        ...
        '''
        return self.orgPayorAggs(min_year = min_year,
                          max_year = max_year,
                        min_month = min_month,
                        max_month = max_month,
                        npi = npi,
    ...
    def providerPayorDiagAggs(self,
        ...
        Description:
            ...
            Query the aggregated case mix for an payor, payor channel, and diagnosis code.
        ...
        Parameters:
            ...
            diag_code (str or list(str) or list(list(str),list(str))):
                ...
                default(s): Defaults to all ICD diagnosis codes if left unspecified
            diag_subcategory (str or list(str) or list(list(str),list(str))):
                Subcategory of the diagnosis code(s) of interest
                option(s): Any valid, registered ICD diagnosis subcategory in the US
                default(s): Defaults to all ICD diagnosis subcategories if left unspecified
            diag_position (list(int)):
                Position of the diagnosis code in the encounter
                option(s): 1 to 5, or [1,3,5] to select multiple positions
                default: None
            ...
            limit (int):
                ...
                default: 10000
        '''
        return self.orgPayorDiagAggs(min_year = min_year,
                                     max_year = max_year,
                        min_month = min_month,
                        max_month = max_month,
                        npi = npi,
    ...
    def providerPayorProcAggs(self,
        ...
        Parameters:
            ...
            limit (int):
                ...
                default: 10000
        '''
        return self.orgPayorProcAggs(min_year = min_year,
                                     max_year = max_year,
                        min_month = min_month,
                        max_month = max_month,
                        npi = npi,
    ...
    def procZipAggs(self,
        ...
        Description:
            ...
            Query the aggregated case mix for a procedure code and zip code.
        ...
        Parameters:
            ...
            proc_code (str or list(str) or list(list(str),list(str))):
                ...
                option(s): Any valid, registered ICD diagnosis code in the US
                default(s): Defaults to all ICD diagnosis codes if left unspecified
            short_zip (str or list(str) or list(list(str),list(str))):
                First three digits of the zip code of interest
                option(s): Any valid, registered first three digits of the zip code in the US
                default(s): Defaults to all first three digits of the zip code if left unspecified
            show (bool):
                Binary indicator as to whether you'd like your query to be printed for review upon it's creation. Regardless, this summary may be accessed via the .summarize(show=True) method on the query object itself.
                option(s): True indicates print, False indicates do not print
                default: True
        ...
        # ensure valid mixed data type
        mixed = {"proc_code": proc_code,"short_zip": short_zip}
        for x in list(mixed.items()):
            if x[1] != None:
                cqu.checkTypeMixed(x[1], x[0])

        # ensure valid integer data type
        ints = {"min_year": min_year, "max_year": max_year, "min_month": min_month, "max_month": max_month, "limit": limit}
        for x in list(ints.items()):
            if (x[1] != None):
                cqu.checkTypeInt(x[1], x[0])
        ...
        short_zip = cqu.listWrapString(short_zip)
    ...
    def procSpecialtyZipAggs(self,
        ...
        Description:
            ...
            Query the aggregated case mix for a procedure code, specialty, and zip code.
        ...
        Parameters:
            ...
            proc_code (str or list(str) or list(list(str),list(str))):
                ...
                default(s): Defaults to all ICD diagnosis codes if left unspecified
            specialty (str or list(str) or list(list(str),list(str))):
                Specialty of the provider(s) of interest
                option(s): Any valid, registered specialty in the US
                default(s): Defaults to all specialties if left unspecified
            short_zip (str or list(str) or list(list(str),list(str))):
                First three digits of the zip code of interest
                option(s): Any valid, registered first three digits of the zip code in the US
        ...
        ### Input Data Type Control ###
        # ensure valid mixed data type
        mixed = {"proc_code": proc_code, "taxonomy_code": specialty, "short_zip": short_zip}
    ...
    def orgDiagAggs(self,
                        ...
                        limit = None):
        ''''''
        return self.providerDiagAggs(min_year = min_year,
                        max_year = max_year,
                        min_month = min_month,
                        max_month = max_month,
                        npi = npi,
