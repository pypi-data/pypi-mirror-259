...
class TaskScheduler:
    def __init__(
        ...
        **kwargs,
        ...
        self.job_id = job_id
        self.logger = logging.getLogger(f"Job.{job_id}.{self.task_id}")
    ...
    async def set_task_status(self, state, error):
        ...
        conn = AsyncDB("pg", dsn=default_pg, loop=event_loop, **options)
        try:
    ...
    def __call__(self, *args, **kwargs):
        try:
            ...
            if self.priority == "direct":
                ...
                task = loop.create_task(self._send_task(self.wrapper, self.worker))
            elif self.priority == "pub":
                # Using Channel Group mechanism (avoid queueing)
                task = loop.create_task(self._publish_task(self.wrapper, self.worker))
            else:
                task = loop.create_task(self._schedule_task(self.wrapper, self.worker))
            ...
            self.logger.error(f"IO: Connection Refused: {exc}")
            send_notification(
                loop,
...
def thread_wrapper(program, task_id, loop, *args, **kwargs):
    logging.info(f"Calling Task: {program}.{task_id} on ThreadPool Executor.")
    ...
    try:
        ...
        raise TaskFailed(f"Error: Task {program}.{task_id} failed: {err}") from err
    # finally:
    #     loop.stop()


def process_wrapper(program, task_id, *args, **kwargs):
    logging.info(f"Calling Task: {program}.{task_id} on ProcessPool Executor.")
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
...
def get_function(job: dict, priority: str = "low", worker: Callable = None):
    ...
    try:
        ...
        raise RuntimeError(f"Error getting Function on Schedule {t}") from e
    if t == "function":
        try:
            fn = globals()[func]
            return fn
        ...
        if priority == "local":
            ...
            if executor == "default":
                ...
                sched = TaskScheduler(program, task, job_id, priority, worker, **params)
