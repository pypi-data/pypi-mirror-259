...
class DownloadManager:
    ...
    def add_to_queue(self, link):
        ...
        self.download_list.append(f"{title} - Pendente")
    ...
    def process_queue(self):
        while not self.queue.empty():
            ...
            self.download_list.remove(f"{title} - Pendente")
            self.download_list.append(f"{title} - Concluído")
            self.update_download_list()

    def download_video(self, title, link, resolution, folder_path):
    ...
    def update_specific_download(self, title, message, complete=False):
        for i, item in enumerate(self.download_list):
            if title in item:
                self.download_list[i] = f"{title} - {message}"
                if complete:
                    self.download_list[i] = f"{title} - Concluído"
                break
        self.update_download_list()
