    ...
    def start_next_download(self):
        with self.download_lock:
            if self.download_queue and not self.is_downloading:
                ...
                # Inicia a thread de download sem bloquear
                threading.Thread(target=self.download_video, args=(video_url, folder_path), daemon=True).start()

    def download_video(self, video_url, folder_path):
        try:
            start_download(video_url, folder_path, self)
        finally:  # Garante que download_finished seja chamado mesmo se ocorrer um erro
            self.download_finished()

    def download_finished(self):
        with self.download_lock:
            self.is_downloading = False
            # Chama de forma assíncrona o próximo download para evitar potenciais bloqueios
