...
class DownloadManager:
    ...
    def add_to_queue(self, link):
        ...
        self.current_downloads.append(f"{title} - Pendente")  # Adiciona o título do vídeo à lista com status pendente
        self.update_download_list()
        if self.queue.qsize() == 1:
            # Inicia o processamento da fila em uma nova thread apenas se for o primeiro item adicionado
    ...
    def process_queue(self):
        while not self.queue.empty():
            ...
            # Atualiza o status do vídeo na lista de downloads após a conclusão
            self.update_specific_download(title, "Concluído")
    ...
    def download_video(self, title, link, resolution):
      ...
      monitor_thread = threading.Thread(target=monitor_download_progress, args=(folder_path, title, resolution, download_ready_event, self.update_specific_download), daemon=True)
    ...
    def update_specific_download(self, title, message, complete=False):
        for i in range(len(self.current_downloads)):
            if title in self.current_downloads[i]:
                # Atualiza o status baseado no parâmetro complete
                new_status = "Concluído" if complete else message
                self.current_downloads[i] = f"{title} - {new_status}"
                break
        self.update_download_list()
