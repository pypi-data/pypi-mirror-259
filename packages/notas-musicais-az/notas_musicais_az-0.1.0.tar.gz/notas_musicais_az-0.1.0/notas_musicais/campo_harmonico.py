...
def _triade_na_escala(nota, notas_da_escala):
    ...
    Examples:
        >>> _triade_na_escala('C', ['C', 'D', 'E', 'F', 'G', 'H', 'A', 'B'])
        'C'
        >>> _triade_na_escala('D', ['C', 'D', 'E', 'F', 'G', 'H', 'A', 'B'])
        'Dm'
        >>> _triade_na_escala('B', ['C', 'D', 'E', 'F', 'G', 'H', 'A', 'B'])
        'B°'
    """
    resposta = str()
...
def campo_harmonico(tonica: str, tonalidade: str) -> dict[str, list[str]]:
    """
    Gera um campo harmônico com base em uma tônica e uma dada tonalidade
