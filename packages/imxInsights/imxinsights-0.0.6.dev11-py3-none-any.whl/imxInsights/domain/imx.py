...
from imxInsights.utils.geodata_gis import get_gis_data_layer
...
class Imx:
    ...
    def __init__(self, imx_file_path: str, add_graph: bool = True, _add_km: bool = True):
    ...
    def _add_km_to_all_items(self, item):
        item.km_values = []
        if isinstance(item.shapely, Point):
            item.km_values.append(self.gis_layer.get_km_linear_referencing(item.shapely.x, item.shapely.y))
        elif isinstance(item.shapely, LineString):
            item.km_values.append(self.gis_layer.get_km_linear_referencing(item.shapely.coords[0][0], item.shapely.coords[0][1]))
            item.km_values.append(self.gis_layer.get_km_linear_referencing(item.shapely.coords[-1][0], item.shapely.coords[-1][1]))
        elif isinstance(item.shapely, Polygon):
            if not item.shapely.is_empty:
                item.km_values.append(self.gis_layer.get_km_linear_referencing(item.shapely.centroid.x, item.shapely.centroid.y))
