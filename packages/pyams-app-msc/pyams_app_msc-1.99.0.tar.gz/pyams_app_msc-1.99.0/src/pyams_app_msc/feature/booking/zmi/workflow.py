...
class BookingSessionsVocabulary(SimpleVocabulary):
    ...
    def __init__(self, context):
        ...
        def get_session_label(session, request):
            ...
            rooms = getVocabularyRegistry().get(session, ROOMS_VOCABULARY)
            return translate(_("{room} - {date} from {start_time} to {end_time}")).format(
...
class BookingSessionChangeForm(AdminModalAddForm):
    ...
    def create_and_add(self, data):
        ...
        if session_key:
            return self.context.set_session(session_key)
...
@adapter_config(name='notify.group',
                required=(IBookingInfo, IAdminLayer, BaseBookingAdminForm),
                provides=IGroup)
class BookingWorkflowNotifyGroup(FormGroupChecker):
...
class BaseBookingAdminFormRenderer(AJAXFormRenderer):
    ...
    def render(self, changes):
        ...
        return {
            'status': 'success',
            'callbacks': [
                get_json_table_row_refresh_callback(IBookingContainer(self.context), self.request,
                                                    IBookingContainerTable, self.context),
...
class BookingReminderWarning(AlertMessage):
    ...
    def message(self):
        ...
        settings = IMovieTheaterSettings(theater)
        return translate(_("The reminder delay is set to {} hours before session start.")).format(
