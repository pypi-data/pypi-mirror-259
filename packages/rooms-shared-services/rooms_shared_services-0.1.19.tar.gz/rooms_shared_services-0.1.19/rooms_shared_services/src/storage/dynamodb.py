...
class DynamodbStorageClient(AbstractStorageClient):
    ...
    def get_by_pages(self, filter_params: dict | None = None, condition: ConditionLiteral = "AND",
        ...
        scan_params = self.create_scan_params(filter_params=filter_params,
                                              condition=condition)
        for page in paginator.paginate(TableName=self.table.name, PaginationConfig={"PageSize": page_size}, **scan_params):
