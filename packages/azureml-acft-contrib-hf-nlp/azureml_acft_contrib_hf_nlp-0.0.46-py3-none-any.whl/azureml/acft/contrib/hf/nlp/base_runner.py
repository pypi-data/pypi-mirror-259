...
class BaseRunner(ABC):
    ...
    def resolve_resume_from_checkpoint(self, component_plus_preprocess_args: Namespace) -> None:
        ...
        model_name_or_path = component_plus_preprocess_args.model_name_or_path
        if resume_from_checkpoint and isinstance(model_name_or_path, Path) and model_name_or_path.is_dir():
            last_checkpoint = get_last_checkpoint(model_name_or_path)
            if last_checkpoint:
                model_name_or_path = last_checkpoint
            component_plus_preprocess_args.resume_from_checkpoint = model_name_or_path
        else:
            component_plus_preprocess_args.resume_from_checkpoint = None
        logger.info(f"Set resume_from_checkpoint path to: {component_plus_preprocess_args.resume_from_checkpoint}")
