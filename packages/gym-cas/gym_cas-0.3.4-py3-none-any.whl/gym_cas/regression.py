...
from sympy import Lambda, N, exp, ln, simplify
...
def regression(
    ...
    *,
    ...
    if show or return_plot:
        ...
        plt = p1 + p2
        avg_step_x = (max(x_points) - min(x_points)) / len(x_points)
        plt.xlim = (min(x_points) - avg_step_x, max(x_points) + avg_step_x)
        avg_step_y = (max(y_points) - min(y_points)) / len(y_points)
        plt.ylim = (min(y_points) - avg_step_y, max(y_points) + avg_step_y)
        if show:
            plt.show()
        if return_plot:
            if return_r2:
