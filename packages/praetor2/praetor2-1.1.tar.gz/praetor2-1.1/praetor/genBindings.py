...
def get_agent_string(modules):
    ...
    try:
        ...
        procPy = subprocess.Popen(['python2', '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    for mod in modules:
        try:
            ...
            proc1 = subprocess.Popen(['pip2', 'show', mod], stdout=subprocess.PIPE)
...
def gen_identifier(variable, naming_template="entity"):
    ...
    try:
        ...
        if 'execute_inEntity_0_praetor_5' in prov_id:
            write_string = 'provid - ' + prov_id + '\nvalue - ' + variable + '\npythonid - ' + str(id(variable)) + '\n'
            with open(praetor_settings.provenance_directory + 'send_me_this_file_kristen.txt', 'a') as f:
...
def track_globs(initial_globals):
    '''
    Find changes in global variables between and old set and a freshly generated one
...
def provcall(inlist, outlist, template, longname, startTime, endTime, memUse, json_name,
    ...
    if global_outputs is not None:
        ...
        for glob_key, glob_val in global_outputs.iteritems():
    ...
    if global_inputs is not None:
        ...
        for glob_key, glob_val in global_inputs.iteritems():
            object_id = gen_identifier(glob_val, naming_template='globals_{}'.format(glob_key))
            g_val, ptype = find_type(glob_val)
            if len(g_val) > maxValueLength:
                with open(longfileDir + object_id + '.txt', 'w') as f:
...
def create_template(func, template_name, template_name_started, function_name, filePath, global_tracking=False):
    '''
    ...
    if isinstance(func, types.FunctionType):
        ...
        source_str = source.encode('ascii', 'ignore')
        output_list = praetorTemplates.separate_output(source_str)
        try:
            arg_names = (inspect.getargspec(func)[0])
            input_list = reduce_lists(arg_names)
        except ValueError:
    ...
    def wrap(*args, **kwargs):
        ...
        for key in input_list:
            if key in kwargs.keys():
                ...
                kwargList.append(defaultsVals[defaultsKeys.index(key)])
        ...
        call_func_list.append(inspect.stack()[1][3])
        return output
...
def find_all(some_list, to_find):
    return [m.start() for m in re.finditer(to_find, some_list)]
...
def decorate_all_in_module(module, decorator):
    ...
    func_blacklist = [x[max(find_all(x,'.'))+1:] for x in mod_spec_blacklist]
