...
class PidCore():
    ...
    def __init__(self, kp=0.0, ki=0.0, kd=0.0, sp=0.0, omax=1000.0, omin=0.0, osp=0.0, srmax=0.0):
        """The constructor.
...
class PidVelocity(PidCore):
    ...
    def __init__(self, kp, ki, kd, sp, omax, omin, osp, srmax):
        """The constructor.
        See PidCore().
        """
        ...
        #self._out = osp  # Set output value @startup
...
class PidPosition(PidCore):
    ...
    def __init__(self, kp, ki, kd, sp, omax, omin, osp, srmax, scale=1.0):
        """The constructor.
        See PidCore().
    ...
    def process(self, value, **kwargs):
        ...
        print(value, self._out, un , self._out - self._srmax)
        
        # Limit output slew rate (dynamic)
        if un < self._out - self._srmax and self._sr_enabled is True:
            
            #print("Too fast down", self._out , value, un , self._out - self._srmax)
            un = self._out - self._srmax
        elif un > self._out + self._srmax and self._sr_enabled is True:
            
            #print("Too fast up", self._out , value, un , self._out + self._srmax)
            un = self._out + self._srmax
        # Save internal variables for next computation
        self._out = un
        # Notify end of computation (ie new data at output of PID)
