...
class PidCore(QObject):
    ...
    in_updated = pyqtSignal((float,), (str,))
    error_updated = pyqtSignal((float,), (str,))
    out_updated = pyqtSignal((float,), (str,))
...
class PidVelocity(PidCore):
    ...
    def process(self, inp):
        ...
        :param inp: current value of corrected signal (str or float)
        :returns: the corrected value (float)
        """
        self._inp = float(inp)
        # Notify new computation (ie new data at input of PID)
        self.in_updated[float].emit(float(inp))
        self.in_updated[str].emit('{:0.10f}'.format(inp))
        # Compute output itself
        er = self._sp - float(inp)
        self.error_updated[float].emit(er)
        self.error_updated[str].emit('{:0.10f}'.format(er))
        ...
        self.out_updated[float].emit(un)
        self.out_updated[str].emit('{:0.10f}'.format(un))
        # Return result
        return un
...
class PidPosition(PidCore):
    ...
    def process(self, inp):
        ...
        :param inp: actual value of corrected signal (float)
        :returns: the corrected value (float)
        """
        self._inp = inp
        # Notify new computation (ie new data at input of PID)
        self.in_updated.emit(inp)
        self.in_updated[str].emit(str(inp))
        # Compute output itself
        er = self._sp - inp
        self.error_updated[float].emit(float(er))
        self.error_updated[str].emit(str(er))
        ...
        # than to the error.
        self._d_term = self._kd * (self._inp1 - inp)
        self._inp1 = inp
        ...
        self._out = un
        # Notify end of computation (ie new data at output of PID)
        self.out_updated.emit(un)
        self.out_updated[str].emit(str(un))
        # Returns result
        return un
...
class PidPositionTime(PidPosition):
    ...
    def process(self, inp):
        ...
        # than to the error.
        if dt > 0:
            self._d_term = self._kd * (self._inp1 - inp) / dt
        self._inp1 = inp
        # Calculates PID output
        un = (self._p_term + self._i_term + self._d_term) * self._scale + \
            self._osp
