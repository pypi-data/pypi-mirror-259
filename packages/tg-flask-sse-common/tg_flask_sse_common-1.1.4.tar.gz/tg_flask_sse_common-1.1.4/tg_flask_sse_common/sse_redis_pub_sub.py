...
from .sse_message import ERROR_MESSAGE, REDIS_MESSAGE
...
class SseRedisPubSub(object):
    ...
    def listen(self):
        ...
        try:
            ...
            while True:
                ...
                # 消息推送到全局sse连接对象
                if not self.is_filter_message(message):
    ...
    def subscribe_channel(self, channel, extra=None):
        ...
        self.publish_message(
            channel=channel, data={'msg': 'connect success'}, event=SseSystemEventType.CONNECT
        )
    ...
    def sse_event_message_handler(sub_message):
        ...
        try:
            if not sub_message:
                return SseSystemEventType.ERROR, ERROR_MESSAGE
            ...
            if sub_message[RedisPubSubField.TYPE] != RedisPubSubField.Type.MESSAGE:
                return SseSystemEventType.REDIS, REDIS_MESSAGE
