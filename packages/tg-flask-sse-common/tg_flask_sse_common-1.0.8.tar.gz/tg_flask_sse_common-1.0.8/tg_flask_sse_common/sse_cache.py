...
class SseChannelMapCache(object):
    """
    节点sse连接信息缓存
    用于过渡一个频道下多个用户的sse连接信息
    channel : [111, 222] - set
    """
    ...
    def key(self, channel):
        return self.prefix + 'channel_map:' + channel

    def add(self, channel, connect_id):
        self.redis.sadd(self.key(channel), connect_id)
        self.redis.expire(self.key(channel), self.expire)

    def get(self, channel):
        data = self.redis.smembers(self.key(channel))
        if data is None:
            return []
    ...
    def delete(self, channel):
        self.redis.srem(self.key(channel))
