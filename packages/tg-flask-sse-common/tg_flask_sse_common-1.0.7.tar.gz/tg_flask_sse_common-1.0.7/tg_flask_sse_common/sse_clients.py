...
class SseClients(object):
    ...
    def connect(self, channel):
        ...
        if sse_connect:
            sse_connect.update({
                self.Field.REPEAT_CONNECT: True
            })
            # 确保前一个while轮训已经结束
            time.sleep(self.listen_interval * 2)
    ...
    def get_sse(self, channel):
        ...
        if not sse_connect:
            return {
                self.Field.MESSAGE_LIST: [],
                self.Field.CONNECT_TIME: datetime.now(),
                self.Field.REPEAT_CONNECT: False
            }
    ...
    def listen_message(self, channel):
        ...
        while (datetime.now() - connect_time).seconds < self.max_connect_time:
            if sse_connect.get(self.Field.REPEAT_CONNECT):
                print({
                    'title': '连接已存在，关闭前一个连接，重新连接',
                    'channel': channel,
