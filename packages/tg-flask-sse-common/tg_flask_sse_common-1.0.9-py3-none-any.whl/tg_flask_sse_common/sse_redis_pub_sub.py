...
from .sse_cache import SseConnectStatsCache, SseEventMessageCache, SseChannelMapCache
from .sse_constant import RedisPubSubField
from .sse_event_message import ERROR_MESSAGE, REDIS_MESSAGE
...
class SseRedisPubSub(object):
    ...
    def __init__(self, redis_client, key_prefix, sse_clients):
        ...
        self.sse_channel_map_cache = SseChannelMapCache(
            redis_client, key_prefix
        )
    ...
    def subscribe_channel(self, channel, extra=None):
        ...
        # 连接建立，推送一条订阅成功消息
        self.publish_message(channel=channel, data={
            'msg': 'connect success', 'channel': channel,
        }, event=SseEventType.CONNECT)
    ...
    def publish_message(self, channel, data, event=None, _id=None, retry=None):
        ...
        channel_connect_list = self.sse_channel_map_cache.get(channel)
    ...
    def sse_event_message_handler(sub_message):
        ...
        try:
            if not sub_message:
                return SseEventType.ERROR, ERROR_MESSAGE
            ...
            if sub_message[RedisPubSubField.TYPE] != RedisPubSubField.Type.MESSAGE:
                return SseEventType.REDIS, REDIS_MESSAGE
