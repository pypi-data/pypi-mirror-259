...
from sympy import Eq, nsolve, Reals, solveset, Symbol
...
def los_numerisk(variabel = Symbol(""),
                 vs       = Symbol(""),
                 hs       = Symbol(""),
                 rund     = None):
    ...
    # Løser likningen for aktuell variabel med nsolve() og får løsningen på formen 1234.56789
    losning = nsolve(likning, variabel, 1)

    # Type caster "sympy-float"/"string" -> float (for å kunne regne med tallet)
    losning = float(losning)
...
if _blokk == 1:
    ...
    n_dobbelt_totalt = los_numerisk(variabel = n,
                                    vs       = B_P + B_K,
                                    hs       = 4 * I,
                                    rund     = 2)
