    ...
    if request.method == 'POST':
        ...
        if form_id == "staff-form":
            if pk:
                ...
                formset = set_formset(request.POST, request.FILES, prefix='gallery', instance=staff)
                formset2 = set_formset2(request.POST, request.FILES, prefix='formset', instance=staff)    
                if all([form.is_valid(), 
                        formset.is_valid(),
                        formset2.is_valid(),
                    ]):
                ...
                formset = set_formset(request.POST, request.FILES, prefix='gallery')
                formset2 = set_formset2(request.POST, request.FILES, prefix='formset')    
                if all([form.is_valid(), 
                        formset.is_valid(),
                        formset2.is_valid(),
                    ]):
            ...
            if data:
                ...
                if all([form.is_valid(), 
                    ...
                    if schedule:
                        ...
                        return JsonResponse({'alert': _('Process failed, please check the errors...'), 'formset_html': obj_data, 'cke':ckeditor, 'schedule':schedule, 'is_cke_formset': cke_formset, 'date':service.published_date})
                    else:
                        obj_data = render_to_string('stela_control/load-data/inventory/dynamic-formset.html', { 
                        'form': form,
                        'formset': formset,
