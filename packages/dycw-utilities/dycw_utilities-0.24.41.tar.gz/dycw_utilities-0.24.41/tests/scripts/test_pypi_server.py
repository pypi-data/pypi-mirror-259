...
class TestPypiServer:
    def test_check_password_file_success(self, *, tmp_path: Path) -> None:
        (path_password := valid_path(tmp_path, "password")).touch()
        _check_password_file(path_password=path_password)

    def test_check_password_file_error(self, *, tmp_path: Path) -> None:
        with raises(FileNotFoundError):
            _check_password_file(path_password=valid_path(tmp_path, "password"))
    ...
    def test_get_args(
        ...
        _ = _get_args(
            port=port,
            path_password=valid_path(root, password),
            path_packages=valid_path(root, packages),
        )

    def test_dry_run(self, *, tmp_path: Path) -> None:
        with temp_cwd(tmp_path):
            path = valid_path("password")
            path.touch()
            runner = CliRunner()
