...
async def find_bad_channels_async(channel_list):
    async def worker(channel):
        out = await asyncio.to_thread(nds.find_channels, channel)
        if not out:
            return channel
    tasks = []
...
def main():
    ...
    if args.check:
        bad_channels = asyncio.run(
            find_bad_channels_async(_template.template_extract_channels(template))
        )
        if bad_channels:
