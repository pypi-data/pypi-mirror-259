...
t.add_column("#")
t.add_column("File")
t.add_row("00", "update kicks")
t.add_row("0", "create wrappers (.bat) in '%%HOME%%/AppData/Local/Microsoft/WindowsApps'")
...
if x == "00":
    while True:
        ...
        update_loop_confirm = CONSOLE.input('i for INSTALL | q for QUIT | EMPTY for RE-CHECK # ')
...
if x == "0":
    ...
    with open(os.path.join(bindir, 'ee.bat'), 'w') as f:
        f.write('%s %%*' % (os.path.join(home, '_kicks', 'e', 'emacs-29.2_1', 'bin', 'runemacs')))

    #e / sublime
    with open(os.path.join(bindir, 'e.bat'), 'w') as f:
        f.write('%s %%*' % (os.path.join(home, '_kicks', 'sub', 'sublime')))
