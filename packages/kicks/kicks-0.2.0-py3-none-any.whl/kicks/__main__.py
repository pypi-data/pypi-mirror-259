...
if x == "2":
    outfile = os.path.join(OP, "chrome-setup.exe")
    cmd = 'curl -C - -L -o "%s" "https://www.google.com/chrome/next-steps.html?brand=FKPE&platform=win64&statcb=0&installdataindex=empty&defaultbrowser=0#"' % outfile
    subprocess.call(cmd, shell=True)
    run_confirm = CONSOLE.input('run it? type "run" to execute or anything else to exit # ')
    if run_confirm == "run":
        subprocess.call(outfile, shell=True)
