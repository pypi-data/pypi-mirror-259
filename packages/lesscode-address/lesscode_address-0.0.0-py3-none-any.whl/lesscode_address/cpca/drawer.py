...
def draw_locations(adcodes, file_path):
    ...
    try:
        ...
        folium_plugins = importlib.import_module("folium.plugins")
    except ImportError:
        raise Exception("folium is not exist,run:pip install folium")
    ...
    # 将热力图添加到前面建立的map里
    folium_plugins.HeatMap(heatData).add_to(map_osm)
    # 保存为html文件
...
def echarts_draw(adcodes, file_path, title="地域分布图"
    ...
    """
    try:
        pyecharts = importlib.import_module("pyecharts")
    except ImportError:
        raise Exception("pyecharts is not exist,run:pip install pyecharts==1.9.1")
    ...
    geo = pyecharts.Geo(title, subtitle, title_color="#fff",
...
def echarts_cate_draw(adcodes, labels, file_path, title="地域分布图", subtitle="location distribute",
    ...
    try:
        ...
        raise Exception("pyecharts is not exist,run:pip install pyecharts==1.9.1")
    geo = pyecharts.Geo(title, subtitle, title_color="#000000",
                        title_pos="center", width=1200,
