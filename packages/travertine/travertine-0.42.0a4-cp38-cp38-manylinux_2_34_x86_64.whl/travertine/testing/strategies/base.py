...
ratios = st.floats(allow_infinity=False, allow_nan=False, min_value=1 / 100, max_value=1.0)
...
getattr_procs = numerical_attributes.map(
    lambda attr: GetAttributeProcedure(TypedAttribute(attr, SimpleType.from_python_type(float)))
)
