...
// РасширенияКонфигурации.Получить() не работает.
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ТипОперации = "CFE Выгрузка" Тогда
		ПроверяемыеРеквизиты.Добавить("ПутьВыгрузкиРасширений");
	КонецЕсли;
...
Функция РеквизитыФормыВСтруктуру()
	...
	НастройкиВыгрузкиРасширений = Новый Структура("ПапкаВыгрузки, ИмяПапки", ПапкаВыгрузкиБезСлеша, ПутьВыгрузкиРасширений);
	Если Не ПустаяСтрока(ИмяРасширения) Тогда
		НастройкиВыгрузкиРасширений.ИмяПапки = СоединитьПути(НастройкиВыгрузкиРасширений.ИмяПапки, ИмяРасширения);
	КонецЕсли;
	...
	Структура.Вставить("ПапкаВыгрузкиРасширений",   ПодставитьПараметрыВСтроку(СоединитьПути("%ПапкаВыгрузки%", "%ИмяПапки%"), НастройкиВыгрузкиРасширений));
	Структура.Вставить("ИмяРасширения",             ИмяРасширения);
	...
	Структура.Вставить("ПапкаЗагрузкиРасширений",   СоединитьПути(ПапкаВыгрузкиБезСлеша, "lib\ext"));
...
Процедура УстановитьВидимостьЭлементов()
	...
	Видимость = (ТипОперации = Операции.ВыгрузкаCFE);
	Элементы.ПутьВыгрузкиРасширений.Видимость = Видимость;
	Элементы.ИмяРасширения.Видимость = Видимость;
...
Функция ПолучитьЭтапы(Знач ВыбраннаяОперация)
	...
	ИначеЕсли ВыбраннаяОперация = Операции.ВыгрузкаCFE Тогда
		...
		Если Не ПустаяСтрока(ИмяРасширения) Тогда
			СтрокаДляЗапуска = СтрокаДляЗапуска + " --name ""%ИмяРасширения%""";
		КонецЕсли;
	...
	ИначеЕсли ВыбраннаяОперация = Операции.ЗагрузитьCFиCFE Тогда
		
		СтрокаДляЗапуска = "--folder ""%ПапкаВыгрузкиКонфигурации%""";
		Массив.Добавить(СоздатьЭтап(СтрокаДляЗапуска, "load_config"));
		
		СтрокаДляЗапуска = "--folder ""%ПапкаЗагрузкиРасширений%""";
...
&НаСервере
Процедура ЗаполнитьИменаРасширений()
	
	Попытка
		Для Каждого РасширениеКонфигурации Из РасширенияКонфигурации.Получить() Цикл
			Элементы.ИмяРасширения.СписокВыбора.Добавить(РасширениеКонфигурации.Имя);
		КонецЦикла;
	Исключение
		Сообщение(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Сообщение("Заполнение имен расширений выключено");
		Элементы.ИмяРасширения.РежимВыбораИзСписка = Ложь;
	КонецПопытки;
...
Операции.Вставить("ЗагрузитьCFиCFE", "Загрузить конфигурацию");
