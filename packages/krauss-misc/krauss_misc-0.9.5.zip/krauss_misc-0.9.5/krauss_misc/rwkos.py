...
mac_roots = {'345':'/Users/kraussry/345_F23', \
...
def glob_search_walking_up(startdir, glob_pat):
    ...
    while curdir:
        ...
        full_pat = os.path.join(curdir, glob_pat)
        curfiles = glob.glob(full_pat)
        mylist.extend(curfiles)
        curdir, folder = os.path.split(curdir)
        if folder == "My Drive":
            break
        

    sorted_by_mtime_descending = sorted(mylist, \
