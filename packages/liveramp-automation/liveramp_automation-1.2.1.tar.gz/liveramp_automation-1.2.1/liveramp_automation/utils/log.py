...
class MyFormatter(logging.Formatter):
    ...
    def __init__(self, fmt='%(asctime)s - %(levelname)s - [ %(test_case_name)s ] [ %(filename)s: %(lineno)d ] - %(message)s '):
...
class LoggerUtils:
    ...
    def get_log_format(log_include_scenario):
        include_scenario = strtobool(log_include_scenario)
        if (include_scenario == 1):
