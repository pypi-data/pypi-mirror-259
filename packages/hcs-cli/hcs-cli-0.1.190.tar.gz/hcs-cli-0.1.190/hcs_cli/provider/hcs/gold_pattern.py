...
def refresh(data: dict, state: dict) -> dict:
    ...
    if state:
        id = state.get("id")
        if id:
            return ims.gold_pattern.get(id, org_id)

    # search by provider id and name, which is the criteria used by service side verification
    provider_id = body["providerInstanceId"]
    image_name = body["goldPatternDetails"]["data"]["imageName"]
    search = f"providerInstanceId $eq {provider_id} AND goldPatternDetails.data.imageName $eq {image_name}"
    ret = ims.gold_pattern.list(org_id, search=search)
    if ret:
        return ret[0]
