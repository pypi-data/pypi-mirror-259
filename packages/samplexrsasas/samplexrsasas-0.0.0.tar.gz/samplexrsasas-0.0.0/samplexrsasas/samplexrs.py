def paravis():
            ...
            params = {
                ...
                "kNR": spyndex.computeKernel(
                    ...
                    params={
                        ...
                        "sigma": snt.sel(
                            band=[selected_bands[4], selected_bands[2]]
                        ).mean("band"),
                    },
                ),
            }
    ...
    def plot_figure(button):
        ...
        cv_value = float(
            indices_dict[selected_dataset][selected_indices]["cv_percentage"]
        )
        ...
        plt.xticks(
            ...
            [
                "{:.2f}\u00b0 {}".format(tick, "W" if tick < 0 else "E")
                for tick in x_ticks
            ],
            size=10,
            fontweight="semibold",
        ...
        plt.yticks(
            ...
            [
                "{:.2f}\u00b0 {}".format(tick, "S" if tick < 0 else "N")
                for tick in y_ticks
            ],
            size=10,
            fontweight="semibold",
    ...
    widgets_row = widgets.HBox(
        ...
        layout=widgets.Layout(
            justify_content="space-between", margin=f"0px {vbox_margin}"
        ),
    )
    display(widgets_row)
    ...
    def update_input(mode_value):
        if mode_value == "unidimensional":
            ...
            input_text.rows = len(
                index_names
            )  # Display more rows for multi-file selection
    ...
    def click(btn):
        ...
        def minkowski_dist(pair_list, p_minkowski):
            return sum(
                [
                    (abs(x[0] - x[1]) ** p_minkowski) ** (1 / p_minkowski)
                    for x in pair_list
                ]
            )
            ...
            for rw in range(row_start, row_end):
                for cl in range(col_start, col_end):
                    ...
                    borderCondition = [
                        np.sum(
                            np.invert(
                                np.isnan(x[rw - w : rw + w + 1, cl - w : cl + w + 1])
                            )
                        )
        ...
        def parallel_raoq(
            ...
            with tqdm(total=len(ray_results)) as pbar:
                for result in ray_results:
                    row_start, row_end, col_start, col_end, raoq_values = ray.get(
                        result
                    )
        ...
        output = parallel_raoq(
            data_input=(
                input_indice if mode.value == "unidimensional" else input_indices
            ),
            output_path=(
    ...
    p_minkowskii = widgets.BoundedIntText(
        description="degree:", value=2, min=2, max=5000
    )
    window = widgets.BoundedIntText(
        description="Window:", min=1, max=333, step=2, value=3
    )
    ...
    input_widgets = widgets.VBox(
        [
            ...
            widgets.HBox(
                [window, na_tolerance, batch_size, num_cpu_cores, memory_slider]
            ),
    ...
    def normalize_data(data):
        if np.isnan(np.nanmin(data)) or np.isnan(np.nanmax(data)):
            ...
            normalized_data = (data - np.nanmin(data)) / (
        ...
        if selected_files:
            ...
            for file_name in selected_files:
                ...
                if file_path:
                    ...
                    IQR = pd.Series(data).quantile(0.75) - pd.Series(data).quantile(
                        0.25
                    )
    ...
    def difference_files(b):
        ...
        if len(selected_files) != 2:
            ...
            return
        file1_name, file1_path = selected_files[0], file_dict.get(
            selected_files[0], None
        )
        file2_name, file2_path = selected_files[1], file_dict.get(
            selected_files[1], None
        )
        output_filename = os.path.join(
        ...
        if file1_path and file2_path:
            ...
            print(
                "Error: Unable to subtract files. Please ensure both files are selected."
            )
    ...
    def heatmap_files(b):
        ...
        if file1_path and file2_path:
            with rasterio.open(file1_path) as src:
                transform = (
                    src.transform
                )  # Get the transform object for one of the files
    ...
    def split_plot(_):
        ...
        def display_split_view(split_size):
            ...
            with MemoryFile() as memfile:
                ...
                with memfile.open() as combined_ds:
                    ...
                    x = (
                        np.arange(0, num_cols) * combined_transform.a
                        + combined_transform.c
                    )
                    y = (
                        np.arange(0, num_rows) * combined_transform.e
                        + combined_transform.f
                    )
                    plt.figure(figsize=(8, 6))
                    plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(nbins=2))
                    plt.gca().yaxis.set_major_locator(ticker.MaxNLocator(nbins=2))
                    ...
                    split_position = int(split_size * left_data.shape[1])
                    plt.axvline(
                        x=split_position, color="red", linestyle="--", linewidth=1
                    )
                    if save_checkbox.value == True:
                        output_filename = os.path.join(
                            output_dir,
        ...
        def save_gif():
            ...
            def animate(split_position):
                ...
                ax.axvline(
                    x=split_position_pixel, color="red", linestyle="--", linewidth=1
                )
                        ...
                        [
                            widgets.VBox(
                                [
                                    ...
                                    dp_dropdown,
                                    widgets.HBox(
                                        [fmt_dropdown, plot_individual_button]
                                    ),
                                ]
                            )
                        ]
