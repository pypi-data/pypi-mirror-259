...
def deploy(app: App, project_directory_name: str) -> None:
    ...
    representation = app.represention()
    if not has_store(SP_APP_STORE):
        create_store(SP_APP_STORE, store_config={"replicas": 1})
    json_representation = json.dumps(representation)
    set_key(SP_APP_STORE, app.name, json_representation)
