...
from pyzjr.Models.bricks.comblock import ConvBNReLU, _DenseBlock

__all__=["FCN8s", "FCN16s", "FCN32s", "FCNs", "VggFCN", "ResFCN", "DenseFCN"]
...
def _get_fun_model(backbone, fcntype, num_classes):
    model = None
    if fcntype == "FCN8s":
        model = FCN8s(model=backbone, num_classes=num_classes)
    elif fcntype == "FCN16s":
        model = FCN16s(model=backbone, num_classes=num_classes)
    elif fcntype == "FCN32s":
        model = FCN32s(model=backbone, num_classes=num_classes)
    elif fcntype == "FCNs":
        model = FCNs(model=backbone, num_classes=num_classes)
    return model
...
if __name__ == "__main__":
    batch_size, num_classes, h, w = 10, 21, 160, 160
    fcn_model = DenseFCN(num_classes=num_classes)
    # fcn_model = ResFCN(num_classes=num_classes)
