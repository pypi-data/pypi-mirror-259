...
class DataboxLoader:
    ...
    def __init__(
        ...
        if self.builder is not None:
            try:
                self.builder_build_method = getattr(self.builder, "_build")
            except:
                pass

            try:
                self.builder_init_method = getattr(self.builder, "_init")
            except:
                pass
        ...
        if self.builder_init_method is not None:
            self._log_event(f"Init {self.item_path()}")
    ...
    def load_databox_item(self, box_instance, key):
        ...
        if not self.root.config.NO_CACHE_READ and self.is_item_path_cacheable(
            ...
            if item is None:
                self._log_event(f"Item {item_path} not found in cache")
            else:
                self._log_event(f"Item {item_path} loaded from cache")
        ...
        if isinstance(item, DataboxLoader):
            ...
            print("Item as loader:", item_loader, item)

        self._log_event(f"Item {item_path} built using {build_method.__name__}")
        if not self.root.config.NO_CACHE_WRITE and self.is_item_path_cacheable(
            item_path
        ):
            self._log_event(f"Item {item_path} written to cache")
