...
class ArgParser:
	...
	def parse(cls, argv: list[str], command: Command) -> tuple[list, dict]:
		...
		logger.debug(f"start parsing arguments for {command.cmd}: {argv}")
		...
		while pointer < len(argv):
			if alias := cls.__match_argument_name(argv[pointer]):
				...
				while next_argument < len(argv) and not cls.__match_argument_name(argv[next_argument]):
					next_argument += 1
				if typehint == bool and argument not in positionals and (
						not defaults.get(argument) or is_last or next_argument == pointer + 1):
					values[argument] = "True"
					pointer += 1
					continue
				elif is_last or next_argument == pointer + 1:
