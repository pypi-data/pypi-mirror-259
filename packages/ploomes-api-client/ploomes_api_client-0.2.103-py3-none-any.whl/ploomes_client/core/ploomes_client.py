...
class PloomesClient:
    ...
    def __init__(self, api_key, rate_limit_tokens=120, token_refresh_rate=2, max_retries = MAX_RETRIES) -> None:
        """
        ...
        while retries > 0:
            ...
            try:
                ...
                if status_code == 401:  # Unauthorized access
                    logger.error("Unauthorized access to %s. Terminating retry.", url)
                    return (
                        response.json()
                    )  # Assuming the response contains relevant error details in JSON format
