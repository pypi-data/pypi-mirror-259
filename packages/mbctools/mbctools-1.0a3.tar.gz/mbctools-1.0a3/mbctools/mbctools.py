...
def trim_2x():
        ...
        if rmenu == "2a":
                ...
                while True:
                        ...
                        os.chdir(f"./results_by_locus/{loc2trim2a}")
        ...
        if rmenu == "2b":
                ...
                while True:
                        ...
                        if orientFileSuffix == None:
                                ...
                                trim_right = in_trim_right(orientFileSuffix)
                                ts = in_ts()
                                ...
                                for sample in samples:
                                        if os.path.exists(f"{sample}_singleEnd_orient{orientFileSuffix}.fas") and os.path.getsize(f"{sample}_singleEnd_orient{orientFileSuffix}.fas") > 0:
                                                with open(f"{sample}_singleEnd_orient{orientFileSuffix}.fas", 'r') as filin, open("trim-select." + scriptExt, "w") as out:
                                                        targets = [line for line in filin if "size" in line]
                                                        a = 0
                                                        for target in targets:
                                                                size = re.search('size=(.+?)$', target).group(1)
        ...
        if rmenu == "2c":
                ...
                while True:
                        ...
                        os.chdir(f"{current_dir}{fileSep}results_by_locus{fileSep}{loc2trim2c}")
                        trim_left = in_trim_left("")
                        trim_right = in_trim_right("")
...
def concat_3(loci):
        ...
        while loci is None or locusIndex < len(loci):
                ...
                if len(files2cat) == 0:
                        ...
                        if nb_tot > 0:
                                sys.stdout.write(f"Results in --> {current_dir}{fileSep}results_by_locus{fileSep}{loc2cat}{fileSep}{loc2cat}_allseq_select.fasta\n")
                        ...
                        if loci is not None:
                                ...
                                outFasta = f"{current_dir}{fileSep}results_by_locus{fileSep}{loc2cat}{fileSep}{loc2cat}" + '_allseq_select_derep.fasta'
                                outTsv = f"{current_dir}{fileSep}results_by_locus{fileSep}{loc2cat}{fileSep}{loc2cat}" + '_allseq_select_derep.tsv'
        ...
        if loci is not None and anySeqsFound is True and "yes" == promptUser("Do you want to generate dereplicated versions of the above mentioned fasta files? " + normalStyle + "Enter yes or no" + promptStyle, None, ["yes", "no"], 1, None, ""):
                ...
                for locus in loci:
                        if locus not in skippedLoci:
                                outFasta = f"{current_dir}{fileSep}results_by_locus{fileSep}{locus}{fileSep}{locus}" + '_allseq_select_derep.fasta'
                                outTsv = f"{current_dir}{fileSep}results_by_locus{fileSep}{locus}{fileSep}{locus}" + '_allseq_select_derep.tsv'
                                derepResults = derepSeveralFastaFiles({locus : f"{current_dir}{fileSep}results_by_locus{fileSep}{locus}{fileSep}{locus}" + '_allseq_select.fasta'}, outFasta, outTsv, logFile)
                                if derepResults[0] > 0:
...
def main_menu1():
        ...
        print(titleStyle + "\n--- MENU 1: BASIC ANALYSIS - only option 1 is strictly mandatory ---" + normalStyle + "\n\n"
                                                                                                                           "1  -> INITIAL ANALYSIS (" + warningStyle + "mandatory" + normalStyle + "): read merging, sample-level dereplication, sequence clustering,\n\tchimera detection, affiliation of sequences to loci, and sequence re-orientation\n\n"
                                                                                                                           "1a -> Re-analyze all loci, from the clustering step, modifying parameters\n\n"
                                                                                                                           "1b -> Re-analyze only one locus of paired-end amplicon (merged reads), modifying parameters\n\n"
                                                                                                                           "1c -> Re-analyze only one locus of single-end amplicon (R1 only), modifying parameters\n\n"
                                                                                                                           "1d -> Re-analyse a given sample, modifying parameters\n\n"
...
def main_menu2():
        ...
        os.system("cls" if winOS else "clear")
        global rmenu
        print(titleStyle + "\n--- MENU 2: PRIMER REMOVAL, SELECTION OF MINIMUM SEQUENCE ABUNDANCE LEVELS ACCORDING TO USER-DEFINED THRESHOLDS ---" + normalStyle + "\n\n"
...
def main_menu4():
        ...
        os.system("cls" if winOS else "clear")
        global rmenu
        print(titleStyle + "\n--- MENU 4: EXPORTING ANALYSIS RESULTS INTO metaXplor FORMAT ---" + normalStyle + "\n\n"
                                                                                                                                  "4a -> Generate sequence files\n"
...
def menu4a():
        ...
        for locus in list(set(lociPEs + lociSEs)):
                files2cat = glob.glob("results_by_locus/" + locus + fileSep + '*_allseq_select.fasta')
                if len(files2cat) > 0:
                        locusToFastaDict[locus] = "results_by_locus/" + locus + fileSep + locus + '_allseq_select.fasta'
        ...
        with open(metaXplorSamples, "w") as outfile:
                ...
                while i < len(lines):
                        ...
                        if splitLine[sampleIndex] not in samples:
                                ...
                                if collDate != None and collDate.strip() != "":
                                        try:
