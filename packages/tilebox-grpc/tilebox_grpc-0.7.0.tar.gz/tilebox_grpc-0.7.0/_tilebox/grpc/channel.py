...
def open_channel(url: str, auth_token: str | None = None) -> Channel:
    ...
    interceptors = [_ErrorHandlerInterceptor()]
    ...
    if channel_info.use_ssl:
        ...
        channel = insecure_channel(channel_info.url_without_protocol, _CHANNEL_OPTIONS, interceptors=interceptors)
...
class _AuthMetadataInterceptor(UnaryUnaryClientInterceptor):  # type: ignore[misc]
...
class _ErrorHandlerInterceptor(UnaryUnaryClientInterceptor):  # type: ignore[misc]
    ...
    async def intercept_unary_unary(
        ...
        try:
            ...
            match err.code():
                ...
                case _:
                    raise InternalServerError(f"Ups, something went wrong: {err.details()}") from None
