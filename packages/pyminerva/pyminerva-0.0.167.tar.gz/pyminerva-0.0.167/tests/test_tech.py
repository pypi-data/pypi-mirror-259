...
if __name__ == "__main__":
    ...
    def tech_func(func, ticker, df, tech_type, TECH_LIST, idx):
        ...
        df, latest_date = func(ticker, df, tech_type, TECH_LIST, idx)
    ...
    for ticker in tickers:
        ...
        for i, ta in enumerate(TECH_LIST):
            print('      ', ta)
            # 문자열을 함수 이름으로 변환하여 함수 가져오기
            func = globals().get(ta)
            df, latest_date = tech_func(func, ticker, df, ta, TECH_LIST, i+1)
