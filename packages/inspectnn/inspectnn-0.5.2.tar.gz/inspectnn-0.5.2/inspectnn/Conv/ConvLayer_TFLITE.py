...
from inspectnn.Conv.Conv_kernel_tflite import convolve_tflite_occorenza,convolve_tflite, convolve_tflite_padding,convolve_tflite_fake,convolve_tflite_mf, convolve_tflite_padding_mf,convolve_tflite_fake_mf, convolve_tflite_mfiber, convolve_tflite_fake_mfiber, convolve_tflite_padding_mfiber #, convolve_tflite_mfiber_cpu
    ...
    def load_weights(self, **kwargs):
        ...
        if self.enable_gpu:
            ...
            if ( kwargs["quant_data"] is not None):
                ...
                self.weights = cuda.to_device(np.ascontiguousarray(np.sum(kwargs["weights"],(1,2,3))*kwargs["quant_data"]*kwargs["quant_input_offset"]*kwargs["quant_input_k"],dtype=np.single))#TODO:parametrizare il diviso 2 con alfa input+offest input
