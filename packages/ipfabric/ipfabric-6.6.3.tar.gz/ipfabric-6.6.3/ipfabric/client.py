...
class IPFClient(IPFabricAPI):
    ...
    def _stream(self, url, payload, export):
        if export == "csv" and self.api_version < "v6.3":  # TODO: No check in v7.0
    ...
    def get_columns(self, url: str, ui: bool = False) -> List[str]:
        ...
        if self.oas.get(url, None) and getattr(self.oas[url], ctype):
            return getattr(self.oas[url], ctype)
        else:
    ...
    def share_link(
        ...
        url = self._check_url(url)
        try:
            web_endpoint = self.oas[url].web_endpoint
        except KeyError:
