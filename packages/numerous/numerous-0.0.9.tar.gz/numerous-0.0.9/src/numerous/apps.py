...
def field(  # type: ignore[no-untyped-def] # noqa: ANN201, PLR0913
    *,
    ...
    return Field(default=default, label=label)
...
@overload
def app(title: str = ...) -> Callable[[Type[ToolT]], Type[ToolT]]: ...


def app(
    *args: Any,
    **kwargs: Any,
) -> Union[Type[ToolT], Callable[[Type[ToolT]], Type[ToolT]]]:
...
def app_decorator(**kwargs: dict[str, Any]) -> Callable[[Type[ToolT]], Type[ToolT]]:
    title = kwargs.get("title", None)

    @dataclass_transform(field_specifiers=(field, html, slider))
    def decorator(cls: Type[ToolT]) -> Type[ToolT]:
