...
class Input(InputBase):
    ...
    def choose_note(self, use_search_request: bool | None = None) -> Note:
        if n(use_search_request):
            use_search_request = self.yes_no("Использвать поиск")
...
class PIH:
    ...
    class ERROR:
        ...
        def global_except_hook(
            exception: BaseException, host: str | None = None, argument_list: list[str] | None = None
        ) -> None:
    ...
    class UPDATER:
        ...
        def package_exists(package_name: str) -> bool:
            executor_path: str = CONST.PYTHON.EXECUTOR
    ...
    class EXECUTOR:
            ...
            complete_process: CompletedProcess = PIH.EXECUTOR.execute(
                DataTool.check_not_none(
                    ...
                    lambda: PIH.EXECUTOR.create_command_for_psexec(
                        ping_command,
                        host,
                        PIH.DATA.VARIABLE.ENVIRONMENT.value(LINK.ADMINISTRATOR_LOGIN, False) or AD.ADMINISTRATOR_LOGIN,
        ...
        def create_command_for_psexec(
            ...
            if not local_host:
                ...
                login = login or PIH.DATA.VARIABLE.ENVIRONMENT.value(
                    LINK.ADMINISTRATOR_LOGIN, False) or AD.ADMINISTRATOR_LOGIN
        ...
        def process_is_exists(
            ...
            login = PIH.DATA.FORMAT.login(
                login or PIH.DATA.VARIABLE.ENVIRONMENT.value(LINK.ADMINISTRATOR_LOGIN, False) or AD.ADMINISTRATOR_LOGIN
            )
            password = password or PIH.DATA.VARIABLE.ENVIRONMENT.value(
                LINK.ADMINISTRATOR_PASSWORD
            )
    ...
    class PATH(PATHS, PathTool):
        ...
        def file_path_list_by_directory_info(
            ...
            if n(confirmed):
                ...
                confirmed_file_list: list[str] = DataTool.map(
                    lambda item: PIH.PATH.join(directory_path, item[0]),
                    info.confirmed_file_list,
                )
                if confirmed:
                    return confirmed_file_list
                return ListTool.diff(
                    confirmed_file_list,
            ...
            def wappi_status(
                ...
                title: str = j(
                    (
                        ...
                        ": ",
                        value.name or ("" if n(profile) else profile.name),  # type: ignore
                        nl(),
                    )
                )
                if value.authorized:
    ...
    class RESULT(ResultTool):
        ...
        class NOTES:
            ...
            def get_by_id(value: str, label: str | None = None) -> Result[Note] | None:
                ...
                note: Note | None = None
                event_ds: EventDS | None = one(
                    PIH.RESULT.EVENTS.get_by_key(event, value)
                )
                if nn(event_ds):
                    return PIH.RESULT.NOTES._convert_from_event(event_ds)
                """
                return None
            ...
            def find(
                ...
                if e(PIH.RESULT.NOTES._local_database_cache[section]):
                    ...
                    if need_update:
                        ...
                        PIH.RESULT.NOTES._local_database_cache[section] = ResultTool.map(
                            lambda item: File(
                            ...
                            ResultTool.map(
                                ...
                                PIH.RESULT.EVENTS.get(event),  # type: ignore
                            ),
                        )
                result: Result[list[File]] = (
                    PIH.RESULT.NOTES._knowledge_database_cache
                ...
                result = (
                    PIH.RESULT.NOTES._local_database_cache[CONST.FILES.SECTION][0]
                    if n(value)
    ...
    class CHECK:
        ...
        class EMAIL:
            @staticmethod
            def accessability(value: str, cached: bool = True) -> bool:
    ...
    class ACTION:
        ...
        class NOTES:
            ...
            @staticmethod
            def _cache_database_by_label(
                getter: Callable[[list[Any]], Result[list[Note]]],
                cache_holder: dict[
                    str, tuple[Result[list[Note]], dict[str, Result[Note]]]
                ],
                value: str | Events = None,
                event: Events | None = None,
            ) -> bool:
                args: list[Any] = [value, event]
            ...
            def cache_knowledge_database_by_label(args: list[Any] | None) -> bool:
                return PIH.ACTION.NOTES._cache_database_by_label(
...
def serve(
    service_role_or_description: ServiceRoles | ServiceDescriptionBase,
    call_handler: Callable[[ServiceCommands, ParameterList], Any | None] | None = None,
