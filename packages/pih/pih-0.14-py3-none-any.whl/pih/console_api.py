...
class ConsoleAppsApi:
    ...
    def resources_and_indications_check(
        ...
        all: bool = False,
        additional_text: dict[CheckableSections, str] | None = None,
    ) -> None:
        with self.output.make_indent(2):
            ...
            if CheckableSections.POLIBASE in checkable_section_list:
                ...
                if nn(additional_text[CheckableSections.POLIBASE]):
                    self.output.write_line(additional_text[CheckableSections.POLIBASE])  # type: ignore
            if CheckableSections.VALENTA in checkable_section_list:
                scanned_file_path_list: list[str] | None = (
                    A.PTH.file_path_list_by_directory_info(A.PTH.MEDICAL_DATA.VALUE, confirmed = False)
                )
