...
class MakeReport:
    def __init__(self, scc):
        ...
        self.template_path = self.path + "/paux_material"
        self.making_path = self.path + "/making_report"

        # Crea el directorio 'reports' si no existe
        if not os.path.exists(self.path + "/reports/"):
    ...
    def makeExcel(self, project=None):
        ...
        if project is None:
            ...
            self.scc.scc_with_assessments.to_excel(
                self.path + "/reports/" + "Report_all_subscriptions_" + str((datetime.date.today()).strftime("%d-%m-%Y")) + ".xlsx",
                index=False
            )
        else:
            ...
            tmp.to_excel(
                self.path + "/reports/" + "Report_" + self.project + "_" + str((datetime.date.today()).strftime("%d-%m-%Y")) + ".xlsx",
                index=False
            )
    ...
    def makeReportCover(self):
        ...
        doc.replace_pic('logo.png', self.template_path + "/images/" + self.scc.client_name + '.png')
    ...
    def makeReportRecommendationPage(self, recommendation):
        ...
        doc.replace_pic('Picture 4', self.path + '/paux_material/images/' + sev.lower())
    ...
    def compose(self):
        ...
        try:
            ...
            docpath = self.path + "/reports/" + "Report_" + self.project + "_" + str((datetime.date.today()).strftime("%d-%m-%Y")) + ".docx"
