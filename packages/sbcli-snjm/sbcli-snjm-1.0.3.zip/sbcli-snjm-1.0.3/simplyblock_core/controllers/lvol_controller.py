...
def add_lvol_on_node(lvol, snode, ha_comm_addrs=None, ha_inode_self=None):
    ...
    if lvol.snapshot_name:
        ...
        label = "label"
        desc = "desc"
        ret = rpc_client.ultra21_lvol_mount_lvol(lvol_name, snapshot_name, label, desc)
        if not ret:
            return False, "Failed to create base lvol bdev"

        base_bdev = lvol.base_bdev
        ret = rpc_client.ultra21_lvol_mount_snapshot(snapshot_name, lvol_name, base_bdev)
        if not ret:
    ...
    ret = rpc_client.nvmf_subsystem_add_ns(lvol.nqn, lvol_name, lvol.uuid, lvol.guid)
