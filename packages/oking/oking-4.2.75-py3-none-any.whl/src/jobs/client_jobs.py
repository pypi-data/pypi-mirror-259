        ...
        try:
            if src.client_data['operacao'].lower().__contains__('okvendas'):
                ...
                clients = [Cliente(
                    ...
                    codigo_representante=c.representative_code) for c in db_clients]
                total = len(clients)
                send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
...
def job_send_points_to_okvendas(job_config_dict: dict):
    with (lock):
        ...
        try:
            ...
            send_log(job_config_dict.get('job_name'), False, False,
                     f"Enviando {pontos_para_okvendas['parceiros'].__len__()} Pontos", 'info', 'PONTOS_PARA_OKVENDAS',
                     '')
            if api_okvendas.post_send_points_to_okvendas(pontos_para_okvendas):
                protocol_pontos_to_okvendas(job_config_dict, db_config, pontos_para_okvendas)
        except Exception as ex:
            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True
...
def points_to_okvendas_dict(pontos_para_okvendas):
    ...
    for i in pontos_para_okvendas:
        retorno['parceiros'].append({
            'codigo_parceiro': i['codparc'],
            'codigo_empresa': i['codemp'],
