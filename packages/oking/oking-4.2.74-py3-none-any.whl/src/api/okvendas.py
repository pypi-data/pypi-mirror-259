...
def get_product_by_code(code) -> (ResponseProductByCode, str):
    try:
        ...
        if response.status_code == 404:
            ...
            return None, result['message'] if 'message' in result else f'Não foi encontrado o produto: {code} na api okvendas'
        ...
        if response.ok:
            ...
            logger.warning(f'Retorno sem sucesso {response.status_code} - {response.url}')
            return None, result['message'] if 'message' in result else f'Não foi possível consultar o produto: {code} na api okvendas'
...
def put_representative(body: List[Representante]):
    try:
        ...
        logger.error(f'Erro ao realizar POST /representante na api okvendas {str(e)}')
...
def send_associated_product(body):
    ...
    try:
        ...
        logger.error(f'Erro ao realizar POST na api okvendas {src.client_data.get("url_api_principal")}/vitrine/produtoassociado'
                     + str(ex), exc_info=True)
        return False
...
def post_send_points_to_okvendas(body):
    try:
        ...
        if result['sucesso']:
            ...
            logger.error(
                ...
                + str(result['mensagem']), exc_info=True)
            return False
