...
class Rtspntf:
    def __init__(self, rtsp_url: str, screen_sx: float, screen_sy: float, screen_ex: float, screen_ey: float, webhook_url: str, diff_threshold: float = 40.0, process_interval: float = 0.2, save_img_dest: Optional[str] = None) -> None:
    ...
    def _detect_motion(self, img_screen: Image, img_else: Image) -> None:
        ...
        if (area_mean_diff > self.diff_threshold):
            self.logger.info("Motion Detected %s %s %s", diff_screen_.mean(), diff_else_.mean(), area_mean_diff)
        else:
            self.logger.info("No Motion Detected %s %s %s", diff_screen_.mean(), diff_else_.mean(), area_mean_diff)
            ...
            while client.isOpened():
                ...
                if _image:
                    ...
                    if self._is_previous_img_exists():
                        self._detect_motion(img_screen, img_else)
