...
class NNInterface():
    ...
    @classmethod
    def transform_batch(cls, x, y=None, n_classes=2):
    ...
    def _transform_input(cls,input):
        """ Transforms a training input to the format expected by the network.
            ...
            When :class:`NNInterface` is subclassed to make new neural_network classes, 
            this method can be overwritten to accomodate any transformations required. 
            Common operations are reshaping of an input.

            Args:
                input:numpy.array
                    An input instance. Must have 2, 3, or 4 dimensions.
        ...
        if input.ndim == 2:
            ...
            transformed_input = input #do nothing
        else:
            raise ValueError("Expected input to have 2, 3, or 4 dimensions, got {} ({}) instead".format(input.ndim, input.shape))
    ...
    def run_on_instance(self, input, return_raw_output=False, transform_input=True):
        ...
        if transform_input == True:
            input = self.transform_batch(input)
    ...
    def run_on_batch(self, input_batch, return_raw_output=False, transform_input=True):
        """ Run the model on a batch of inputs
            ...
            Args:
                ...
                return_raw_output:bool
                    If False, the model output will be transformed by :func:`transform_output`.
                    If true, the model output will return the output given by the tensorflow predict_on_batch function.
                    In this case, the return is a numpy array(s) of prediction scores with shape (batch_size, n_classes).   
