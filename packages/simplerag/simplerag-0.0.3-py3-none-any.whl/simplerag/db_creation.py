...
class Data:
    ...
    def process_documents(self, source_directory, chunk_size=500, chunk_overlap=50, ignored_files=[]):
        """
...
class DataBase:
    ...
    def append_in_db(self, texts, similarity_embeddings , chroma_settings):
        # Update and store locally vectorstore
        print(f"Appending to existing vectorstore at {self.database_directory}")
        db = Chroma(persist_directory=self.database_directory, embedding_function=similarity_embeddings,
    ...
    def create_new_db(self, texts, similarity_embeddings, chroma_settings):
        #texts = process_documents()
        print(f"Creating embeddings. May take some minutes...")
        db = Chroma.from_documents(texts, similarity_embeddings, persist_directory=self.database_directory,
                                   client_settings=chroma_settings)
