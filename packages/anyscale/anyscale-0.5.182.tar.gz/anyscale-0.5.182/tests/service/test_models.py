...
class TestServiceConfig:
    ...
    def test_applications(self):
        applications = [
            {"import_path": "main:app"},
            {"import_path": "main_app", "name": "app2"},
        ]
    ...
    def test_image(self):
        ...
        config = ServiceConfig(
            applications=[{"import_path": "main:app"}], image="my-custom-image:1"
        )
        assert config.image == "my-custom-image:1"

        with pytest.raises(TypeError, match="'image' must be a string"):
            ServiceConfig(applications=[{"import_path": "main:app"}], image=123)
    ...
    def test_override_working_dir_import_path(self, working_dir: Optional[str]):
        config = ServiceConfig(
            applications=[{"import_path": "main:app"}], working_dir=working_dir,
        )

        if working_dir is None:
            assert "runtime_env" not in config.applications[0]
        else:
            config.applications[0]["runtime_env"] = {
                "working_dir": working_dir,
            }

    @pytest.mark.parametrize("working_dir", [None, ".", "s3://path.zip"])
    def test_override_working_dir_applications(self, working_dir: Optional[str]):
        ...
        config = ServiceConfig(applications=applications, working_dir=working_dir)
