...
class PandasFeatureViewPipelineNode(PandasExecNode):
    ...
    def _call_python_udf_and_verify_result(
        ...
        def batch_iterator(chunk_size=1_000_000):
            input_batch = itertools.islice(input_it, chunk_size)
            try:
                ret = [user_function(*args, **kwargs) for args, kwargs in input_batch]
            except Exception as exc:
                msg = (
            ...
            try:
                yield pyarrow.RecordBatch.from_pylist(ret, schema=tecton_schema_to_arrow_schema(expected_schema))
