...
_SNOWFLAKE_TYPE_TO_PA_TYPE = [e.pa_type for e in FIELD_TYPES]
...
class SnowflakeCompute(QueryTreeCompute):
    ...
    def run_sql(
        ...
        with self.monitoring_ctx(sql_string) as progress_logger:
            ...
            first_table = next(table_it, None)

            if first_table is None or first_table.num_rows == 0:
                ...
                return pyarrow.RecordBatchReader.from_batches(schema, [])

            if expected_output_schema:
                diff = validate_schema_arrow_table(table=first_table, schema=expected_output_schema)
                if diff:
                    msg = "Result from Snowflake transformation does not match expected schema.\n"
                    msg += diff.as_str()
                    raise TectonValidationError(msg, can_drop_traceback=True)

            def batch_iterator(chunk_size=1_000_000):
                first_table_processed = self._post_process(first_table, expected_output_schema)
                for batch in first_table_processed.to_batches(chunk_size):
                    yield batch

                for table in table_it:
                    table = self._post_process(table, expected_output_schema)
                    for batch in table.to_batches(chunk_size):
                        yield batch

            progress_logger(1.0)
            output_schema = (
                tecton_schema_to_arrow_schema(expected_output_schema) if expected_output_schema else first_table.schema
            )
            return pyarrow.RecordBatchReader.from_batches(output_schema, batch_iterator())

    @staticmethod
    def _post_process(table: pyarrow.Table, expected_output_schema: Optional[Schema] = None) -> pyarrow.Table:
        ...
        if expected_output_schema:
            ...
            processed = processed.cast(tecton_schema_to_arrow_schema(expected_output_schema))
