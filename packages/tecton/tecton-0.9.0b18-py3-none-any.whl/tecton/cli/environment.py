...
def resolve_requirements(
    ...
    if not on_demand_transform_runtime_version and not rift_runtime_version:
        printer.safe_print(
            f"{ERROR_MESSAGE_PREFIX} Please include at least one of the supporting libraries:\n"
            f"💡  `tecton-runtime` package (https://pypi.org/project/tecton-runtime) to support On Demand and Streaming jobs\n"
            f"💡  `tecton[rift]` package (https://pypi.org/project/tecton) to support Rift batch materialization jobs",
            file=sys.stderr,
        )
        sys.exit(1)
