...
# Setup OpenAI
openai.api_key = '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'
...
def create_gui(file_path):  # Accept `file_path` as an argument
    global root
    root = tk.Tk()
    root.title("Dunkin")
    ...
    canvas.grid(row=0, column=0, sticky="nsew")  # Use grid for the canvas layout management

    # Setup the GUI layout and widgets
    start_button = Button(root, text="START", command=lambda: start_questions(file_path))
    start_button.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)  # Use grid for the start_button
...
if __name__ == "__main__":
    file_path = r"D:\Chatgpt_voice_assessment\Voice\ss.txt"  # Ensure this path is correct for your file
    create_gui(file_path)  # Pass `file_path` as an argument to `create_gui`