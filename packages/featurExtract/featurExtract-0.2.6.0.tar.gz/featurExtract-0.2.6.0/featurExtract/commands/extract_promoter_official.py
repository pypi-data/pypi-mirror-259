...
def get_promoter(args):
    ...
    if not args.gene:
        # loop all gene
        param_list = [(g, args.database, args.genome, args.promoter_length, 
                       args.utr5_upper_length, args.output_format) 
                     for g in db.all_features(featuretype='gene', order_by="seqid")]
        with Pool(processes=args.process) as p:
            promoter_seq_list = list(tqdm(p.imap(sub_promoter, param_list), 
                                     total=len(param_list), ncols = 80, desc='Promoter processing:'))
