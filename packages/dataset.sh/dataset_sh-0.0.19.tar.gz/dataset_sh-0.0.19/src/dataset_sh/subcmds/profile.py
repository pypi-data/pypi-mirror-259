...
@click.option('--name', '-n', 'name', help='profile name.', default='default')
...
def add_profile(name, host, force):
    ...
    Args:
        name: profile name.
        host: HOST url, default is https://remote.dataset.sh
    ...
    if host is None or host.strip() == '':
        host = ask_hostname('https://remote.dataset.sh')
...
def remove_profile(name):
    """
    Remove existing profile on file.
    """
