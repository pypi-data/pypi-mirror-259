...
def check_1 ():
	import non_venomous
	import non_venomous.db as non_venomous_db
	...
	records_1 = non_venomous_db.records (
		db_directory = normpath (join (variable, f"status_db"))
	)

	scan = non_venomous.start (
	...
	records_2 = non_venomous_db.records (
		db_directory = normpath (join (variable, f"status_db"))
	)
	assert (len (records_2) == (len (records_1) + 1))
	
	last_record = non_venomous_db.last_record (
		db_directory = normpath (join (variable, f"status_db"))
	)
