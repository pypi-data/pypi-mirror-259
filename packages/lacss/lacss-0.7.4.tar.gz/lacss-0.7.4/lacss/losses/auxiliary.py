...
def _compute_edge(preds, height, width):
    ...
    patch_edges = jnp.where(preds["instance_mask"], patch_edges, 0)
    ...
    return jnp.tanh(combined_edges)
...
def self_supervised_edge_loss(batch, prediction):
    ...
    instance_edge = _compute_edge(prediction, height, width)
