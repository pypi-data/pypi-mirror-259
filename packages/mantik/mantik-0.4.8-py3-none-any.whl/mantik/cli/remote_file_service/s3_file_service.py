...
import mantik.cli.remote_file_service._options as _options
...
@_arguments.SOURCE
@_arguments.TARGET
@_options.CONNECTION_ID
@_options.DATA_REPOSITORY_NAME
@_options.DATA_REPOSITORY_DESCRIPTION
def copy_file(
    source: str,
    target: str,
    ...
    file = s3_fs.copy_file(
        source=source,
        target=target,
        data_repository_name=name,
        data_repository_description=description,
    )
    click.echo(file)
...
def remove_file(
    ...
    s3_fs.remove_file(
        target=target, data_repository_name=name, data_repository_id=id
    )
...
def create_file_if_not_exists_file(
    ...
    file = s3_fs.create_file_if_not_exists(
        target=target,
        data_repository_name=name,
        data_repository_description=description,
    )
...
def create_directory(
    ...
    directory = s3_fs.create_directory(
        target=target,
        data_repository_name=name,
        data_repository_description=description,
    )
