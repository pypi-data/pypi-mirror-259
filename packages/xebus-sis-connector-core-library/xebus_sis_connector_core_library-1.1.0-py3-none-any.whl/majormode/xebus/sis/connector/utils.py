...
def normalize_names_codes_mapping(mapping: dict[str, Any]) -> dict[str, Any]:
    """
    Normalize the keys of a names codes mapping.

    Key normalization makes it possible to support minor differences
    (basically lowercase or uppercase letters, and accents) when searching
    for the code corresponding to a name.


    :param mapping: A mapping between names (the keys) and their
        respective codes (the values), such as, for example, language
