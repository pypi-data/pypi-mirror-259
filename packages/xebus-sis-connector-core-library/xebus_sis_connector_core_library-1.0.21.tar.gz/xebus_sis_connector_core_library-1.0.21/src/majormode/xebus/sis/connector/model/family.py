...
class FamilyList:
    ...
    def __build_primary_parent_guardianship(
        ...
        contacts: list[Contact] = [Contact(ContactName.EMAIL, email_address)]
        if phone_number:
            contacts.append(Contact(ContactName.PHONE,phone_number))
    ...
    def __build_secondary_parent_guardianship(
        ...
        if email_address:
            contacts.append(Contact(ContactName.EMAIL, email_address))
        if phone_number:
            contacts.append(Contact(ContactName.PHONE, phone_number))
            ...
            for cached_parent in cached_parents:
                if cached_parent and cached_parent != parent:
                    if parent.sis_account_id == cached_parent.sis_account_id:
                        ...
                        # information.
                        raise ValueError(
                            f"The parent \"{parent.sis_account_id}\" has already been defined but "
                        ...
                        if parent.email_address == cached_parent.email_address:
                            # Two parents MUST NOT be declared with the same email address.
                            raise ValueError(
                                f"The parent \"{parent.sis_account_id}\" has been declared with the same "
                                f"email address \"{parent.email_address}\" than the other parent "
                                f"\"{cached_parent.sis_account_id}\""
                            )
                            ...
                            logging.warning(
                                f"The parent \"{parent.sis_account_id}\" has been declared with the same "
                                f"phone number \"{parent.phone_number}\" than the other parent "
                                f"\"{cached_parent.sis_account_id}\"; we are removing this phone number "
                                f"from the parent  \"{parent.sis_account_id} information"
                            )
                            ...
                            raise ValueError(
                                f"The parent \"{parent.sis_account_id}\" has been declared with a same "
                                f"identifier than the other parent \"{cached_parent.sis_account_id}\". "
