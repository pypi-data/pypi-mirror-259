...
class SisConnector(ABC):
    ...
    assert len(PRIMARY_GUARDIAN_PROPERTY_NAMES) == len(SECONDARY_GUARDIAN_PROPERTY_NAMES), \
        "Primary parent's property list is not the same as secondary parent's " \
        "property list"
    ...
    def _cleanse_rows(
        ...
        :raise ValueError: If a child is declared with parents without a
        ...
        for row_index, row in enumerate(rows):
            ...
            if cast.is_undefined(row[FamilyPropertyName.primary_guardian_email_address]):
                ...
                logging.info(
                    f"The primary guardian \"{row[FamilyPropertyName.secondary_guardian_sis_id]} "
                    "is not declared with an email address; replacing them with the "
                    "secondary guardian..."
                )
                self._replace_primary_guardian_with_secondary_parent(row)
    ...
    @classmethod
    def _replace_primary_guardian_with_secondary_parent(
            cls,
            row: dict[FamilyPropertyName, Any],
    ) -> None:
        """
