...
class S01(BasePage):
    def __init__(self, parent, controller):
        ...
        explain2 = tk.Label(master=self.headframe, text = 'NOTE: OPTION 2 - Ensure BHTPBANK file exist in the current path',font = controller.text_font)
        explain2.pack()

        explain3 = tk.Label(master=self.headframe, text = 'WARNING: Option 2 will freeze this window once RUN due to Plotting Function Limitation.', font = controller.text_font)
        explain3.pack()

        choice1 = tk.Label(master=self.optionframe, text = 'Option 1: BHTPBANK File Creator',font = controller.text_font)
        choice1.grid(row = 0, column = 0, sticky = "w", padx= 10, pady=5)

        filename = 'bhtpbank_file_creator_v10.xlsm'
        button1 = tk.Button(master=self.optionframe, text="Import Creator",command = lambda: SharedMethods.copy_example_files(filename),width = 20, height =1)
        button1.grid(row = 1, column = 0, sticky = "w", padx= 10, pady=5)

        choice2 = tk.Label(master=self.optionframe, text = 'Option 2: BHTPBANK File Check (Fill Below Information and Click Run)',font = controller.text_font)
...
class S02(BasePage):
    def __init__(self, parent, controller):
        ...
        filename = 'ac_section_impedance.xlsm'
        ...
        filename2 = 'dc_section_impedance.xlsm'
...
class S03(BasePage):
    def __init__(self, parent, controller):
        ...
        # add widgets here
        head = tk.Label(master=self.headframe, text = 'Page 3: Extra OSLO Creator / Impedance Calculation',font = controller.sub_title_font)
        head.pack()

        explain = tk.Message(master=self.headframe, text = 'This support the creating of Extra.OSLO file. Excel creator is recommended due to the complexity of information involved. ',aspect = 1200, font = controller.text_font)
        explain.pack()

        explain1 = tk.Label(master=self.headframe, text = 'NOTE: OPTION 1 - Import extra.oslo Creator ',font = controller.text_font)
        explain1.pack()
...
class S04(BasePage):
    def __init__(self, parent, controller):
        ...
        explain = tk.Message(master=self.headframe, text = 'This will do some common pre-CIF analysising and fitering',aspect = 1200, font = controller.text_font)
        explain.pack()
    ...
    def run_cif_check(self):
        try:
            ...
            thread = threading.Thread(target=SharedMethods.common_thread_run, 
                                      args=("xxxxx.py",sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step))
            thread.start()
...
class S05(BasePage):
    def __init__(self, parent, controller):
        ...
        # add widgets here
        head = tk.Label(master=self.headframe, text = 'Page 5: Provision for development',font = controller.sub_title_font)
        head.pack()

        explain = tk.Message(master=self.headframe, text = 'Provision for development',aspect = 1200, font = controller.text_font)
        explain.pack()
