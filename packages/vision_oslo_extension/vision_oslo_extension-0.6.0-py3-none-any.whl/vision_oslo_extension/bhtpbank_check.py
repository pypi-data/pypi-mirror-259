...
#=================================================================
"""
This module was built to check BHTPBANK File.
...
def main(sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
    ...
    if not os.path.isfile(bhtpname): # if the bhtpbank file exist
       print("BHTPBANK file {} does not exist. Exiting...".format(bhtpname))
       return False
    ...
    if (carnumber <= 0) or (carnumber > 20):
        print("Invalid Car Number. Please Re-enter a number between 1-20. Existing...")
        return False
    ...
    if len(warning_sum) != 0:
        ...
        print("WARNING: Unexpected value occurs.Please check screen output for detail ! Warning Summary.txt was saved !")
...
#        print('\033[1;33m') # color control warning message Yellow
    ...
    if BE_flag == False:
        print('\033[1;0m') # color control warning message reset
        print("Stationary Auxiliary Power at Different Voltage Levels:")
        for index, value in enumerate(vol_list):
...
def list_data_action(line,vol_list,speed_list,TE_list,Re_I_list,Im_I_list, \
    ...
    if spe == True:
        data = float(line[8:17].strip()) # get the value of speed
        speed_list[vo_index].append(data)
        data = float(line[21:29].strip()) # get the value of TE
        TE_list[vo_index].append(data)
        data = float(line[40:52].strip()) # get the value of Real Current
        Re_I_list[vo_index].append(data)
        data = float(line[52:].strip()) # get the value of Imag Current
        Im_I_list[vo_index].append(data)

    if pec == 1:
        data = float(line[:9].strip()) # get the value of TE percentage
        TE_list_p1[vo_index].append(data)
        data = float(line[9:19].strip()) # get the value of Real perc
        Re_I_list_p1[vo_index].append(data)
        data = float(line[19:].strip()) # get the value of Imag perc
...
if __name__ == '__main__':
    ...
    text_input = "BSENACFR"  # Adjust as needed
