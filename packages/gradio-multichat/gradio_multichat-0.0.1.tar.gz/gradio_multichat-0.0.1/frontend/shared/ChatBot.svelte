<script lang="ts">
	...
	// TODO: Add a CSS Handler towards each message
  ...
  >
	<div class="message-wrap" class:bubble-gap={layout === "bubble"} use:copy>
	  {#if value !== null}
		<!--Instead of 'message_pair' it now uses 2 separated entries, one for entries other for entities  -->
			...
			>
			  ...
			  {#if entity.avatar !== null}
				<div class="avatar-container">
				  <Image
					...
					alt="{entity.role === 'user' ? 'user' : 'bot'} avatar"
				  />
				</div>
			  {/if}
			  ...
			  >
				<button
				  ...
				  dir={rtl ? "rtl" : "ltr"}
				  aria-label={(entity.role === "user" ? "user" : "bot") +
					"'s message: " +
					(typeof message === "string"
					  ? message
					  : `a file of type ${message.file?.mime_type}, ${
						  message.file?.alt_text ?? message.file?.orig_name ?? ""
						}`)}
				>
			  ...
			  {#if (likeable && entity.role !== "user") || (show_copy_button && message && typeof message === "string")}
				<div
				  class="message-buttons-{entity.role === 'user'
					? 'user'
					: 'bot'} message-buttons-{layout} {entity.avatar !== null &&
					'with-avatar'}"
