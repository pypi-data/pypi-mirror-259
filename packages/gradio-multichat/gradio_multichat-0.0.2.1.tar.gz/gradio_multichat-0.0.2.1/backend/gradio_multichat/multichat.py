...
# TODO: Add CCS customization for each entity/character, to make everything more flexible.

# class CSSData(GradioModel):
...
class ChatbotData(GradioRootModel):
    root: List[Tuple[Entity, Optional[Union[str | FileMessage]]]] | None
...
class MultiChat(Component):
    ...
    def __init__(
        ...
        *,
        ...
        if characters:
            self._setup_characters(characters)


    def _set_character(self, character: dict, name:str) -> dict:
        role = character.get("role")
        if not isinstance(role, str) or role not in ["user", "bot"]:
            raise ValueError(
                "The role provided is not valid.",
                f"It must be 'user' or 'bot': {character}.",
            )
        return {
            name: {
                "role": role,
                "avatar": processing_utils.move_resource_to_block_cache(
                    character.get("avatar", None), self
                ),
            }
        }

    def _setup_characters(self, characters: dict):
        if not isinstance(characters, dict):
            raise TypeError(
                f"Expected a dictionary of entities. Received: {characters}, {type(characters)}"
            )
        for content in characters:
            elements = characters.get(content)
            if not elements:
                print(f"Empty entity '{content}'! Skiping!")
                continue
            self.characters.update(self._set_character(elements, content))

    def _preprocess_chat_message(
        ...
        elif not contents:  # none or other type of empty values
            return None

        else:
            raise ValueError(f"Invalid message for RoleplayChat component: {contents}")

    def preprocess(
        self,
        payload: (
            list[tuple[str, str | None] | list[str, str | None]] | ChatbotData | None
        ),
        ...
        Returns:
            Passes the messages in the chatbot as a `list[list[str, str | None]]`. The inner list has 2 elements:
            1. dict = The entity assigned to the message.
            2. str = The pre-processed message.
        """
        ...
        for message in payload:
            if not isinstance(message, (list, tuple)):
                raise TypeError(
                    f"Expected a list of lists or list of tuples. Received: {message}"
                )

            if isinstance(message[0], str):
                entity = self.characters.get(message[0], None)
                if not entity:
                    raise TypeError(
                        f"The character/entity '{entity}' does not exists",
                    )
            else:
                raise TypeError(
                    f"Expected a string with the agent name, but received: {message}",
                )

            processed_messages.append(
                [
                    entity,
                    self._preprocess_chat_message(message[1]),
                ]
            )
        return processed_messages

    def _postprocess_entity(self, entity_name: str) -> dict:
        if not isinstance(entity_name, str):
            raise ValueError(
                f"Invalid message for RoleplayChat, it musb be a string. Received: {entity_name}"
            )

        entity = self.characters.get(entity_name)
        
        if not entity:
            raise ValueError(f"'{entity}' is not a valid registred character.")
    ...
    def _postprocess_chat_message(
        self, chat_message: Optional[Union[str | tuple | list]]
        ...
        if not chat_message:
            ...
            raise ValueError(
                f"Invalid message for RoleplayChat component: {chat_message}"
            )
    ...
    def postprocess(
        ...
        for message_pair in value:
            ...
            processed_messages.append(
                [
                    self._postprocess_entity(message_pair[0]),
                    self._postprocess_chat_message(message_pair[1]),
                ]
            )
        return ChatbotData(root=processed_messages)

    def example_inputs(self) -> Any:
        return [["user", "Hello"], ["bot", f"World!"]]
