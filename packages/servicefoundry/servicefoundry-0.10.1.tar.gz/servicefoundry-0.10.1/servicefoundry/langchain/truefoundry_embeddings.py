...
class TrueFoundryEmbeddings(BaseModel, Embeddings):
    ...
    def _remote_embed(self, texts, query_mode=False):
        ...
        params = {**self.model_parameters} if self.model_parameters else {}

        payload = {
            "input": texts,
            "model": {"name": self.model, "parameters": params},
        }
        url = f"{self.tfy_llm_gateway_url}/api/inference/embedding"
        logger.debug(
