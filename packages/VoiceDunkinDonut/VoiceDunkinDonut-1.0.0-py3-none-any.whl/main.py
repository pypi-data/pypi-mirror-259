...
def create_gui():
    ...
    # logo_img_url = 'https://drive.google.com/file/d/128wsCfumenUyMjMFgtNrAg2EpY9buu1s/view?usp=sharing'
    # response = requests.get(logo_img_url)
   
    # if response.status_code == 200:
    #     try:
    #         logo_img = Image.open(BytesIO(response.content)).convert("RGBA").resize((200, 100), Image.LANCZOS)
    ...
    start_button = Button(root, text="START", command=lambda: start_button_clicked(start_button), **start_button_style)
...
def start_button_clicked(start_button):
    hide_start_button(start_button)
    start_questions(file_path)
 
if __name__ == "__main__":
    file_path = r"ss.txt"  # Adjust this to your file path
    create_gui()