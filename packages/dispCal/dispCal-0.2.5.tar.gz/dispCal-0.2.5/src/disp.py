...
def calDisp(thickness, vp, vs, rho, periods,dc0=0.0025,domega=0.0001,wave='rayleigh', mode=1, velocity='phase', flat_earth=True,ar=6370.0,parameter='vp',smoothN=1,Qp=[],Qs=[],fRef=1.0):
    ...
    if (vs<=0.01).sum()<=1:
        ...
        if velocity == 'ellipticity':
            return np.array(cyDisp.ellipticity(thickness.astype(np.float64), vp.astype(np.float64), vs.astype(np.float64),rho.astype(np.float64),periods.astype(np.float64),Qp,Qs,fRef,dc0=dc0,domega=domega,isR=(wave =='rayleigh'),isFlat=flat_earth,mode=mode,ar=ar))
        if velocity =='kernel':
