...
def parse_url(url):
    ...
    url_split = split_forward_slash(url)
    final_url_split = []
    parameters = []
    for u in url_split:
        if len(u) == 0:
        ...
        if name is not None:
            u = "{" + name + "}"
        u = u.replace("<", "{").replace(">", "}")
        final_url_split.append(u)
    final_url = "/".join(final_url_split).replace("^", "").replace("$", "").replace("?", "").replace("*", "")
    if len(final_url) == 0 or (final_url[-1] != "/" and final_url[-1] != "."):
        final_url += "/"
    _PATH_PARAMETER_COMPONENT_RE = re.compile(r"<(?:(?P<converter>[^>:]+):)?(?P<parameter>\w+)>")
    return {
        "url": re.sub(_PATH_PARAMETER_COMPONENT_RE, r"{\g<parameter>}", final_url),
