...
from tool_kit.register import find_tools_by_package_name, get_main_module_name, get_doc_dir, get_settings
from tool_kit.summary_widget import SummaryWidget
...
class MainUI(QMainWindow):
    def __init__(self, category: Enum = None):
        ...
        if SETTINGS.WINDOW_TITTLE:
            self.setWindowTitle(SETTINGS.WINDOW_TITTLE)

        doc_dir = get_doc_dir()  # 存放工作流程的帮助文档的dir路径
        tools = find_tools_by_package_name(package_name)  # 被管理的工具的子类列表

        self.main_layout = QHBoxLayout()
        layout_summary_and_category = QVBoxLayout()

        # 概要组件
        self.summary_widget = SummaryWidget(title='Summary')
        ...
        layout_summary_and_category.addWidget(self.summary_widget)
        layout_summary_and_category.addWidget(self.category_list_widget)

        # self.main_layout.addWidget(self.category_list_widget)
        self.main_layout.addLayout(layout_summary_and_category)
