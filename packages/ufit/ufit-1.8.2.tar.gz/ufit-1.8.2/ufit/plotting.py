...
class DataPlotter:
    ...
    def plot_params(self, params, chisqr):
        ...
        for p in params:
            if p.expr:
                continue
            s.append('%-12s = %9.4g ± %9.4g' % (p.name, p.value, p.error))
            if p.expr:
                s[-1] += ' (fixed)'
            if p.overall:
                s[-1] += ' (global)'
        s.append('%-12s = %9.3f' % ('chi²/NDF', chisqr))
        s = '\n'.join(s)
    ...
    def plot_image(self, imgdata, multi=False):
        axes = self.axes
        norm = LogNorm() if getattr(self.canvas, 'logz', False) else None
