...
import time
import traceback
from logging import DEBUG, INFO, WARNING, Formatter, Handler, StreamHandler, \
    getLogger, root
from os import path
...
class ConsoleFormatter(Formatter):
    ...
    def __init__(self, fmt=None, datefmt=None, colorize_fn=None):
        Formatter.__init__(self, fmt, datefmt)
        if colorize_fn:
            self.colorize = colorize_fn
        else:
...
class LogfileHandler(StreamHandler):
    ...
    def _open(self):
        ...
        return open(self.baseFilename, 'a', encoding='utf-8')
...
fmter = ConsoleFormatter(colorize_fn=colorize)
