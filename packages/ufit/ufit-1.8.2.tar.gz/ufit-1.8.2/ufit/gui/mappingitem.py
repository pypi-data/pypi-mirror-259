...
from ufit.data.mapping import bin_mapping, get_xss_yss_zss
...
class MappingItem(SessionItem):
    ...
    def __init__(self, datas, settings):
        ...
        if settings is None:
            for key in self.settings:
                if 'map_' + key in session.props:
                    self.settings[key] = getattr(session.props, 'map_' + key)
            if self.settings.xaxis and self.settings.yaxis:
                self._auto_select_yscale(self.settings.xaxis,
                                         self.settings.yaxis)
        SessionItem.__init__(self)

    def _auto_select_yscale(self, xaxis, yaxis):
        xss, yss, _ = get_xss_yss_zss(xaxis, yaxis, self.datas)
        xptp, yptp = ptp(xss), ptp(yss)
        if yptp:
            self.settings.yscale = float('%.2g' % (xptp / yptp))
    ...
    def update_htmldesc(self):
        ...
        self.htmldesc = (
            '<img src=":/map.png">&nbsp;&nbsp;<big><b>%d</b></big> '
            '- %s<br>%s' % (self.index, self.settings.title,
...
class MappingPanel(QFrame):
    ...
    def _update_settings(self):
        ...
        if xaxis != s.xaxis or yaxis != s.yaxis:
            # new axes selected => discard limits on replot and rescale
            self.mapdata = None
            self.item._auto_select_yscale(xaxis, yaxis)
            self.scaleEdit.setText('%.2g' % s.yscale)
        if xaxis == yaxis:
        ...
        for key in s:
            setattr(session.props, 'map_' + key, s[key])
        session.set_dirty()
