...
from numpy import absolute, corrcoef, diag, dot, finfo, infty, isnan, \
    maximum, minimum, nonzero, ones, sqrt, zeros
...
def __leastsq(xyw, func, p, dp, niter=50, mode='print', eps=1e-3):
    ...
    OPTIONS IN:
      ...
      mode (string) : can be 'print' or 'silent'. In silent mode output is not
                      written to standard out
                      but is saved in a message string.

    Author:  EF <manuf@ldv.univ-montp2.fr>, RM <rfm2@ds2.uh.cwru.edu>,
    AJ <jutan@charon.engga.uwo.ca>
    ...
    # python function object of the form 'y=f(x,instr_p)'.
    # func = self.__getfunc(self.func, self.add_pars)
    ...
    if len(options) == 0:
        ...
        if nor != n:
            raise FitError('Options and parameter matrices must have same '
                           'number of rows!')
    ...
    for iteration in range(niter):
        ...
        prt, s, v = svd(prt, full_matrices=False)
        # prt=unit matr, s=eigenval : prt=prt*s*v.T
        v = v.T
        g = dot(prt.T, r)  # gradient by Gauss-Newton formula

        for k in range(len(epstab)):  # pylint: disable=consider-using-enumerate
            epsl = maximum(epsllast*epstab[k], 1e-7)
            se  = sqrt(s*s+epsl)
            gse = g/se
            chg = dot(v, gse)*nrm  # change on params
            # check the change constraints and apply if necessary
            for l in range(n):
                if maxstep[l] == infty:
                    break
    ...
    if not isnan(jtgjinv).all():
        ...
        errmsg += "Warning : leasqr : couldn\'t compute squared Jacobian " \
            "inverse matrix (no covp, no corp).\n"
        covp = zeros((n, n))
        corp = ones((n, n))
    ...
    for p_i, d_i in zip(p, deltap):
        res['values'].append(p_i)
        res['errors'].append(d_i)
