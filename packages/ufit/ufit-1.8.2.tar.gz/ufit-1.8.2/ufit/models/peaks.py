...
class Gauss(Model):
    ...
    * `fwhm` - Full width at half maximum
    ...
    def convert_pick(self, p, w):  # pylint: disable=arguments-differ
        return {
...
class LorentzInt(Model):
    ...
    * `fwhm` - Full width at half maximum
    ...
    def __init__(self, name='', pos=None, int=None, fwhm=None):
        ...
        self.fcn = lambda p, x: 2 * abs(p[pint]) / (pi * p[pf]) / \
            (1 + 4*(x - p[pp])**2/p[pf]**2)
...
class DHO(Model):
    ...
    * `tt` - Temperature in K
    ...
    def convert_pick(self, p1, w, p2):  # pylint: disable=arguments-differ
        return {
