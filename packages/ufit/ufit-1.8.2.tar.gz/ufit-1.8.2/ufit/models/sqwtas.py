...
from ufit.rescalc import CFGNAMES, PARNAMES, calc_MC, calc_MC_cluster, \
    calc_MC_mcstas, load_cfg, load_par, plot_resatpoint, resmat
    ...
    *instfiles* must be ('instr.cfg', 'instr.par').
    ...
    def __init__(self, sqw, instfiles, NMC=2000, name=None, cluster=False,
        ...
        if isinstance(sqw, str):
            ...
            with open(mod.__file__.rstrip('c'), encoding='utf-8') as fp:
                code = fp.read()
        ...
        for par in init:  # pylint: disable=consider-using-dict-items
            if par == 'par_ALL':
                par_all = True
            elif par == 'cfg_ALL':
                cfg_all = True
        ...
        if cfg_all:
            ...
            for i, cfgname in enumerate(CFGNAMES):
                init['cfg_' + cfgname] = str(cfg_orig[i])

        # numba compat
        arg_sqw = getattr(self._sqw, 'py_func', self._sqw)
        self._pvs = self._init_params(name,
                                      ['NMC', 'bkgd'] +
                                      inspect.getfullargspec(arg_sqw)[0][6:] +
                                      instparnames, init)
    ...
    def simulate(self, data):
        _, _, dependent, _ = prepare_params(self.params, data.meta)
