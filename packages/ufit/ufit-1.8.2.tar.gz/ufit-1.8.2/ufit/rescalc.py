...
from numpy import abs, arange, arccos, arcsin, arctan2, array, concatenate, \
    cos, cross, degrees, delete, diag, dot, loadtxt, matrix, pi, radians, \
    real, reshape, sin, sqrt, tan, zeros
...
""" % info
    ...
    def run_mcstas(self, NMC, QE):
        ...
        if len(arr.T) == 4:
            return [], [], [], [], []
        kix, kiy, kiz, kfx, kfy, kfz, _x, _y, _z, pi_, pf = arr.T
    ...
    def showCFG(self):
        ...
        msg += '===============================\n'
        for i, cfgitem in enumerate(self.cfg):
            msg += '%2i' % i + ('%1.3f   :  ' % cfgitem).rjust(16) + \
                '%s\n' % CFGDESC[i]
        print(msg)
...
def calc_MC(x, fit_par, sqw, resmat, NMC, use_caching=True):
    ...
    for QE in x:
        QE = tuple(QE)
        if QE in resmat.m_cache and use_caching:
            b_mat, sigma, R0_corrected = resmat.m_cache[QE]
        else:
            ...
            resmat.m_cache[QE] = b_mat, sigma, R0_corrected
