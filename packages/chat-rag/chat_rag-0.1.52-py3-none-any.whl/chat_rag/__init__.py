...
class RAG:
    ...
    def retrieve(
        ...
        contexts = [context["content"] for context in contexts] # get unique contexts
        last_contexts = [contexts[:prompt_structure_dict["n_contexts_to_use"]]] # structure for references

        # Use a list comprehension to preserve order and not adding duplicates
        seen = set()
        contexts = [x for x in prev_contexts + contexts if not (x in seen or seen.add(x))]

        return contexts, last_contexts
