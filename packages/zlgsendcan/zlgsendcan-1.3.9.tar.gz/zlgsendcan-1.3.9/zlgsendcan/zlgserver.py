...
class ZlgSend:
    ...
    def __init__(self, handle, data, dbc_file,chanl=0, pro_name=None, is_key=False):
        ...
        # if GlobleValue.dbcinit is None:
        GlobleValue.dbcinit=get_dbc_db(dbc_file)
        if not is_key:
                ...
                if k not in ['cycle_time', 'send_num', 'msg_id', 'is_crc','chanl']:
                    ...
                    for sn, sv in v.items():
                        if isinstance(sv, dict):
                            sigchange_th = ThreadSingChange(GlobleValue.dbcinit,chanl)
                        ...
                        ob = DbcInit(k, v, GlobleValue.dbcinit)
                        if is_crc == '1':
