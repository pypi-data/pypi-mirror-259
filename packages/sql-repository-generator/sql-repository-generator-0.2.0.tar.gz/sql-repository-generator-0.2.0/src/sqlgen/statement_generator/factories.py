...
def make_object_bound_statement_generator_class_for[S, D](
    ...
    """
    joins: list[RelationshipProperty] = resolve_model_joins(cls, model_to_join)
    primary_key: Column = class_mapper(model_to_join).primary_key[0]
    return type(f"{model_to_join.__name__}Bound{cls.__name__}StatementGenerator", (ObjectBoundStatementGenerator,),
                {"cls": cls, "joins": joins, "primary_key": primary_key})
