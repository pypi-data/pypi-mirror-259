...
def _resolve_model_joins(
    ...
    for relation in inspected_mapper.relationships:
        ...
        if relation.entity.class_ is model_to_join:
            return previous_joins + [relation.class_attribute]
        try:
...
def resolve_model_joins[S, D](model_source: type[S], model_destination: type[D]) -> list[RelationshipProperty]:
    """
    ...
    """
    return _resolve_model_joins(class_mapper(model_source), model_destination, [], [])
