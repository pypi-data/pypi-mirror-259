...
class CmFormSubmission(models.Model):
    ...
    is_map_crowdfunding_target = fields.Boolean(string=_("Target lead"), compute="compute_is_map_crowdfunding_target",store="True")
    ...
    def compute_is_map_crowdfunding_target(self):
        for record in self:
            filtered_metadata_ids = record.form_submission_metadata_ids.filtered(
                lambda meta: meta.key == record.place_id.map_id.metadata_key and meta.value in record.place_id.map_id.metadata_values.split(',')
            )
            if filtered_metadata_ids:
                record.is_map_crowdfunding_target = True
            else:
                record.is_map_crowdfunding_target = False
