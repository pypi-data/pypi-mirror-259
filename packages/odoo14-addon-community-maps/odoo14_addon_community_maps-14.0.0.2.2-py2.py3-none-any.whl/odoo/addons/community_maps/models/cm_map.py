...
class CmMap(models.Model):
    ...
    metadata_values = fields.Char(string="Metadata values")
    # Proposal
...
#     )
    ...
    @api.constrains('metadata_key','metadata_values')
    def propagate_is_map_crowdfunding_target(self):
        submissions = self.env['crm.lead'].search([('submission_type','in',["place_submission","place_proposal_submission"])])
        for submission in submissions:
            submission.compute_is_map_crowdfunding_target()
