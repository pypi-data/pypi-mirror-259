...
#         first_overwrite['place_id'] = self.id
        ...
        for request in self.map_id.crowdfunding_notification_request_ids:
            if self.completed_percentage >= request.percentage:
                ...
                if existing_notification:
                    ...
                    self.crowdfunding_notifications(request)
    ...
    def crowdfunding_notifications(self, request):
        if request.notify_admins:
            ...
            for admin in request.map_id.admins_to_notify:
                ...
                template.send_mail(res_id=self.id, email_values=email_values)
        for submission in self.form_submission_ids:
            filtered_metadata_ids = False
            if request.notify_target_submissions:
                filtered_metadata_ids = submission.form_submission_metadata_ids.filtered(
                    lambda meta: meta.key == request.map_id.metadata_key and meta.value in request.map_id.metadata_values.split(',')
                )
                if filtered_metadata_ids:
                    template = request.template_target_submissions
                    email_values = {'email_to': submission.email_from}
                    template.send_mail(res_id=submission.id, email_values=email_values)
            if request.notify_submissions and not filtered_metadata_ids:
