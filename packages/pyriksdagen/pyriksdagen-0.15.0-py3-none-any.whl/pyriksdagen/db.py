...
def load_metadata(metadata_folder="corpus/metadata", processed_metadata_folder="input/matching"):
    if metadata_folder[-1] == "/":
        metadata_folder = metadata_folder[:-1]
    if processed_metadata_folder[-1] == "/":
        processed_metadata_folder = processed_metadata_folder[:-1]
    party_mapping = pd.read_csv(f'{metadata_folder}/party_abbreviation.csv')
