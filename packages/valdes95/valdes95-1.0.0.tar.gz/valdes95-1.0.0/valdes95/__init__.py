...
def wait_download(url,ichunk=0,index=0,file=None,session=None):
    ...
    f.close()
    if os.path.exists('valdes95/' + filename):
        os.unlink('valdes95/' + filename)
    shutil.move(filename,'valdes95/'+filename)
...
def initi():
    while (True):
        ...
        session = None
        init()
        print(Fore.YELLOW + '> Bienvenido a valdes95 <')
        print(Fore.YELLOW + 'Ingrese su enlace')
        url = input()
        if os.path.exists('valdes95/'):
            pass
        else:
            os.mkdir('valdes95/')
