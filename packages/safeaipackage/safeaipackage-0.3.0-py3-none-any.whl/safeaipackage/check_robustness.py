...
class Robustness():
    def __init__(self, xtrain, xtest, ytrain, ytest, model, perturbation_percentage= 0.05):
        """
    ...
    def perturb(self, data, variable):
        """
        ...
        percentile_5_index = int(np.ceil(self.perturbation_percentage * len(sorted_variable)))
        percentile_95_index = int(np.ceil((1-self.perturbation_percentage) * len(sorted_variable)))
    ...
    def rgr_single(self, variable):
        ...
        perturbed_xtrain = self.perturb(self.xtrain, variable)
    ...
    def rgr_all(self):
        ...
        for i in self.xtrain.columns:
            perturbed_xtrain[i] = self.perturb(self.xtrain, i)[i]
        rf_perturbed = self.model.fit(perturbed_xtrain, self.ytrain)
        yhat_pert = rf_perturbed.predict(self.xtest)
        rgr_val= _rga(self.yhat, yhat_pert)
        return rgr_val
    
    def rgr_statistic_test(self, problemtype, secondmodel= np.nan, variable= np.nan):
        """
