...
class AMQPManager:
    ...
    def _queue_declare(self, queue_name, durable=False):
        self.channel.queue_declare(queue=queue_name, durable=durable)
        print(f"Queue {queue_name} declared")

    def queue_declare(self, queue_name, durable=False, retry_conn=False):
        try:
            if self.channel is None:
                self.connect(retry=retry_conn)
            self._queue_declare(queue_name, durable)
        except Exception as e:
            print(f"An error occurred when trying to declare queue: {e}")

    def _queue_bind(self, queue_name, exchange_name, routing_key):
    ...
    def queue_bind(self, queue_name, exchange_name, routing_key, retry_conn=False):
        try:
            if self.channel is None:
                self.connect(retry=retry_conn)
            self._queue_bind(queue_name, exchange_name, routing_key)
        except Exception as e:
    ...
    def queue_unbind(self, queue_name, exchange_name, routing_key, retry_conn=False):
        try:
            if self.channel is None:
                self.connect(retry=retry_conn)
