...
class ExternalEntitiesParser:
    ...
    def parse_specification_chain(
        self,
        entities: GaarfReport,
        specifications: Sequence[BaseExclusionSpecification],
    ...
    def _parse_via_external_parser(self, entities: GaarfReport,
                                   specification: BaseExclusionSpecification,
        ...
        Args:
            ...
            parse_options:
                Options for performing parsing.
        """
        repo = getattr(uow, specification.repository_name)
        if not_parsed_entities := self._get_not_parsed_entities(
                entities, specification, repo):
            i = 0
            while batch := list(
                    itertools.islice(not_parsed_entities, i,
                                     i + parse_options.batch_size)):
                parsed_entities_info = specification.parser().parse(batch)
                i += parse_options.batch_size
    ...
    def _get_not_parsed_entities(self, entities: GaarfReport,
                                 specification: BaseExclusionSpecification,
        ...
        for entity_info in entities:
            if (entity_info.placement_type ==
                    specification.corresponding_placement_type.name):
