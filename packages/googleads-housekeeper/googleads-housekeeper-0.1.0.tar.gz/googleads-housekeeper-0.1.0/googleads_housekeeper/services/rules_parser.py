...
from googleads_housekeeper.services.exclusion_specification import BaseExclusionSpecification
from googleads_housekeeper.services.exclusion_specification import create_exclusion_specification
...
class RulesParser:
    """Helper class for handling specification generation."""

    def generate_specifications(
        self, raw_rules: str | Sequence[str]
    ) -> list[list[BaseExclusionSpecification]] | None:
        ...
        for rules in parsed_rules:
            ...
            for rule in rules:
                specification_entry.append(
                    create_exclusion_specification(rule.exclusion_type,
                                                   rule.exclusion_rule))
            specifications.append(specification_entry)
        return specifications

    def generate_rules(self,
                       raw_rules: str | Sequence[str]) -> list[list[Rule]]:
