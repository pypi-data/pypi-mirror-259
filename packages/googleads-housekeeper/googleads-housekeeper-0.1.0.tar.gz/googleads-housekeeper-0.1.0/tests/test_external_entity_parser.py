...
from googleads_housekeeper.services.exclusion_specification import (
    ContentExclusionSpecification)
from googleads_housekeeper.services.external_parsers import (
...
def test_external_entity_parser(mocker, bus):
    ...
    specifications = [ContentExclusionSpecification('placement regexp example')]
    report = GaarfReport(
        results=[['parsed-example.com', 'WEBSITE']],
        column_names=['placement', 'placement_type'])
    mocker.patch(
        'googleads_housekeeper.services.external_parsers.website_parser.'
        'WebSiteParser.parse',
        return_value=[
    ...
    external_parser.parse_specification_chain(
        entities=report, specifications=specifications)
    with bus.uow as uow:
