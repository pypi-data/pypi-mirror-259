...
from .demography import current_country, current_timezone, current_lat, current_lon, get_next_holiday
...
def is_holiday(country_code: str = current_country, date: datetime = datetime.now(), city: str = None) -> bool:
    """Determina si una fecha específica es festiva en un país específico.
    Args:
        country_code (str): Código de país ISO 3166-1 alfa-2.
        date (datetime): Fecha a verificar.
    Returns:
        bool: Verdadero si la fecha es festiva, Falso si no lo es.
    """
    data_holiday = get_next_holiday(country_code)
    holiday_date = str_to_date(data_holiday['date'])

    if compare_dates(date, holiday_date) == 0:
        if not data_holiday['global']:
            print('--------------------------------------------------------------')
            print(f"La fecha festiva {data_holiday['name']} no es global.")
            print(f"Se aplica solo a las siguientes regiones: {', '.join(data_holiday['cities'])}")
            print(f"La ciudad actual es {city}.")
            print('--------------------------------------------------------------')
            if city is not None:
                return city in data_holiday['cities']
            else:
                return False
        else:
            return True
