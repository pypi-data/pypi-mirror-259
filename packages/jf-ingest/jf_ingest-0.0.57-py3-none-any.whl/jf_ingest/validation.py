...
from jf_ingest.utils import retry_for_429s, test_jira_object_access
...
def validate_jira(
    ...
    for project_key in projects:
        ...
        healthcheck_result.project_data[project_key].issues, _ = test_jira_object_access(
        ...
        healthcheck_result.project_data[project_key].versions, _ = test_jira_object_access(
            jira_connection.project_versions, project_key, return_objs=False
        )
        healthcheck_result.project_data[project_key].components, _ = test_jira_object_access(
    ...
    healthcheck_result.fields.access_ok, healthcheck_result.fields.available_objects = (
        test_jira_object_access(jira_connection.fields)
    )
    ...
    healthcheck_result.resolutions.access_ok, healthcheck_result.resolutions.available_objects = (
        test_jira_object_access(jira_connection.resolutions)
    )

    logger.info('Checking access to issue types')
    healthcheck_result.issue_types.access_ok, healthcheck_result.issue_types.available_objects = (
        test_jira_object_access(jira_connection.issue_types)
    )
    ...
    ) = test_jira_object_access(jira_connection.issue_link_types)

    logger.info('Checking access to priorities')
    healthcheck_result.priorities.access_ok, healthcheck_result.priorities.available_objects = (
        test_jira_object_access(jira_connection.priorities)
    )

    logger.info('Checking access to boards')
    healthcheck_result.boards.access_ok, healthcheck_result.boards.available_objects = (
        test_jira_object_access(jira_connection.boards)
    )
    ...
    if healthcheck_result.boards.access_ok:
        ...
        for board in boards:
            ...
            ) = test_jira_object_access(jira_connection.sprints, board.id, is_sprint=True)
