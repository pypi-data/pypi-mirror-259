...
class CloneProgress(RemoteProgress):
    def update(self, op_code, cur_count, max_count=None, message=''):
        if max_count:
            ...
            print(f"Progress: {cur_count}/{max_count} ({percent:.2f}%) {message}")
...
# Create the directory if it does not exist
    ...
    if os.listdir(LOCAL_PATH):
        print('The directory is not empty. Please clear the directory and try again.')
        return
    
    

    try:
        Repo.clone_from(repo_url, LOCAL_PATH, progress=CloneProgress())
...
def undo_local_commits_but_keep_changes(repo_path='codebase'):
    repo = Repo(repo_path)
