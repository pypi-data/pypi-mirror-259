...
def task_master_system_prompt():
    return """You are a genius level-10 engineer who has been give a software task to complete on a codebase. You will plan everything needed to complete the task step by step. 
...
Length is not a concern. Write as much as you need.
Do not include any non-functional changes like testing or security concerns in your plan, unless the task explicitly asks you to.
"""

def task_master_user_prompt(task, context, feedback, old_plan):
    new_line = '\n'
    return f"""HERE ARE THE RELEVANT CODE SNIPPETS FROM THE CODEBASE: {context}
#TASK:
{task}

{f"You have previously created a plan for this step and the user provided a feedbackv for you to fix. Here your old plan and the feedback:{new_line}# OLD PLAN:{new_line}{old_plan}{new_line}{new_line}# FEEDBACK:{new_line}{feedback}" if feedback else ""}
"""