...
from .. import _get_global_var
from ..base import Serie
from .._service import get_service_edb
from .._service.edb import ParamGtjaqhid, ParamEdbdata
from ..util import Tool
from .h_grammar_to_list import grammar_to_list
...
class Calculate:
    ...
    def serie(gtjaqh_id: str, option: dict) -> Serie:
        try:
            ...
            return Calculate.adjust_with_option(s, option)
    ...
    def op_add(data_list: List, option: dict = None) -> Serie:
        ...
        for i in range(1, len(data_list)):
            tmp = data_list[i]
            _rst = Calculate.handle_num_op(rst, tmp, "+")
            if _rst:
                rst = _rst
            else:
                d1, d2 = Calculate.init_data(rst, tmp)
                rst = d1 + d2
        return Calculate.adjust_with_option(rst, option)
    ...
    def op_sub(data_list: List, option: dict = None):
        if len(data_list) == 1:
            return Calculate.op_mul([data_list[0], -1])
        else:
            rst = data_list[0]
            for i in range(1, len(data_list)):
                tmp = data_list[i]
...
class Sexp:
    _SUPPORT_OP = {
        "SERIE": Calculate.serie,
        'RESAMPLE': Calculate.resample,
        'SHIFT': Calculate.shift,
        'MA': Calculate.ma,
        '+': Calculate.op_add,
        '-': Calculate.op_sub,
        '*': Calculate.op_mul,
