...
class EdukaConnector(SisConnector):
    ...
    #    appropriate format.
    FIELD_VALUE_CONVERTERS: dict[FamilyPropertyName, tuple[Callable[[Any, str], Any], bool]] = {
        ...
        for eduka_row in csv_reader:
            ...
            for eduka_field_name, eduka_field_value in eduka_row.items():
                if eduka_field_name not in ignored_eduka_field_names:
                    ...
                    field_value_converter = self.FIELD_VALUE_CONVERTERS.get(field_name)

                    if field_value_converter is None:
                        field_value = eduka_field_value
                    else:
                        field_value_converter_function, is_field_required = field_value_converter
                        try:
                            field_value = field_value_converter_function(self, eduka_field_value)
                        except ValueError as error:
                            # If the field's value is of the wrong format, while the field is not
                            # required, we ignore this value, replacing it with a null value.
    ...
    def __convert_language_value(self, value: str) -> Locale:
        ...
        """
        key = unidecode.unidecode(value.lower())

        try:
            locale = cast.string_to_locale(key)
        except Locale.MalformedLocaleException as error:
            locale = self.__eduka_languages_names_mapping and self.__eduka_languages_names_mapping.get(key)
            if not locale:
    ...
    def __convert_nationality_value(self, value: str) -> Country:
        ...
        try:
            ...
            country = self.__eduka_nationalities_names_mapping and self.__eduka_nationalities_names_mapping.get(key)
            if not country:
    ...
    def __fetch_eduka_list_url(
        ...
        if response.status_code == 200:
            data = response.text
            data = '{"confirm":"OK","url":"https:\/\/lpehcm.eduka.school\/download\/ticket\/261\/vlwra5cllvkks0ycprys138rxgtoid"}'
    ...
    def __init__(
        ...
        :param eduka_properties_mapping: The mapping between the Xebus and the
            Eduka properties containing information about a child and their
            parents.

        :param eduka_grades_names_mapping: The mapping between the names of
            the education grades as declared in Eduka and their corresponding
            levels.
        ...
        self.__eduka_grades_names_mapping = {
            unidecode.unidecode(grade_name.lower()): grade_level
            for grade_name, grade_level in eduka_grades_names_mapping.items()
        }

        self.__eduka_languages_names_mapping = eduka_languages_names_mapping or \
            self.__load_eduka_languages_names_mapping_from_default_csv_file(FRENCH_LOCALE)

        self.__eduka_nationalities_names_mapping = eduka_nationalities_names_mapping or \
            self.__load_eduka_nationalities_names_mapping_from_default_csv_file(FRENCH_LOCALE)
