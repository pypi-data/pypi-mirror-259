...
for i in range(n_energ):
    ...
    temp_Energ = f'{E[i]:_.4f}'
    
    if (E[i] <= 0.0): c_energ = temp_Energ
    if (E[i] > 0.0):  c_energ = '+' + temp_Energ
    ...
    if (save_png == 1): plt.savefig(dir_output + 'figures/SFermi_' + number + '_[' + c_energ + '].png', dpi = quality, bbox_inches='tight', pad_inches = 0)
    if (save_pdf == 1): plt.savefig(dir_output + 'SFermi_' + number + '_[' + c_energ + '].pdf', dpi = quality, bbox_inches='tight', pad_inches = 0)
    if (save_svg == 1): plt.savefig(dir_output + 'SFermi_' + number + '_[' + c_energ + '].svg', dpi = quality, bbox_inches='tight', pad_inches = 0)
    if (save_eps == 1): plt.savefig(dir_output + 'SFermi_' + number + '_[' + c_energ + '].eps', dpi = quality, bbox_inches='tight', pad_inches = 0)  
