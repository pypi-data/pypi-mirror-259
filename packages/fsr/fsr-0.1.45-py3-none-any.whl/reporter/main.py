...
def generate_report(file_path, test_type, image_folder):
    ...
    files_and_directories  = os.listdir(source_dir)
    ...
    for item in files_and_directories:
        ...
        destination_item_path = os.path.join(report_dir, item)
        if os.path.isfile(source_item_path):
            shutil.copy(source_item_path, destination_item_path)
            logging.debug("File copied: %s", item)
        elif os.path.isdir(source_item_path):
            shutil.copytree(source_item_path, destination_item_path)
            logging.debug("Directory copied: %s", item)
