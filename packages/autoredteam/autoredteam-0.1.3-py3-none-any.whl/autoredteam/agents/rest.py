...
class RestAPI(Agent):
    """
    Generic class for creating an Agent based on an URL and API token.
    Takes as input a json file or dict.
    """
    ...
    def __init__(
        self,
        config,
        token="REST_API_TOKEN", # supply name of the env variable, not the variable itself
        generations: int = 10
    ):
        ...
        if token not in os.environ:
            raise ValueError(
                f"Put the {self.family} API token in the {token} environment variable\n \
                e.g.: export {token}='abc123'"
            )
        os.environ["REST_API_TOKEN"] = os.getenv(token)
    ...
    def _call_model(self, prompt: str):
        ...
        payload = self.config['prompt_handler']
