...
from zippy_form.models import Account, FormField, Form, FormStep, FormSubmission, FormSubmissionData, FormFieldOption, Webhook,WebhookForm, AccountPaymentSettings,FormPaymentSettings, PaymentGatewayWebhook, FormSubmissionPaymentDetails, \
    FormMetaData, FormMetaDataOption, FormSubmissionMetaDataValue, FormPageViews, FormReCaptcha
...
def fields_mapped_to_form_step(req, form_id, step_id = None):
    ...
    if hasattr(settings, 'ZF_API_LIST_ALL_FORM_FIELDS_BEFORE_RESPONSE'):
        try:
            # Import Callback Function Provided In Project Settings File
            callback_function = import_string(settings.ZF_API_LIST_ALL_FORM_FIELDS_BEFORE_RESPONSE)
    ...
    if hasattr(settings, 'ZF_API_LIST_ALL_FORM_FIELDS_FORMAT_SUCCESS_RESPONSE'):
        try:
            # Import Callback Function Provided In Project Settings File
            callback_function = import_string(settings.ZF_API_LIST_ALL_FORM_FIELDS_FORMAT_SUCCESS_RESPONSE)
            ...
            if _is_payment_collect_enabled:
                ...
                if form_payment_settings:
                    ...
                    if primary_payment_gateway == PAYMENT_GATEWAYS[0][0]:
                        ...
                        if application_fee_amount_in_percentage > 0:
                            # Handle "Application Fee Amount" based on "Application Fee Amount Percentage" configured in project settings file
                            application_fee_amount = (application_fee_amount_in_percentage / 100) * int(price)
                        else:
