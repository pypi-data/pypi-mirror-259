...
def status(
    ...
    if resource_type == "htcondor":
        ...
        if "MemoryUsage" in job_ad:
            memory_usage = (
                ...
                f"{readable_size(job_ad.eval('RequestMemory')
                                 * 10**6)} requested"
            )
        ...
        if job_status == htcondor.JobStatus.IDLE:
            ...
            logger.info(f"It was submitted {
                        readable_time(job_queue_time.seconds)} ago.")
            ...
            if job_holds > 0:
                logger.info(f"It has been held {
                            job_holds} time{s(job_holds)}.")
            if job_atts > 0:
                logger.info(f"HTCondor has attempted to start the job {
                            job_atts} time{s(job_atts)}.")
                logger.info(f"The job has started {
                            job_execs} time{s(job_execs)}.")
            ...
            logger.info(f"It started running {
                        readable_time(job_running_time.seconds)} ago.")
            logger.info(f"It was submitted {
                        readable_time(job_queue_time.seconds)} ago.")
            if memory_usage:
            ...
            if job_suspends > 0:
                logger.info(f"It has been suspended {
                            job_suspends} time{s(job_suspends)}.")
            if job_holds > 0:
                logger.info(f"It has been held {
                            job_holds} time{s(job_holds)}.")
            if job_atts > 1:
            ...
            logger.info(f"It has been suspended for {
                        readable_time(job_suspended_time.seconds)}.")
            logger.info(f"It has been suspended {
                        job_suspends} time{s(job_suspends)}.")
            logger.info(f"It was submitted {
                        readable_time(job_queue_time.seconds)} ago.")
            ...
            logger.info(f"It started transferring output {
                        readable_time(job_transfer_time.seconds)} ago.")
            if memory_usage:
                logger.info(f"Its last memory usage was {memory_usage}.")
            if disk_usage:
                logger.info(f"Its last disk usage was {disk_usage}.")
            ...
            logger.info(f"It has been held for {
                        readable_time(job_held_time.seconds)}.")
    ...
    # additional information from the provisioning DAGMan log
        ...
        if job_status == "COMPLETED":
            ...
            if job_status == "PROVISIONING REQUEST PENDING":
                logger.info(f"Job is waiting for {
                    resource_type.upper()} to provision pending request", end='')
            else:
        ...
        raise ValueError(f"Error: The 'job status' command does not support {
