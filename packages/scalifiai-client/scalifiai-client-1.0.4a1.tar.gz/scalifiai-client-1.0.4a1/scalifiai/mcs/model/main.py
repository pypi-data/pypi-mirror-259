...
from .exceptions import ModelNotFoundException, raise_custom_exception
...
class Model(BaseSubModule, MCS):
    ...
    @classmethod
    def create_model(cls, *, name=None, description=None, api_key=None):

        try:
            model_obj = CreateModel(name=name, description=description)
        ...
        if resp.status_code == 201:
            ...
            raise_custom_exception(response=resp, func_name=cls.create_model.__name__)
    ...
    def add_metadata_simple(self, *, data_type=None, key=None, value=None):
        ...
        if resp.status_code == 201:
            ...
            raise_custom_exception(
                response=resp, func_name=self.add_metadata_simple.__name__
            )
    ...
    def add_metadata_storage(self, *, data_type=None, key=None, file_path=None):
        ...
        with open(metadata_obj_data["file_path"], "rb") as fp:
            ...
            if resp.status_code != 204:
                ...
                # TODO[VIMPORTANT] ADD CUSTOM EXCEPTIONS HERE TO HANDLE THIS IF FILE CANNOT BE UPLOADED TO S3, THINK OF DELETING THE INITIATED METADATA ENTRY, OR ATLEAST CALL METADATA COMPLETE UPLOAD, THINK MORE ON THIS
                raise BackendNotAvailableException()
    ...
    # TODO[VIMPORTANT] THIS THIS AGAIN BY ADDING MORE FILE TYPE METADATA
    # TODO[VIMPORTANT] ADD OPTION TO PASS FILTERS WITH PROPER DYNAMIC CHECK VIA PYDANTIC
