    ...
    def save_and_upload(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(dir=base_dir_path) as temp_dir:
            ...
            # TODO[VIMPORTANT] CALL COMPLETE MODEL VERSION CREATION REQUEST BEFORE RAISING ANY ERROR OR HANDLE IT VIA TRY EXCEPT BLOCK, THINK MORE ON THIS AS NOW STRUCTURE HAS CHANGED
            ...
            for file in files_data:
                metadata_create_body = {
                    "metadata": {
                        "key": file["file_relative_path"],
                        "data_type": "file",
                        "properties": {"file_size": file["file_size"]},
                    }
                }

                metadata_instance = model_version_instance.add_metadata(
                    metadata_create_body=metadata_create_body, model_core_file=True
                )
                ...
                with open(file["file_full_path"], "rb") as fp:

                    file_data = {"file": (file["file_full_path"].name, fp)}

                    resp = requests.post(
                        url=metadata_instance.instance_data["properties"][
                            "upload_config"
                        ]["presigned_post_data"]["url"],
                        data=metadata_instance.instance_data["properties"][
                            "upload_config"
                        ]["presigned_post_data"]["fields"],
                        files=file_data,
                    )
                    ...
                    if resp.status_code != 204:
                        print(f"resp.status_code: {resp.status_code}")
                        print(f"resp.text: {resp.text}")
                        # TODO[VIMPORTANT] ADD CUSTOM EXCEPTIONS HERE TO HANDLE THIS IF FILE CANNOT BE UPLOADED TO S3, THINK OF DELETING THE INITIATED METADATA ENTRY, OR ATLEAST CALL METADATA COMPLETE UPLOAD OR MODEL COMPLETE UPLOAD, THINK MORE ON THIS
                        raise BackendNotAvailableException()

                model_version_instance.complete_metadata_upload(
