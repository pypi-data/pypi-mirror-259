...
SOURCECREATEPAYLOAD_ANY_OF_SCHEMAS = ["SourceCreateAzure", "SourceCreateGCS", "SourceCreateS3"]
...
class SourceCreatePayload(BaseModel):
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(SOURCECREATEPAYLOAD_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = SourceCreatePayload.construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in SourceCreatePayload with anyOf schemas: SourceCreateAzure, SourceCreateGCS, SourceCreateS3. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SourceCreatePayload:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> SourceCreatePayload:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into SourceCreatePayload with anyOf schemas: SourceCreateAzure, SourceCreateGCS, SourceCreateS3. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
