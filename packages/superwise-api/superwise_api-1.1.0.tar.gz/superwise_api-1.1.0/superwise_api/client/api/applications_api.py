...
class ApplicationsApi:
    ...
    def test_tool_connection(self, payload: Payload, **kwargs) -> None:  # noqa: E501
        ...
        Test a tool connection.  Parameters:  For `ToolConfigSQLDatabase`: - **type**: Literal type indication for SQL database tool (`SQL_DATABASE`). - **connection_string**: Connection string for the SQL database.  For `ToolConfigPGVector`: - **type**: Literal type indication for PG Vector tool (`PGVector`). - **connection_string**: Connection string for the PG Vector database. - **table_name**: Name of the table in the PG Vector database. - **embedding_model**: Configuration of the embedding model to be used.     - **provider**: Provider of the embedding model (`VertexAIModelGarden`).     - **project_id**: Project ID for Vertex AI.     - **endpoint_id**: Endpoint ID for the model.     - **location**: Location of the Vertex AI endpoint.     - **service_account**: Service account information as a dictionary with keys and values as strings.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
