...
def create_directory_and_zip(
    ...
    if (path_obj.exists() and how != "zip") or (
        how == "zip" and path_obj.with_suffix(".zip").exists()
    ):
        if not overwrite:
    ...
    if how == "zip":
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp_path_obj = Path(tmpdir) / path_obj.stem
            path_obj_with_files = _create_files_in_directory(
                tmp_path_obj, files, tmpdir=True
            )
            _write_files_to_zip(path_obj.with_suffix(".zip"), path_obj_with_files)
    elif how == "local":
        if path_obj.suffix:
            raise ValueError(
