...
def validateinput(parameters,tool=None):
	...
	if 'survey' in keys:
		...
		if tool=='imagequery':
			...
			if survey not in ['ztf','atlas','gaia','asassn','any']:
				raise Exception(f'Unsupported lightcurvequery survey. Supported lightcurvequery surveys are [ztf,atlas,gaia,asassn,any].')
