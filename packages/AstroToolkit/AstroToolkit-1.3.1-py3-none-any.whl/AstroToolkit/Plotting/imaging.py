...
def plot_image(image_dict):
	...
	survey=image_dict['metadata']['survey']
	...
	image_data=image_dict['header']
	
	if not isinstance(image_dict['data'],np.ndarray) or not isinstance(image_data,astropy.io.fits.header.Header):
		return None
	...
	plot.image(image=[image_dict['data']],x=x_points[0],y=y_points[0],dw=x_range,dh=y_range,palette='Greys256',level="image",origin='bottom_right',anchor='bottom_right')
	...
	overlay_data=image_dict['metadata']['overlay']
