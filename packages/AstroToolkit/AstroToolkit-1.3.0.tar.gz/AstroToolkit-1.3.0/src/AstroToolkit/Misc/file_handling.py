...
def create_file(data_copy): 
    ...
    # handles image files
    if file_type=='image':
        if data['metadata']['pos']!=None:
            ra,dec=data['metadata']['pos'][0],data['metadata']['pos'][1]
            
        # makes an astropy fits hdu
        hdu=fits.PrimaryHDU(data['data'],header=data['header'])

        # sets up some values that ATK uses in fits header
        hdu.header['atk_survey']=data['metadata']['survey']
        hdu.header['atk_source']=data['metadata']['source']
        # fits header doesn't support arrays, so split pos into pos_ra and pos_dec
        if data['metadata']['pos']!=None:
            hdu.header['atk_pos_ra']=data['metadata']['pos'][0]
            hdu.header['atk_pos_dec']=data['metadata']['pos'][1]
        else:
            hdu.header['atk_pos_ra']=None
            hdu.header['atk_pos_dec']=None
        hdu.header['atk_location_ra']=data['metadata']['location'][0]
        hdu.header['atk_location_dec']=data['metadata']['location'][1]
        hdu.header['atk_size']=data['metadata']['size']
        hdu.header['atk_time_year']=data['metadata']['image_time'][0]
        hdu.header['atk_time_month']=data['metadata']['image_time'][1]
            
        overlay_data=data['metadata']['overlay']
...
def read_file(file_name):
    ...
    if 'ATKimage' in file_name:
        ...
        image_dict={'type':'image','data':data,'header':header,'metadata':{'survey':survey,'source':source,'pos':pos,'location':location,'size':size,'image_time':image_time,'wcs':wcs,'overlay':overlay_data}}
