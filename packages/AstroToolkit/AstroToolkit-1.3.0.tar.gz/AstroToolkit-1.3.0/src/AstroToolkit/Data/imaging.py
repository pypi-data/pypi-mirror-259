...
def get_image_dict(survey,source=None,pos=None,location=None,size=30,band='g'):
	...
	image_dict={'type':'image','data':image,'header':image_header,'metadata':{'survey':survey,'source':source,'pos':pos,'location':[ra,dec],'size':size,'image_time':imageTime,'wcs':wcs,'overlay':None}}
...
def image_correction(survey,source=None,pos=None,size=30,band='g',overlay=None):
	...
	if source!=None:
		...
		if image_dict!=None:
			image_time=image_dict['metadata']['image_time']
		else:
			return f_return
