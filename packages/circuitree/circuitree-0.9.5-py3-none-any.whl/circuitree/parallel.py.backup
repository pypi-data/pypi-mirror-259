...
class MultithreadedCircuiTree(CircuiTree):
    ...
    def traverse(self, **kwargs) -> tuple[list[Hashable], float | int]:
        """To be run in a parallel thread. Performs a single iteration of MCTS
        asynchronously."""
        ...
        # Get the reward and backpropagate
    ...
    def search_mcts(
        ...
        print(
            f"Running MCTS search for {n_steps_per_thread} iterations each in "
            f"{self.threads} threads"
        )

        _ = self.thread_group.map(
            self.search_mcts_in_thread,
            [n_steps_per_thread] * self.threads,
            callback=callback,
