...
class ClaudeClient:
    ...
    def __init__(
        ...
        """
        Initializes a new ClaudeClient instance with various configurations for HTTP requests and service interactions.
...
class GeminiSession:
    ...
    def __init__(
        ...
        self.gemini: ClaudeClient = gemini
...
def running(func) -> callable:
    async def wrapper(self: "ClaudeClient", *args, **kwargs):
        if not self.running:
            ...
            raise Exception(
                f"Invalid function call: ClaudeClient.{func.__name__}. Client initialization failed."
            )
        else:
            return await func(self, *args, **kwargs)
