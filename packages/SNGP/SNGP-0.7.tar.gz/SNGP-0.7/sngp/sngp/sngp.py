    ...
    def forward(self, X, with_variance=False, update_precision=False):
        ...
        if not with_variance:
            ...
            with torch.no_grad():
                ...
                regularized_covariance = self.covariance + epsilon * torch.eye(self.covariance.size(0), device=self.covariance.device)
                variances = torch.bmm(features[:, None, :], (features @ regularized_covariance)[:, :, None], ).reshape(-1)
