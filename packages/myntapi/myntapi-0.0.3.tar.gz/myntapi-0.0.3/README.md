...
- [place_basket](#md-place_basket)
- [close_all_position](#md-close_all_position)
...
ret = mt.login(userid=uid, password=pwd, twoFA=factor2, vendor_code=vc, api_secret=app_key, imei=imei)
```
Request Details :
...
|vc*||Vendor code provided by zebu team, along with go.mynt.in|
|api_secret*||you will get from go.mynt.in API key (app key)|
...
```
ret = mt.place_order(buy_or_sell='S', product_type='C',
                        exchange='NSE', tradingsymbol='TCS-EQ', 
                        quantity=1, discloseqty=0,price_type='SL-LMT', price=2000.00, trigger_price=1990.50,
                        retention='DAY', remarks='Algo traders')
```
Request Details :
...
|prd*|C / M / H|Product name (Select from ‘prarr’ Array provided in User Details response, and if same is allowed for selected, exchange. Show product display name, for user to select, and send corresponding prd in mt call)|
...
ret = mt.modify_order(exchange='NSE', tradingsymbol='CANBK-EQ', orderno=orderno,
...
```
orderno = ret['norenordno'] #from placeorder return value
ret = mt.cancel_order(orderno=orderno)
```
...
```
ret = mt.get_positions()
#converts the first position from existing product to intraday
p = ret[0]
ret = mt.position_product_conversion(p['exch'], p['tsym'], p['netqty'], 'I', p['prd'], 'S', 'DAY')
```
...
```
orderno = ret['norenordno'] #from placeorder return value
ret = mt.single_order_history(orderno=orderno)
```
Request Details :
...
#### <a name="md-place_basket"></a>  place_basket(order)

Order multiple scripts

```
order=[{'buy_or_sell': 'B', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'TCS-EQ', 'quantity': '1', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0'},{'buy_or_sell': 'S', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'IDEA-EQ', 'quantity': '6', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0'},{'buy_or_sell': 'B', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'SBIN-EQ', 'quantity': '5', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0'}]


ret = mt.place_basket(order)


Response Details :
Response data will be in array format with below fields in case of Success:
[{'buy_or_sell': 'S', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'TCS-EQ', 'quantity': '1', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0', 'request_time': '18:51:40 10-01-2024', 'stat': 'Ok', 'norenordno': '24011000052942'},{'buy_or_sell': 'S', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'IDEA-EQ', 'quantity': '1', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0', 'request_time': '18:51:40 10-01-2024', 'stat': 'Ok', 'norenordno': '24011000052943'},{'buy_or_sell': 'S', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'SBIN-EQ', 'quantity': '1', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0', 'request_time': '18:51:40 10-01-2024', 'stat': 'Ok', 'norenordno': '24011000052944'}]
```
#### <a name="md-close_all_position"></a>  close_all_position()

Close all positions

```
For close all position
ret = mt.close_all_position()

For close partially position
ore=[{
...
ret = mt.close_all_position(ore)

Response data will be in array format with below fields in case of Success:
[{'buy_or_sell': 'S', 'product_type': 'I', 'exchange': 'NSE', 'tradingsymbol': 'TCS-EQ', 'quantity': '1', 'discloseqty': '0', 'price_type': 'MKT', 'price': '0', 'request_time': '18:51:40 10-01-2024', 'stat': 'Ok', 'norenordno': '24011000052942'},{'error': 'quantity is greater than position', 'tradingsymbol': 'IDEA-EQ', 'order_type': 'MIS'}]
...
```
ret = mt.span_calculator(actid,positionlist)
```
Request Details :
...
```
ret = mt.option_greek(expiredate ='24-NOV-2022',StrikePrice='150',SpotPrice  = '200',InitRate  = '100',Volatility = '10',OptionType='5')
```
Request Details :
...
ret = mt.searchscrip(exchange=exch, searchtext=query)
...
Example 2:
```
mt.searchscrip(exchange='NSE', searchtext='REL')
```
...
exch  = 'NSE'
token = '22'
ret = mt.get_security_info(exchange=exch, token=token)
```
Request Details :
...
exch  = 'NSE'
token = '22'
ret = mt.get_quotes(exchange=exch, token=token)
```
...
ret = mt.get_time_price_series(exchange='NSE', token='22', starttime=lastBusDay.timestamp(), interval=5)
```
Request Details :
...
Example:
```
ret =mt.get_daily_price_series(exchange="NSE",tradingsymbol="PAYTM-EQ",startdate="457401600",enddate="480556800")
```
Request Details :
...
First configure the endpoints in the mt_helper constructor. 
...
ret = mt.login(userid=user, password=pwd, twoFA=factor2, vendor_code=vc, api_secret=app_key, imei=imei)
print(ret)
```

## <a name="md-example-market"></a> Example Symbol/Contract : Example_market.py
This Example shows mt usage for finding scrips and its properties
...
```
mt.searchscrip(exchange='NSE', searchtext='NES')
```
This will reply as following
```
{
...
```
mt.get_security_info(exchange='NSE', token='22')
```
...
```
mt.subscribe(['NSE|22', 'BSE|522032'])
```
...
mt.start_websocket( order_update_callback=event_handler_order_update,
...
```
    mt.place_order(buy_or_sell='S', product_type='H',
                        exchange='NSE', tradingsymbol='INFY-EQ', 
                        quantity=1, discloseqty=0,price_type='LMT', price=1500, trigger_price=None,
                        retention='DAY', remarks='Algo traders', bookloss_price = 1490)
```
    Place a Bracket Order as follows
```
    mt.place_order(buy_or_sell='S', product_type='I',
                        exchange='NSE', tradingsymbol='TCS-EQ', 
                        quantity=1, discloseqty=0,price_type='LMT', price=2000, trigger_price=None,
                        retention='DAY', remarks='Algo traders', bookloss_price = None, bookprofit_price = None)
```
### Modify Order
    Modify a New Order by providing the OrderNumber
```
    mt.modify_order(exchange='NSE', tradingsymbol='TCS-EQ', orderno=orderno,
                                   newquantity=2, newprice_type='LMT', newprice=2005)
```
### Cancel Order
    Cancel a New Order by providing the Order Number
```
    mt.cancel_order(orderno=orderno)
```
