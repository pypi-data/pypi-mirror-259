    ...
    def load_from_cdf(
        ...
        for head in heads:
            training_data = client.sequences.data.retrieve_dataframe(
                id=head.id, start=0, end=None
            )
            if head.metadata is None:
                raise ValueError(
                    f"Metadata for sequence {head.id} is missing. Cannot retrieve well name."
                )
            try:
        ...
        cdf_data = pd.concat(data)
        if save_as:
            cdf_data.to_pickle(save_as)
        return self.save_df_to_cls(cdf_data)
