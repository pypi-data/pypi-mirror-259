...
def encode_columns(df: pd.DataFrame, **kwargs) -> pd.DataFrame:
    ...
    formations_map: Dict[str, float] = kwargs.get("formations_map", None)
    groups_map: Dict[str, float] = kwargs.get("groups_map", None)
    systems_map: Dict[str, float] = kwargs.get("systems_map", None)
    ...
    df = utilities.normalize_group_formation_system(
        df, add_systems="SYSTEM" in columns_to_encode
    )
...
def onehot_encode_columns(df: pd.DataFrame, **kwargs) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: dataframe with categorical columns encoded
    """
    columns_to_onehot_encode: List[str] = kwargs.get("columns_to_onehot_encode", None)
    categories_to_keep: List[str] = kwargs.get("categories_to_keep", None)
...
def fill_columns(df: pd.DataFrame, **kwargs) -> pd.DataFrame:
    ...
    """
    columns_to_fill: Dict[str, List[str]] = kwargs.get("columns_to_fill", None)
    missing_value: float = kwargs.get("missing_value", None)
