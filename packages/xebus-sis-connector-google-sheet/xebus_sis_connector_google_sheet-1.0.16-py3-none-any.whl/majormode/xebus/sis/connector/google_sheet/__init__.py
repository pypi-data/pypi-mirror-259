...
class GoogleSheetConnector(SisConnector):
    ...
    # The deafult name of the sheet in a school organization's Google
    ...
    def __fetch_grades_names_mapping_from_google_sheet(
        ...
        education_grades_names_mapping = {
            unidecode.unidecode(grade_name.lower()): int(grade_level)
...
# organization's Google Sheets.
        self.__grades_names_mapping: dict[str, int] = None #  self.__fetch_grades_names_mapping_from_google_sheet(DEFAULT_GRADES_NAMES_MAPPING_SHEET_NAME)
        self.__languages_names_mapping: dict[str, Locale] = None  # self.__load_languages_names_mapping_from_default_csv_file()
        self.__nationalities_names_mapping: dict[str, Country] = None  # self.__load_nationalities_names_mapping_from_default_csv_file()
    ...
    def __load_languages_names_mapping_from_default_csv_file() -> dict[str, Locale]:
        ...
        :note: This function is not used at the moment.  The language names
            mapping is dynamically loaded from a the sheet ``(Languages)`` of
    ...
    def fetch_families_list(self) -> FamilyList:
        ...
        # Fetch families data.
        logging.debug(
            f"Fetching families data from the sheet \"{self.__families_sheet_name}\"..."
        )
        ...
        if self.__grades_names_mapping is None:
            ...
            self.__grades_names_mapping = self.__fetch_grades_names_mapping_from_google_sheet(
                self.__google_client,
                self.__spreadsheet_url,
        ...
        if self.__languages_names_mapping is None:
            ...
            self.__languages_names_mapping = self.__fetch_languages_names_mapping_from_google_sheet(
                self.__google_client,
                self.__spreadsheet_url,
        ...
        if self.__nationalities_names_mapping is None:
            ...
            self.__nationalities_names_mapping = self.__fetch_nationalities_names_mapping_from_google_sheet(
                self.__google_client,
                self.__spreadsheet_url,
