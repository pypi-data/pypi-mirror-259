...
from numerous.sdk.connect.optimization import load_target_scenario_data_and_set_job_id
...
class OptimizationConfiguration:
    def __init__(
        ...
        if scenario_document.get("optimize", False):
            raise RuntimeError(
                f"Optimization was not enabled on scenario {scenario_id} in project {project_id}."
            )

        [target_scenario, target_job_id] = load_target_scenario_data_and_set_job_id(
            spm_stub, project_id, scenario_document
        )
        self.target_job_id = target_job_id
        self.settings = list(self._parse_optimization_settings(target_scenario))
        components = self._components_by_uuid(scenario_document)
