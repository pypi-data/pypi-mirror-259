...
def calibrated_header(is_mosaic: bool):
...
@pytest.fixture(scope="function")
def write_l1_task(calibrated_header, num_mosaic_repeats):
...
@pytest.mark.parametrize(
    "is_mosaic", [pytest.param(True, id="Mosaic"), pytest.param(False, id="Non-Mosaic")]
)
def test_write_l1_frame(write_l1_task, is_mosaic, num_mosaic_repeats, mocker):
    """
    ...
    for file in files:
        ...
        if num_mosaic_repeats > 1:
            ...
            assert header["DINDEX3"] == 5  # Because of how we set up the header fixture
        else:
        ...
        if is_mosaic:
            ...
            assert header["MINDEX1"] == 1  # Because VBISTP = 5 from the fixture, which corresponds
            assert header["MINDEX2"] == 1  #  to mosaic field #1, which has MINDEX (1, 1)
        else:
