...
def validate_bag(test_bag: bagit.Bag, bag_errors_p: [()]) -> bool:
    """
    Validates a bag
...
def debag(bag_arcname: str, output_path_p: str):
    """
    ...
    try:
        with ZipFile(bag_arcname, 'r') as bz:
            ...
            for bag_zip_info in bag_zip_infos:
                ...
                if not validate_bag(bag_zip_info, bag_errors):
    ...
    # Now validate the test_bag we unzipped. We're assuming the zip contains 1 test_bag
    # TOMaybeDO (?) Extend the dfn to pass in a list of works which are known to be in the test_bag
    # Something like
    # bags_in_arc=[ab for ab in bz.infolist() where ab.name.endswith(".test_bag/")
    # and
    # works_in_arc = [ wa for wa in bz.infolist() where wa.name.endswith("/test_bag/data/")
...
def bag(bag_src: str, bag_dest: str, preserve_source: bool):
    """
    ...
    # Test if we're zipping an already existing test_bag
        try:
            ...
            if preserve_source:
                ...
                shutil.copytree(bag_src,Path(bag_temp_dir) / dest_base_name)
                bag_src =  str(Path(bag_temp_dir, dest_base_name))
            dest_bag = bagit.make_bag(bag_src, processes=6, checksums=['sha512'])

        bag_errors: [] = []
        # If the given folder is not a test_bag, make one from it
        if not validate_bag(dest_bag, bag_errors):
            log_bag_errors(bag_errors)
            sys.exit(1)

        dest_bag.info['BDRC-RID'] = dest_base_name # Assume it's the RID
        dest_bag.save(manifests=True, processes=6)
