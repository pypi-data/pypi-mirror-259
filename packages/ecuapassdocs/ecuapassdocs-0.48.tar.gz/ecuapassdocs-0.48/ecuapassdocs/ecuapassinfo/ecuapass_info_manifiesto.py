...
class EcuMNF (EcuInfo):
	...
	def getTransportistaInfo (self):
		...
		try:	
			transportista ["procedimiento"]     = "IMPORTACION||LOW"
			...
			transportista ["MCI"]               = self.getNumeroMCI ()
	...
	def getVehiculoRemolqueInfo (self, vehicleType):
		...
		try:
			...
			vehiculo ["tipo"]        = Utils.addLow ("TRACTOCAMION" if vehicleType=="vehiculo" else "SEMIRREMOLQUE")
	...
	def formatCertificadoString (self, text, vehicleType):
		try:
			...
			Utils.printx (f"Excepción formateando certificado en <{self.fieldsJsonFile}>")
			certificadoString = ""
	...
	def getBultosInfo (self):
		...
		try:
			bultos ["cartaporte"] = self.getNumeroCPIC ()
