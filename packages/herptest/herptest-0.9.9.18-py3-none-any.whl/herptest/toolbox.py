...
def replace(replacements, target, is_recursive=False, delete_empties=False):
    if type(target) == str:
        return Template(target).substitute(**replacements)

    if not is_recursive or not is_iterable(target):
        return None

    result = [replace(replacements, entry, True) for entry in target]
    return [entry for entry in result if entry or delete_empties]


def contains_empties(target):
    for entry in target:
        if not entry:
            return True
