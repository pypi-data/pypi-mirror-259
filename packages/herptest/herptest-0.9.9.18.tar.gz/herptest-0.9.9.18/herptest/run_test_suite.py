...
def build_project(source_root, build_root, build_cfg, is_framework=False):
    result_error = None
    error_output = None
    ...
    # Prepare to make substitutions to the prep / build commands if applicable. Remove empty elements.
    command_sets = ('prep_cmd', 'compile_cmd', 'post_cmd')
    subs = {key : value for key, value in build_cfg.__dict__.items() if not key in command_sets}
    subs["source_dir"] = (source_root if source_root else __NULL_PATH__)
    subs["build_dir"] = (build_root if build_root else __NULL_PATH__)

    try:
        for entry in command_sets:
            if is_framework and entry in build_cfg.framework_flags and not build_cfg.framework_flags[entry]:
                continue # We are building the framework, and this command set flag is off (False). Skip it.

            if entry in build_cfg and build_cfg[entry]: # If the entry exists and is non-null, replace it.
                cmds = toolbox.replace(subs, build_cfg[entry], is_recursive=True, delete_empties=True)

                for cmd in cmds:
                    if any([__NULL_PATH__ in entry for entry in cmd]):
                        logging.info("\nSkipping entry without path:\n%s\n" % cmd)
                    else:
...
# Initialize environment and build & initialize framework (only need to do this once.)
def prepare_environment_and_framework(cfg):
    # Run initialization command line commands (pre-build) if applicable.
    logging.info("Initializing environment...")
    if cfg.build.init_cmd:
        for cmd in cfg.build.init_cmd:
            subprocess.check_output(cmd, stderr=subprocess.STDOUT, text=True)
        logging.info("done.\n")
    else:
        logging.info("no tasks; skipping.\n")

    # Run build prep, build, and post-build commands.
    if cfg.build.prep_cmd or cfg.build.compile_cmd or cfg.build.post_cmd:
        logging.info("Prepping / building framework environment... ")
        result_error, error_output = build_project(cfg.build.framework_src, cfg.build.framework_bin, cfg.build, is_framework=True)
        if result_error:
...
def shutdown_framework_and_environment(cfg, framework_context):
    # Shutdown framework.
    cfg.shutdown_framework(framework_context)

    # Shutdown environment if applicable.
    logging.info("Shutting down environment...")
    if cfg.build.shutdown_cmd:
        for cmd in cfg.build.shutdown_cmd:
            subprocess.check_output(cmd, stderr=subprocess.STDOUT, text=True)
        logging.info("done.\n")
    else:
        logging.info("no tasks; done.\n")
...
def main():
    ...
    # Load and format the configuration.
    ...
    # Run basic initialization routines.
    framework_context = prepare_environment_and_framework(cfg)
...
#            executor.join()
    ...
    shutdown_framework_and_environment(cfg, framework_context)
