...
class Model(object):
    ...
    @staticmethod
    def fetchMany(name=None, cursor=[], pageSize=10):
        """
            获取用户可以运行的项目列表
        ...
        query = """query($input:ModelsInput!){models(input:$input){cursor total count items{rid name description owner tags updatedAt }}}"""
        variables = {
            "cursor": cursor,
            "limit": pageSize,
            "orderBy": ["updatedAt<"],
        ...
        data = graphql_request(query, {"input": variables})
