...
class PublisherAdoptedBooks():
    ...
    def get_all_adopted_books(self):
        # Creiamo un file nella directory corrente
        with open(self.json_file_name, 'w', encoding='utf-8') as fh:
            # Dobbiamo creare un json ben formato a partire da blocchi successivi di json
            fh.write('[')
            if not self.user_vars['uni_cod']:
                ...
                for uni_cod in uni_list:
                    ...
                    self.append_to_file(fh, content['data'])
...
def programmi(filename, uni_cod, a_a, laurea_nome, laurea_tipo, 
    ...
    """
    Scarica i programmi dei corsi di studio per università e anno accademico.
    """
    if not filename:
