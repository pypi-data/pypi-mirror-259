...
class TritonPythonModel:
        ...
        for request in requests:
            ...
            for each in required_output:
                if each["name"] in inference_outputs:
                    ...
                    if type(each_inference_output) == list:
    ...
    def finalize(self, args):
        inferless_model.finalize(args)

    def create_config_from_json(output):
        ...
        for key in output:
            ...
            type_of_output = TritonPythonModel.get_type(output[key])
            each_output["data_type"] = TritonPythonModel.map_data_type(type_of_output)
            each_output_for_return["datatype"] = TritonPythonModel.map_triton_data_type(
                type_of_output
            )

            shape_of_output = TritonPythonModel.get_shape(output[key])
    ...
    def map_data_type(variable_type):
        return model_data_type_maping[variable_type]

    def map_triton_data_type(variable_type):
    ...
    def get_shape(value):
        ...
        if isinstance(value, list):
            ...
            if shape[0] > 0 and isinstance(value[0], list):
                shape.extend(TritonPythonModel.get_shape(value[0]))
    ...
    def get_type(variable):
        if not isinstance(variable, (list, np.ndarray)):
            ...
            return TritonPythonModel.get_type(variable[0])
        elif not variable:
            return type(None)
        else:
            return TritonPythonModel.get_type(variable[0])
