...
@st.cache_data
def blip_call(images, project_name, task, question = None):
    rows_list = []
    folder = f"output/{project_name}/ads_images"
    for file in images:
        raw_image = Image.open(os.path.join(folder, file)).convert('RGB')
        numbers = AD_PATTERN.findall(file)
...
project_name = "testmeet" # this needs to be user-defined
...
with col7:
    ...
    st.write("Topic modeling will come here.")
...
st.subheader(f'Image examples (5 out of {len(images)})')

img_examples = st.columns(5)
for i in range(0, 5):
    with img_examples[i]:
        st.image(os.path.join(img_path, images[i]))


# extract image captions and contents first
img_captions = blip_call(images, "testmeet", task = tasks[0]) # img_captioning
img_contents = blip_call(images, "testmeet", task = tasks[1], question = ["what objects do you see in this picture?"])

# then analyze the img-to-text content:
img_cpt_tokens = img_captions["img_caption"].apply(preprocess)
img_cont_tokens = img_contents["img_content"].apply(preprocess)

img_outcomes = st.columns(2)
with img_outcomes[0]:
    st.subheader('Image captions')
    _, wc = get_word_freq(img_cpt_tokens) 
    wc_image = wc.to_array()
    st.image(wc_image, use_column_width=True)

    textblb_sent, nltk_sent = get_sentiment(img_captions["img_caption"]) 
    avg_polarity = textblb_sent.mean()
    st.write(f"Average Sentiment Polarity: {avg_polarity}")

with img_outcomes[1]:
    st.subheader('Image contents')
    _, wc = get_word_freq(img_cont_tokens) 
    wc_image = wc.to_array()
    st.image(wc_image, use_column_width=True)

    textblb_sent, nltk_sent = get_sentiment(img_contents["img_content"]) 
    avg_polarity = textblb_sent.mean()
    st.write(f"Average Sentiment Polarity: {avg_polarity}")
