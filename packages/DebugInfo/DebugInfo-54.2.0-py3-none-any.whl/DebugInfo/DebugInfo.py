...
class posix搜索接口类(搜索接口基类):
    """
    这是一个基于 ssh 管道,通过 paramiko.SSHClient() 执行 locate/find 命令,并分析返回结果来进行的搜索
    """
    ...
    # 定义 locate搜索 方法，以使用locate服务进行文档/路径搜索
    def __locate搜索(self,
                     搜索关键字: str = None,
                     大小写敏感: bool = False,
                     参考路径: str = None,
        ...
        # 提供搜索引擎的功能实现
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(self.__class__, self.__locate搜索)
        ...
        if '/' in 搜索关键字:
            ...
            搜索关键字 = 真正关键字
        搜索关键字 = 搜索关键字.replace('.*', '*')  # 先把关键字中的 .* 统一替换成 *
        while True:
            if "**" not in 搜索关键字:
                break
            else:
                搜索关键字 = 搜索关键字.replace("**", "*")
        搜索关键字 = 搜索关键字.replace('*', '.*')  # 再把关键字中的 * 统一替换成 .*, 以支持正则处理

        大小写敏感 = True if 大小写敏感 else False
        画板.调试消息(f'大小写敏感: {大小写敏感}')

        参考路径存在: bool = True if 参考路径 else False

        参考路径, 路径分隔符 = 路径格式化(路径=参考路径, 分隔符='/')
        ...
        搜索字串: str = 搜索关键字
        if not 搜索字串.startswith('^'):
            搜索字串 = f'^{搜索字串}'
        if not 搜索字串.endswith('$'):
            搜索字串 = f'{搜索字串}$'

        if 参考路径存在:
            搜索字串 = 搜索字串.lstrip('^')
            搜索字串 = f'{参考路径.rstrip(路径分隔符)}{路径分隔符}{搜索字串}'
        # endregion

        # 如果条件满足,则执行搜索过程
        if self.可用 and 搜索字串:
            locate指令: str = ''
            locate参数字: str = f'-er{"" if 大小写敏感 else "i"}{"w" if 路径分隔符 in 搜索字串 else "b"}'
            if 指定命令参数:
                locate指令 = f'locate {指定命令参数.lstrip("locate ")}'
            elif 搜文档:
                # 如果被要求搜文档，则处理搜文档逻辑，不再关注是否要求搜路径
                locate指令 = f'''locate {locate参数字} "{搜索字串}" | xargs -I {{}} sh -c 'if [ -f "{{}}" ]; then echo {{}}; fi' '''
            elif 搜路径:
                locate指令 = f'''locate {locate参数字} "{搜索字串}" | xargs -I {{}} sh -c 'if [ -d "{{}}" ]; then echo {{}}; fi' '''
            # 调试模式下展示指令字符串
            ...
            if locate指令:
                try:
                    if self.ssh接口.无效 and self.在linux系统中:
                        # 如果 ssh 接口无效, 且当前在linux中, 直接进行 locate 搜索
                        ...
                        # 如果ssh接口有效, 则通过 ssh 管道进行 locate 搜索
                        执行结果 = self.ssh接口.执行远程指令(指令=locate指令, 超时限制ms=1000, 画板=画板.副本.缩进())
                        if 执行结果.指令执行完成:
                            if not 执行结果.存在代码执行错误:
                                if 执行结果.结果存在(self.ssh接口):
                                    ssh接口指令结果 = 执行结果.执行结果(self.ssh接口)
            ...
            if not locate结果:
                if locate错误:
                    ...
                    locateExist: bool = True  # 用于标记目标服务器上是否安装了 locate 服务
                    ...
                    if locate错误 and not locateExist:
                        画板.提示错误('目标主机可能没有安装 locate 指令,请确认目标主机安装 locate 服务后再试')
                        画板.消息(黄字('你可以通过 sudo apt-get install plocate 来安装 locate 服务'))

                    搜索结果.状态码 = 1
                    搜索结果.错误消息 = locate错误
                else:
                    画板.消息(黄字(
                        '基于 locate 的搜索结果可能不是最新的,如果必要,请更新目标主机上的 locate 数据库后再尝试(sudo updatedb)'))
            else:
                文档命中: bool = True
    ...
    # 定义 find搜索 方法，以使用find服务进行文档/路径搜索
    def __find搜索(self,
                   搜索关键字: str = None,
                   大小写敏感: bool = False,
                   参考路径: str = None,
        ...
        # 提供搜索引擎的功能实现
        画板 = 画板 if isinstance(画板, 打印模板) else 打印模板()
        画板.执行位置(self.__class__, self.__find搜索)
        ...
        if self.可用 and 搜索字串:
            ...
            if 指定命令参数:
                ...
                if '*' == 搜索字串:
                    ...
                    find指令 = f'find {参考路径} -{"" if 大小写敏感 else "i"}name "{搜索字串}" -type f'
            elif 搜路径:
                if '*' == 搜索字串:
                    find指令 = f'find {参考路径} -maxdepth 1 -type d'
                else:
                    find指令 = f'find {参考路径} -{"" if 大小写敏感 else "i"}name "{搜索字串}" -type d'
            # 调试模式下展示指令字符串
            ...
            if find指令:
                try:
                    if self.ssh接口.无效 and self.在linux系统中:
                        ...
                        执行结果 = self.ssh接口.执行远程指令(指令=find指令, 超时限制ms=1000, 画板=画板.副本.缩进())
                        if 执行结果.指令执行完成:
                            if not 执行结果.存在代码执行错误:
                                if 执行结果.结果存在(self.ssh接口):
                                    ssh接口指令结果 = 执行结果.执行结果(self.ssh接口)
    ...
    def 搜索(self,
        ...
        搜索结果 = self.__locate搜索(搜索关键字=搜索关键字,
                                     大小写敏感=大小写敏感,
                                     参考路径=参考路径,
                                     指定命令参数=指定命令参数,
                                     搜文档=搜文档,
        ...
        if not 搜索结果.结果列表:
            搜索结果 = self.__find搜索(搜索关键字=搜索关键字,
                                       大小写敏感=大小写敏感,
                                       参考路径=参考路径,
                                       指定命令参数=指定命令参数,
                                       搜文档=搜文档,
...
class 本地搜索接口类(搜索接口基类):
    ...
    def 列出文档(self, 路径: str, 限制数量: int = 0, 画板: 打印模板 = None) -> list[str]:
        ...
        文档数量: int = 0
        if self.存在路径(路径=路径, 画板=画板.副本.缩进()):
    ...
    # 根据平台,自动判断使用 everything or locate 进行搜索
    def 搜索(self,
             搜索关键字: str = None,
             大小写敏感: bool = False,
             参考路径: str = None,
