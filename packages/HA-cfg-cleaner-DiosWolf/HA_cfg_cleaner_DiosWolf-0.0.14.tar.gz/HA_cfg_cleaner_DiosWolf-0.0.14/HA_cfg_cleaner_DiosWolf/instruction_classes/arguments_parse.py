...
class ConsoleArguments:
    ...
    def __get_args_dict(self):
        ...
        if "inst_path" not in args_dict.keys():
            # args_dict["inst_path"] = "instructions_wind.json"
            raise Exception("Cant find instructions.json file")

        if "cfg_ha_path" not in args_dict.keys():
            # args_dict["cfg_ha_path"] = "./config_HA.json"
            cfg_path = dirname(__file__).replace("instruction_classes", "")
            args_dict["cfg_ha_path"] = join(cfg_path, "configs_HA/config_HA.json")

            # args_dict["cfg_ha_path"] = "C:\\MyFiles\\Python work\\work_python_pipenv\\HA_cfg_cleaner_DiosWolf\\HA_cfg_cleaner_DiosWolf\\configs_HA\\config_HA_wind.json"
        return args_dict
