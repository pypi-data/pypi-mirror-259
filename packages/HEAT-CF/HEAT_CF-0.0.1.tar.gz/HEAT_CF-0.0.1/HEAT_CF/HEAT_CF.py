...
def onmf(data, dataset_name, ncells=3000, nfactors=list(range(5, 16)), nreps=2, niters=500, njobs=2, alpha=1,
         SS_weight=0.3):
    """
    ...
    # with open(os.path.join(dataset_path, 'oNMFres.pkl'), 'wb') as f:
    #     pickle.dump(oNMFres, f)

    if len(nfactors) > 1:  # if more than one factor numbers in nfactors, find the optimal k
        # bestk = find_best_k(oNMFres, dataset_path, alpha=alpha, multiplier=multiplier)
...
def HEAT_train(data_path, dataset_name, seed=0, perturbation_key='condition', split_key=None,
    ...
    The embeddings (basal state, factor-level expression), the orthogonal loading matrix,
     the model file (.pt file), the loss and metric curves are stored in dataset_name/HEAT.
    Parameters
    ----------
    data_path: string
    ...
    if not os.path.exists(dataset_name):
        os.makedirs(dataset_name)  # makedir
    dataset_path = os.path.join(dataset_name, 'HEAT')
    if not os.path.exists(dataset_path):
        ...
        if ((epoch + 1) % 5) == 0 or stop:
            ...
            if stop:
                ...
                torch.save(
                    ...
                    os.path.join(
                        dataset_path,
                        "stored_model_seed={}.pt".format(seed),
                    ),
                )
                break
        # if ((epoch + 1) % 20) == 0 or stop:
        #     torch.save(
        #         (autoencoder.state_dict(), autoencoder.history,autoencoder.W),
        #         os.path.join(
        #             dataset_path,
        #             "model_seed={}_epoch={}.pt".format(seed, epoch),
        #         ),
        #     )
        # if verbose:
        #     print('Finish epoch:{}'.format(epoch))
    ...
    plot_loss(history=autoencoder.history, save_path=dataset_path, seed=seed)
    plot_metric(history=autoencoder.history, save_path=dataset_path, seed=seed, epoch_min=9)
    ...
    np.save(os.path.join(dataset_path, 'model_seed={}_gene_loading.npy').format(seed), gene_loading)
    ...
    basal_adata.write(os.path.join(dataset_path, 'model_seed={}_basal.h5ad'.format(seed)))
    tr_adata.write(os.path.join(dataset_path, 'model_seed={}_treated.h5ad'.format(seed)))
