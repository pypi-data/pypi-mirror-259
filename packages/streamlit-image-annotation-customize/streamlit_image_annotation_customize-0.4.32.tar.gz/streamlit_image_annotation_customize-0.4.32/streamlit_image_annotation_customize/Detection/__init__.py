...
#[0,3]
#'''
def detection(image_path, masked_image, label_list, bboxes=None, points=None, neg_points=None,labels=None, height=800, width=512, line_width=5.0, key=None, color_map=None, is_processing=False, need_update_annotation=0) -> CustomComponent:
    image = None
    ...
    component_value = _component_func(image_url=image_url, image_size=image.size, label_list=label_list, bbox_info=bbox_info, points_info=points_info, neg_points_info=neg_points_info,color_map=color_map, line_width=line_width, is_processing=is_processing, need_update_annotation=need_update_annotation, key=key)
    print("component_value", component_value)
    #print([list(x * scale for  in p["points"]) for p in component_value["points"]])
    if component_value is not None:
        component_value_bbox = {"bboxes":[[x * scale for x in bbox["bboxes"]] for bbox in component_value["bboxes"]], "labels":[bbox["labels"] for bbox in component_value["bboxes"]]}
        component_value_point = {"points": [[x * scale for x in p["points"]] for p in component_value["points"]], "labels": [p["labels"] for p in component_value["points"]]}  
        component_value_neg_point = {"neg_points": [[x * scale for x in p["neg_points"]] for p in component_value["neg_points"]], "labels": [p["labels"] for p in component_value["neg_points"]]} 
        return {
