...
DEFAULT_CONFIGFILE = ospath.join(ospath.expanduser("~"), ".caerp_pdf_split_config.yaml")


_UNSET = object()
_FILENAMESRE = re.compile(
    r"(?P<DOCTYPE>[A-Za-z_-_]+)_(?P<YEAR>" r"[0-9]+)_(?P<MONTH>[^_]+)\.pdf",
    re.IGNORECASE,
)
...
class Config(object):
    DEFAULTS = {
        ...
        "use_syslog": False,
        "log_to_mail": False,
        "outline": {"no_entr_name": False},
        "preprocessor": "caerp-split-pdf-page2text",
        "restrict": 0,
        "mail": {
            "subject": "[%(hostname)s] Log of caerp pdf splitter",
        },
    ...
    def load_args(self, parsed_args):
        ...
        if configstream:
            self.confvalues.update(yaml.safe_load(configstream))
    ...
    def _parse_inputfiles(self, parsed_args):
        inputfile = namedtuple(
            "inputfile", ["doctype", "year", "month", "filepath", "filedescriptor"]
        )
            ...
            if parsed is None:
                raise AutosplitError(
                    "Given filename '{}' doesn't match the regexp:\n{}".format(
                        bare_filename, _FILENAMESRE.pattern
                    )
                )
        ...
        for item in name:
            ...
            if intermediate is None:
                if default is not None:
                    ...
                    raise AutosplitError(
                        "Missing mandatory configuration parameter {0}".format(name)
                    )
