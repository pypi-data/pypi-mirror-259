...
from common.data.handle_common import req_expr, convert_json,format_caseName,get_system_key,print_info,print_debug
                        ...
                        if DataProcess.isNotNull(get_system_key(Constant.TEST_SRTCYCLE_ID)):
                            ...
                            if 'status' in _list:
                                if _list['status'] == Constant.STATUS_PRE or _list['status'] == Constant.STATUS_AUTOTEST:
                                    ...
                                    testdata[Constant.CASE_NO] = _list['caseid']
                                    testdata[Constant.CASE_TITLE] = _list['casename']
                                    _testDatas.append(testdata)
                                else:
    ...
    def list_script_removal_byKey(self, list_dict, key):
        ...
        if list_dict is not None and list_dict.__len__() > 0:
            ...
            for dict in list_dict:
                k = 0
                for item in a:
                    if key in dict and dict[key][0:dict[key].find('[')] != item[key][0:item[key].find('[')]:
                        k = k + 1
                    else:
                        break
                    if k == len(a):
