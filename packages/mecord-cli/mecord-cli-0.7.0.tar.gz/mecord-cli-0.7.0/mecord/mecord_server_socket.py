...
class MecordLongConnectThread(Thread):
    ...
    def get_url(self):
        if self.service_country == "test":
            return "wss://mecord-beta.2tianxin.com/proxymsg/ws" 
        else:
            return "wss://api.mecordai.com/proxymsg/ws" 
    ws = None
    extend_config = {}
    token = ""
    device_id = ""
        ...
        while self.is_running:
            try:
                ...
                try:
                    ...
                    taskUtils.taskPrint(taskUUID, f"{current_thread().name}=== receive {self.service_country} task : {taskUUID}")
                    _appendTask(taskUUID, self.service_country)
                    is_ok, msg, result = task.runTask(data, timeout, self.service_country)
                    self.TaskNotify(taskUUID, is_ok, msg, result)
                    if is_ok == False:
                        taskUtils.notifyTaskFail(taskUUID, self.service_country, msg)
                    _removeTask(taskUUID)
                except Exception as e:
                    taskUtils.taskPrint(taskUUID, f"{current_thread().name}=== task exception : {e}")
                    taskUtils.notifyScriptError(taskUUID, self.service_country)
                finally:
        ...
        while (self.is_running):
            ...
            try:
                ...
                for it in datas:
                    taskUtils.taskPrint(taskUUID, f"{current_thread().name}=== receive {self.service_country} task : {taskUUID}")
                    _appendTask(taskUUID, self.service_country)
                    is_ok, msg, result = task.runTask(it, timeout, self.service_country)
                    xy_pb.TaskNotify(self.service_country, taskUUID, is_ok, msg, result)
                    if is_ok == False:
                        taskUtils.notifyTaskFail(taskUUID, self.service_country, msg)
                    _removeTask(taskUUID)
            except Exception as e:
                taskUtils.taskPrint(taskUUID, f"{current_thread().name}=== {self.service_country} task exception : {e}")
                taskUtils.notifyScriptError(taskUUID, self.service_country)
            finally:
                taskUtils.taskPrint(taskUUID, None)
