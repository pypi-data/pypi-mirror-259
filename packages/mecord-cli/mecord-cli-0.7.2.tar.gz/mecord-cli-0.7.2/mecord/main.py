...
def status():
    ...
    if platform.system() == 'Windows':
        ...
        if service_is_running:
            print(scr_str(f"运行中 ({thread_num} 线程)".ljust(ll*3-2)))
        else:
            print(scr_str("未运行".ljust(ll*3-2)))
        print(scr_line("-" * ll*3))
        ...
        try:
            while True:
                ...
                idx+=1
                if service_is_running:
                    stdscr.addstr(idx, 0, scr_str(f"运行中 ({thread_num} 线程)".ljust(ll*3-2)), curses.color_pair(1))
                else:
                    stdscr.addstr(idx, 0, scr_str("未运行".ljust(ll*3-2)), curses.color_pair(2))
                idx+=1
                stdscr.addstr(idx, 0, scr_line("-" * ll*3))
                idx+=1
