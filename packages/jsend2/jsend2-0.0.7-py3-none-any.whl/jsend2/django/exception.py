...
from jsend2.jfast.response import JsendResponse
...
def jsend_exception_handler(exc, context):
    ...
    if isinstance(exc, JsendException):
        ...
        response.data = JsendResponse(status="error", message=detail).dict(exclude_none=True)

    return response


def myhandler400(request, exc):
    return JsonResponse({"msg": "400 raised!"}, status=400)


def myhandler403(request, exc):
    return JsonResponse({"msg": "403 raised!"}, status=403)


def myhandler404(request, exc):
    return JsonResponse({"msg": f"404 raised!"}, status=404)


def myhandler500(request):
    return JsonResponse({"msg": "500 raised!"}, status=500)
