...
class InstallRequirements:
    ...
    def __init__(self):
        self.version_library_comparisons_path = ensure_path("\\yiXie_2_util\\install_tool") \
                                                + "\\version_library_comparisons.txt"
        self.requirements_path = ensure_path("\\requirements.txt")
        # 初始化时，获取最新的版本库
    ...
    def text_comparison(self):
        ...
        if read_version_library_comparisons_txt == read_requirements:
            Info.log("程序中未检查到更新版本库，已为您跳过自动安装库")
        # 程序中如出现不同的文件，则安装
        else:
            Info.log("程序中检测到您更新了依赖库，已为您自动安装")
