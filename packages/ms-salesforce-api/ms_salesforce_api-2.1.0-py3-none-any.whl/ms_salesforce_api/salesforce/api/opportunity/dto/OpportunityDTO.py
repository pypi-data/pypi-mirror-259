    ...
    def from_salesforce_record(record):
        ...
        return OpportunityDTO(
            ...
            account_billing_country=normalize_value(
                record.get("Account_Billing_Country__c", None)
            ),
            account_owner=normalize_value(
                record.get("Account_owner__c", None)
            ),
            account_id=normalize_value(record.get("AccountId", None)),
            amount=normalize_value(record.get("Amount", None)),
            amount_eur=normalize_value(record.get("Amount_EUR__c", None)),
            campaign_id=normalize_value(record.get("CampaignId", None)),
            close_month_formula=normalize_value(
                record.get("Close_Month_Formula__c", None)
            ),
            close_date=normalize_value(record.get("CloseDate", None)),
            contact_id=normalize_value(record.get("ContactId", None)),
            curreny_iso_code=normalize_value(
                record.get("CurrencyIsoCode", None)
            ),
            end_date=normalize_value(record.get("EndDate__c", None)),
            expected_revenue=normalize_value(
                record.get("ExpectedRevenue", None)
            ),
            fiscal=normalize_value(record.get("Fiscal", None)),
            fiscal_quarter=normalize_value(record.get("FiscalQuarter", None)),
            fiscal_year=normalize_value(record.get("FiscalYear", None)),
            jira_default_name=normalize_value(
                record.get("FRM_JiraDefaultName__c", None)
            ),
            ga_client_id=normalize_value(record.get("GACLIENTID__c", None)),
            ga_track_id=normalize_value(record.get("GATRACKID__c", None)),
            ga_user_id=normalize_value(record.get("GAUSERID__c", None)),
            is_global=normalize_value(record.get("Global__c", None)),
            has_proposal=normalize_value(record.get("Has_proposal__c", None)),
            has_opportunity_lineitem=normalize_value(
                record.get("HasOpportunityLineItem", None)
            ),
            has_overdue_task=normalize_value(
                record.get("HasOverdueTask", None)
            ),
            hasproposal=normalize_value(record.get("HasProposal__c", None)),
            is_closed=normalize_value(record.get("IsClosed", None)),
            is_won=normalize_value(record.get("IsWon", None)),
            jira_component_required=normalize_value(
                record.get("Jira_component_required__c", None)
            ),
            jira_estimated_work_load=normalize_value(
                record.get("Jira_Estimated_Work_Load__c", None)
            ),
            jira_number=normalize_value(record.get("Jira_Number__c", None)),
            jira_work_load=normalize_value(
                record.get("Jira_Work_Load__c", None)
            ),
            jira_component_url=normalize_value(
                record.get("JiraComponentURL__c", None)
            ),
            lead_id=normalize_value(record.get("LeadId__c", None)),
            lead_source=normalize_value(record.get("LeadSource", None)),
            google_funding_opportunity=normalize_value(
                record.get("LKP_GoogleFundingOpportunity__c", None)
            ),
            loss_reason=normalize_value(record.get("Loss_Reason__c", None)),
            market_scope=normalize_value(record.get("Market_Scope__c", None)),
            ms_account_company_invoicing=normalize_value(
                record.get("MS_Account_Company_Invoicing__c", None)
            ),
            ms_account_invoice_country_code=normalize_value(
                record.get("MS_Account_Invoice_Country_Code__c", None)
            ),
            ms_company_origin=normalize_value(
                record.get("MS_Company_Origin__c", None)
            ),
            opportunity_name=normalize_value(record.get("Name", None)),
            opportunity_name_short=normalize_value(
                record.get("Opportunity_Name_Short__c", None)
            ),
            opportunity_requestor=normalize_value(
                record.get("Opportunity_requestor__c", None)
            ),
            owner_id=normalize_value(record.get("OwnerId", None)),
            probability=float(record.get("Probability", 0.0)),
            record_type_id=normalize_value(record.get("RecordTypeId", None)),
            roi_analysis_completed=normalize_value(
                record.get("ROI_Analysis_Completed__c", None)
            ),
            associated_services=normalize_value(
                record.get("Servicios_Asociados__c", None)
            ),
            stage_name=normalize_value(record.get("StageName", None)),
            start_date=normalize_value(record.get("Start_Date__c", None)),
            tier_short=normalize_value(record.get("Tier_Short__c", None)),
            total_opportunity_quantity=normalize_value(
                record.get("TotalOpportunityQuantity", None)
            ),
            year_created=normalize_value(record.get("Year_Created__c", None)),
