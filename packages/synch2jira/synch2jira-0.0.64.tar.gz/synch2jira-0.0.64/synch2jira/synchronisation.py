...
from synch2jira.implementation_issue_S2 import IssueImplementationS2
from synch2jira.implementation_table_correpondance_key_id import verifier_existence_id, save_correspondance_table,rebuild_correspondence_table,get_correspondence_table, \
    save_correspondence_dict

logging.basicConfig(filename=config.log_file, level=logging.INFO, format=config.log_format)
log_file = config.main_directory + "/log/file.log"
log_format = '%(asctime)s - %(levelname)s - %(message)s'

class Synchronisation:
    issue1 = IssueImplementationS1.issue1_factory(summary='', description='', updated=None, status='')

    @staticmethod
    def synchronisation_S2_S1():
        if not(os.path.exists(config.table_correspondance_file)) or os.path.getsize(config.table_correspondance_file) == 0:
    ...
    def synchronisation_S1_S2_v2():
        ...
        issue = IssueImplementationS1.issue1_factory("","",None,"")
        issue_list = issue.all()
        new_correspondences = {}
        for issue in issue_list:
            if str(issue.id) not in table_id_list:
                miror = IssueImplementationS2(summary=issue.summary,description=issue.description, status=issue.status)
                miror.miror = issue
                key = miror.save()
                if len(key) > 10 :
                    print("error  ",key)
