...
block((sub, Row', Col'), (Row, Col)) :- Row = 1..9; Col = 1..9; Row' = (Row-1) / 3; Col' = (Col-1) / 3.
    ...
    # print(Model.of_program(program).filter(lambda atom: atom.predicate_name == "assign").as_facts)
    start = time.time()
    explain = Explain.the_program(
        program,
        the_answer_set=model,
        the_atoms_to_explain=Model.of_program("assign((3,9),6).")
    )
    # print(explain.minimal_assumption_set())
    # explain.compute_explanation_dag()
    print(len(explain.explanation_dag()))
    end = time.time()
    print(end - start)
