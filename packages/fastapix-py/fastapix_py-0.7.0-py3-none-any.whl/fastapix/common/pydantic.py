        ...
        *,
        ...
        for name, field in model_fields(cls).items():
            if field.alias and field.alias in values:
                ...
                if err:
                    for e in err:
                        e['loc'] = (field.field_info.title or field.alias,)
                    errs.extend(err)
            elif name in values:
                _v = values.get(name)
                fields_values[name], err = field.validate(_v)
                if err:
                    for e in err:
                        e['loc'] = (field.field_info.title or name,)
                    errs.extend(err)
            elif not field.required:
                _v = field.get_default()
                defaults[name], err = field.validate(_v)
                if err:
                    if err[0].get('input') is None and field.field_info.primary_key:
                        # 主键默认None, 将自动生成主键值
                        continue
                    for e in err:
                        e['loc'] = (field.field_info.title or name,)
                ...
                errs.extend([InitErrorDetails(type='missing', loc=(field.field_info.title or name,), input=values)])
        if errs:
