...
class HistoricalDataManager:
    ...
    async def get_client(self, exchange: Exchanges) -> ExchangeAPIClient:
        if exchange.value not in self.clients_pool:
            if exchange == Exchanges.BINANCE_FUTURES:
                ...
                self.clients_pool[exchange.value] = BitgetFuturesAPI()
            else:
    ...
    async def shutdown(self):
        bitget_futures_client: BitgetFuturesAPI = self.clients_pool.get(Exchanges.BITGET_FUTURES.value)
        if bitget_futures_client:
            await bitget_futures_client.request_executor.http_session.close()
