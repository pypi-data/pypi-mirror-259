...
namespace sasktran_disco_lowlevel {
    ...
    // Exposed, untemplated main interface function, basically just validates
    // the input and then dispatches to a templated internal calculate
    ...
    void internal_calculate(const Atmosphere* atmosphere, const Config* config,
                            const WeightingFunctions* weightingfunctions,
                            const ViewingGeometry* geometry,
                            const Output* output);
    ...
    class CPPApi {
      private:
        ...
        std::vector<int>
            m_d_layerindex; // [deriv], assuming the same for every wavel
        ...
        int linear_index(int lyr, int wav) { return lyr + wav * m_nlyr; }
      ...
      public:
        CPPApi(int nstr, int nlyr, int nwavel, int nstokes, int nlos,
               int nderiv) {
            this->m_nstr = nstr;
            this->m_nlyr = nlyr;
        ...
        void initialize_c_api(Atmosphere* atmosphere, Config* config,
                              ViewingGeometry* geometry,
                              WeightingFunctions* weightingfunctions,
                              Output* output) {
        ...
        double& od(int lyr, int wav) { return m_od[linear_index(lyr, wav)]; }

        double& d_od(int deriv, int wav) {
            return m_d_od[d_linear_index(deriv, wav)];
        }

        int& d_layerindex(int deriv) { return m_d_layerindex[deriv]; }

        double& ssa(int lyr, int wav) { return m_ssa[linear_index(lyr, wav)]; }

        double& d_ssa(int deriv, int wav) {
            return m_d_ssa[d_linear_index(deriv, wav)];
        }

        double& f(int lyr, int wav) { return m_f[linear_index(lyr, wav)]; }

        double& albedo(int wav) { return m_albedo[wav]; }
        ...
        double& cos_sza() { return m_cos_sza; }

        double& cos_vza(int los) { return m_cos_vza[los]; }

        double& saa(int los) { return m_saa[los]; }

        double& earth_radius() { return m_earthradius; }
    };
