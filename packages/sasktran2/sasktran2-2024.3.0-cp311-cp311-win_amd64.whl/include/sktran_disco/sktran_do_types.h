...
#pragma endregion
    ...
    struct LegendreSumMatrixStorage : LegendreSumMatrixCommon {
        std::vector<sasktran_disco::TripleProductDerivativeHolder<NSTOKES>>
            storage;

        // This is actually unique ownership but we can't use a smart ptr
        // because it is inside a VectorDim2 where the internal VectorDim1 is
        // default copy? But it is never copied or moved so we just treat it as
        // unique
