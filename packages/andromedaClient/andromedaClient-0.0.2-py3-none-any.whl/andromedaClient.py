...
class AndromedaClient:
    ...
    def paginate(self, method, url, payload=None):
    ...
    def get(self, url_path):
        ...
        return self.paginate('get', url)

    def put(self, url_path, payload):
        ...
        if isinstance(payload, list) and len(payload) > 1000:
            ...
            for chunk in self.divide_chunks(payload, 200):
                res = self.session.put(url, json=chunk)
                res.raise_for_status()
    ...
    def post(self, url_path, payload):
        ...
        if isinstance(data, dict) or len(data) < 1000:
            return data
        logging.info("Too many results, start retrieving pages!")
        return self.paginate('post', url, payload)