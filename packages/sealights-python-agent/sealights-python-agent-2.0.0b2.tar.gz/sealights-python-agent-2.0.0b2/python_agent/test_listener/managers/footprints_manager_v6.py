...
from python_agent.test_listener.managers.code_coverage_manager_v6 import CodeCoverageManager
...
class FootprintsManager(object):
    def __init__(self, config_data: ConfigData, backend_proxy, agent_events_manager):
        ...
        self.build_mapping: BuildMapper = BuildMapper(config_data).scan_files_and_calculate_metrics()
        ...
        self.check_active_execution_interval = constants.ACTIVE_EXECUTION_INTERVAL_IN_MILLISECONDS / 1000
        self._add_coverage_interval = 1
        self.scheduler_manager = SchedulerManager()
        self.scheduler_manager.add_job(self.send_footprints_task, self.send_footprints_interval)
        self.scheduler_manager.add_job(self.add_coverage_task, self._add_coverage_interval)
        if not self.auto_execution:
            self.scheduler_manager.add_job(self.get_active_execution, self.check_active_execution_interval)
            log.debug(
                "Anonymous execution is enabled, will check for active execution every %d seconds" % (
                    int(self.check_active_execution_interval)))
        else:
            log.debug("Anonymous execution is disabled, will open and close execution manually")
        ...
        with self._add_coverage_lock:
            ...
            if file_lines_coverage:
                for file_name, lines in file_lines_coverage.items():
                    if not self.build_mapping.has_file(file_name):
                        log.debug(f"File: {file_name} not found in build context, ignoring lines: {lines}")
                        continue
                    for line in lines:
                        unique_id = self.build_mapping.get_method_unique_id(file_name, line)
                        if not unique_id:
                            continue
            ...
            if coverage_data:
                ...
                if self.config_data.drop_init_footprints:
                    log.info(f"Dropping init footprints is active, will ignore {len(coverage_data)} methods")
                else:
                    self.footprints_service.add_coverage(coverage_data, not has_active_execution)

    def send_footprints_task(self, *args, **kwargs):
        with self._send_footprints_lock:
            self.add_coverage_task()
            execution_is_active, execution_id, test_stage, execution_build_session_id = self.get_execution_data()
            has_coverage_recorded = self.footprints_service.has_coverage_recorded()
            if not execution_is_active:
                if has_coverage_recorded:
                    log.debug("Coverage is recorded but no execution is active. Will not send footprints")
                return
            try:
                if has_coverage_recorded:
                    log.info("Execution is active,sending footprints...")
                    log.info(self.build_mapping.get_coverage_metrics())
                    self.footprints_service.send(execution_id, test_stage, execution_build_session_id)
                else:
                    log.debug("Execution is active but no coverage is recorded. Will not send footprints")
    ...
    def get_active_execution(self):
        execution_response = self.backend_proxy.has_active_execution_v4(self.config_data.labId)
        send_now = False
        with self._current_execution_lock:
            if not self._current_execution and execution_response:
                log.debug(
                    "Execution is now active, Details: %s, pending coverage data will be sent now." % execution_response)
                send_now = True
            if self._current_execution and not execution_response:
                log.debug("Execution is not active anymore, new coverage data will be kept until next active execution")
    ...
    def start_execution(self):
        ...
        self.backend_proxy.start_execution(execution_request)
        with self._current_execution_lock:
        ...
        log.debug(
            "Started execution for labid: %s, testgroupid: %s" % (self.config_data.labId, self.config_data.testGroupId))

    def end_execution(self):
        execution_id = None
        with self._current_execution_lock:
            execution_id = self._current_execution.get("executionId", None)
        self.backend_proxy.end_execution(self.config_data.labId, self.config_data.testGroupId, execution_id)
        with self._current_execution_lock:
            self._current_execution = None
        log.debug(
            "Ended execution for labid: %s, testgroupid: %s" % (self.config_data.labId, self.config_data.testGroupId))
