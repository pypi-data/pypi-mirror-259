...
from core.utils.exceptions import ProjectExistException, DataStudioDatabaseException
...
@method_decorator(name='get', decorator=swagger_auto_schema(
    ...
    To perform most tasks with the Data Studio API, you must specify the project ID, sometimes referred to as the `pk`.
    To retrieve a list of your Data Studio projects, update the following command to match your own environment.
...
@method_decorator(name='post', decorator=swagger_auto_schema(
    ...
    operation_description="""
    Create a project and set up the labeling interface in Data Studio using the API.
...
class ProjectListAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # - Public projects of any organization
        # - Projects of the user's organization where the user is a contributor or the project owner
        projects = Project.objects.filter(
            Q(is_public=True) |
            Q(organization=self.request.user.active_organization, 
              user_roles_projects__user=self.request.user) |
            Q(organization=self.request.user.active_organization, 
    ...
    def perform_create(self, ser):
        try:
            ...
            raise DataStudioDatabaseException('Database error during project creation. Try again.')
...
class ProjectAPI(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        ...
        serializer.is_valid(raise_exception=True)
        fields = serializer.validated_data.get('include')
        return Project.objects.with_counts(fields=fields).filter(Q(is_public=True) |
            Q(organization=user.active_organization, 
              user_roles_projects__user=user) |
            Q(organization=user.active_organization, 
              created_by=user)).distinct()
