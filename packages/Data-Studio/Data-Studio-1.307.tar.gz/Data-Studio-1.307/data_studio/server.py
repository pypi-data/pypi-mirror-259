...
# from data_studio.projects.models import Project
from data_studio.users.models import User
...
def _init(input_args, config):
    ...
    user = authenticate(username=input_args.email, password=input_args.password)
    if user is not None:
    ...
    #     print('Project "{0}" already exists'.format(input_args.project_name))
        if not _project_exists(input_args.project_name):
                # Define the sampling strategy map
...
def main():
    ...
    if input_args.command == 'login_init':
        ...
        if input_args.email:
            try:
                user = User.objects.get(email=input_args.email)
        ...
        user = authenticate(username=user.username, password=input_args.password)
        if user is not None:
            print("Login successful.")
            ...
            if input_args.project_name:
                project, created = Project.objects.get_or_create(
                    title=input_args.project_name, defaults={'created_by': user}
                )
                if created:
    ...
    if input_args.command == 'delete_project':
        ...
        try:
            ...
            confirm = input(f'Are you sure you want to delete the project "{project_name}"? [y/N]: ').lower()
            ...
            print(f'An error occurred while trying to delete the project: {e}')
    ...
    # on `start` command, launch browser if --no-browser is not specified and start Data Studio server
    if input_args.command == 'start' or input_args.command is None:
        ...
        if cert_file or key_file:
            logger.error("Data Studio doesn't support SSL web server with cert and key.\n"
