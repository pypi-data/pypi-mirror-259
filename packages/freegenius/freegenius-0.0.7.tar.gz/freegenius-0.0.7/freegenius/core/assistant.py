...
class FreeGenius:
    ...
    def runPlugins(self):
        ...
        letmedoit.config.outputTransformers = config.outputTransformers = []
...
{userInput}""")
                    ...
                    if action in ("python_qa", "other_request") and not action in config.freeGeniusActionMethods:
                        ...
                        if parameters:
                            if response := config.freeGeniusActionMethods[action](parameters):
                                if response == "[INVALID]":
...
"""
    ...
    def extractActionParameters(self, ongoingMessages, userInput, action) -> dict:
        def getKeyDescriptions(parameters) -> str:
            descriptions = [f"""* "{parameter}" - {parameter["description"]}""" for parameter in parameters]
            return "\n".join(descriptions)

        parameters = config.freeGeniusActionParameters[action]
        # create a template
        template = {parameter: "" if parameter["type"] == "string" else [] for parameter in parameters} # support string and list/array first, more later
        # prompt
