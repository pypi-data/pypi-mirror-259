...
class FaultInjectionController(object):
    ...
    def get_fib_state(self):
        """
        Returns the full Pin State of the FIB as a dictionary:
        """
        states = {}
        out = {}
        pins = list(range(70))
    ...
    def get_fib_card(self, channel):
        ...
        -------
        tuple of int
            fib card number starting at 0, signal number on card
...
class StandardSignalCard(SignalCard):
    ...
    def get_status(self):
        ...
        for i in range(10):
            out[i] = {
                "fr1" : bool((self.state >> 3*i) & 0b010),
                "fr2" : bool((self.state >> 3*i) & 0b001),
                "oc" : bool((self.state >> 3*i) & 0b100),
            }
        return out
