...
def fetch_tles(requested_norad_ids, sources=get_tle_sources(), spacetrack_config=None,
    ...
    if spacetrack_config:
        # Limit the requested NORAD IDs to 400 per request. The tested limit is 574 NORAD IDs.
        limit = 400
        requests_num = len(requested_norad_ids) // limit + 1
        try:
            for request in range(0, requests_num):
                norad_ids = list(requested_norad_ids)[request * limit:(request + 1) * limit]
                new_tles = fetch_tles_from_spacetrack(norad_ids, spacetrack_config)
                logging.debug('Found TLEs for {}'.format(list(new_tles.keys())))
                for norad_id, tle in new_tles.items():
