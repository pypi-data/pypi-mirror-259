...
def _ensure_provider(provider_id: str, state: dict):
    ...
    with _provider_lock:
        ...
        if not existing:
            provider = import_module(_get_provider_module_name(provider_id))
...
def get_deployment_data(data: dict, additional_context: dict = None):
    blueprint, state, state_file = _prepare_data(data, additional_context, None)
    input = {}
    output = {}
    for k in blueprint["resource"]:
        output[k] = state["output"].get(k)
        input[k] = state["resource"].get(k)
