...
def update(
    ...
    func_defaults = {
        k: v.default
        for k, v in inspect.signature(update_deps).parameters.items()
    ...
    config_obj = utils.merge_config_with_params(
        config_file=config_file, params=final_params
    )
    ...
    if update_config_from_files(
        ...
        utils.save_config_to_toml_file(cerebrium_config, config_file)
