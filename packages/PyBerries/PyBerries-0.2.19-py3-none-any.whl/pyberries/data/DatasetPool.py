    ...
    def add_metadata(self, metadata, inplace=False):
        ...
        for target in metadata.keys():
            ...
            for obj in objects:
                if not dp[obj].empty:
                    df = dp[obj]
                    for var in metadata[target]:
                        df[var.replace(' ', '_')] = pd.Series()
                        if var == 'DateTime':
                            df['TimeDelta'] = pd.Series()
                            df['Time_min'] = pd.Series()
                        for i, ds in enumerate(dp.dsList):
                            if ds in list(dp[obj].Dataset):
                                ...
                                for pos in df.loc[df['Dataset'] == ds].Position.unique():
                                    ...
                                    elif exists(join(ds_path, ds, 'SourceImageMetadata', f'{pos}_c0_t0.txt')):
                                        metadata_filename = f'{pos}_c0_t0.txt'
                                    else:
                                        metadata_filename = f'{pos}_c0_t1.txt'
                                    with open(join(ds_path, ds, 'SourceImageMetadata', metadata_filename)) as f:
                                        ...
                                        df.loc[(df['Position'] == pos) & (df['Dataset'] == ds),
                                               var.replace(' ', '_')] = value.split('=')[-1][:-1]
