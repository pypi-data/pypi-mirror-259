...
class TextBlocks(GenericCRUD):
    ...
    #TODO profile_id -> text_block_profile_id
    ...
    def process_and_update_field(self, profile_id: int, fields_dict: dict, field_id: int, match: str) -> None:
        ...
        try:
            ...
            self.cursor.execute("SELECT `schema`, table_name, profile_mapping_table_id FROM `database`.table_definition_view WHERE table_definition_id = %s", (table_id,))
            schema, table_name, profile_mapping_table_id = self.cursor.fetchone()
            view_name = self._get_view_name(table_name)
            # Get field information
            self.cursor.execute("SELECT `schema`, table_name FROM `database`.table_definition_view WHERE table_definition_id = %s",
            ...
            if profile_id and profile_mapping_table_id:
                ...
                mapping_id = self.select_one_dict_by_id(view_table_name="user_general_view",
                                                        select_clause_value=select_clause_value,
                                                        id_column_name="profile_id",
                                                        id_column_value=profile_id)[select_clause_value]
                id_column_name = TextBlocks._get_id_column_name(table_name)
                sql = "SELECT %s FROM %s.%s WHERE %s = %s" % (database_field_name, schema, view_name, id_column_name, mapping_id)
                self.cursor.execute(sql)
                field_old = self.cursor.fetchone()
                if field_old is not None:
                    sql = "UPDATE %s.%s SET %s = '%s' WHERE %s = %s" % (
                        schema, table_name, database_field_name, match, id_column_name, mapping_id)
                    if database_sub_field_name and database_sub_field_value:
                        sql = "UPDATE %s.%s SET %s = '%s', %s = '%s' WHERE %s = %s" % (
                            ...
                            database_sub_field_value, id_column_name, mapping_id)
                    self.cursor.execute(sql)
    ...
    # TODO: move this method to GenericCRUD
    @staticmethod
    def _get_id_column_name(table_name: str) -> str | None:
        if table_name:
