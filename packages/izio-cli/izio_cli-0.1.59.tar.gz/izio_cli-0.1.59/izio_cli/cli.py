...
import inquirer
from izio_cli.helper.fs_helper import create_file
...
def new_project(
    ...
    if not type:
        type = inquirer.list_input(
            "Select the type of project, flutter or netCore | netCore is not suported on MacOs",
    ...
    if not path:
        ...
        path = getProjectPath(projectName, path)
    ...
    if type == "flutter":
        ...
        if entity:
            new_entity(path, solution=projectName)
...
def new_entity(
    ...
    name: str = Option("", "--name", "-n", help="Entity name"),
    solution: str = Option("", "--solution", "-s", help="Solution name"),
):
    """'
    ...
    if not name:
        name = inquirer.text("Enter the entity name")
        if not name:
            raise ValueError("Entity name is required")

    if not solution:
        solution = inquirer.text("Enter the solution name")
        if not solution:
            raise ValueError("Solution name is required")

    if not path:
        path = f"{os.getcwd()}{os.sep}{solution}" if solution not in os.getcwd() else os.getcwd()

    create_entity(path, name, console, solution)
