...
from izio_cli.values.dot_net_payload import deValidationPayload, envPayload, responseArrayInterfacePayload, responseArrayPayload, responseInterfacePayload, responsePayload
...
def addIzioLib(projectName, path):
    ...
    """
    # TODO: Add the Izio Library package to the DataAccess and  project
    run_command(
        [
            "dotnet",
            "add",
            f"{projectName}.Infra.DataAccess{os.sep}{projectName}.Infra.DataAccess.csproj",
            "package",
...
def create_entity(path, solution ,name: str, console: Console):
    ...
    with console.status("Creating a new entity", spinner="arc", spinner_style="bold green") as status:
        ...
        console.print(
            f"🚀 [green]Successfully created the[/] [b blue].NET Core[/] Entity: {name.captalize}",)

    create_file(
        f"{path}{os.sep}Application{os.sep}Infra.DataAccess{os.sep}Repository",
