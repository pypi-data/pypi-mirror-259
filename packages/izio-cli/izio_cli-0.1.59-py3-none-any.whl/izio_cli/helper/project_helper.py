...
def getProjectPath(projectName, path=os.getcwd()):
    path = f"{path}{os.sep}{projectName}"
...
def getProjectName( type="flutter", solution="IzPay") -> tuple[bool, str]:
    projectName = to_snake_case(inquirer.text("Enter the project name"))
    (
        f"Mb_{solution}.flutter.{to_pascal_case_with_underscore(projectName)}"
        if type == "flutter"
        else f"Be_{solution}.{to_pascal_case_with_underscore(projectName)}"
    )
