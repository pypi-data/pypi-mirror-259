...
SITE_STATS = namedtuple("SiteStats", ['spike_count', 'spike_count_labeled', 'spike_count_unlabeled',
                                      ...
                                      'noise', 'snr', 'neurons'])
SPK_LABEL = Enum('SpikeLabel', ['LABELED', 'UNLABELED', 'BAD', 'TOTAL'], start=0)
SPIKE_COMP_LEN = namedtuple("SpikeComponentLen", ['waveform', 'feature', 'feature2', 'zscore'])
...
class SpikesSet():
    ...
    def _reserved_has_component(self, comp: str) -> bool:
        if comp not in ['timestamps', 'waveforms', 'ftrs', 'ftrs2']:
            raise ValueError('comp not valid, use [`timestamps`|`waveforms` | `ftrs` | `ftrs2`')
...
class SpikesMetadata():
    ...
    def __init__(self, msg):
        ...
        self._sensor_ext_metadata = {'instance_UID': msg.sensorExt.sensorInstanceUID}
        ...
        for site in msg.site:
            ...
            self._site['neurons'].append([make_neuron_desc(elem) for elem in site.neuron])
            self._site['num_neurons'].append(site.numNeurons)
            self._site['spike_comp_len'].append(SPIKE_COMP_LEN(waveform=site.compLen.waveform, feature=site.compLen.feature,
