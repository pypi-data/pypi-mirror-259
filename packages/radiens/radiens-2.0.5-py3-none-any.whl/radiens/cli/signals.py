...
from radiens.utils.util import (time_now, make_time_range)
    ...
    try:
        if ctx.obj['cmd_app'] in ['videre']:
            ...
            ctx.obj['videre_client'].signals().get_psd(
                time_range=make_time_range(time_range=[tstart, tstart+dur], fs=dsource.time_range.fs),
        ...
        click.echo('Error: {}'.format(ex))
        return
    click.echo('{} ({}) : calculated PSD heatmap at {}'.format(ctx.obj['cmd_app'].upper(), msg_id, time_now()))
    if plot_id:
        click.echo('          plotted to screen')
    if file:
