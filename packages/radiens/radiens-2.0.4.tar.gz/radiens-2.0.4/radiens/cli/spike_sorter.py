...
def sort(ctx, arg, verbose):
    ...
    if arg[0] not in ['on', 'off']:
        click.echo(click.style('`arg` must be `on` or `off`', fg=ERROR_COLOR_FG))
        return
    try:
        ...
        return
    click.echo('ALLEGO real-time spike sorter set {} at {}'.format(arg[0], time_now()))
...
def init(ctx):
    ...
    try:
        ...
        click.echo('Error: {}'.format(ex))
        return
    click.echo('ALLEGO real-time spike sorter initialized at {}'.format(time_now()))
...
def clear_spikes(ctx):
   ...
   Clears all spike data from the spike sorter, but does not clear it's sorting settings or spike templates.
    ...
    try:
        ...
        click.echo('Error: {}'.format(ex))
        return
    click.echo('ALLEGO real-time spike sorter spikes cleared at {}'.format(time_now()))
