...
from radiens.grpc_radiens import (allegoserver_pb2, allegoserver_pb2_grpc,
                                  ...
                                  common_pb2, spikesorter_pb2, datasource_pb2)
...
def dsrc_list_dir(addr, dirL: list, sort_by: str):
    ...
    if len(dirL) == 1:
        req = common_pb2.ListDataSourcesRequest(directory=dirL[0], sortBy=sort_enum.index(sort_by))
    else:
        req = common_pb2.ListDataSourcesRequest(directory='na', dirList=dirL, sortBy=sort_enum.index(sort_by))
...
def get_protocol(addr, protocol_id: str):
    with new_server_channel(addr) as chan:
        stub = radiensserver_pb2_grpc.RadiensCoreStub(chan)
        try:
            resp = stub.GetProtocol(common_pb2.ProtocolRequest(protocolID=protocol_id))
        except grpc.RpcError as ex:
            handle_grpc_error(ex, CLIENT_NAME)
        return make_protocol_from_pb2(resp)
...
def rename_protocol(addr, protocol_id: str, new_protocol_id: str):
    with new_server_channel(addr) as chan:
        stub = radiensserver_pb2_grpc.RadiensCoreStub(chan)
        try:
            resp = stub.RenameProtocol(common_pb2.RenameProtocolRequest(protocolID=protocol_id, newProtocolID=new_protocol_id))
        except grpc.RpcError as ex:
            handle_grpc_error(ex, CLIENT_NAME)
        return make_protocol_from_pb2(resp)
...
def apply_protocol(addr, protocol_id: str):
    ...
    stub = radiensserver_pb2_grpc.RadiensCoreStub(chan)
    try:
        stream = stub.ApplyProtocol(common_pb2.ProtocolRequest(protocolID=protocol_id))
    except grpc.RpcError as ex:
        handle_grpc_error(ex, CLIENT_NAME)
    return stream
