...
# from radiens.utils.constants import ()
...
def make_node_from_protobuf(pb_node: common_pb2.TransformNode):
    if pb_node.invalid is True:
        raise ValueError('node: {} -- INVALID::{}'.format(pb_node.id, pb_node.errorMessage))
    ...
    if pb_node.type in [common_pb2.LOWPASS_FILTER]:
        ...
        node.bandpass(pb_node.bandParams.lowFrequency, pb_node.bandParams.highFrequency)
    elif pb_node.type in [common_pb2.BANDSTOP_FILTER]:
        node.bandstop(pb_node.bandParams.lowFrequency, pb_node.bandParams.highFrequency)
    elif pb_node.type in [common_pb2.NOTCH_FILTER]:
        node.notch(pb_node.notchParams.notchFrequency, pb_node.notchParams.notchBandwidth)
    elif pb_node.type in [common_pb2.PAIRED_REF]:
        node.paired_ref(pb_node.pairedRefParams.refNtvChanIdx, pb_node.pairedRefParams.targetNtvChanIdx)
        ...
        node.slice_time(pb_node.sliceTimeParams.timeStart, pb_node.sliceTimeParams.timeEnd)
...
class TransformNode:
    ...
    def bandpass(self, low_freq: float, high_freq: float):
        ...
        self._params['band'] = {'lowFrequency': low_freq, 'highFrequency': high_freq}
        return self
    ...
    def notch(self, freq: float, bandwidth: float):
        ...
        self._params['notch'] = {'notchFrequency': freq, 'notchBandwidth': bandwidth}
        return self

    def paired_ref(self, ref_ntv_chan_idx: int, target_ntv_chan_idx: int):
        self._node_type = common_pb2.PAIRED_REF
        self._params['pairedRef'] = {'refNtvChanIdx': ref_ntv_chan_idx, 'targetNtvChanIdx': target_ntv_chan_idx}
        return self
    ...
    def slice_time(self, time_start: float, time_end: float):
        ...
        self._params['sliceTime'] = {'timeStart': time_start, 'timeEnd': time_end}
        return self
    ...
    def as_protobuf(self):
        ...
        raise ValueError('invalid transform node type {}'.format(self._node_type))
...
class ProtocolAPI:
    def __init__(self, protocol_id: str):
        self._protocol_id = '{}__{}'.format(protocol_id, str(uuid.uuid4())[4:13])
