...
from radiens.grpc_radiens import (allegoserver_pb2, allegoserver_pb2_grpc,
                                  biointerface_pb2, common_pb2,
                                  spikesorter_pb2)
from radiens.utils.constants import (KEY_IDXS, NEIGHBORS_DESC, NEURON_DESC,
                                     NEURON_EXTENDED_METADATA, SIG_SELECT,
                                     SITE_POS_CARTESIAN, TIME_RANGE, TIME_SPEC)
...
def ntv_to_dset_dict(idxs: KEY_IDXS) -> dict[int, int]:
    return {n: d for n, d in zip(idxs.ntv, idxs.dset)}


def dset_to_ntv_dict(idxs: KEY_IDXS) -> dict[int, int]:
