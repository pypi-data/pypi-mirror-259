...
from radiens.utils.constants import (DEFAULT_HUB_ID, NEURONS1_ADDR,
                                     NEURONS_SINK_DSOURCE_ID, TIME_RANGE)
...
class SpikesClient():
    ...
    def get_spike_waveforms(self, dataset_id: str, time_range: list, ntv_idxs: list = [], hub_name=DEFAULT_HUB_ID):
        """
        ...
        if self._is_videre():
            return api_videre.get_spike_waveforms(self.__parent._server_address(hub_name, RadiensService.CORE), dataset_id, time_range, ntv_idxs)
        else:
            raise Exception('{} cannot get spike waveforms',
                            self.__parent.type)

    def get_spike_timestamps(self, dataset_id: str, time_range:  TIME_RANGE, ntv_idxs: list = [], hub_name=DEFAULT_HUB_ID):
        """
        ...
        if len(ntv_idxs) != 0:
            warnings.warn(
                f"ntv_idxs was supplied, but is not used in the current implementation of get_spike_timestamps")
    ...
    def get_neuron_timestamps(self, dataset_id: str, time_range: TIME_RANGE, ntv_idxs: list):
