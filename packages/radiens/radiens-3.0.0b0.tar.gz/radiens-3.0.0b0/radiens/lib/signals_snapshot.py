...
from radiens.utils.constants import SIGNALS, TIME_RANGE
from radiens.utils.enums import FftWindow, PsdScaling
...
def concat_sigs(sigs_list: list[Signals]) -> Signals:
    """
    Concatenates a list of Signals objects into a single Signals object.
    """
    sigs = sigs_list[0]
    for sigs_ in sigs_list[1:]:
        for k, v in sigs_.signals._asdict().items():
            sigs.signals._asdict()[k] = np.concatenate(
                (sigs.signals._asdict()[k], v), axis=0)
    return sigs
