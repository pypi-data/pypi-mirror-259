...
def link_datasource(ctx, datafile, metrics, force, hub):
    ...
    '''
    try:
        dsource = ctx.obj['videre_client'].link_data_file(datafile, calc_metrics=metrics, force=force)
    except Exception as ex:
        click.echo('Error: {}'.format(ex))
        return
    click.echo('Linked {} (id={}) to Radiens hub {} at {}'.format(Path(datafile), dsource.id, hub, time_now()))
...
def list_dsource(ctx, hub):
    ...
    try:
        ...
        click.echo('Error: {}'.format(ex))
        return
    click.echo('Linked dataset table [hub {}, N={}]'.format(hub, len(df_table)))
    print(df_table)
