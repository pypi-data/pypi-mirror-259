...
from radiens.utils.constants import SYSTEM_MODE_DECODE
...
class AllegoState():
    ...
    def __init__(self, raw=allegoserver_pb2.ConfigAndStatus):
        ...
        for k, port in enumerate(raw.ports):
            self._port_spec[common_pb2.Port.Name(
                port.port)] = port.channelCount
        ...
        for p in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']:
            self._cable_lengths[p] = eval('raw.cableLengths.{}'.format(p))
        self._backbone_mode = SYSTEM_MODE_DECODE[raw.backboneMode]
    ...
    @ property
    def port_num_channels(self) -> dict:
        """
    ...
    def print(self, detail: str = 'brief'):
        ...
        if detail in ['verbose']:
            ...
            print('  port channels : {}'.format(self.port_num_channels))
            print('  cable lengths : {}'.format(self.cable_length_ft))
