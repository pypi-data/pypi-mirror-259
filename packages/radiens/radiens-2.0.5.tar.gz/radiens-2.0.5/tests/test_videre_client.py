...
class base_utest(unittest.TestCase):
    ...
    def test_get_signals(self):
        ...
        chunk = 0.01
        TR = make_time_range(time_range=[dsource.time_range.sec[0], dsource.time_range.sec[0] + chunk], fs=dsource.time_range.fs)
        sig = self.client.signals().get_signals(TR, TRS_MODE.SUBSET, sig_sel=None, dataset_metadata=dsource)
        self.assertEqual(sig.signals.amp.shape[0], dsource.channel_metadata.num_sigs(SignalType.AMP))
        self.assertEqual(sig.signals.gpio_ain.shape[0], dsource.channel_metadata.num_sigs(SignalType.AIN))
        ...
        self.assertEqual(dsource.time_range.sec[0]+chunk, sig.time_range.sec[1])
    ...
    def test_get_psd(self):
        ...
        chunk = 0.2
        TR = make_time_range(time_range=[dsource.time_range.sec[0], dsource.time_range.sec[0] + chunk], fs=dsource.time_range.fs)
        psd = self.client.signals().get_psd(TR, TRS_MODE.SUBSET, sig_sel=None, samp_freq=2000.0, dataset_metadata=dsource)
        self.assertEqual(psd.psd.shape[0], dsource.channel_metadata.num_sigs(SignalType.AMP))
    ...
    def test_signal_metrics(self):
        ...
        self.assertFalse(Path(self.data_path, '64chan_1s.kpi').expanduser().absolute().exists())

        dsource = self.client.link_data_file(Path(self.data_path, '64chan_1s.xdat'), calc_metrics=False)
        ...
        time.sleep(2)
        self.assertTrue(Path(self.data_path, '64chan_1s.kpi').expanduser().absolute().exists())
        status = self.client.signal_metrics().get_metrics_status(dsource)
        ...
        self.client.clear_dataset(dataset_id='all')
        dsource = self.client.link_data_file(Path(self.data_path, '64chan_1s.xdat'), calc_metrics=True)
        self.assertEqual("64chan", dsource.id[0:6])
        time.sleep(2)
        self.assertTrue(Path(self.data_path, '64chan_1s.kpi').expanduser().absolute().exists())
