...
from radiens.utils.constants import CORE_SERVICE, DEFAULT_HUB
...
class CurateClient(BaseClient):
    ...
    def dsp_progress_bar(self, stream_id):
        ...
        """
        pbar = tqdm(total=self.dsp_stream[stream_id]['data_source'].duration_sec, desc=stream_id,
                    smoothing=0.9,
    ...
    def dsp_low_pass_filter(self, cutoff_freq: float, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_high_pass_filter(self, cutoff_freq: float, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB):
        """
    ...
    def dsp_band_pass_filter(self, low_cutoff_freq: float, high_cutoff_freq, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_band_stop_filter(self, low_cutoff_freq: float, high_cutoff_freq, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_notch_filter(self, freq: float, bandwidth, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_paired_diff(self, source_chan: int, ref_chan: int, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_virtual_ref(self, ref_chan: int, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_car(self, source_path: any, target_path: any, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_time_decimate(self, decimate: int, source_path: Path, target_path: Path, force=False, hub_name=DEFAULT_HUB) -> str:
        """
    ...
    def dsp_slice_time(self, time_start: float, time_end: float, source_path: Path, target_path: Path, force=False, hub_name=DEFAULT_HUB):
        """
    ...
    def dsp_slice_channels(self, target_channels: list, source_path: Path, target_path: Path, force=False, hub_name=DEFAULT_HUB):
        """
    ...
    def _do_protocol(self, protocol: ProtocolAPI, hub_name=DEFAULT_HUB) -> str:
        api.set_protocol(self._server_address(
            hub_name, CORE_SERVICE), protocol)
        return api.apply_protocol(self._server_address(hub_name, CORE_SERVICE),  protocol.id)

    def _clear_dataset(self, source=[], dataset_id=[], hub_name=DEFAULT_HUB) -> str:
        ...
        for _id in self._get_dataset_ids(hub_name):
            ...
            for src in source:
                if _id.find(Path(src).stem) > 0 and _id.find(self.id) > 0:
                    _get_ids.append(_id)
        return api_videre.unlink_datasource(self._server_address(hub_name, CORE_SERVICE), _get_ids)

    def _get_dataset_ids(self, hub_name=DEFAULT_HUB) -> list:
        return api_videre.list_datasource_ids(self._server_address(hub_name, CORE_SERVICE))

    def _link_data_file(self, source: any, calc_metrics=False, force=False, hub_name=DEFAULT_HUB) -> DatasetMetadata:
        ...
        req = datasource_pb2.DataSourceSetSaveRequest(path=str(source.parent), baseName=source.stem, fileType=to_radiens_file_type(
            source), dsourceID=source.stem+self.id, isBackgroundKPI=calc_metrics, isForce=force)
