...
def deploy_container_with_docker_compose(
    ...
    remote_path = os.path.join(Config.REMOTE_WS, "cache", "docker-compose.yml")
    local_path = os.path.join(Config.DATAPATH_BASE, "cache", tmp_file.execution_cmd)
...
class DeploymentBotFoundation:
    ...
    def load_system_paths(self, c: Connection):
        r = c.run("which bash", warn=True, tty=True)
        if str(r.stdout.strip()) != "":
            Config.BASH = str(r.stdout.strip())
        r = c.run("which docker", warn=True, tty=True)
        if str(r.stdout.strip()) != "":
            Config.DOCKER = str(r.stdout.strip())
        r = c.run("which docker-compose", warn=True, tty=True)
        if str(r.stdout.strip()) != "":
