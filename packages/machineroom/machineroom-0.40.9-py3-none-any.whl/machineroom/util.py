...
def extract_block_from_line(head_line_json: str, endline_str: str, temp_file: str = TEMP_FILE, isJson: bool = True) -> \
        Union[dict, str, int]:
...
def extract_by_regx(regex, temp_file: str = TEMP_FILE) -> list:
    path = os.path.join(DATAPATH_BASE, temp_file)
    payload_x = []
    with open(path, 'r') as fp:
        dat = fp.read()
...
def capture_js_block(start_cap_line: str, end_cap_line: str, offset_back_line: int = 1,
    ...
    with open(path, 'r') as fp:
        ...
        for line in dumps:
            ...
            nb += 1
        fp.close()
    path_tmp = os.path.join(DATAPATH_BASE, TEMP_JS)
...
def store_txt(file_put: str, content: str):
    path_tmp = os.path.join(DATAPATH_BASE, file_put)
...
class BufferFile:
    ...
    @property
    def path(self):
        return os.path.join(DATAPATH_BASE, self._file)
    ...
    def save_as(self, file_name: str):
        base_database = os.path.join(DATAPATH_BASE, file_name)
        content = ""
        with open(self.path, "r") as f_io:
            content = f_io.read()
            f_io.close()
...
class Servers:
    ...
    def path_file(self) -> str:
        return os.path.join(DATAPATH_BASE, self._meta_file)
