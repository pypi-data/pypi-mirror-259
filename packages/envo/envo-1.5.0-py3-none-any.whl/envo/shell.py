...
import envo
from envo import comp, logger
from envo.exceptions import UserError, user_code
from envo.utils import Callback, is_windows
...
class Shell(BaseShell):  # type: ignore
    ...
    def set_variable(self, name: str, value: Any) -> None:
        ...
        if "." in name:
            ...
            self._add_namespace_if_not_exists(namespace)
    ...
    def _run_code(self, code: str) -> None:
        ...
        exc, value, _ = run_compiled_code(c, self.ctx, None, "single")

        if exc:
            raise UserError(msg=f'"{code}" -> {repr(value)}', exc=exc)
    ...
    def set_full_traceback_enabled(self, enabled: bool = True):
        self.environ["XONSH_SHOW_TRACEBACK"] = enabled

    def convert_fire_to_python(self, input_str: str) -> str:
        matches = re.findall(r'[\"\'](.*?)[\"\']', input_str)
        if matches:
            args = ', '.join([f'"{match}"' for match in matches])
        else:
            args = ''
        function_name = re.match(r'^(.*?)(?: [\"\']|$)', input_str).group(1)
        return f'{function_name}({args})'

    def _pre_cmd(self, command: str) -> Optional[str]:
        if self._is_python_fire_cmd(command):
            ret = self.convert_fire_to_python(command)
            return ret
        else:
            return command
    ...
    def _is_python_fire_cmd(self, cmd: str) -> bool:
        ...
        try:
            env_obj = eval(f"env.{command_name}")
            if isinstance(env_obj, (FunctionType, MethodType)):
                return True
        except:
            pass

        try:
            obj = eval(command_name)
