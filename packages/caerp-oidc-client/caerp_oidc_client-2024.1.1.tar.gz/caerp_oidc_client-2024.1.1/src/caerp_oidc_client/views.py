...
from endi.utils.session import delete_session_from_cookie_value, get_session_id

from .models import OidcSession
from .utils import check_settings, parse_id_token

logger = logging.getLogger("endi." + __name__)
...
class OidcLoginView:
    ...
    def __call__(self):
        ...
        if self.request.identity is not None:
            ...
            return HTTPFound(self.auth_endpoint_url())
    ...
    def _get_id_token_content(self, json_result: str) -> dict:
        """
    ...
    def _store_session_in_database(self, id_token):
        ...
        if oidc_sid:
            ...
            oidc_session = OidcSession.get_from_login(self.request, oidc_sub)
    ...
    def oidc_callback_view(self):
        ...
        if not code:
            ...
            if get_identity(self.request, login) is None:
    ...
    def backend_logout_view(self):
        ...
        request.response.headerlist.append(('Cache-Control', 'no-store'))
        return {}
