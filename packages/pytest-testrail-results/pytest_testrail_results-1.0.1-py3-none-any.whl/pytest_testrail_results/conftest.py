...
from utility.custom_pytest_testrail_plugin.plugin import PyTestRailPlugin
from _pytest.config import Config
from _pytest.config.argparsing import Parser
from utility.custom_pytest_testrail_plugin.testrail_api import APIClient
import configparser
    ...
    if config.getoption("--testrail"):
        ...
        config.pluginmanager.register(
            PyTestRailPlugin(
                ...
                tr_name=str(
                    config_manager.getoption("tr-testrun-name", "name", "TESTRUN") or ""
                ),
                tr_description=str(
                    config_manager.getoption(
...
class ConfigManager(object):
    def __init__(self, cfg_file_path: Optional[str], config: Config) -> None:
        """
        ...
        """
        self.cfg_file = None
        if cfg_file_path is not None and (
            os.path.isfile(cfg_file_path) or os.path.islink(cfg_file_path)
        ):
