...
def pad_out_table(df, pad_t, unit):
    ...
    pad_before = pd.date_range(pad_before_start, periods=pad_t, freq=unit)
    pad_after = pd.date_range(pad_after_start, periods=pad_t, freq=unit)

    df = pd.concat(
        [
            pd.DataFrame([[0] * len(df.columns)] * pad_t, index=pad_before),
            df,
            pd.DataFrame([[0] * len(df.columns)] * pad_t, index=pad_after),
        ]
    )
