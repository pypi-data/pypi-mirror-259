    ...
    def inner_function(*args, **kwargs):
        try:
            ...
            log.critical(
                ...
                'Provide this log output along with the InnerTune data.'
            )
...
def extract(data: Dict) -> Container:
    ...
    for entry in data_contents:
        # workaround 
        if entry == "content": 
            entry = data_contents[entry]  # type: ignore
...
def _extract_contents(data: Dict) -> Optional[Union[Dict, List]]:
    if ResultStructType.CONTINUATION.value in data:
        key = ResultStructType.CONTINUATION.value
        if "sectionListContinuation" in data[key]:
            contents = data[key]["sectionListContinuation"]["contents"][0]
        else:
            raise UnexpectedState

    elif ResultStructType.SINGLE_COLUMN_BROWSE_RESULTS.value in data["contents"]:
        tmp = data["contents"]["singleColumnBrowseResultsRenderer"][
...
def _extract_item(entry_item: Dict, type: Optional[ItemType] = None) -> Optional[Item]:
    if ShelfStructType.MUSIC_CARD_SHELF.value in entry_item:
        ...
        endpoint = _extract_endpoint(entry_item[key]["title"]["runs"][0]["navigationEndpoint"])
        ...
        match type:
            ...
            case ItemType.ARTIST:
                subscribers = convert_number(entry_item[key]["subtitle"]["runs"][-1]["text"])
                item = ArtistItem(  # type: ignore
                    name=name,
                    endpoint=endpoint,
                    thumbnail_url=thumbnail_url,
        ...
        match type:
            ...
            case ItemType.ARTIST:
                subscribers = convert_number(entry_item[key]["subtitle"]["runs"][0]["text"])
                item = ArtistItem(  # type: ignore
                    name=name,
                    endpoint=endpoint,
                    thumbnail_url=thumbnail_url,
        ...
        length = convert_length(
            entry_item[key]["lengthText"]["runs"][-1]["text"]
        )
        ...
        # In this case it is impossible to determine the type of the items
        ...
        # Podcasts, episodes and videos are collected here.
            ...
            try:
                views = convert_number(entry_item[key]["longBylineText"]["runs"][-3]["text"])
        ...
        item = ArtistItem(  # type: ignore
            ...
            thumbnail_url=entry_item[key]["thumbnail"]["musicThumbnailRenderer"][
                "thumbnail"]["thumbnails"][-1]["url"],
            subscribers=entry_item[key]["subscriptionButton"]["subscribeButtonRenderer"][
        ...
        match type:
            ...
            case ItemType.PLAYLIST:
                views = convert_number(entry_item[key]["secondSubtitle"]["runs"][0]["text"])
                item = PlaylistItem(  # type: ignore
                    name=name,
                    endpoint=None,
                    thumbnail_url=thumbnail_url,
