...
def _extract_contents(data: Dict) -> Optional[Union[Dict, List]]:
    if ContinuationStrucType.CONTINUATION.value in data:
        ...
        if ContinuationStrucType.SECTION_LIST.value in data[key]:
            ...
            # workaround
            contents = data[key][ContinuationStrucType.MUSIC_PLAYLIST_SHELF.value]

        elif ContinuationStrucType.MUSIC_SHELF.value in data[key]:
            contents = data[key][ContinuationStrucType.MUSIC_SHELF.value]["contents"][0]

        else:
            raise UnexpectedState

    elif ResultStructType.TWO_COLUMN_BROWSE_RESULT.value in data["contents"]:
        tmp = data["contents"][ResultStructType.TWO_COLUMN_BROWSE_RESULT.value][
            "secondaryContents"
        ]["sectionListRenderer"]
        ...
        tmp = data["contents"]["tabbedSearchResultsRenderer"]["tabs"][0]["tabRenderer"][
            "content"
        ]["sectionListRenderer"]
        contents = tmp["contents"]
        ...
        # workaround
        contents = data["contents"]["singleColumnMusicWatchNextResultsRenderer"][
            "tabbedRenderer"
...
def _extract_item(entry_item: Dict, type: Optional[ItemType] = None) -> Optional[Item]:
    if ShelfStructType.MUSIC_CARD_SHELF.value in entry_item:
        ...
        | (
            ItemStructType.MUSIC_MULTI_ROW_LIST_ITEM.value in entry_item
        )  # FIXME include this
