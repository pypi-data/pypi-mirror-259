...
class GenericCRUD(metaclass=MetaLogger, object=LOGGER_CRUD_CODE_OBJECT):
    ...
    def upsert(self, *, schema_name: str = None, table_name: str = None, data_json: dict = None,
               data_json_compare: dict = None, order_by: str = None) -> int:
        ...
        table_id = self.select_one_value_by_where(
            schema_name=schema_name, view_table_name=view_name, select_clause_value=id_column_name,
            where=" AND ".join([f"{key}=%s" for key in processed_data_json_compare.keys()]),
            params=tuple(processed_data_json_compare.values()),
            order_by=order_by)
        if table_id:
            self.update_by_id(schema_name=schema_name, table_name=table_name, id_column_name=id_column_name,
