...
class Cli(object):
    ...
    def meta(self, workdir=None, port=9999, build="cr.yandex/crpm0p0gmosuerb6vnrp/metaplatform:latest", auto_git_add=True, eap=False):
        """
        ...
        :param eap: bool. Early access program build. if true, build param rewrite to cr.yandex/crpm0p0gmosuerb6vnrp/metaplatform:eap-latest. By default is false.
        :return:
        """
        if not workdir:
            workdir = self.cwd
        if eap:
            build = "cr.yandex/crpm0p0gmosuerb6vnrp/metaplatform:eap-latest"
