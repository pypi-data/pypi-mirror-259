# This is: https://github.com/micropython/micropython/blob/master/tools/pydfu.py
# 
#!/usr/bin/env python
...
URL_README = 'https://github.com/hmaerki/mpfshell2/blob/master/readme_firmware_update.md'
...
def init():
    ...
    if not devices:
        raise SystemExit(f"ERROR: No pyboards in DFU mode found. Please follow 'Bring pyboard in DFU-mode' on {URL_README}")
    if len(devices) > 1:
        raise SystemExit("Multiple DFU devices found")
    __dev = devices[0]
...
def get_dfu_devices(*args, **kwargs):
    ...
    # ms32_or_64 = 'MS32' if '32 bit' in sys.version else 'MS64'
    # library_path = DIRECTORY_OF_THIS_FILE / 'libusb-1.0.24' / 'VS2017' / ms32_or_64 / 'dll' / 'libusb-1.0.dll'
    # assert library_path.exists()
    # backend = usb.backend.libusb1.get_backend(find_library=lambda x: str(library_path))
    # return list(usb.core.find(*args, find_all=True, custom_match=FilterDFU(), backend=backend, **kwargs))
