...
class PrrteSystem(MCASystemModule):
    ...
    def _set_system_topology(self, topo_graph):
        ...
        # Start PRRTE
        if self.verbosity > 9:
            cmd = "prte --report-pid "+filename+" --daemonize --mca ras timex --mca odls_base_verbose 10 --mca ras_base_verbose 10 --mca pml_base_verbose 10 --host "+hosts
        else:
            cmd = "prte --report-pid "+filename+" --daemonize --mca ras timex --host "+hosts
    ...
    def _finalize_psetop(self, psetop_id):
        ...
        return self._send_setop_cmd(ready_successors)
    ...
    def define_new_psetop(self, id, op, input, op_info):
        ...
        if op == MCASystemModule.PSETOP_CANCEL:
            ...
            cancel_psetop = MCAPSetopModule("new_op", self._pmix_dynrm_convert_psetop(op), in_psets)
        ...
        if None == psetop:        
            psetop = MCAPSetopModule("new_op", self._pmix_dynrm_convert_psetop(op), in_psets)
        ...
        for info in op_info:
            ...
            # Get the monitoring data
                ...
                kvs = info['value'].split(',')
                for kvt in kvs:
                    self._insert_kvt(monitoring_data, kvt)
            ...
            # Update paramters for the input PSet 
                ...
                kvs = info['value'].split(',')
                for kvt in kvs:
                    self._insert_kvt(input_pset_model_params, kvt)
            ...
            # Update paramters for the input PSet 
                ...
                kvs = info['value'].split(',')
                for kvt in kvs:
                    self._insert_kvt(input_pset_model_monitoring[index], kvt)
        ...
        for index in input_pset_model_monitoring.keys():
            pset_model = in_psets[index].run_service("GET", "PSET_MODEL")
            pset_model.run_service("ADD", "MONITORING_ENTRY", time.time(), input_pset_model_monitoring[index])
    ...
    def _send_setop_cmd(self, psetops):
        ...
        for psetop in psetops:
            ...
            if  psetop.run_service("GET", "PSETOP_OP") == MCAPSetopModule.PSETOP_OP_ADD and \
                ...
                v_print("Sending setop cmd with info "+str(info), 5, self.verbosity)
