...
class MCASystemModule(MCAGraphModule):
    ...
    def __init__(self, parent = None, parent_dir = ".", verbosity = 0, enable_output = False):
        ...
        self.register_component(MCALoggerComponent(enable_output = self.enable_output))
        logger_comp = self.get_component(MCALoggerComponent)
        logger_comp.register_module(MCANodeLogger(enable_output = self.enable_output))
        logger_comp.register_module(MCASetOpLogger(enable_output = self.enable_output))
        logger_comp.register_module(MCASetLogger(enable_output = self.enable_output))
        logger_comp.register_module(MCATaskLogger(enable_output = self.enable_output))
