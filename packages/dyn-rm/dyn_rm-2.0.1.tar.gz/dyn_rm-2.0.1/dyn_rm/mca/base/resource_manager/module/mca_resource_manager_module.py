...
class MCAResourceManagerModule(MCAModule, MCAClass):
    ...
    def __init__(self, parent = None, parent_dir = ".", verbosity = 0, enable_output = False):
        ...
        # SYSTEMS
        self.register_component(MCASystemComponent(parent_dir=parent_dir, enable_output = enable_output))

        # POLICIES
        self.register_component(MCAPolicyComponent(parent_dir=parent_dir, enable_output = enable_output))
    ...
    def _psetop_defined_callback(self, conn_name, event_name, system, psetop):
        ...
        #print("PSETOP '"+psetop.run_service("GET", "NAME")+"' DEFINED in system '"+system.run_service("GET", "NAME")+"'")
    ...
    def _task_terminated_callback(self, conn_name, event_name, system, task):
        #print("TASK '"+task.run_service("GET", "NAME")+"' TERMINATED in system '"+system.run_service("GET", "NAME")+"'")

        task_graph = task.run_service("GET", "TASK_GRAPH")
        #if task_graph.run_service("GET", "TASK_STATUS") == MCATaskModule.TASK_STATUS_TERMINATED:
            #print("")
            #print("### TASK GRAPH "+task_graph.run_service("GET", "GID")+" COMPLETED ###")
            #print("")
    ...
    def _object_submitted_callback(self, module, object, params):
        #print("OBJECT_SUBMITTED "+str(object)+" "+str(params))
    ...
    def add_policy(self, name, policy):
        p = policy(parent_dir = self.parent_dir, enable_output = self.enable_output, verbosity=self.verbosity)
    ...
    def add_system(self, name, system_module, topo_creation_module, topology_file):
        system = system_module(parent_dir = self.parent_dir, enable_output = self.enable_output)
