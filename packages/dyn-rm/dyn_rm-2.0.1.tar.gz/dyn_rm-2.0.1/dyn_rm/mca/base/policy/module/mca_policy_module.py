...
class MCAPolicyModule(MCAModule, MCAClass):
    ...
    def __init__(self, parent = None, parent_dir = ".", verbosity = 0, enable_output = False):
        ...
        self.params = dict()
        logger_comp = MCALoggerComponent(parent_dir=parent_dir, verbosity=verbosity, enable_output=enable_output)
        logger_comp.register_module(MCAPolicyLogger(enable_output=enable_output))
    ...
    def eval_policy(self, system):
        if self.enable_output and self.verbosity > 0:
