...
class StructModel:
    def __init__(self, infile, depths=None, verbose=True):
        ...
        self.maxdepth = max(self.residuedepths)
        self.xlscores = {}
        self.avexlscore = 0
        self.avemlscore = 0
        self.depviols = 0
    ...
    def scoremodel(self, xlmslist, verbose=True):
        ...
        for key in self.xlscores:
            self.xlscores[key] /= xlweightbychain[key]
        if verbose:
...
class Crosslink:
    ...
    def probability(self, dist):
        if self.siteA.linker == 'BS3/DSS':    # default situation, BS3 or DSS
            ...
            return logistic_function(dist, 1.07, 0.18, 20.25)
        elif self.siteA.linker == 'PhoX':
            return logistic_function(dist, 1.05, 0.34, 15.50)
        else:
            print ('No information about linker. Using distance distribution for BS3/DSS')
            return 1/(1+np.exp(0.33*dist-7))

    def cutoff(self):
        cutoffs = {'BS3/DSS':33, 'DSBSO':40, 'PhoX':30}
...
class Monolink:
    ...
    def probability(self, depth):
        if self.linker == 'BS3/DSS':
            ...
            tagcoefs = np.array([2583, 5.38])
            lyscoefs = np.array([318, 3.94])
            if func2(depth, *lyscoefs) > 0:
                return func2(depth, *tagcoefs)/func2(depth, *lyscoefs)
            else:
                return 0
        elif self.linker == 'PhoX':
            tagcoefs = np.array([2896, 5.47])
            lyscoefs = np.array([288, 3.88])
            if func2(depth, *lyscoefs) > 0:
                return func2(depth, *tagcoefs)/func2(depth, *lyscoefs)
            else:
                return 0
    ...
    def cutoff(self):
        cutoffs = {'BS3/DSS':15, 'DSBSO':15, 'PhoX':17}
