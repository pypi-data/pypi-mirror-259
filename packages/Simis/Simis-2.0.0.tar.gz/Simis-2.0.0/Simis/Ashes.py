...
import sys
from PySide6.QtCore import QIODevice, QDataStream, QByteArray, QCborValue, QCborMap, QDateTime
...
class Ashes:
    ...
    def __buildModelRecursively(self, cborMap: QCborMap, model: Model) -> None:
        ...
        for key in cborMap.keys():
            ...
            if cborMap.value(key).isMap():
                ...
                # For now only considder this flag. might need other solution for more bools.
                # Needs more research
                if key.toString() in ["keep", "init"]:
                    model.__setattr__(modified_key, cborMap.value(key).toBool())
                elif key.toString() in ["user_defined_script_input"]:
                    model.__setattr__(modified_key, cborMap.value(key).toString())
                else:
    ...
    def __callCallbacks(self, callbackFunction: Callable[[Model], Model], data: QDataStream) -> QCborMap:
        ...
        hasInitKey = False
        if cborMap.contains("init"):
            hasInitKey = True
        initKeyIsTrue = cborMap.value("init").toBool()
        # Build a model from the data
        if hasInitKey and initKeyIsTrue:
            self.model = Model()
