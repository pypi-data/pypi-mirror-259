"""
Provides a `TargetWriter` class, for buffering data and writing to arbitrary streams, usually specified by a (text) CLI parameter.
"""
# !EXPORT_TO_README
import os
import sys
...
class PagerWriter( BufferedWriter ):
    def __init__( self, *args, **kwargs ):
        super().__init__( *args, **kwargs )
        ...
        try:
            ...
            raise ValueError( "Cannot use ClipboardWriter without first installing the `pydoc` package." ) from ex
...
TTarget = Union[str, Callable[[], WriterBase], None]
...
class TargetWriter:
    ...
    default_map: TOpenWriteOpts = { __fmt( "stdout" ): StdOutWriter,
                                    ...
                                    __fmt( "pager" ) : PagerWriter,
                                    ""               : TerminalWriter }
    stdout_key = __fmt( "stdout" )
    ...
    def write_all_text( self, file_name: TTarget, content: str ) -> None:
        with self.open( file_name, ".txt", str ) as fout:
            fout.write( content )
    ...
    def open( self, file_name: TTarget, extension: str = "", mode: type = str ) -> Union[WriterBase, TextIO, BinaryIO]:
        """
        Opens a file - see class documentation for details.
        
        :param file_name:     Name of the file *or* the name of one of the
                              special handlers listed in `map`:arg:.
                              `None` is treated the same as `""`.
                              
                              The class/factory function for a handler may also
                              be provided.
        ...
        if intermediate:
            ...
            open_fn = partial( open, encoding = "utf8" )
        else:
            open_fn = open
