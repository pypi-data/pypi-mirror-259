...
class TestClass:
    ...
    def test_Instantiation(self):
        outliers = self.sample_columns_to_test_p_col.method_IQR()
        outliers = self.sample_columns_to_test_p_col.method_IQR(distance = 2.7)
        outliers = self.sample_columns_to_test_p_col.method_IQR(threshold_included=True)
        outliers = self.sample_columns_to_test_p_col.method_IQR(distance=2.4, threshold_included=True)
    ...
    def test_threshold_value(self):
        threshold = {'art_looking_time': (610.497, 3097.789), 'discrimination_performance': (0.697, 1.085)}
    ...
    def test_manage_method(self):
        ...
        assert(np.isnan(df2.loc["P11", "art_looking_time"]))
