...
class StickProtocol():
    ...
    def __init__(self, UserModel, DeviceModel, GroupModel, session_life_cycle, CustomParty=None):
        self.User = UserModel
        self.Device = DeviceModel
        self.Group = GroupModel
        self.session_life_cycle = session_life_cycle
        self.Party = CustomParty if CustomParty else Party
    ...
    def process_pre_key_bundle(self, data, user):
        ...
        self.Party.objects.create(user=user)
        self.Party.objects.create(user=user, individual=True)
    ...
    def get_sender_key(self, data, user):
        ...
        if is_invitation:  # An invited user is authorized
            ...
            if group:
                ...
                party = self.Party.objects.filter(id=stick_id[:36]).first()
                if not party:
                    return {'party_exists': False}
    ...
    def get_stick_id(self, data, user):
        ...
        if not is_profile and 'party_id' not in data:
            if len(groups_ids) == 1 and len(connections_ids) == 0:  # Sharing with a single group
                ...
                party = self.Party.objects.filter(party_hash=party_hash).first()
                if party == None:  # Create a new Party object if does not exist
                    party = self.Party.objects.create(party_hash=party_hash)
            ...
            members_ids = [user.id]
            if data['type'] != 'group':
                party = self.Party.objects.get(id=data['party_id'])
                if party.user:
            ...
            party_id = self.Party.objects.get(user=user, individual=is_individual).id
        dict = {}
        bundles_to_fetch = []
        response_dict = {}
