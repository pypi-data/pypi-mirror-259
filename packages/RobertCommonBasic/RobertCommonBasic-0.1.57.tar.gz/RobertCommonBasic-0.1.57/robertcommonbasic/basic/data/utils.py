...
def format_value(value_old: str, scale: str = '1', decimal: int = 2, revert: bool = False, replaces: dict = {'on': 1, 'true': 1, 'active': 1, 'off': 0, 'false': 0, 'inactive': 0}) -> str:
    ...
    try:
                    ...
                    try:
                        if is_scale_num is True:
                            ...
                            if revert is True:
                                ...
                                # 添加自定义函数
                                g_s.functions.update(convert=convert_value, max=max, min=min, sum=sum, _round=_round, _zero=_zero, bit=bit, _in=_in, signed=signed, inverse_long=inverse_long, inverse_float=inverse_float, inverse_double=inverse_double, _and=_and, _or=_or)
                                g_s.names = {'v': value}
        ...
        logging.error(f"format_value({value_old} - {scale})-({e.__str__()})")
    return str(value)
