...
functions.update(convert=convert_value, max=max, min=min, sum=sum, _round=_round, _zero=_zero, bit=bit, _in=_in, signed=signed, inverse_long=inverse_long, inverse_float=inverse_float, inverse_double=inverse_double, _and=_and, _or=_or)


def eval_express(value: Any, scale: Any, revert: bool = False, fail_retry: bool = True):
    """计算表达式"""
    value_old = value
    scale_old = scale
    try:
    ...
    try:
        ...
        if is_value_num is True:
            if -Decimal(str(value)).as_tuple().exponent > decimal:
                _format = f"%.{decimal}f"
                return remove_exponent_str(str(_format % value))    # 格式化小数点
    except Exception as e:
        raise Exception(f"invalid decimal({e.__str__()})")
    return value
