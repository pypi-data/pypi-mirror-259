...
class AsyncLedger(BaseModel):
    ...
    async def subtract(self, user_id: UUID, amount: int) -> None:
        ...
        active_balance_grouped = await self.storage.get_active_balance_grouped_by_expire_at(user_id)
        # Sum the active balance for the user and check if it is greater than the amount
        active_balance = sum([entry.sum for entry in active_balance_grouped]) #entry.sum is the sum of the amount column
