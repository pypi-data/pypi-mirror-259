...
def hf_save_as_mlflow_model(component_args: Namespace,
    ...
    vision_model = transformers.pipeline(
        task="image-classification", model=model.cpu(), image_processor=image_processor
    )
    image_url = \
        "https://automlcesdkdataresources.blob.core.windows.net/finetuning-image-models/images/multiclass/can/1.jpg"
    signature = infer_signature(
        image_url, generate_signature_output(vision_model, image_url),
    )
