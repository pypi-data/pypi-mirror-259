...
class baseml:
    ...
    def valid(self, path=None, x=None ,y=None ,metrics='accuracy'):
        ...
        from sklearn.metrics import accuracy_score,precision_score,recall_score,f1_score,r2_score,mean_squared_error,mean_absolute_error,auc
        if metrics == 'accuracy' or metrics=='acc':
            acc = accuracy_score(y, y_pred)
            print('验证准确率为：{}%'.format(acc * 100))
        elif metrics == 'precision':
            acc = precision_score(y, y_pred,average='weighted')
            print('验证精确率为：{}%'.format(acc * 100))

        elif metrics =='recall':
            acc = recall_score(y, y_pred,average='weighted')
            print('验证召回率为：{}%'.format(acc * 100))

        elif metrics == 'f1':
            acc = f1_score(y, y_pred,average='weighted')
            print('验证f1-score为：{}%'.format(acc * 100))
        
        elif metrics == 'auc':
            acc = auc(y, y_pred)
            print('验证auc为：{}%'.format(acc * 100))
            ...
            print('验证r2-score为：{}%'.format(acc * 100))
            
        elif metrics =='mse':
            acc = mean_squared_error(y, y_pred)
            print('验证均方误差为：{}%'.format(acc * 100))

        elif metrics =='mae':
            acc = mean_absolute_error(y, y_pred)
            print('验证平均绝对误差为：{}%'.format(acc * 100))
