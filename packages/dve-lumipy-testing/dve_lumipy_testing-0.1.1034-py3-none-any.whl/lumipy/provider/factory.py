...
from lumipy.provider.common import get_dll_path, get_latest_local_semver, get_certs_path, find_dotnet_sdks, semver_has_dll
...
def ensure_directory(
    ...
    get_binaries = sdk_version is None or not semver_has_dll(sdk_version)
    ...
    sdk_version = get_latest_local_semver()
    if sdk_version is None or not semver_has_dll(sdk_version):
...
class Factory:
    ...
    def __print_process_output(self):
        ...
        while self.process.poll() is None:
            ...
            output_errors = self.process.stderr.readline().decode('utf-8', errors="replace").rstrip()
