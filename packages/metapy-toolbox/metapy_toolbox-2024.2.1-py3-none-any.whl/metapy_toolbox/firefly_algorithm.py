...
def gamma_parameter(x_lower, x_upper, n_dimension, m=2):
    ...
    Args:
        ...
        m (int): Light absorption factor. Default is 2
...
def discriminant_factor(fit_male, fit_female):
    """
    Calculation of the discriminating factor 
    of the male and female fireflies population

    Args:
        fit_male (float): Fitness of the i firefly
        fit_female (float): Fitness of the k firefly
    
    Returns:
        d_1 (int): Discriminating factor
    """
...
def male_movement(obj_function, beta_0, gamma, x_i_old, fit_i_old, y_j_old,\
                    fit_j_old, y_k_old, fit_k_old, n_dimensions, x_lower,\
                    x_upper, none_variable=None):
    """
    ...
    Args:
        ...
        beta_0 (float): Attractiveness at r = 0
        gamma (list): Light absorption coefficient  1 / (X_U - X_L) ** M
...
def female_movement(obj_function, beta_0, gamma, x_i_old_best, y_j_old,\
                        n_dimensions, x_lower, x_upper,\
                        none_variable=None):
    """
    ...
    Args:
        ...
        beta_0 (float): Attractiveness at r = 0
        gamma (list): Light absorption coefficient  1 / (X_U - X_L) ** M
        x_i_old_best (list): Design variables i (
...
def gender_firefly_algorithm_01(settings):
    """
    ...
    beta_0 = algorithm_parameters['beta zero']
    gamma = algorithm_parameters['gamma']
    n_pop_female = algorithm_parameters['number of females']
    ...
    # Mutation control
    type_mut = algorithm_parameters['type mutation']
    if type_mut == 'chaotic map 01':
        n_tries = algorithm_parameters['number of tries']
        alpha = algorithm_parameters['alpha']
        ch = algorithm_parameters['first value chaotic map']
    elif type_mut == 'hill climbing':
        std = algorithm_parameters['cov (%)']
    ...
    for iter in range(n_iterations):
        ...
        if type_mut == 'chaotic map 01':
            report_move += "    Chaotic Map 01\n"
            x_i_temp, of_i_temp,\
                fit_i_temp, neof,\
                ch, report_mov = metapyco.mutation_02_chaos_movement(obj_function, x_best, x_lower, x_upper,
                                                                n_dimensions, ch, alpha, n_tries, iter, n_iterations,
                                                                none_variable=none_variable)
        elif type_mut == 'hill climbing':
            report_move += "    Hill Climbing\n"
            x_i_temp, of_i_temp,\
                fit_i_temp, neof,\
...
def firefly_algorithm_01(settings):
    """
    Gender firefly algorithm.
    ...
    for iter in range(n_iterations):
        ...
        for pop in range(n_population):
            report += f"Pop id: {pop} - particle movement - male procedure\n"
            pos = [int(i) for i in range(n_pop_female)]
            report += f"    current x = {x_male_pop[pop]}, of = {of_male_pop[pop]}, fit = {fit_male_pop[pop]}\n"
            id_y_j, id_y_k = np.random.choice(pos, 2, replace = False)
            report += f"    selected female id y_j = {id_y_j}, id y_k = {id_y_k}\n"
            x_i_temp, of_i_temp,\
                fit_i_temp, neof,\
                report_mov = male_movement(obj_function,
                                            beta_0,
        ...
        for tr in range(n_tries):
            report += f"Pop id best: {id_best} - try: {tr} - particle movement - mutation procedure\n"
            report += f"    best = {x_best}, of = {of_best}, fit = {fit_best}\n"
            if type_mut == 'chaotic map 01':
                x_i_temp, of_i_temp,\
                    fit_i_temp, neof,\
                    ch, report_mov = metapyco.mutation_02_chaos_movement(obj_function, x_best, x_lower, x_upper,
                                                                    n_dimensions, ch, alpha, iter, n_iterations,
                                                                    none_variable=none_variable)
            elif type_mut == 'hill climbing':
                x_i_temp, of_i_temp,\
                    fit_i_temp, neof,\
                    report_mov = metapyco.mutation_01_movement(obj_function,
                                                                x_best,
                                                                x_lower, x_upper,
                                                                n_dimensions,
                                                                pdf, std,
