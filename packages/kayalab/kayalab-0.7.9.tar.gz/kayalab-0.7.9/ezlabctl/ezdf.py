...
from ezlab.parameters import DF, INSTALL_COMMANDS
...
def cluster(
    ...
    commands.insert(
        0,
        f"[ -f /tmp/mapr-setup.sh ] || wget -q {user_str} {token_str} {ez_repo}/installer/redhat/mapr-setup.sh -P /tmp; chmod +x /tmp/mapr-setup.sh",
    )
    commands.insert(1, f"curl -s -k https://127.0.0.1:9443/ > /dev/null || sudo /tmp/mapr-setup.sh -y -r {ez_repo}")
    ...
    print(
        f"echo Check installation status at https://{first_host}:8443/"
    )
