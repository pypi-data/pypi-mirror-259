...
from ezlab.utils import fire_and_forget, ezapp
from ezinfra import pve
from ezinfra import kvm
...
def prepare(
    ...
    if dryrun:
        response["task"] = (
            f"Prepare {hostname} ({hostip}) for {prepare_for} {'with hosts: ' + ','.join([h['ip'] for h in addhosts]) if len(addhosts) > 0 else 'as single node'}"
        )
    ...
    if dryrun:
        ...
        try:
            for filepath, content in files.items():
                ...
                ezapp.queue.put(f"[ {hostip} ] COPY {filepath}")
                for result in ssh_content_into_file(
                    ...
                    ezapp.queue.put(f"[ {hostip} ] COPYOUT: {result}")
...
def clone(
    ...
    if target == PVE:
        ...
        ezapp.queue.put("Unknown target")
        return False
