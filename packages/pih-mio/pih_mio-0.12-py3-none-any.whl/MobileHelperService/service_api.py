...
class MobileHelperService:
    ...
    @staticmethod
    def as_admin() -> bool:
        return MobileHelperService.is_admin or not A.D.contains(
            A.SYS.host(), SD.host
        )
            ...
            try:
                ...
                if sender in MobileHelperService.mobile_helper_client_map:
                    ...
                    if mobile_helper.do_pih(
                        line, sender_user, external_flags, return_result_key, args
                    ):
            ...
            MIO.create_output(A.D.get(A.CT_ME_WH.GROUP.PIH_CLI)).write_line(
                j(
                    (
                        ...
                        nl() * 2,
                        get_wappi_status(
                            space, profile.CALL_CENTRE
                        ),
                        nl() * 2,
                        get_wappi_status(
                            space, profile.MARKETER
                        ),
                    )
                )
            )
