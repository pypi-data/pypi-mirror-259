...
class MobileHelper(OutputStub):
        ...
        marketer_unit_node: CommandNode = CommandNode(
            ...
            text=lambda: j(
                (
                    ...
                    ".",
                    nl() * 2,
                    A.D_F.wappi_status(A.CT_ME.WHATSAPP.WAPPI.Profiles.MARKETER, self.output),
                )
            ),
        )
        self.menu.unit = [
    ...
    def check_email_address_handler(
        ...
        try:
            ...
            if not only_for_polibase_person:
                if ne(value):
                    if A.C.email(value):  # type: ignore
                        result = A.C.EMAIL.accessability(value, not self.is_forced)  # type: ignore
                        self.output.separated_line()
                        text: str = js(
                            (
    ...
    def check_resources_and_indications_handler(
        ...
        additional_text: dict[CheckableSections, str] = defaultdict(str)
        with self.output.make_instant_mode():
            if CheckableSections.POLIBASE in section_list:
                with self.output.make_redirection():
                    self.run_command(
                        A.CT_CMDT.POLIBASE,
                        one(A.R_F.find("doctemplets statistics")).text,
                    )
                    additional_text[CheckableSections.POLIBASE] = jnl(
                        self.output.redirected_text_list
                    )
    ...
    def reboot_workstation_handler(self, all: bool = False) -> None:
        ...
        self.workstation_action_handler(
            (
                name
                if nn(name)
                and A.C_WS.name(name)
                and A.C_R.accessibility_by_smb_port(name)
                else None
            ),
            0,
            all,
    ...
    def find_note_handler(self, strict_equality: bool = False) -> None:
        value: str | None = self.arg()
            ...
            if e(gkeep_item_list_result):
                ...
                for gkeep_item in gkeep_item_list:
                    with self.output.make_loading(1, "Идет загрузка заметки. Ожидайте"):
                        note_result: Result[Note] = A.R_N.get_by_id(gkeep_item.id)
