...
from postlanganalytics.request import APIError
from postlanganalytics.test.test_utils import TEST_API_KEY, TEST_HOST
...
class TestClient(unittest.TestCase):
    ...
    def setUp(self):
        self.failed = False
        self.client = Client(TEST_API_KEY, host=TEST_HOST, on_error=self.set_fail)
    ...
    def test_basic_capture(self):
        ...
        self.assertEqual(msg["properties"]["$lib"], "postlang-python")
        self.assertEqual(msg["properties"]["$lib_version"], VERSION)

    def test_basic_capture_with_project_api_key(self):

        client = Client(project_api_key=TEST_API_KEY, host=TEST_HOST, on_error=self.set_fail)
    ...
    def test_groups_capture(self):
        success, msg = self.client.capture(
            "distinct_id",
            "test_event",
            groups={"company": "id:5", "instance": "127.0.0.1:8000"},
        )

        self.assertTrue(success)
        self.assertEqual(msg["properties"]["$groups"], {"company": "id:5", "instance": "127.0.0.1:8000"})
    ...
    def test_synchronous(self):
        client = Client(TEST_API_KEY, host=TEST_HOST, sync_mode=True)
    ...
    def test_overflow(self):
        client = Client(TEST_API_KEY, host=TEST_HOST, max_queue_size=1)
    ...
    def test_unicode(self):
        Client(six.u("unicode_key"), host=TEST_HOST)
    ...
    def test_debug(self):
        Client("bad_key", host=TEST_HOST, debug=True)

    def test_gzip(self):
        client = Client(TEST_API_KEY, host=TEST_HOST, on_error=self.fail, gzip=True)
        for _ in range(10):
    ...
    def test_user_defined_flush_at(self):
        client = Client(TEST_API_KEY, host=TEST_HOST, on_error=self.fail, flush_at=10, flush_interval=3)
        ...
        # each time.
        with mock.patch("postlang.consumer.batch_post", side_effect=mock_post_fn) as mock_post:
            for _ in range(20):
                client.identify("distinct_id", {"trait": "value"})
            time.sleep(1)
    ...
    def test_user_defined_timeout(self):
        client = Client(TEST_API_KEY, host=TEST_HOST, timeout=10)
        for consumer in client.consumers:
            self.assertEqual(consumer.timeout, 10)

    def test_default_timeout_15(self):
        client = Client(TEST_API_KEY, host=TEST_HOST)
        for consumer in client.consumers:
            self.assertEqual(consumer.timeout, 15)

    @mock.patch("postlang.client.Poller")
    @mock.patch("postlang.client.get")
    def test_load_feature_flags(self, patch_get, patch_poll):
        ...
        client = Client(TEST_API_KEY, host=TEST_HOST, personal_api_key="test")
    ...
    def test_load_feature_flags_wrong_key(self):
        client = Client(TEST_API_KEY, host=TEST_HOST, personal_api_key=TEST_API_KEY)
        with freeze_time("2020-01-01T12:01:00.0000Z"):
    ...
    @mock.patch("postlang.client.decide")
