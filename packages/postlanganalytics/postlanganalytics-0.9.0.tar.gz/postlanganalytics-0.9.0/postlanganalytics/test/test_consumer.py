...
from postlanganalytics.consumer import MAX_MSG_SIZE, Consumer
...
class TestConsumer(unittest.TestCase):
    ...
    def test_upload(self):
        q = Queue()
        consumer = Consumer(q, TEST_API_KEY, host=TEST_HOST)
    ...
    def test_flush_interval(self):
        ...
        q = Queue()
        flush_interval = 0.3
        consumer = Consumer(q, TEST_API_KEY, host=TEST_HOST, flush_at=10, flush_interval=flush_interval)
        with mock.patch("postlang.consumer.batch_post") as mock_post:
            consumer.start()
            for i in range(0, 3):
    ...
    def test_multiple_uploads_per_interval(self):
        ...
        flush_interval = 0.5
        flush_at = 10
        consumer = Consumer(q, TEST_API_KEY, host=TEST_HOST, flush_at=flush_at, flush_interval=flush_interval)
        with mock.patch("postlang.consumer.batch_post") as mock_post:
            consumer.start()
    ...
    def test_request(self):
        consumer = Consumer(None, TEST_API_KEY, host=TEST_HOST)
        track = {"type": "track", "event": "python event", "distinct_id": "distinct_id"}
    ...
    def _test_request_retry(self, consumer, expected_exception, exception_count):
        ...
        with mock.patch("postlang.consumer.batch_post", mock.Mock(side_effect=mock_post)):
            track = {"type": "track", "event": "python event", "distinct_id": "distinct_id"}
    ...
    def test_request_retry(self):
        ...
        consumer = Consumer(None, TEST_API_KEY, host=TEST_HOST, retries=3)
    ...
    def test_max_batch_size(self):
        q = Queue()
        consumer = Consumer(q, TEST_API_KEY, host=TEST_HOST, flush_at=100000, flush_interval=3)
        properties = {}
        ...
        with mock.patch("postlang.request._session.post", side_effect=mock_post_fn) as mock_post:
            consumer.start()
