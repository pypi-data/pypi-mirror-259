...
from postlanganalytics.utils import remove_trailing_slash
from postlanganalytics.version import VERSION

_session = requests.sessions.Session()

USER_AGENT = "postlang-python/" + VERSION
...
def post(
    ...
    url = remove_trailing_slash(host) + path
...
def _process_response(
    ...
    if not res:
        raise APIError(
            "N/A",
            "Error when fetching postlang API, please make sure you are using your public project token/key and not a private API key.",
        )
    if res.status_code == 200:
...
def get(api_key: str, url: str, host: Optional[str] = None, timeout: Optional[int] = None) -> requests.Response:
    url = remove_trailing_slash(host) + url
