  ...
  <dynamic-pagination :endpoint="endpoint"
    ...
    <template #default="paginationProps">
      <div class="row align-items-center">
        ...
        <div class="col-md-6 col-xl-4 mb-3 mb-md-2 d-flex justify-content-end"
             v-if="paginationProps.totalUnfiltered > 0">
          <div class="flex-grow-1 mr-2" v-if="enableSort && paginationProps.total > 1">
            <div class="input-group input-group-sm">
              <div class="input-group-prepend">
                <label class="input-group-text" :for="`sort-${id}`">{{ $t('Sort by') }}</label>
              </div>
              <select class="custom-select" :id="`sort-${id}`" v-model="sort">
          ...
          <div v-if="paginationProps.total > 0">
            <button class="btn btn-sm btn-light" :title="$t('Change layout')" @click="changeView">
              <i class="fa-solid fa-table" v-if="listView"></i>
              <i class="fa-solid fa-list-ul" v-else></i>
            </button>
          </div>
        </div>
      </div>
      <ul class="list-group" v-if="listView">
        <li class="list-group-item py-1" v-for="item in paginationProps.items" :key="item.id">
          <a class="text-default stretched-link" :href="item._links.view">
            <div class="form-row">
              <div class="col-md-4" v-trim-ws>
                <small>
                  <resource-visibility :visibility="item.visibility"></resource-visibility>
                </small>
                <strong class="wb-break-word ml-2">{{ item.title }}</strong>
                <resource-type class="d-inline d-md-none badge-mt-plus float-right ml-3"
                               :type="item.type"
              ...
              <div class="col-md-2 d-flex justify-content-end">
                <span>
                  <resource-type class="d-none d-md-inline" :type="item.type" :is-template="Boolean(item.data)">
                  </resource-type>
                </span>
              </div>
            </div>
      ...
      <card-deck :items="paginationProps.items" v-else>
...
export default {
  ...
  methods: {
    changeView() {
      ...
      // Dispatch a custom 'resourceviewlayout' event from the window as well in order to synchronize layout changes
      // between multiple components.
      window.dispatchEvent(new CustomEvent('resourceviewlayout', {detail: {id: this.id, listView: this.listView}}));
  ...
  mounted() {
    ...
    window.addEventListener('resourceviewlayout', (e) => {
      if (this.id !== e.detail.id) {
        this.listView = e.detail.listView;
      }
    });
  },
};
