...
@qparam(
    ...
    " :func:`kadi.modules.records.utils.search_records` for a more detailed description"
    " (in Python syntax).",
)
@qparam(
    "sort",
...
@qparam(
    ...
    description="The order of the returned record links. One of ``last_modified``,"
    " ``-last_modified``, ``created_at``, ``-created_at``, ``name`` or ``-name``.",
)
@qparam(
    "action",
    multiple=True,
...
@qparam(
    ...
    " ``-last_modified``, ``created_at``, ``-created_at``, ``name``, ``-name``,"
    " ``size`` or ``-size``.",
)
...
def get_uploads(id):
    ...
    Only uploads owned by the current user will be returned. For chunked uploads, the
    required size for uploading chunks (except for the last chunk) is returned as the
    ``_meta.chunk_size`` property of each upload.
    """
    record = Record.query.get_active_or_404(id)
    uploads = record.uploads.filter(
        Upload.user_id == current_user.id, Upload.state != UploadState.INACTIVE
    ).order_by(Upload.created_at)

    data = {
        "items": UploadSchema(many=True).dump(uploads),
        # For backwards compatibility.
        "_meta": {"chunk_size": const.UPLOAD_CHUNK_SIZE},
    }
...
def get_upload(record_id, upload_id):
    ...
    Will return the upload specified by the given *upload_id* of the record specified by
    the given *record_id*. Only uploads owned by the current user can be retrieved.
    ...
    ``_meta.task`` property, and includes its ``state`` and, for uploads that have
    finished processing, either the newly created ``file`` when the processing task
    finished successfully or an ``error`` message.
    """
    record = Record.query.get_active_or_404(record_id)
