...
class S3StorageBackend(StorageBackend):
    ...
    def list_dir(self, path: str) -> list[str]:
        ...
        return [f"s3://{obj.bucket_name}/{obj.object_name}" for obj in objects]
    ...
    def dataset_artifact_md5hash(
        ...
        response = None
        try:
            response = client.get_object(bucket_name, f"{dataset_id}/{artifact_path}")
            if response is None:
                raise ValueError(
