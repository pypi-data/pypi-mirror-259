...
from common import SpotData


class WxPush:
    def __init__(self, redis_clt: aioredis.Redis):
        self.redis_clt: aioredis.Redis = redis_clt

    async def send_wx_message(self, title, message, openid_list=None):
    ...
    async def send_price_wx(self, openid_list, spot_data: SpotData):
        ...
        for openid in openid_list:
            ...
            pre_detail_md5 = await self.redis_clt.getset(key_md5, detail_md5)
            await self.redis_clt.expire(key_md5, 24 * 60 * 60)
            if pre_detail_md5 != detail_md5:
                await self.send_wx(openid, data, 'pto-35_QZ_IfDfu1-mwKS7jJWa1GrFZtCpm61Px_4X0')
