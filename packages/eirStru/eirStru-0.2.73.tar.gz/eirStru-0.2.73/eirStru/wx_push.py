...
class WxPush:
    def __init__(self, host, port, password, db):
        self.r_pool = aioredis.ConnectionPool(
            host=host,
            password=password,
            port=port,
            db=db,
            decode_responses=True)
        self.redis_clt = aioredis.Redis(connection_pool=self.r_pool)
    ...
    async def send_price_wx(self, spot_data: SpotData, openid_list=None):
