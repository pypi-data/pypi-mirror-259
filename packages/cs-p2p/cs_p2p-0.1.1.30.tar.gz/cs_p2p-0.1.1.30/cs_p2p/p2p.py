...
def __publisher(channel, text):
    ...
    messaggio_parte1 = b"zero"
    messaggio_parte2 = b"Messaggio dal pubblicatore"
    
    socket.send_multipart([messaggio_parte1, messaggio_parte2])
...
def __subscriber(channel, ip_address):
    ...
    canale = b"zero"
    socket.setsockopt(zmq.SUBSCRIBE, canale)

    # socket.setsockopt_string(zmq.SUBSCRIBE, channel)  # Sottoscrivi a tutti i messaggi
   
    while True:
        messaggio = socket.recv_multipart()
        canale_ricevuto = messaggio[0]
        testo = messaggio[1]
