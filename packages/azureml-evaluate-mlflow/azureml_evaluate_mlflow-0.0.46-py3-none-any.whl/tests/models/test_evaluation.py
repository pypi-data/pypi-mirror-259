...
from azureml.evaluate.mlflow.models.evaluation import (
    evaluate
)
...
from azureml.evaluate.mlflow.models.evaluation.base import _gen_md5_for_arraylike_obj
...
def test_classifier_evaluate(multiclass_logistic_regressor_model_uri, iris_dataset):
    ...
    # confusion_matrix_artifact = eval_result.artifacts["confusion_matrix"]
...
def test_evaluate_with_multi_evaluators(multiclass_logistic_regressor_model_uri, iris_dataset):
    with mock.patch.object(
        ...
        for evaluators in [["test_evaluator1", "test_evaluator2"]]:
            with mock.patch.object(
                    FakeEvauator1, "can_evaluate", return_value=True
...
def test_normalize_evaluators_and_evaluator_config_args():
    ...
    with mock.patch.object(
            ...
            {"default": DefaultEvaluator,
             "azureml": AzureMLEvaluator},
    ):
        assert _normalize_config(None, None) == (["default", "azureml"], {})
        assert _normalize_config(None, {"default": {"a": 3}, "azureml": {"a": 3}}) == (
            ["default", "azureml"], {"default": {"a": 3}, "azureml": {"a": 3}})
        assert _normalize_config(["default"], {"default": {"a": 3}}) == (
            ["default"],
            {"default": {"a": 3}},
        )
