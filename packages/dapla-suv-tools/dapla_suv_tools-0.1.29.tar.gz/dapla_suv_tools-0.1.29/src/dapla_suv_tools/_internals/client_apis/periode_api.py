...
def get_periode_by_id(self, *, periode_id: int, context: SuvOperationContext = None) -> OperationResult:
    ...
    """
    try:
        content: str = client.get(path=f"{constants.PERIODE_PATH}/{periode_id}", context=context)
...
def get_perioder_by_skjema_id(self, *, skjema_id: int, context: SuvOperationContext = None) -> OperationResult:
    try:
        content: str = client.get(path=f"{constants.PERIODE_PATH}/skjema/{skjema_id}", context=context)
        content_json = json.loads(content)
...
def create_periode(
    ...
    try:
        ...
        content: str = client.post(path=constants.PERIODE_PATH, body_json=body, context=context)
...
def delete_periode(self, *, periode_id: int, context: SuvOperationContext = None) -> OperationResult:
    try:
        content: str = client.delete(path=f"{constants.PERIODE_PATH}/{periode_id}", context=context)
