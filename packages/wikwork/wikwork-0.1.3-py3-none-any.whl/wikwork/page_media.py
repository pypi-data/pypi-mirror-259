...
class AudioFile:
    ...
    wikitext : str, optional
        ...
        never requested.
    prob_author : str
        Probable author. Derived from parsing the wikitext with regular
        expressions. Will search for a match using this regular expression:
           r'[Aa]uthor=\[\[.*?\]\]|[Aa]uthor=..*\||[Aa]uthor=..*\}\}'
        and then removing 'Author=' or 'author=' from the start of the
        returned string. Users should compare this with the `wikitext`
        output to confirm correct extraction before using.
    prob_license : str
        Probable license. Derived from parsing the wikitext with the
        following regular expression list (joined with a '|' delimiter)
        and then enclosed in parentheses. Users should compare this with
        the `wikitext` attribute to confirm correct extraction before
        using. The regular expression list is:
    ...
    request_status : str
        Contains information whether the `filename` and `revisioni` are in
    ...
    def __init__(self, filename: str, headword: str, headword_lang_code: str,
        ...
        self.html_etag: str = ''
        self.wikitext: str = ''
        self.prob_license: str = ''
        self.prob_author: str = ''
    ...
    def __repr__(self) -> str:
        return(f'AudioFile(filename="{self.filename}"'
            ...
            f'    prob_license="{self.prob_license}",'
            f'    prob_author="{self.prob_author}",'
    ...
    def str_wikitext_etal(self) -> str:
        """Create a string with prob_license, prob_author, wikitext, etc...
        ...
        Returns
        -------
        String. First line is '-' repeated 79 times, then the attributes
        with the attribute name and value, followed by newline. So the
        first line is 'filename: {filename}'. This is done for `filename`,
        `headword`, `headword_lang_code`, `prob_license`, `prob_author`,
        and `wikitext`.  For readability, additional newlines are added
        after the `headword_lang_code` value, the `prob_author` value, and
        the label for `wikitext` (so that the `wikitext` value starts on a
        new line).
...
class Headword:
    ...
    def fetch_word_page(self, io_options: io_opts.IOOptions) -> None:
        ...
        if self.revision>0:
            ...
            if self.html:
                _html_to_wikitext(self, self.lang_code, io_options=io_options)
    ...
    def fetch_audio_info(self, io_options: io_opts.IOOptions,
        ...
        parse_wikitext_fn : Callable([AudioFile], None), optional
            ...
            default parser that sets the `prob_license` and `prob_author`
            attributes of each `AudioFile` object. The default uses
            regular expressions (possibly imperfectly) to determine the
            probable license and probable author of the audio file from
            the wikitext of the file-namespace file.  If passed, this
            should set the `prob_license` and `prob_author` attributes of
            the `AudioFile` object it is called on.
        ...
        media_list = self._get_media_list(io_options=io_options)

        try:
            media_list_items = media_list['items']
        except KeyError:
            logging.error(("Lang %s: Word %s: 'items' not found "
        ...
        for media_list_position, item in enumerate(media_list_items):
            if item['type'] == 'audio':
                if _should_audio_item_be_skipped(item, pattern_str):
                    continue
                audio_file = AudioFile(filename = item['title'].split(':')[1],
        ...
        for aud_file in self.audio_files:
            ...
            if not valid_audio_html_lang_code:
                logging.error('fetch_audio_info: Lang %s: Word %s: '
            ...
            _html_to_wikitext(aud_file, lang_code=audio_html_lang_code,
                           io_options=io_options)
            if parse_wikitext_fn is None:
                aud_file._parse_wikitext_regex()  # don't want to make public, so pylint: disable=protected-access
            else :
    ...
    # Private methods
    #-------------------
    def _get_media_list(self, io_options: io_opts.IOOptions) -> MediaList:
...
def _fetch_current_revision(self: Union[Headword, AudioFile],
    ...
    if ( (io_options.cache_mode is io_opts.CacheMode.READ_AND_WRITE
        ...
        url = (f'{base_url}/page/title/'
               f'{_quote_url(self.title_uncoded)}')
        response = requests.get(url, headers=io_options.headers,
                                timeout=io_options.timeout)
    ...
    if 'title' in resp_json and resp_json['title'] == 'Not found.':
        self.status_msg = 'Page not found'
        self.revision = 0
        logger.warning('File %s: page not found', self.title_uncoded)
    else:
        try:
            self.revision = resp_json['items'][0]['rev']
        except KeyError:
            self.status_msg = 'Revision not in response (bad key)'
            logger.error('Lang %s, %s %s: revision not in response (bad key)',
                lang_code, self.desc, self.title_uncoded)
            self.revision = 0
        except TypeError:
            self.status_msg = 'Revision not in response (type error)'
            logger.error(('Lang %s, %s %s: revision not in response '
                          '(type error)'),
                lang_code, self.desc, self.title_uncoded)
            self.revision = 0

def _html_to_wikitext(self: Union[Headword, AudioFile],
                      lang_code: str,
                      io_options: io_opts.IOOptions) -> None:
    ...
    if ( (io_options.cache_mode is io_opts.CacheMode.READ_AND_WRITE
        ...
        url = (f'{base_url}/transform/html/to/wikitext/'
               f'{_quote_url(self.title_uncoded)}')
