...
class IOOptions:
    ...
    audio_out_mode : {AudioOutMode.NO_OUTPUT (default)}
        Controls whether existing audio files are overwritten when creating
        output. AudioOutMode.OVERWRITE and AudioOutMode.NO_OVERWRITE are
        also defined as part of the `AudioOutMode` class but the variable
        cannot currently be assigned to these.
    sleep_time : float
    ...
    def __init__(self, output_dir: str,
        ...
        for leafdir in (['rev', 'medlist', 'html',
                         'wtxt','fnsprev', 'fnsphtml', 'fnspwtxt']):
    ...
    def audio_out_mode(self) -> AudioOutMode:
        """Future use: Controls download/overwriting of audio files.

        Currently must be AudioOutMode.NO_OUTPUT.
        AudioOutMode.OVERWRITE and AudioOutMode.NO_OVERWRITE are also
        defined as part of the `AudioOutMode` enumerator class but the
        variable cannot currently be assigned to these.
        """
        return self._audio_out_mode

    @audio_out_mode.setter
    def audio_out_mode(self, audio_out_mode: AudioOutMode) -> None:
        if audio_out_mode is not AudioOutMode.NO_OUTPUT:
            raise ValueError('IOOptions: audio_out_mode must be '
                             'AudioOutMode.NO_OUTPUT')
        else:
