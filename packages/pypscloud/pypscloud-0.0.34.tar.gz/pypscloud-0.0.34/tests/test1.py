...
def test_account(ps):
    ...
    a2 = {}
    a = ps.get_account(acc_edge_test_bench)
    print(a)
    if a['useFirmwareAutomaticUpdates'] == True:
        a2['useFirmwareAutomaticUpdates'] = False
        a2['address'] = a['address']
        a2['isPartner'] = a['isPartner']
        a2['name'] = a['name']
        a2['subscriptionExpiry'] = a['subscriptionExpiry']
        a2['subscriptionType'] = a['subscriptionType']

        info = ps.set_account(acc_edge_test_bench,a2)
        print(info)
    a2 = ps.get_account(acc_edge_test_bench)
    print(a2)
