...
def getKeyCols(filePath):
    """
    desc:
        A Function for getting the primary key cols from the given file path
        (Works only if source file path is Bronze and src_key_cols is present in the delta table)

    args:
        filePath: String - A file path name that is passed as an argument

    return:
        keyCols: String - returns the value of the column "src_key_cols" in the latest record
        of the given bronze delta table

    example:
        getKeyCols("abfss://bronze@eabase.dfs.core.windows.net/test/afpo/")

    tip:
        Make sure the source file is in delta format and has the columns "src_key_cols" and "src_commit_time"
    """
    try:
        
        keyCols = (spark.read.format("delta").load(filePath)
        ...
        raise Exception(
            f"the column src_key_cols is not present in the given delta table in {filePath}")    
