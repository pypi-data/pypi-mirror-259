...
def master_data_check(master_df,column_excel_df, lookup_column_name, excel_column_name,primary_key_in_excel="", primary_key_in_master=""):
    logging.info("master data check is started")

    if primary_key_in_master != "":
        # logging.info("master data check is started\n inside if ")
        column_excel_df = column_excel_df.rename(columns={primary_key_in_excel: primary_key_in_master})
        # logging.info(f"{column_excel_df.head()}")
        
        merged_df = pd.merge(column_excel_df, master_df, on=primary_key_in_master, how='left')
        ...
        merged_df[lookup_column_name] = merged_df[lookup_column_name].fillna("").astype(str)
        merged_df[excel_column_name] = merged_df[excel_column_name].fillna("").astype(str)
        ...
        merged_df[error_column_name] = merged_df.apply(certification_concat_check, axis=1)
        # print(merged_df)
        logging.info("merged_df of lookup column done")
        failed_master_data_check_rows = merged_df[~merged_df[error_column_name]].index.tolist()
