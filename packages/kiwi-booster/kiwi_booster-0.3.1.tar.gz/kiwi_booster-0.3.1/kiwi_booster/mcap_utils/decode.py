...
class DecoderFactoryCustom(DecoderFactory):
    """
...
def decode_compressed_image(msg: Any) -> Tuple[np.ndarray, dict]:
    ...
    log_time = datetime.fromtimestamp(msg.header.stamp.sec) + timedelta(
        microseconds=msg.header.stamp.nanosec / 1000
    )
...
def decode_image(msg: Any) -> Tuple[np.ndarray, dict]:
    ...
    image = np.frombuffer(msg.data, dtype=np.uint8).reshape((height, width, channel))
        ...
        decoded_transforms.append(
            {
                ...
                "timestamp": datetime.fromtimestamp(transform.header.stamp.sec)
                + timedelta(microseconds=transform.header.stamp.nanosec / 1000),
                "translation": {
