...
class AsyncClient:
    ...
    async def query_user_memory(
        self, user_id: ID_T, text: str, k: int = 200, query_state: bool | str = False
    ) -> List:
        ...
        response = await self.client.post(
            ...
            data=json.dumps(
                {"text": text, "k": k, "query_state": query_state},
    ...
    async def update_memory_function(
        ...
        *,
        ...
        response = await self.client.patch(
            f"/memory-functions/{_as_uuid(memory_function_id)}", json=data
        )
        response.raise_for_status()
        return response.json()
