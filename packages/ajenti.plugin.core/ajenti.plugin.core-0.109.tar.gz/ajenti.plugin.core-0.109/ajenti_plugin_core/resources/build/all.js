...
angular.module('core').controller('CoreLoginController', function ($scope, $log, $rootScope, $routeParams, identity, notify, gettext, customization) {
    ...
    $scope.sanitizeNextPage = function () {
    ...
    $scope.verify = function ($event) {
        ...
        if (code.toString().length == 6) {
            $scope.totp_attempts++;
            identity.auth($scope.username, code, "totp").then(function (response) {
                $scope.success = true;
    ...
    $scope.closeTotpDialog = function () {
        $scope.working = false;
        $scope.verif_code = false;
        $scope.totp_attempts = 0;
        $scope.username = '';
    ...
    $scope.login = function () {
        ...
        identity.auth($scope.username, $scope.password, $scope.mode).then(function (response) {
...
angular.module('core').controller('CoreTotpController', function ($scope, $log, $rootScope, $routeParams, $http, $window, identity, notify, gettext, messagebox, customization) {
    $scope.add_new = false;
    identity.promise.then(function () {
        return $scope.user = identity.user;
    });
    $scope.tmp_totp = {
        'description': '',
        'secret': ''
    ...
    $http.get('/api/core/totps').then(function (resp) {
        $scope.totp = resp.data || '';
    });

    $scope.random_secret = function () {
    ...
    $scope.verify = function ($event) {
        ...
        if (code.toString().length == 6) {
            $http.post('/api/core/totps/verify', {
                ...
                'code': code }).then(function (resp) {
                return $scope.verified = resp.data;
            });
        }
    };

    $scope.delete = function (totp) {
        messagebox.show({
            text: gettext('Really delete this TOTP?'),
            positive: gettext('Delete'),
            negative: gettext('Cancel')
        }).then(function () {
            $http.delete('/api/core/totps/' + totp.created).then(function (resp) {
    ...
    $scope.save = function () {
        ...
        $http.post('/api/core/totps', { 'secret': $scope.tmp_totp }).then(function (resp) {
            notify.success(gettext('Command successfully sent'));
...
angular.module('core').service('config', function ($http, $q, initialConfigContent) {
    ...
    this.getAuthenticationProviders = function (config) {
        return $http.get("/api/core/authentication-providers").then(function (response) {
            return response.data;
        });
    };

    this.getPermissions = function (config) {
        return $http.get("/api/core/permissions").then(function (response) {
            return response.data;
        });
    };
...
angular.module('core').service('identity', function ($http, $location, $window, $timeout, $q, urlPrefix, ajentiBootstrapColor) {
    ...
    this.init = function () {
        return $http.get('/api/core/identity').then(function (resp) {
            identity = resp.data.identity;
            _this.user = identity.user;
            _this.uid = identity.uid;
            _this.effective = identity.effective;
            _this.elevation_allowed = identity.elevation_allowed;
            _this.profile = identity.profile;
            _this.machine = resp.data.machine;
    ...
    this.manage_totp = function () {
        return $window.location.assign(urlPrefix + '/view/totp');
    };
...
angular.module('core').service('messagebox', function ($timeout, $q) {
    ...
    this.prompt = function (prompt, value) {
        var input_type = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'text';
