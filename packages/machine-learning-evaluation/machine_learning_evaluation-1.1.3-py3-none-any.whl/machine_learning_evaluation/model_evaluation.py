...
class DataExplorer:
    ...
    def content_recommendation_system(self, text_data):
        """
        ...
        def recommendation_system(item_name, item_column, n_recommendations):
            item_index = self.df[item_column][self.df[item_column] == item_name].index[0]
            sim_scores = list(enumerate(cosine_sim[item_index]))
            sim_scores.sort(key=lambda x: x[1], reverse=True)
            top_indices = [i[0] for i in sim_scores[1:n_recommendations + 1]]  # Exclude the item itself
            return self.df[item_column].iloc[top_indices].tolist(), item_column
