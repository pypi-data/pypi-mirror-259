...
def repr_args(*args, **kwargs):
    return "{args}{mark}{kwargs}".format(
        args=", ".join(list(map(lambda x: str(x), args))) if args else "",
        kwargs=", ".join(
    ...
    try:
        ...
        if is_skipped_hosts and browser_errors:
            for host in ast.literal_eval(is_skipped_hosts):
                browser_errors = list(filter(
                    lambda x: host not in x, browser_errors
                ))
