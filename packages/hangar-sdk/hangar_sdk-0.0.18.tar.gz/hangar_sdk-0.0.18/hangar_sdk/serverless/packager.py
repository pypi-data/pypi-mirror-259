...
class BaseLambdaDeploymentPackager(object):
    ...
    def _get_requirements_filename(self, project_dir: str,req_file_name : str) -> str:
        # Gets the path to a requirements.txt file out of a project dir path
        return self._osutils.joinpath(project_dir, req_file_name)
        ...
        for rootdir, dirs, filenames in self._osutils.walk(
            ...
            for filename in filenames:
                ...
                if filename in gitignore:
                    continue
                #  project dir : abs path to the root of project directory should be considered as the root of the zip file
                #  file path: abs path to the file on the users machine
                #  zip path: path to the file in the zip file
                #  zip path: Relative(file path, project dir) -> zip path
                fullpath = self._osutils.joinpath(rootdir, filename)
                zip_path = self._osutils.joinpath(os.path.relpath(rootdir,self._osutils.abspath(project_dir)), filename)
...
class LayerDeploymentPackager(BaseLambdaDeploymentPackager):
    ...
    def create_deployment_package(self, project_dir: str, python_version: str, requirements_filename : str = "requirements.txt") -> str:
        msg = "Creating shared layer deployment package."
        self._ui.write("%s\n" % msg)
        logger.debug(msg)
        package_filename = self.deployment_package_filename(project_dir, python_version, req_file_name=requirements_filename)
        self._create_output_dir_if_needed(package_filename)
        if self._osutils.file_exists(package_filename):
            self._ui.write("  Reusing existing shared layer deployment package.\n")
            return package_filename
        with self._osutils.tempdir() as tmpdir:
            requirements_filepath = self._get_requirements_filename(project_dir,requirements_filename)
            self._build_python_dependencies(
                python_version, requirements_filepath, site_packages_dir=tmpdir
            )
            with self._osutils.open_zip(
    ...
    def deployment_package_filename(self, project_dir: str, python_version: str, req_file_name: str) -> str:
        return self._deployment_package_filename(
            project_dir, python_version, prefix="managed-layer-", req_file_name=req_file_name
        )

    def _deployment_package_filename(
        self, project_dir: str, python_version: str, prefix: str = "", req_file_name: str = "requirements.txt"
    ) -> str:
        requirements_filename = self._get_requirements_filename(project_dir,req_file_name)
        if not self._osutils.file_exists(requirements_filename):
            contents = b""
        else:
            contents = cast(
