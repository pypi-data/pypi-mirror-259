...
class CreateableIOResource:
    ...
    def _post_input_request(
        ...
        body, initialized_input = self._get_scene_uuid(body)

        body, validated_imu_data = self._validate_imu_data(body, imu_data, initialized_input.scene_uuid, dryrun)
    ...
    def _get_scene_uuid(self, body: Dict) -> Tuple[Dict, InitializedSceneJob]:
        json_resp = self._client.post(INPUTS_ROUTE + "/initialize")

        init_input = InitializedSceneJob.from_json(json_resp)
        body["internalId"] = init_input.scene_uuid
