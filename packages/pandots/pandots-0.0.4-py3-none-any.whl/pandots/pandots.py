...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        '--paf',
        required=True,
        help='path to paf alignment file'
    )
    parser.add_argument(
        '-r',
        '--refchrom',
    ...
    parser.add_argument(
        ...
        '--outprefix',
        required=True,
        help='prefix for outfile name. appends to *.pandots.svg'
    )
    parser.add_argument(
        '--pad',
        default=10000000,
        help='padding value for distance between genome lines'
    )
    parser.add_argument(
        '--key',
        action="store_true",
        help='show genome key'
    )
    parser.add_argument(
        '--flip',
        action="store_true",
        help='make paf column 6 the reference. column 1 is reference by default'
    )
    parser.add_argument(
        '--invert',
        action="store_true",
        help='invert reference coordinates in relation to pangenome'
    )
    parser.add_argument(
        '--reorder',
        action="store_true",
        help='adjust order of genomes plotted by length and inversions'
    )
    parser.add_argument(
        '--recolor',
        action="store_true",
        help='use alternating colors'
    )
    parser.add_argument(
        '--orient',
        default='NO_FILE',
        help='two column file to orient genome direction to reference. col1: chrom name, col2: True/False'
    )
...
def main():
    ...
    if args.orient != "NO_FILE":
        orients = load_orients(args.orient)
        if args.invert:
    ...
    plot = draw_ggplot(pairs,args.refchrom,'','','Genome',args.key,args.recolor)

    ggsave(plot,args.outprefix+'.pandots.svg',format="svg")
