...
class page(page):
	...
	def GS_Loader(self):
		...
		def fun_reset(Engine=None,FileDir=None,FileType=None,Interval=None):
				...
				for key,value in cfg.items():
					if key in ("MultiprocessEngine","IO","Loader"): eval(f"self.con_{key}(**value)")
						...
						for k,v in value.items():
							if k in ("quantity_name","result"):anaf[k]=v
							else: anaf["arguments"][k]=v
						anafs.append(anaf)
		...
		def Test(event):
			...
			for key,value in get_anafs().items():
				try:anaf=self.pageargs["Infobj"].Config["AnalysisTool"][key].get
					...
					try:anaf=self.pageargs["Infobj"].Config["AnalysisLib"][i]
					except:
