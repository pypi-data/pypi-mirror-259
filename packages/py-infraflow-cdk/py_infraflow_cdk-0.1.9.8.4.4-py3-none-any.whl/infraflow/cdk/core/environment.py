...
class Env(IEnv):
    ...
    def vpc_subnets(self, subnet_type: SubnetType=None) -> list[ISubnet]:
        subnet_select = self.vpc.select_subnets(subnet_type=subnet_type, one_per_az=True).subnets
        for subnet in subnet_select:
            print(f"**** subnet_id: {subnet.subnet_id} subnet_select: {subnet}")
        #.select_subnets(subnet_type=subnet_type).subnets
        return subnet_select

    def service_subnets(self, subnet_type: SubnetType):

        print("**** environment.py. subnet_type:", subnet_type)
        vpc_subnets = self.vpc_subnets(subnet_type)
        print("**** environment.py. self.vpc_subnets(subnet_type):", vpc_subnets)
        for sn in vpc_subnets:
            print(f"**** environment.py. subnet_id: {sn.subnet_id} subnet: {sn}")

        if not self.config.subnet_map:
            print("**** environment.py. NO self.config.subnet_map:", self.config.subnet_map)
            print("**** environment.py. no config.subnet_map, so vpc_subnets:", vpc_subnets)
            return vpc_subnets

        print("**** environment.py. [found] self.config.subnet_map:", self.config.subnet_map)
