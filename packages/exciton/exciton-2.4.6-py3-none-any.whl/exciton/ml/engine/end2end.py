...
class End2EndBase:
    ...
    def _build_pipeline(self, train_base_model: bool = False):
    ...
    def fit(
        ...
        for epoch in range(n_epoch):
            ...
            for batch_id, batch_raw in enumerate(batch_pool):
                ...
                loss = self._build_pipeline(train_base_model=train_base_model)
                optimizer.zero_grad()
                loss.backward()
                ...
                if ((batch_id + 1) * n_checkpoints) % n_batches == 0 or batch_id == 0:
