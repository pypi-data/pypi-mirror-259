...
angular.module('ajenti.plugins').controller('PluginsIndexController', function ($scope, $q, $http, $rootScope, notify, identity, pageTitle, messagebox, tasks, core, gettext) {
    ...
    $scope.refresh = function () {
        $http.get('/api/plugins/installed').then(function (resp) {
            $scope.installedPlugins = resp.data;
            $scope.repoList = null;
            $scope.repoListOfficial = null;
            $scope.repoListCommunity = null;
            $http.get('/api/plugins/pypi/ajenti-plugins').then(function (rsp) {
        ...
        $http.post('/api/plugins/core/check-upgrade').then(function (resp) {
            return $scope.coreUpgradeAvailable = $scope.needUpgrade($rootScope.ajentiVersion, resp.data);
        });

        $scope.pypiList = null;
        $http.get('/api/plugins/pypi/installed').then(function (resp) {
            return $scope.pypiList = resp.data;
        });
    };
