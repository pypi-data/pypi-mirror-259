...
def my_ContinousParameter():
    return data.ContinousParameter("test_continous", 0, 1, 4, "uniform", 0.1, 0.1)
...
@pytest.mark.parametrize(
    ...
    [(0, (0.0, 0.25)), (1, (0.25, 0.50)), (2, (0.50, 0.75)), (3, (0.75, 1.0))],
)
def test_continous_parameter_get_level_bounds(
    my_ContinousParameter, level, expected_output
):
...
def test_continous_parameter_distributions_ppf(distribution, expected_output):
    my_parameter = data.ContinousParameter(
        "test_continous", 0, 1, 4, distribution, 0.1, 0.1
    )

    assert (
        my_parameter.ppf([0.25, 0.5, 0.75], 0.5).round(3) == expected_output.round(3)
    ).all()
...
def test_discrete_parameter_repr(my_DiscreteParameter):
    assert (
        str(my_DiscreteParameter)
        == f'id:{my_DiscreteParameter.id} - Discrete Parameter "{my_DiscreteParameter.name}"\n{my_DiscreteParameter.n_levels} Levels\n'
    )
...
def test_objective_repr():
    ...
    assert (
        str(my_objective)
        == f"Objective {my_objective.name} : {my_objective.operand}: Requirement: {my_objective.get_requirement()} : P_sat = {my_objective.p_satisfaction}\n"
    )


@pytest.mark.parametrize(
    "operand, has_requirement, expected_output",
    [
        ("min", None, None),
        ("max", None, None),
        ("min", 1, ("lt", 1)),
...
def test_objective_get_requirement(operand, has_requirement, expected_output):
    my_objective = data.Objective(
        "test_objective", operand, min_requirement=has_requirement
    )

    assert my_objective.get_requirement() == expected_output


@pytest.mark.parametrize("operand, expected_output", [("min", 1), ("max", -1)])
...
def test_constraint_repr():
    ...
    assert (
        str(my_constraint)
        == f"Constraint {my_constraint.name} : {my_constraint.get_constraint()} : P_sat = {my_constraint.p_satisfaction}\n"
    )


@pytest.mark.parametrize(
    "operand, expected_output",
    [("lt", ("lt", 1)), ("gt", ("gt", 1)), ("let", ("let", 1)), ("get", ("get", 1))],
)
...
def test_DesignSet_samples(my_DesignSet):
    ...
    assert (
        my_DesignSet.sample(10, [par1, par2], debug=True).round(4)
...
class Dummy_Optimisation_Problem:
    var = [
        data.ContinousParameter("par1", 0, 1, 4, None, None, None),
        data.DiscreteParameter("par2", 4),
    ]

    obj = [data.Objective("obj", "min")]

    cst = [data.Constraint("con1", "lt", 1), data.Constraint("con2", "gt", 0.1)]

    x_mask = ["c", 0]


class Dummy_Optimisation_Results:
    X = np.array([[0.2, 0], [0.04, 0]])

    F = np.array([1.0, 2.0]).reshape(-1, 1)
    G = np.array([[-0.5, 0.1], [0.5, -0.1]])
...
def test_DesignSpace_creation_from_parameters(my_DesignSpace):
    parameter_1 = data.ContinousParameter("par1", 0, 10, 4, "uniform", 0.1, 0.2)
    parameter_2 = data.DiscreteParameter("par2", 2)

    objective = data.Objective("obj", "min", min_requirement=5, p_sat=0.25)
    ...
    DesignSpace_from_parameters = data.DesignSpace(parameters, objectives, constraints)
...
def test_DesignSpace_get_optimum_results(my_DesignSpace):
    ...
    s_id_list = [0.0, 0.0, 1.0, 1.0, 2.0, 2.0, 3.0, 3.0, 4.0, 4.0, 5.0, 5.0]
