...
LOAD_MODULE = html.Div(
    children=[
        dcc.Upload(
            id="upload-data",
            children=[html.A("Load .csv file")],
            style={
                "width": "fit-content",
                "height": "60px",
                "lineHeight": "60px",
...
def get_HTML_Layout(ext_load=True, load_figures=False):
    ...
    return html.Div(
        [
            ...
            html.H5(
                children="This webapp allows to visualize .csv outputs from P-DOPT"
            ),
            html.Div(id="debug"),
            # Load-in data
                        ...
                        children=[
                            html.Div(
                                children=[
                                    ...
                                    dcc.Dropdown(
                                        id="dropdown-parcoord",
                                        options=g_labels
                                        if load_figures
                                        else NONE_LABELS,
                                        value=g_labels[-1]["value"]
                                        if load_figures
                                        else NONE_LABELS[0]["value"],
                                    ),
                                ],
                                style={
                                    "width": "20vw",
            ...
            html.Br(),
            html.Div(id="copyright", children=["V. 0.4, Copyright Andrea Spinelli"]),
            # Local Variables
    ...
    if len(g_selected_points_parc) > 0:
        ...
        if color_label != "none":
            ...
            color_values = np.linspace(
                (df[color_label].min() - q) / k,
                (df[color_label].max() - q) / k,
                len(PARCOORD_COLOR_LIST),
            )

            color_scale = [
                [color_values[i], PARCOORD_COLOR_LIST[i]]
            ...
            dummy = [1 if int(x) in g_selected_points_parc else 0 for x in df.index]
            df["dummy"] = dummy
...
def generate_scatter(x_label, y_label, color_label, scatter_id):
    ...
    if color_label != "none":
        m = dict(
            ...
            showscale=True,
            colorbar={"title": color_label, "titleside": "right"},
            line=dict(width=1, color="DarkSlateGrey"),
        )
    else:
        m = dict(
            size=10, color="LightSkyBlue", line=dict(width=1, color="DarkSlateGrey")
        )
    ...
    fig_sca.update_layout(xaxis_title=x_label, yaxis_title=y_label, hovermode="closest")
    ...
    if len(g_selected_points_sca) > 0:
        fig_sca.update_traces(
            unselected={"marker": {"color": "LightGray", "opacity": 0.1}},
    ...
    fig_sca.update_traces(
        hovertemplate="<br>".join(
            [
                "{}:%{{customdata[{}]:.3f}}".format(g_dataframe.columns[k], k)
                for k in range(len(g_dataframe.columns))
            ]
        )
    )
...
def update_parallel_coordinates(c_label, dummy):
    ...
    g_parallel_coord_figure = generate_parallel_coordinates(color_label=c_label)

    if len(g_par_coord_selected_ranges) > 0:
        # Add the selected ranges back
        for axis, values in g_par_coord_selected_ranges.items():
            g_parallel_coord_figure.data[0].dimensions[
                int(axis)
            ].constraintrange = values

    return g_parallel_coord_figure


@g_app.callback(
    Output("update_parc", "data"),
...
def get_selected_data_parc(
    scatter_data1, scatter_data2, scatter_data3, scatter_data4, scatter_data5, dummy
):
    global g_dataframe, g_selected_points_parc, g_par_coord_selected_ranges

    # Get the selected rows from each scatter plot
    scatter1_points = (
        [p["pointIndex"] for p in scatter_data1["points"]] if scatter_data1 else []
    )
    scatter2_points = (
        [p["pointIndex"] for p in scatter_data2["points"]] if scatter_data2 else []
    )
    scatter3_points = (
        [p["pointIndex"] for p in scatter_data3["points"]] if scatter_data3 else []
    )
    scatter4_points = (
        [p["pointIndex"] for p in scatter_data4["points"]] if scatter_data4 else []
    )
    scatter5_points = (
        [p["pointIndex"] for p in scatter_data5["points"]] if scatter_data5 else []
    )
...
def get_selected_data_scatters(
    restyleData,
    scatter_data1,
    scatter_data2,
    scatter_data3,
    ...
    if g_par_coord_selected_ranges:
        for key, val in g_par_coord_selected_ranges.items():
            ...
            temp_points = g_dataframe[g_dataframe[col].between(val[0], val[1])].index
    ...
    g_scatter_selection_list[0] = (
        scatter_data1["range"]
        if scatter_data1 and "range" in scatter_data1.keys()
        else {}
    )
    g_scatter_selection_list[1] = (
        scatter_data2["range"]
        if scatter_data2 and "range" in scatter_data2.keys()
        else {}
    )
    g_scatter_selection_list[2] = (
        scatter_data3["range"]
        if scatter_data3 and "range" in scatter_data3.keys()
        else {}
    )
    g_scatter_selection_list[3] = (
        scatter_data4["range"]
        if scatter_data4 and "range" in scatter_data4.keys()
        else {}
    )
    g_scatter_selection_list[4] = (
        scatter_data5["range"]
        if scatter_data5 and "range" in scatter_data5.keys()
        else {}
    )
...
def main_inline(dataframe, debug=False):
    ...
    g_labels = [{"label": column, "value": column} for column in g_dataframe.columns]
    g_labels = [{"label": "None", "value": "none"}] + g_labels
    default = g_labels[1]["value"]

    g_customdata = np.empty(
        shape=(len(g_dataframe), len(g_dataframe.columns), 1), dtype="object"
    )

    for i in range(len(g_dataframe.columns)):
        g_customdata[:, i] = np.array(g_dataframe.iloc[:, i]).reshape(-1, 1)

    g_parallel_coord_figure = generate_parallel_coordinates(color_label=default)

    g_scatter_list = [
        generate_scatter(
            g_dataframe.columns[-1], g_dataframe.columns[-2], g_dataframe.columns[-3], i
        )
        for i in range(5)
    ]
...
def main_standalone(debug=True):
    ...
    @g_app.callback(
        Output("dropdown-parcoord", "options"),
        ...
        if file_contents is None or file_contents is []:
            ...
            try:
                if "csv" in filename:
                    ...
                    g_dataframe = pd.read_csv(io.StringIO(decoded.decode("utf-8")))
                    g_labels = [
                ...
                g_scatter_list = [
                    generate_scatter(
                        g_dataframe.columns[-1],
                        g_dataframe.columns[-2],
                        g_dataframe.columns[-3],
                        i,
                    )
                    for i in range(5)
                ]
