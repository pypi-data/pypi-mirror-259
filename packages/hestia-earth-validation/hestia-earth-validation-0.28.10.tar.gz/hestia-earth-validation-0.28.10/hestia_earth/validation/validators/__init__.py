...
# only run basic validation or complete validation
VALIDATE_AGGREGATED_AVANCED = os.getenv('VALIDATE_AGGREGATED_AVANCED', 'false') == 'true'
VALIDATE_TYPE = {
...
VALIDATE_TYPE_AGGREGATED = {
    SchemaType.CYCLE.value: lambda n, nodes: validate_cycle_aggregated(n, nodes) + (
        validate_cycle(n, nodes) if VALIDATE_AGGREGATED_AVANCED else []
    ),
    SchemaType.IMPACTASSESSMENT.value: lambda n, nodes: validate_impact_assessment_aggregated(n, nodes) + (
        validate_impact_assessment(n, nodes) if VALIDATE_AGGREGATED_AVANCED else []
    )
}
