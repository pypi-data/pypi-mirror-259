...
from autotda.functions.classes import Weight
import pandas as pd
...
def find_relationships(autofeat, relationship_threshold: float = 0.5, matcher: str = "coma", 
    ...
    if explain:
        print_string = f" 1. AutoFeat computes the relationships between {len(tables)} tables from the datasets: {autofeat.datasets} repository using {matcher} similarity score with a threshold of {relationship_threshold} (i.e., all the relationships with a similarity < {relationship_threshold} will be discarded) " 
        if len(autofeat.extra_tables) > 0:
            print_string += f"using extra tables: {autofeat.extra_tables} "
        if len(autofeat.exclude_tables) > 0:
            print_string += f"and excludes tables: {autofeat.exclude_tables}"
        print_string += f""
        print(print_string)
    if verbose:
        print("Calculating relationships...")

    filename = f"{autofeat.weights_location}/{autofeat.base_table}_{relationship_threshold}_{matcher}_weights.txt"
    ...
    def profile(combination, matcher="coma"):
        ...
        df1 = pd.read_csv(autofeat.repository_location + "/" + table1)
        df2 = pd.read_csv(autofeat.repository_location + "/" + table2)
    ...
    if use_cache:
        os.mkdir(autofeat.weights_location + "/" + autofeat.base_table.split("/")[0])
        f = open(f"{autofeat.weights_location}/{autofeat.base_table}_{relationship_threshold}_{matcher}_weights.txt", "w")
        stringlist = []
