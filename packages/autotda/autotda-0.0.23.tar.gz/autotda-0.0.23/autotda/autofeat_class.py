...
from autotda.functions.helper_functions import RelevanceRedundancy, get_df_with_prefix
...
class TDA:
    ...
    def __init__(self, repository_location: str = "data/benchmark", weights_location = "saved_weights"):
    ...
    def set_base_table(self, base_table: str, target_column: str):
        ...
        X_train = get_df_with_prefix(self.repository_location, self.base_table, self.targetColumn)
    ...
    def set_dataset_repository(self, dataset_repository: List[str] = [], all_tables: bool = False):
        ...
        if all_tables:
            datasets = [i.split("/")[-1] for i in glob.glob(self.repository_location + "/*")]
    ...
    def get_tables_repository(self):
        ...
        for dataset in self.datasets:
            for table in glob.glob(self.repository_location + "/" + dataset + "/*.csv"):
                tables.append((table.split("/")[-2]) + "/" + table.split("/")[-1])
        tables.extend(self.extra_tables)
        for i in self.exclude_tables:
            if i in tables:
                tables.remove(i)
        return tables

    def add_table(self, table: str, rerun=False):
        """
        ...
        if self.relationship_threshold is not None and self.matcher is not None and rerun:
            relationship_functions.rerun(self, self.relationship_threshold, self.matcher)

    def remove_table(self, table: str, rerun=False):
        """
    ...
    def get_all_results(self):
        ...
        """
        return evaluation_functions.get_all_results(self)
    ...
    def materialise_join_tree(self, tree_or_result: int):
        """
        Materializes the join tree with the given tree_id or result.

        Args:
            tree_or_result (any): The ID of the tree or the result to materialize.

        Returns:
            The materialized join tree.
        """
        return tree_functions.materialise_join_tree(self, tree_or_result)
    ...
    def augment_dataset(self, algorithm="GBM", relationship_threshold: float = 0.5, non_null_threshold=0.5, matcher="coma", 
        ...
        self.matcher = matcher
        if use_cache:
            if os.path.isfile(f"{self.weights_location}/{self.base_table}_{relationship_threshold}_{matcher}_weights.txt"):
                if verbose:
                    print("Reading from cache file: " + f"{self.weights_location}/{self.base_table}_{relationship_threshold}_{matcher}_weights.txt")
                    self.read_relationships(f"{self.weights_location}/{self.base_table}_{relationship_threshold}_{matcher}_weights.txt")
            else:
...
if __name__ == "__main__":
    ...
    autofeat.add_table("school/qr.csv")
