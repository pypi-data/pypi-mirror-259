...
import tqdm
from autotda.functions.classes import Result, Join
...
def evaluate_table(autofeat, algorithm, tree_id: int, verbose=False, multiple=False):
    ...
    base_df = get_df_with_prefix(autofeat.repository_location, autofeat.base_table, autofeat.targetColumn)
    i: Join
    for i in tree.joins:
        df = get_df_with_prefix(autofeat.repository_location, i.to_table).groupby(i.get_to_prefix()).sample(n=1, random_state=42)
...
def get_all_results(autotda):
    return [(i.tree.id, i.accuracy) for i in autotda.results]
