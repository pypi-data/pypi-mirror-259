from autotda.functions.classes import Tree, Join, Weight, Result
import networkx
...
def stream_feature_selection(autofeat, top_k_features, queue: set, non_null_ratio_threshold: float,
    ...
    while len(queue) > 0:
        ...
        for n in neighbours:
            ...
            right_df = get_df_with_prefix(autofeat.repository_location, highest_join_keys[0].to_table)
            current_queue = set()
...
def materialise_join_tree(self, tree_id_or_result):
    if isinstance(tree_id_or_result, int):
        tree = get_tree_by_id(self, tree_id_or_result)
    elif isinstance(tree_id_or_result, Result):
        tree = get_tree_by_id(self, tree_id_or_result.tree.id)
    else:
        print("not a valid input: ", tree_id_or_result)
    base_df = get_df_with_prefix(self.repository_location, self.base_table, self.targetColumn)
    i: Join
    for i in tree.joins:
        df = get_df_with_prefix(self.repository_location, i.to_table)
