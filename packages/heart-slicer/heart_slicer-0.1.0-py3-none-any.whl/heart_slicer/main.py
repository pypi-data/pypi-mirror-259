"""
Main file for the heart 'package'.
"""
...
def run_heart() -> None:
    ...
    all_modes = ['all', '-A', 'cutter', '-X', 'counter_SC2', '-SC2', 'example', 'counter']

    common.logger.info(f"Selected mode: {common.settings['mode']}")
    common.logger.info(f"Processing directory:\n\t{common.settings['folder_input']}")

    if common.settings['mode'] not in all_modes:
        common.logger.info(f"unknown mode {common.settings['mode']}, choose from {all_modes}")
        raise NotImplementedError(f"unknown mode {common.settings['mode']},"+\
                                  f" supported modes are: {all_modes}")
#,os.path.dirname(__file__),
                                    #   '..', 'example', 'input'),
    if common.settings['mode'] == 'example':
        common.logger.info("running in EXAMPLE mode. Process all slices for example image")
        # overwrite output folder to test
        common.settings['folder_output'] = \
            os.path.join(os.path.dirname(__file__),'..', 'example', 'output')
        common.settings['folder_input'] = \
            os.path.join(os.path.dirname(__file__),'..', 'example', 'input')
        # cutter
        common.logger.info(slicer.process_heart_images_in_directory(
    ...
    if common.settings['mode'] in ['all', '-A', 'cutter', '-X']:
        output = (slicer.process_heart_images_in_directory(
            directory_in=common.settings['folder_output'],
            directory_out=common.settings['folder_output'],
            move_file=common.settings['move_slice_file'],
    ...
    #     p = pstats.Stats(pr)
    #     p.strip_dirs().sort_stats(SortKey.TIME).logger.info_stats(10)
