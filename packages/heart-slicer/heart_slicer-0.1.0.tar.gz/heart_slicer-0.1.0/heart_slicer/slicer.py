...
def cut_polygon_from_image(im: Image, polygon: list, trim: bool=None) -> Image:
    """
    ...
    if trim:
        bounding_box = find_bounding_box(cut_image)
        # crop to size and add border
        cut_image = crop_image(im=cut_image, bbox=bounding_box, border=settings['border'])

    return cut_image

@common.logging_decorator
def boundingbox_old(im: Image, background: set) -> list:
    """
    Old method to find the bounding box of the image using the 'locals'
    ...
    """
    logger.warn("Using a depricated method to find the bounding box."+\
                "use 'boundingbox_new' or 'boundingbox_newest' instead")
    ...
    if 'y_min' not in locals():
        return [0, 0, 1, 1]
    # get y_max
    for y in range(im.height-1, y_min, -1):
        for x in range(im.width - 1):
            if px[x, y] != background:
                y_max = y
                break
    ...
    for x in range(im.width - 1):
        ...
        if 'x_min' in locals():
            break
    # get x_max
    for x in range(im.width - 1, x_min, -1):
        for y in range(y_min, y_max):
            if px[x, y] != background:
                x_max = x
                break
...
@common.logging_decorator
def boundingbox_new(im: Image, background: set) -> list:
    """
    New method to find the bounding box of the image using
    a list of image rgb values to determine if a pixel is a
    background pixel. Then Looks trough list to find the
    bounding box.
    '
    environment variable (not preferred)
        im: Image
    ...
    # use new method
    px = list(im.getdata())
    # next step takes a lot of time
    px = np.array([px[i] == background for i in range(len(px))]).reshape(im.width, im.height)

    def find_min_max(px, axis, direction):
        """axis:
                0 = x
                1 = y
            direction:
                ...
                -1 = maximum
        """
        # When pixel is bachground, value is True
        if (axis == 'x') and (direction == 'min'):
            for i in range(len(px[:,0])):
                if px[:,i].all() is False:
                    return i
            return 0
        if (axis == 'x') and (direction == 'max'):
            for i in range(-1, -len(px[:,0]), -1):
                if px[:,i].all() is False:
                    return i + len(px[:,0])
            return 1
        if (axis == 'y') and (direction == 'min'):
            for i in range(len(px[0,:])):
                if px[i].all() is False:
                    return i
            return 0
        if (axis == 'y') and (direction == 'max'):
            for i in range(-1, -len(px[0,:]), -1):
                if px[i].all() is False:
                    return i + len(px[0,:])
            return 1
    ...
    x_min = find_min_max(px, 'x', 'min')
    x_max = find_min_max(px, 'x', 'max')
    y_min = find_min_max(px, 'y', 'min')
    y_max = find_min_max(px, 'y', 'max')

    return [x_min, y_min, x_max, y_max]

@common.logging_decorator
def boundingbox_newest(im: Image, background: set) -> list:
    """
...
@common.logging_decorator
def find_bounding_box(im, background=(255, 255, 255), method: callable=boundingbox_newest):
    """
...
def get_image_center(im: Image) -> list:
    """
...
def get_corner_angles(im: Image, offset: float=-math.pi/2):
    """
...
@common.logging_decorator
def create_slice_polygon(
        p_center: tuple, p0: tuple, angle: float,
         radius: int, im_size: tuple, corner_angles: list) -> list:
    """
...
def create_folders_heart_slice(
        directory: str, heart: str,
        slice_nr: str, segmentation_type: str) -> dict:
    """
    Check if a folder for the heart exists in the given directory,
    if not create it. Then create a folder for the slice in the heart folder.
    """
...
@common.logging_decorator
def images_for_segmentation_type(
        im: Image,
    ...
    """
    segment_names, offset = \
        segment_names_by_segmentation_type(
            segmentation_type=segmentation_type,
            section=section)

    # get relevant angles and points
    corner_angles = get_corner_angles(im=im, offset=offset)
    nsegments = len(segment_names)
    point_center = get_image_center(im=im)
    ...
    p0 = (point_center[0] + math.cos(offset) * im.width / 2,
    ...
    for cur_segmentnr in range(1, nsegments + 1):
        ...
        polygon_segment = create_slice_polygon(
        ...
        # create segment
        im_segment = cut_polygon_from_image(im, polygon_segment)
...
def segment_names_by_segmentation_type(segmentation_type, section):
    ...
    match segmentation_type:
        ...
        case _:
            raise NotImplementedError(f"Segmentation not supported: {segmentation_type}.")
...
def move_image_file(file, directory_from: str, directory_to: str) -> bool:
    ...
    os.rename(os.path.join(directory_from, file),  # from
                os.path.join(directory_to, os.path.split(file)[1]))  # to
    logger.info(f"Moved image file: {file}\n"+\
...
@common.logging_decorator
def process_heart_images_in_directory(directory_in=None, directory_out=None, move_file=True,
                                 overwrite_existing=True, segmentation_types=None):
    """
    ...
    if directory_in is None:
        ...
        logger.info("No directory providid using directory of current file:\n" + \
              f"..{directory_in}")
    ...
    for filename in images_to_proces:
        ...
        # im = common.load_image(file)
            ...
            for segmentation_type in segmentation_types:
                ...
                for segment_im, segment_name in segmentation_images:
                    ...
                    segment_folder = os.path.join(folders['slice_folder'], segmentation_type)
        ...
        output[file] = {'segments':counters['segments'],
