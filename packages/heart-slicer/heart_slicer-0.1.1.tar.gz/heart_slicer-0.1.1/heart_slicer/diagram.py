...
def CAG_border_plot(fig, ax, data, value_type, cmap=None, hatchtype=None, norm=None):
	...
	plt.colorbar(mpl.cm.ScalarMappable(cmap=cmap, norm=norm),
				cax=ax_bar, orientation='horizontal', label=value_type)
	...
	for el in ['vt','cw','ccw','sup','inf']:
		...
		rect = patches.Rectangle(xy, 1, 1, linewidth=1, edgecolor=(0,0,0), facecolor=color, hatch=hatch)
		line = lines.Line2D(xline, yline, linewidth=data[el]['border']*border_scale, color=(0,0,0))
...
def CAG_bullseye_plot(plot_type, fig, ax, data, value_type, border=None, cmap=None, norm=None, label=None, norm_border=None, title=None):
	"""
	...
	DEFAULT_CMAP = 'hsv'  # default colormap used if 'value_type' can't match
	
	# set colormap
	# see: https://matplotlib.org/3.1.1/tutorials/colors/colorbar_only.html#sphx-glr-tutorials-colors-colorbar-only-py
	...
	if plot_type == 'CAG':
		...
		raise ValueError(f"{plot_type} is not a supported plot type." +
						"choose from: 'AHA', 'CAG")
	
	cmap = get_cmap_by_valuetype(value_type, norm=[min(values), max(values)])
	...
	data = sorted(data, key=lambda point: str(point['slice']).zfill(2) + point['segment'])
	...
	# use min, max by default
		...
		raise ValueError(f"norm_border not list of two floats (provided: {norm_border})")
	...
	for segment in data:
		...
		if segment['slice'] != last_slice:
			...
			def prep_segment(key_to_find):
				...
				raise ValueError(f"segment '{key_to_find}' not supported in plot type: {plot_type}")
		...
		index = max([index_segment(segment['segment'], ks) for
				ks in keys])
	...
	# based on: https://matplotlib.org/gallery/pie_and_polar_charts/nested_pie.html#sphx-glr-gallery-pie-and-polar-charts-nested-pie-py
		# draw line plot_data
		...
		ax.bar(x = x,
				...
				color = [cmap(norm(value)) for value in plot_data[i]],
...
def create_bullseye_plot(df, heart, plot_type, value_type):					
	...
	for i in range(len(data_slice)):
		data.append({'slice': data_slice[i], 'segment': data_segment[i], 'value': data_value[i], 'border':data_border[i], 'label':None})
	...
	# run bullseye plot
	CAG_bullseye_plot(plot_type, fig, ax, data,
	...
	if SAVE_TO_FILE is True:
		savefile = os.path.join(folder, f"{heart}_{value_type}_{plot_type}.png")
...
if __name__ == '__main__':		
	logger.info('Running "Heart_Diagram_CAG6_ .. _v3.py"')
	
	FILENAME = 'testfile.csv'
	Base_path = os.getcwd()  # assume everything is in the folder of the python file.
	OUTPUTFOLDER = 'output_CAG'
	SAVE_TO_FILE = True
	SHOW_PLOT = not SAVE_TO_FILE
	...
	def data_csv_line_to_data(csv_line):
		...
		data_csv_line = 'filename,value_type,vt_value,vt_border,vt_hatch,ccw_value,ccw_border,ccw_hatch,cw_value,cw_border,cw_hatch,sup_value,sup_border,sup_hatch,inf_value,inf_border,inf_hatch'
		"""
	...
	if False:
		...
		df = pandas.read_csv(FILE, delimiter=';', decimal=',')
		...
		value_types = np.setdiff1d(list(df.keys()),
			['HEART', 'SLICE', 'AHA_CAG_MINI', 'ABL_State', 'segment', 'ABL_loss_perc'])

		# if saving to file, check if output folder exists, if not create it
		folder = os.path.join(os.path.split(FILE)[0], OUTPUTFOLDER)
		if SAVE_TO_FILE is True and not os.path.exists(folder):
			# create folder
			os.makedirs(folder)
		...
		for heart in hearts:
			for value_type in value_types:
				for plot_type in plot_types:
					plot_nr += 1
					logger.info(f"running: {heart} - {value_type} ({plot_type}-plot) ({plot_nr}/{total_plots})")
