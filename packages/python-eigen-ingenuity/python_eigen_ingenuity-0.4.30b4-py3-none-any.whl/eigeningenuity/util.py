...
def dataMap(j):
    try:
        if type(j["timestamp"]) == int:
            h = {datetime.fromtimestamp(j["timestamp"] / 1000): j["value"]}
        else:
            h = {datetime.fromisoformat(j["timestamp"]): j["value"]}
    except:
        h = j
    return h
...
def multi_response_to_df(api_response):
    ...
    for key, value in api_response.items():
        ...
        value_data = value['dataPoint']['value']
    ...
    # Create DataFrame
    df = pd.DataFrame(data_dict).T  # Transpose the DataFrame
