...
from eigeningenuity.util import _do_eigen_json_request, force_list, time_to_epoch_millis, is_list, \
    ...
    aggToDf, divide_chunks, flattenDict, csvWriter, multi_response_to_df
...
class HistorianMulti (object):
    ...
    def getCurrentDataPoints(self,tags: Union[str, list], output: str = "json", filepath:str=None, multi_csv:bool=False):
        ...
        response = requests.post(self.serverurl,json=body).json()
    ...
    def getInterpolatedPoints(self, tags: Union[str, list], timestamps: Union[list, int, float, str, datetime],
        ...
        for tag in tags:
            for timestamp in timestamps:
                ...
                body[tag +"-"+ timestamp] = request


        response = requests.post(self.serverurl,json=body)
        items = response["results"]
...
def get_historian_multi(eigenserver: EigenServer = None) -> HistorianMulti:
    ...
    return HistorianMulti(eigenserver.getEigenServerUrl() + "historian-servlet" + "/multi/")
