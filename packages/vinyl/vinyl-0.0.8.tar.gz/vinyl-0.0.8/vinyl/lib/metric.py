...
class Metric:
    ...
    def __init__(
        ...
        # set name
        self._name = ColumnBuilder(
            self._tbl.tbl, self._agg, passthrough_deferred=True
        )._name
...
class MetricStore:
    ...
    def copy(self) -> MetricStore:
        return MetricStore(list(self._metrics_dict.values()), self._default_tbl)

    def _set_columns(self):
        # set columns
        for i, name in enumerate(["ts", *self._dimension_names, *self._metrics_dict]):
    ...
    def _address_naming_conflicts(cls, new_metrics_dict: dict[str, Metric]):
        ...
        new_all_dimension_names = _get_dimension_names(new_metrics_dict.values()) | set(
            cls._all_dimension_names
        )
...
class MetricSelect:
    ...
    def __init__(
        ...
        for i, vals in enumerate(list(self._metric_store._tbl_dict.values())):
            if i == 0:
                self._combined_schema = VinylTable.create_from_schema(
                    vals[_SCHEMA_LABEL]
                )
            else:
                self._combined_schema += VinylTable.create_from_schema(
    ...
    def _process_unaltered_col(self, col, col_name):
        if col_name in self._metric_store._dimension_names:
            ...
            key_it = met._tbl.mutate(met._ts(met._tbl))  # use ts column as unique
    ...
    def _process_derived_col(self, col, col_name):
        ...
        for source in sources:
            ...
            if len(set(source_types)) > 1:
                ...
                self._temp_dimensions.append(col)
    ...
    def _build_final_tbls(self):
        for interval in self._intervals:
            ...
            for tbl, met_names in self._tbl_ts_metrics_to_select.items():
                ...
                vinyl_ts = ColumnBuilder(tbl.tbl, self._ts, passthrough_deferred=True)
                source_ts = [vinyl_ts._name]
                ts_original_source = vinyl_ts._sources
                ...
                if interval is None:
                    ...
                    collected_tbl_with_trailing = tbl.select_all(
                        ...
                        sort=[vinyl_ts._name_as_deferred_resolved(tbl)],
