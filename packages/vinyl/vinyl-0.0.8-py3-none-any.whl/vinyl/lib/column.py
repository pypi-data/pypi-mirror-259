...
class VinylColumn:
    ...
    def __init__(self, _col: Column):
        self._col = _col
        self.array = self.ArrayFunctions()
        self.math = self.MathFunctions()
        self.url = self.URLFunctions()
        self.re = self.RegexFunctions()
        self.str = self.StringFunctions()
        self.dict = self.MapFunctions()
        self.obj = self.StructFunctions()
        self.dt = self.TemporalFunctions()
    ...
    def __getitem__(self, key: Any) -> VinylColumn:
        ...
        if isinstance(key, slice):
            ...
            return self._col.substr(start, stop - start)
    ...
    def in_(self, values: Value | Sequence[Value]) -> BooleanValue:
        """
    ...
    def combine(self, sep: str = ", ", where=None) -> Value:
        """
    ...
    class MathFunctions:
        ...
        def modf(self) -> VinylColumn:
            ...
            return [self._col - self._col.floor(), self._col.floor()]
        ...
        def dist(
            ...
            """
            if type_ == "Euclidian":
                return (self - VinylColumn(other)).norm("L2")

            if type_ == "Manhattan":
                return (self._col - VinylColumn(other)).norm("L1")

            if type_ == "Cosine":
                return (self._col * other).sum() / (
                    self.norm("L2") * VinylColumn(other).norm("L2")
                )
    ...
    class StringFunctions:
        ...
        def center(
            ...
            adj: VinylColumn = self.ljust(width, fillchar).rjust(
                width, fillchar
