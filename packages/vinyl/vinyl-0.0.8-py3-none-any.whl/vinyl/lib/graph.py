...
class VinylGraph(rx.PyDiGraph):
    ...
    def _visualize(cls, expr: ir.Expr):
        g_rust = cls._new_init_from_expr(expr, label_func=_get_label)
        g = _rustworkx_to_networkx(
            g_rust,
            pre_convert=lambda x: x.markup,
        )  # pre_convert handles hashing issues with Text class
        nx.set_node_attributes(
            g, lambda n, d, s: Text.from_markup(n), "$content-renderer"
        )
