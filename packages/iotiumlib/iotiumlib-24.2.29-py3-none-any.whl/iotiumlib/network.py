...
from iotiumlib.requires.api_endpoints import Network
...
class network(object):
    ...
    def __init__(self, action, payload=None, network_id=None, filters=None):
        ...
        api = Network()
        self.uri = {
            add_network: api.v1_network,
            get_network: api.v1_network,
            getv2_network: api.v2_network,
            edit_network: api.v1_network_id,
            delete_network: api.v1_network_id,
            get_network_id_network: api.v1_network_id,
            reset_counter_network: api.v1_network_id_resetcounter,
        }
    ...
    class device_discovery(object):
        ...
        """
        Enable/Disable DD on Network
        """

        def __init__(self, action, filters=None, payload=None, network_id=None, report_id=None, scan_id=None):

            if payload is None:
                payload = {}

            def get_report_device_discovery(uri):
                return network.device_discovery.api(self, method='getv2', uri=uri, filters=filters)

            def download_report_diff_device_discovery(uri):
                return network.device_discovery.api(self, method='post', uri=uri)

            def get_report_id_device_discovery(uri):
                return network.device_discovery.api(self, method='get', uri=uri)

            def get_scan_device_discovery(uri):
                return network.device_discovery.api(self, method='getv2', uri=uri, filters=filters)

            def get_scan_id_device_discovery(uri):
                return network.device_discovery.api(self, method='get', uri=uri)

            def download_device_discovery(uri):
                return network.device_discovery.api(self, method='get', uri=uri)

            def add_scan_device_discovery(uri):
                return network.device_discovery.api(self, method='post', uri=uri)

            def edit_scan_device_discovery(uri):
                return network.device_discovery.api(self, method='put', uri=uri)

            def delete_scan_device_discovery(uri):
                return network.device_discovery.api(self, method='delete', uri=uri)

            def run_device_discovery(uri):
                return network.device_discovery.api(self, method='post', uri=uri)

            _function_mapping = {
                'get_report': get_report_device_discovery,
                'download_report_diff': download_report_diff_device_discovery,
                'get_report_id': get_report_id_device_discovery,
                'download': download_device_discovery,
                'get_scan': get_scan_device_discovery,
                'add_scan': add_scan_device_discovery,
                'get_scan_id': get_scan_id_device_discovery,
                'edit_scan': edit_scan_device_discovery,
                'delete_scan': delete_scan_device_discovery,
                'run': run_device_discovery,
            }

            api = Network()
            self.uri = {
                get_report_device_discovery: api.v2_network_id_discovery_report,
                download_report_diff_device_discovery: api.v2_network_id_discovery_report_diff,
                get_report_id_device_discovery: api.v2_network_id_discovery_report_id,
                download_device_discovery: api.v2_network_id_discovery_report_id_download,
                get_scan_device_discovery: api.v2_network_id_discovery_scan,
                add_scan_device_discovery: api.v2_network_id_discovery_scan,
                get_scan_id_device_discovery: api.v2_network_id_discovery_scan_id,
                edit_scan_device_discovery: api.v2_network_id_discovery_scan_id,
                delete_scan_device_discovery: api.v2_network_id_discovery_scan_id,
                run_device_discovery: api.v2_network_id_discovery_scan_id_run,

            }

            self.payload = resourcePaylod.DeviceDiscovery(payload).__dict__
            self.orgId = orch.id
            self.networkId = network_id
        ...
        @staticmethod
        def report(network_id, report_id=None, filters=None):
            if report_id is None:
                return network.device_discovery(action='get_report', network_id=network_id, filters=filters)
            if report_id is not None:
                return network.device_discovery(action='get_report_id', report_id=report_id, network_id=network_id,
                                                filters=filters)

        @staticmethod
        def download(network_id, report_id, diff_with_report_id=None):
            if diff_with_report_id is None:
                return network.device_discovery(action='download', network_id=network_id, report_id=report_id)
            if diff_with_report_id is not None:
                return network.device_discovery(action='download_report_diff', network_id=network_id, payload=locals())

        @staticmethod
        def get(network_id, filters=None, scan_id=None):
            if scan_id is None:
                ...
                if frequency is not None:
                    if frequency.lower() == "weekly":
                        ...
                        if by_day is not None:
                            by_day = [str(day[:2]) for day in by_day.split(',')]
                        else:
                            raise ArgValueError('by_day value is missing')
                    elif frequency.lower() == "monthly":
                        by_day = None
                        if by_month_day is not None:
                            try:
                                by_month_day = [int(day[:2]) for day in by_month_day.split(',')]
                            except Exception:
                                raise ArgValueError('by_month_day value should be numbers')
                        else:
        ...
        def edit(network_id, scan_id, name=None, profile=None, runonce=None, recurrence=None, frequency=None,
            ...
            #
            # return network.device_discovery(action='edit_scan', scan_id=scan_id, network_id=network_id,
            #                                 payload=locals())

        @staticmethod
        def delete(network_id, scan_id):
            return network.device_discovery(action='delete_scan', network_id=network_id, scan_id=scan_id)

        @staticmethod
        def run(network_id, scan_id):
            return network.device_discovery(action='run', network_id=network_id, scan_id=scan_id)
    ...
    def edit(network_id, network_name=None, cidr=None, gateway_ip=None, start_ip=None, end_ip=None, label=None,
             default_destination=None, connect_networks=None, firewall_selector=None, vlan_id=None,
             firewall_policy=None,
             static_routes=None, interface_ip=None, enable_inter_tan=None):
        ...
        if network_name != "WAN Network":
            ...
            if 'tan_interface_ip' in resp.Response.output['config']['network']:
                interface_ip = resp.Response.output['config']['network'][
                    'tan_interface_ip'] if interface_ip is None else interface_ip
            if 'reserved' in resp.Response.output['config']['network']:
                start_ip = resp.Response.output['config']['network']['reserved'][0][
                    'start'] if start_ip is None else start_ip
        ...
        firewall_policy = resp.Response.output['policy']['firewall'][
            'debug'] if firewall_policy is None else firewall_policy
        ...
        if connect_networks is None:
            ...
            for cn in connect_networks:
                ...
                if _connect_networks:
                    ...
                    if temp:
                        _connect_networks[(
                            [i for i, _ in enumerate(_connect_networks) if _['network_id'] == cn['network_id']][
                                0])].update(
                            cn)
                        continue
            ...
            if DISCONNECT_FLAG:
                for cn in connect_networks:
                    ...
                    if temp:
                        del _connect_networks[
                            ([i for i, _ in enumerate(_connect_networks) if _['network_id'] == cn['network_id']][0])]
