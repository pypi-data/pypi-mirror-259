...
class CertificateGenerator:
    ...
    def _parse_cert(self, content: str) -> X509:
        ...
        cert = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_PEM, content.encode())
        not_after = cert.get_notAfter().decode('utf-8')  # Get the notAfter field of the certificate
        not_after_bytes = not_after.encode('utf-8')  # Encode the expiration date to bytes
        cert.set_notAfter(not_after_bytes)  # Set the expiration date of the certificate
        ic(cert)
        ic(not_after)
        ic(cert.get_notAfter().decode('utf-8'))  # Verify the expiration date
        return cert
