...
class KYZS(Spider):
    ...
    def __init__(self, cookies):
        ...
        self.base_url = 'http://fund.keyanzhiku.com/Index/index/start_year/0/end_year/0/xmid/0/search/1/p/1.html'
    ...
    async def get_pages(self):
        ...
        pages = int(re.findall('\d+ 条数据 \d+/(\d+) 页', html)[0])
        return html, pages
        ...
        async with async_playwright() as playwright:
            ...
            for row in data_list:
                ...
                if not href.startswith(http):
                    href = f'http://fund.keyanzhiku.com{href}'
                name = row.xpath('.//h3/text()').get('').strip()
                span = row.xpath('./span').get('')
                ratify_no = re.findall('项目批准号：(.*?)</span>', span)[0]
                print("ratify_no:", ratify_no)
